...
- `technical_survey.md` is optional. Its absence means the latest sketch did
  not require a separate proof-framework search artifact.
- `global_proof.md` is a theorem-level diagnostic, binding downstream only as
  context when paired with an accepted `global_proof_review.md`.
...
A sketch revision invalidates all step proof/review and assembly artifacts from
the earlier sketch attempt. Every required step must receive a fresh proof and
review under the new accepted sketch. Likewise, rerunning a step producer makes
the old accepted step review stale until a new review identifies and accepts
the repaired proof artifact.

Prior same-branch artifacts may be supplied as narrowly scoped repair context
when their claims and assumptions still match. They remain diagnostic inputs,
not current proof evidence.
...
as source PDFs, problem indexes, evaluation reports, `run-metadata.yaml`, and
`MANIFEST.sha256`. Those files document and verify the published snapshot; they
are not generated by the controller's normal run contract.
