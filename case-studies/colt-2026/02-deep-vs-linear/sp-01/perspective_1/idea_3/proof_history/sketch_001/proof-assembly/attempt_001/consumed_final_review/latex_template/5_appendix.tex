...
Under Assumptions~\ref{assump:fixed-source-witnesses} and
\ref{assump:robust-tube}, Proposition~\ref{prop:p1-i3-step001-gradient},
and Lemma~\ref{lem:p1-i3-step002-one-step}, every initialization
$\theta^{(0)}\in E_r$ and every labeled history $\omega$ above satisfy
\[
...
and Proposition~\ref{prop:p1-i3-step003-ball-gates}, every initialization in
$E_r$ and every labeled history satisfy, for $L\geq2$,
\[
|u_{\ell,j}(\theta^{(t)},x)|
\geq M_r(\theta^{(0)})>0,
\qquad
...
Under Assumptions~\ref{assump:fixed-source-witnesses} and
\ref{assump:robust-tube}, Proposition~\ref{prop:p1-i3-step004-aggregate},
and Lemma~\ref{lem:p1-i3-step005-pathwise-risk}, define
\[
...
\begin{proof}
For an initialization $\vartheta$ and tuple $\mathbf x\in\mathcal X^T$, let
\[
Z_{\mathcal D,h}(\vartheta,\mathbf x):=
\mathcal L_{\mathcal D,h}(\widehat h_{\vartheta,\mathbf x})\in[0,1]
\tag{85}
\]
be the prescribed learner loss on the source-consistent labeled tuple.  For
fixed $\mathbf x$, the forward operations, the Borel selector
$\rho_\kappa$, logistic derivative, finite back-propagation recurrence, tie
map, and finite risk sum are Borel in $\vartheta$.  Thus $Z$ is jointly
measurable.  Define the conditional sample average
\[
...
Under Assumptions~\ref{assump:fixed-source-witnesses},
\ref{assump:universal-expected-success}, and~\ref{assump:robust-tube}, and
Proposition~\ref{prop:p1-i3-step006-dc-witness}, the same law satisfies
\[
\mathbb E_{\varphi\sim\mathcal P_{\rm gate}}
\left[\inf_{w\in\mathbb R^{d_{\rm path}}}
