...
2. The attempt-3 compile log contains two shared-style font-shape fallback warnings. It contains no LaTeX error, unresolved reference or citation, duplicate-label warning, or overfull/underfull box.
...
- Hidden subclaims: PASS. No independent unreviewed lemma, external theorem, optimizer selector, trajectory invariant, conditioned feature law, compactness shortcut, probability conversion, or asymptotic claim is hidden in the assembly. Static scans also find no public claim/hypothesis/invariant environments, workflow scaffolding, placeholder text, malformed dependency references, or citation issue. The 102 manually tagged displays have exactly 102 distinct labels `eq:p1-i3-eq001` through `eq:p1-i3-eq102`; every tag is paired with its matching label, and the compile log reports no unresolved or duplicate reference.
