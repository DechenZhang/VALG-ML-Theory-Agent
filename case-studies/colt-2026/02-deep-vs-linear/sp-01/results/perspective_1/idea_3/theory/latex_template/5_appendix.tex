...
\end{aligned}
\tag{24}\label{eq:p1-i3-eq024}
\]
Since $0\leq\sigma(v)\leq|v|$ for every real $v$, the same bound holds
for $z_{\ell,j}$.  Finite induction proves \eqref{eq:p1-i3-eq022}, and the recurrence also
gives
\[
a_\ell=B^\ell\prod_{q=0}^{\ell-1}n_q,
\tag{25}\label{eq:p1-i3-eq025}
\]
...
\tag{27}\label{eq:p1-i3-eq027}
\]
for every layer and coordinate.  Consequently
\[
G_r(\theta^{(0)})\leq C_r(\theta^{(0)})<\infty.
\tag{28}\label{eq:p1-i3-eq028}
\]
When $L=1$, $C_r(\theta^{(0)})=1$ and $G_r(\theta^{(0)})\leq1$.
\end{proposition}

\begin{proof}
Fix $\theta,x,y$ in the domains of \eqref{eq:p1-i3-eq027}.  The forward equations are finite
...
$\rho_\kappa(0)=\kappa$, the induction hypothesis and \eqref{eq:p1-i3-eq023} give
\[
|\delta_{\ell,j}|
\leq\sum_{q=1}^{n_{\ell+1}}
|\theta_{\ell+1,qj}|\,|\delta_{\ell+1,q}|
\leq n_{\ell+1}B b_{\ell+1}=b_\ell.
\tag{32}\label{eq:p1-i3-eq032}
\]
Backward induction yields
\[
b_\ell=B^{L-\ell}\prod_{q=\ell+1}^{L}n_q,
\tag{33}\label{eq:p1-i3-eq033}
\]
where $n_L=1$.  The layerwise selected-gradient formula is
\[
[\nabla_\theta^{(\kappa)}\ell(yf_\theta(x))]_{\ell,jk}
=\delta_{\ell,j}z_{\ell-1,k}(\theta,x).
\tag{34}\label{eq:p1-i3-eq034}
\]
Combining \eqref{eq:p1-i3-eq032}, \eqref{eq:p1-i3-eq034}, and Lemma~\ref{lem:p1-i3-step001-forward} proves
\eqref{eq:p1-i3-eq027}, and taking the supremum over the domain in \eqref{eq:p1-i3-eq009} proves \eqref{eq:p1-i3-eq028}.  The
finiteness of \eqref{eq:p1-i3-eq025}, \eqref{eq:p1-i3-eq033}, and the finite layer maximum in \eqref{eq:p1-i3-eq026} gives
...
If $L=1$, there are no hidden adjoints and \eqref{eq:p1-i3-eq034} reduces to
\[
\left|[\nabla_\theta^{(\kappa)}\ell(yf_\theta(x))]_{1,1k}\right|
=\frac{|x_k|}{1+e^{yf_\theta(x)}}\leq1,
\tag{35}\label{eq:p1-i3-eq035}
\]
...
\qquad0\leq t<T,
\tag{36}\label{eq:p1-i3-eq036}
\]
and put
\[
D_t:=\|\theta^{(t)}-\theta^{(0)}\|_\infty,
\qquad G:=G_r(\theta^{(0)}).
\tag{37}\label{eq:p1-i3-eq037}
\]
...
and the definitions \eqref{eq:p1-i3-eq036}--\eqref{eq:p1-i3-eq037}, if $0\leq t<T$ and $D_t\leq r$, then
\[
\|g_t\|_\infty\leq G,
\qquad D_{t+1}\leq D_t+\eta G.
\tag{38}\label{eq:p1-i3-eq038}
\]
...
\tag{39}\label{eq:p1-i3-eq039}
\]
Using \eqref{eq:p1-i3-eq036}, the triangle inequality, $\eta>0$, and \eqref{eq:p1-i3-eq039},
\[
\begin{aligned}
D_{t+1}
...
\[
\eta T G\leq r.
\tag{42}\label{eq:p1-i3-eq042}
\]
At the entry state,
\[
D_0=\|\theta^{(0)}-\theta^{(0)}\|_\infty=0=0\cdot\eta G\leq r.
\tag{43}\label{eq:p1-i3-eq043}
\]
Suppose $0\leq t<T$ and $D_t\leq t\eta G$.  Since $t\leq T$, \eqref{eq:p1-i3-eq042} gives
\[
D_t\leq t\eta G\leq T\eta G\leq r,
\tag{44}\label{eq:p1-i3-eq044}
\]
...
\tag{45}\label{eq:p1-i3-eq045}
\]
and $t+1\leq T$ gives
\[
(t+1)\eta G\leq T\eta G\leq r.
\tag{46}\label{eq:p1-i3-eq046}
\]
Finite induction proves \eqref{eq:p1-i3-eq041}.  The first transition is
$D_1\leq\eta G\leq r$ because $T\geq1$; if $T=1$, \eqref{eq:p1-i3-eq043} and this bound
...
\eqref{eq:p1-i3-eq041} is the derived all-history containment result that will be used below.  It applies in
...
\eqref{eq:p1-i3-eq048} is a finite sum of continuous functions.  If all paths at layer
$\ell-1$ are continuous, then coordinatewise ReLU is continuous, and \eqref{eq:p1-i3-eq049} is
...
\tag{53}\label{eq:p1-i3-eq053}
\]
for every hidden coordinate and input.  Along the segment \eqref{eq:p1-i3-eq047}, \eqref{eq:p1-i3-eq053} gives
...
$q_{\ell,j}(s_\star)=0$, contradicting \eqref{eq:p1-i3-eq053}.  Thus the endpoint signs agree;
since both are nonzero, \eqref{eq:p1-i3-eq052} follows.
...
\eqref{eq:p1-i3-eq054}.  The history was arbitrary, so the conclusion is all-history rather
than almost-sure.  No gate stability was assumed before \eqref{eq:p1-i3-eq055} was established.
\end{proof}
...
\tag{63}\label{eq:p1-i3-eq063}
\]
At layer zero, $z_{0,i_0}=x_{i_0}=P_{0,i_0}$.  If the assertion holds at
layer $\ell-1$, substituting it into \eqref{eq:p1-i3-eq060} gives \eqref{eq:p1-i3-eq062}, and distributing the
finite sum appends exactly the new factor
$g_{\ell,i_\ell}^{(0)}(x)(\theta_\ell)_{i_\ell,i_{\ell-1}}$, proving \eqref{eq:p1-i3-eq063}.
...
\tag{68}\label{eq:p1-i3-eq068}
\]
The set $J_T$ is finite and nonempty because $T\geq1$.  Summing \eqref{eq:p1-i3-eq068} and
using bilinearity proves \eqref{eq:p1-i3-eq067}.  On a source-consistent history the left side
is exactly \eqref{eq:p1-i3-eq005}, so equal real scores have equal
...
equality of the error indicators, hence \eqref{eq:p1-i3-eq072}.  The conclusion remains exact
...
\tag{75}\label{eq:p1-i3-eq075}
\]
The infimum need not be attained.
\end{proposition}

\begin{proof}
Every risk in \eqref{eq:p1-i3-eq074} is a probability in $[0,1]$, and the coefficient space is
...
\tag{76}\label{eq:p1-i3-eq076}
\]
The left side is fixed before the tuple is drawn.  Multiplying \eqref{eq:p1-i3-eq076} by the
...
$d_{\rm path}|\mathcal X|$ coordinates, proving measurability and \eqref{eq:p1-i3-eq080}.
...
\tag{82}\label{eq:p1-i3-eq082}
\]
Every risk is in $[0,1]$ and the coefficient space is nonempty, so the table
is finite-valued even if its infimum is not attained.  Equation \eqref{eq:p1-i3-eq079} is
...
&\leq\mathbb E[\mathbf1_{E_r}g_{\mathcal D,h}]
 +\Pr(E_r^c).
\end{aligned}
\tag{89}\label{eq:p1-i3-eq089}
\]
By independence and the finite sum \eqref{eq:p1-i3-eq086}, bounded tower averaging gives
\[
\begin{aligned}
...
\tag{91}\label{eq:p1-i3-eq091}
\]
Substitution in \eqref{eq:p1-i3-eq089} proves \eqref{eq:p1-i3-eq084}.  The complement is charged once by its
...
\tag{92}\label{eq:p1-i3-eq092}
\]
and hence
\[
\operatorname{dc}_{\varepsilon+\delta_0}(\mathcal H)
\leq d_{\rm path}.
\tag{93}\label{eq:p1-i3-eq093}
\]
\end{proposition}
...
one law independent of later choices.  For arbitrary $\mathcal D,h$, \eqref{eq:p1-i3-eq081}
and \eqref{eq:p1-i3-eq084} give \eqref{eq:p1-i3-eq092}.  Since the law was fixed before this arbitrary choice, the
quantifier order is $\exists\mathcal P_{\rm gate}\,\forall\mathcal D\,\forall h$.
The definition \eqref{eq:p1-i3-eq001} then gives \eqref{eq:p1-i3-eq093}.
\end{proof}
...
\tag{100}\label{eq:p1-i3-eq100}
\]
The witness bound \eqref{eq:p1-i3-eq092} therefore proves \eqref{eq:p1-i3-eq098} under the unchanged law and
...
Proposition~\ref{prop:p1-i3-step006-dc-witness} proves \eqref{eq:p1-i3-eq018} with one
unconditional law and gives
\[
\operatorname{dc}_{\varepsilon+\delta_0}(\mathcal H)
\leq d_{\rm path}.
\tag{102}\label{eq:p1-i3-eq102}
\]
Lemma~\ref{lem:p1-i3-step007-path-count} gives
$d_{\rm path}\leq S^L\leq S^{L_0}$, proving \eqref{eq:p1-i3-eq019}.  Finally,
Proposition~\ref{prop:p1-i3-step007-public-specialization} reuses that same
law and gives the first inequality in \eqref{eq:p1-i3-eq020}; combining it with \eqref{eq:p1-i3-eq102} and the
