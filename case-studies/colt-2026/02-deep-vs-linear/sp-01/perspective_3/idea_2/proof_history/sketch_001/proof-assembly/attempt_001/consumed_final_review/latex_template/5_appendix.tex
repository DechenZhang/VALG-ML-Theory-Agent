...
$\mathcal D$ and then taking expectation under this same trajectory law gives
\[
\begin{aligned}
...
\begin{proof}
Apply Proposition~\ref{prop:step-003-expected-transfer} and then
...
Suppose that no $w\in\mathbb R^n$ satisfies
...
\]
If $u_j=h(q_j)q_j$ with $q_j\in Q_h$, one may retain one such $q_j$ for
...
Suppose distinct $q_1,\ldots,q_k\in Q_h$ and positive coefficients satisfy
\[
...
If the strict system
\[
\nexists w\in\mathbb R^n\quad
\forall q\in Q_h,\qquad h(q)\langle w,q\rangle>0
\]
is infeasible, then there are distinct representatives
...
Under Assumption~\ref{assump:high-accuracy}, for every fixed
$h\in\mathcal H$ the strict system
\[
...
Under Assumption~\ref{assump:high-accuracy} and the conclusions of
Propositions~\ref{prop:step-003-approximate-separator},
\ref{prop:step-004-strict-equivalence}, and
\ref{prop:step-005-obstruction}, every fixed $h\in\mathcal H$ has a vector
$w_h\in\mathbb R^n$ satisfying
\[
...
\begin{proof}
Proposition~\ref{prop:step-006-feasibility} supplies a vector satisfying the
strict inequalities on $Q_h$.  The reverse implication in
