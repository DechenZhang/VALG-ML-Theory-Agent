...
Fix the learner \(A\), complete-tape law \(\mu\), and parameters satisfying the two primitive setting assumptions. All constructions through Block B2 are made before fixing an instance. For the risk comparison, then fix an arbitrary \((\mathcal D,h)\); since neither the construction nor any bound below depends on a special property of this pair, the final conclusion is simultaneous over all pairs.
...
\[
\Delta_0=0,\qquad \Delta_t\le\Delta_{t-1}quad(1\le t\le m)
\]
for every transition that occurs. Hence \(\Delta_t=0\) throughout. The same state equality also gives the same stopping decision and terminal predictor. Once the common run stops, the replay coordinate ignores all suffix symbols, so padding cannot change that predictor. Therefore, as functions on \(\mathcal X\),
\[
