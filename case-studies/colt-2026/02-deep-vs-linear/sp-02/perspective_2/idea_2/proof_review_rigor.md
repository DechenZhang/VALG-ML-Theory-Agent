...
- Statement self-containedness: The public theorem states its conditional scope, cites all four numbered assumptions, defines its theorem-local map and weights before use, and exposes the deterministic pointwise conclusion and dimension bound. Appendix results state their assumptions and named dependencies before their proofs, all internal references resolve to uniquely labeled theorem-style results, and the proof sketch now cites Sion's Theorem 3.4 at the optimization-order exchange.
...
- Hidden subclaims: No independent theorem-critical claim is left in prose or assembly. Finite-dimensional separation and Heine--Borel, exact-center legality and bounded Fubini, the Sion object mapping and orientation, arbitrary-family compact closure, and exact coordinate reconstruction are each proved or cited with their assumptions discharged. Both public uses of Sion identify Theorem 3.4, and the final assembly uses only named accepted results and direct logical implications.
