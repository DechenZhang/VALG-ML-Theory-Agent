...
For each nonempty finite set $F$, define its probability simplex by
\[
\Delta_F:=\left\{r=(r_x)_{x\in F}\in\mathbb R_+^F:
\sum_{x\in F}r_x=1\right\}.
\]
Thus the coordinates of every $r\in\Delta_F$ are nonnegative and sum to one.
...
least $\rho$.  After enumerating $F$, the defining constraints make
$\Delta_F$ a closed and bounded subset of $\mathbb R^{|F|}$, hence compact by
the finite-dimensional Heine--Borel theorem
\citep[Theorem~2.41]{rudin1976principles}.  The objective is a finite maximum
of linear functions of $r$ and is continuous, so the extreme-value theorem
\citep[Theorem~4.16]{rudin1976principles} gives the asserted attained minimum.
\end{proof}
...
\begin{proof}
The simplex $\Delta_L$ is closed and bounded in $\mathbb R^L$, hence compact
by the finite-dimensional Heine--Borel theorem
\citep[Theorem~2.41]{rudin1976principles}.  The linear map $w\mapsto Aw$ is
continuous, so its image is compact by preservation of compactness under
continuous maps \citep[Theorem~4.14]{rudin1976principles}.  Thus consider the
compact convex set of column mixtures
\[
...
\end{equation}
The right side is continuous.  Since $C$ is nonempty and compact, the
extreme-value theorem \citep[Theorem~4.16]{rudin1976principles} gives a point
...
The defining constraints make both nonempty finite simplices closed and
bounded in their finite-dimensional coordinate spaces, hence compact by the
finite-dimensional Heine--Borel theorem
\citep[Theorem~2.41]{rudin1976principles}.  The extreme-value theorem
\citep[Theorem~4.16]{rudin1976principles} therefore gives attainment.  Let
\[
...
\end{equation}
Each set on the right is the inverse image of a closed subset of $\mathbb R$
under a continuous coordinate or sum functional.  The closed-set form of the
inverse-image characterization of continuity
\citep[Theorem~4.8, by complements]{rudin1976principles} therefore shows that
$\Delta_L$ is closed.  For
$w\in\Delta_L$,
\[
\lVert w\rVert_2\le\lVert w\rVert_1
=\sum_{i=1}^Lw_i=1,
\]
so it is bounded.  The finite-dimensional Heine--Borel theorem
\citep[Theorem~2.41]{rudin1976principles} therefore makes this one simplex
compact.
...
\]
is continuous.  Since $[\rho,\infty)$ is closed, the closed-set form of the
inverse-image characterization of continuity
\citep[Theorem~4.8, by complements]{rudin1976principles} makes its inverse
image closed, and
