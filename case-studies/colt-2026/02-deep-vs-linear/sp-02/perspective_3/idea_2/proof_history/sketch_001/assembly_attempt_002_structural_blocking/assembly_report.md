...
Assembly identity: `sketch_attempt=1`; assembly `unit_attempt=2`. This repair
restores only 17 missing `\qquad` command prefixes and the main-theorem
inequality `\operatorname{dc}(H)\le L`; every theorem environment and all
proof content otherwise remain unchanged from the archived first attempt.
...
- A clean full `<TECTONIC_BIN>` compile wrote only to
  `<TEMP_ROOT>/p3-assembly-attempt2.WqqIS2/out`, produced a 17-page `main.pdf`, and
