...
Assembly identity: `sketch_attempt=1`; assembly `unit_attempt=3`. Relative to
the archived second attempt, the sole repair is an appendix-local definition
of the probability simplex
\(\Delta_F=\{r=(r_x)_{x\in F}\in\mathbb R_+^F:\sum_{x\in F}r_x=1\}\)
for each nonempty finite set \(F\), placed in `Finite Simultaneous Feasibility`
before its first use. This resolves the structural self-containment finding;
no theorem claim, theorem environment, accepted proof logic, label, or
dependency changes.
...
Validation evidence for attempt 3:
...
- The label/reference audit counts 77 labels and 163 `\ref`/`\eqref` uses,
  with no duplicate label and no missing target. Public-hygiene, placeholder,
  workflow-language, malformed-reference/citation, and citation-command scans
  all return no defect; `main.tex` suppresses the unused bibliography.
- A clean full `<TECTONIC_BIN>` compile wrote only to
  `<TEMP_ROOT>/p3-assembly-attempt3.LgyYhg/out`, produced a 17-page `main.pdf`, and
  required one normal auxiliary-file rerun. The final log has no warning,
  error, unresolved or undefined reference/citation, multiply defined label,
  missing character, or overfull/underfull box. No generated compile artifact
  appears in `latex_template/`.
