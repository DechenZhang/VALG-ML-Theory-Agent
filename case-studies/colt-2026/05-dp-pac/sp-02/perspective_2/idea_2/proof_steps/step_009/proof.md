...
- Derived invariants supplied by accepted dependencies:
  ...
  2. Proposition~\ref{prop:step-006-kernel} supplies the constructed
     full-cube kernel \(B_\Pi\).  Proposition~\ref{prop:step-006-exact-dp}
     is the privacy certificate proving that this same kernel is exactly
  ...
  4. Proposition~\ref{prop:step-008-utility} supplies the strict upper
     ...
     Proposition~\ref{prop:step-008-auxiliary} supplies only the
     conditional one-arm certificate.
...
| `unit_003` | proposition | Under all three setting assumptions, Proposition~\ref{prop:step-009-constants}, Lemma~\ref{lem:step-009-arithmetic}, `N>=N_0`, and the temporary inequality `m<c k log^*N`, the instantiated tuple `(n,N,epsilon_0,delta_m)` satisfies `N>=N_th`, `n<a_th log^*N`, `0<epsilon_0<=1`, and `0<=delta_m<=a_delta/[n^2 log(en)]`; hence the accepted prior exists, Proposition~\ref{prop:step-006-kernel} produces `B_Pi`, and Proposition~\ref{prop:step-006-exact-dp} certifies it as an exact-private competitor. | Instantiates `(HP)` with no delta relabeling or added assumption. |
| `unit_004` | proposition | Under all three setting assumptions, for the same `(n,N,epsilon_0,delta_m)`, prior `Pi`, and kernel `B_Pi`, Proposition~\ref{prop:step-009-admissibility} (HP admissibility), Proposition~\ref{prop:step-005-hard-prior} (HP), Proposition~\ref{prop:step-006-kernel} (kernel producer), Proposition~\ref{prop:step-006-exact-dp} (exact-DP certificate), and Proposition~\ref{prop:step-008-utility} (unconditional UT), the temporary inequality \(m<c k\log^*N\) is impossible; hence (T9.4) holds. | Gives the strict lower-versus-upper contradiction. |
| `unit_005` | proposition | Under all three setting assumptions, Proposition~\ref{prop:step-009-admissibility} (used under the temporary counterassumption for the instantiated HP conditions), Propositions~\ref{prop:step-006-one-arm} and \ref{prop:step-007-boundaries}, Proposition~\ref{prop:step-008-utility} (unconditional UT), Proposition~\ref{prop:step-008-auxiliary} (conditional one-arm certificate only), and Proposition~\ref{prop:step-009-main-closure}, setting `k=1` gives `n=2m+12`, `R=m<n`, zero overflow, exact one-chain sampling/privacy/risk interfaces, and `m>=(a_th/4)log^*N`. | Verifies exact one-chain recovery without a residual or weaker surrogate. |
...
- Restated statement in current notation: once the accepted prior is
  available, Proposition~\ref{prop:step-006-kernel} is the producer
  that defines
  ...
  while Proposition~\ref{prop:step-006-exact-dp} is the privacy
  certificate establishing both eventwise replacement-adjacency
...
- Source: accepted `step_008`, specifically
  Proposition~\ref{prop:step-008-utility} for unconditional `(UT)` and,
  for the baseline's conditional certificate,
  Proposition~\ref{prop:step-008-auxiliary}.
- Restated statement in current notation:
  Proposition~\ref{prop:step-008-utility} gives the unconditional bound,
  for the same prior and hidden-arm kernel,
  ...
  Proposition~\ref{prop:step-008-auxiliary} is used only for the
  conditional certificate: at \(k=1\), zero overflow makes the ideal
  and constructed runs identical, the arm-mixture identity tautological,
  and the conditional auxiliary success probability equal to one.
...
- Proposition~\ref{prop:step-009-admissibility}: transfers the same
  `delta_m` into `(HP)`, instantiates the prior, invokes
  Proposition~\ref{prop:step-006-kernel} as the (B_\Pi) producer, and
  invokes Proposition~\ref{prop:step-006-exact-dp} as its privacy
  certificate.
...
counterassumption (9.6), with \(N\ge N_0\), the instantiated tuple

\[
(n,N,\varepsilon,\delta)=(n,N,\varepsilon_0,\delta_m)
\]

satisfies all four conditions in (C9.1), namely
...
class of the same `(HP)` inequality.
...
`(n,N,epsilon_0,delta_m)`.  With that prior fixed,
...
`(HP)` conditions, Proposition~\ref{prop:step-005-hard-prior} supplies
...
Proposition~\ref{prop:step-008-utility} supplies `(UT)` for the same
prior and kernel.  Under these named interfaces, if the temporary
counterassumption (9.6) is made for the contradiction,
...
exact-private competitor.  Applying the `(HP)` conclusion of
...
Proposition~\ref{prop:step-008-utility} supplies the unconditional `(UT)`
...
Accepted Proposition~\ref{prop:step-008-auxiliary} says only that the
...
`delta_m` comparison (9.18)--(9.20) applies.  Under this temporary
counterassumption, Proposition~\ref{prop:step-009-admissibility} gives
the four instantiated HP conditions explicitly listed in (9.13).  Thus
`(HP)` is
...
Proposition~\ref{prop:step-006-kernel} produces \(B_\Pi\), and accepted
Proposition~\ref{prop:step-006-exact-dp} certifies it as a competitor in
the exact `(HP)` infimum.  Proposition~\ref{prop:step-009-main-closure}
then invokes Proposition~\ref{prop:step-008-utility} for the accepted
strict `(UT)` upper value and compares it with the accepted lower value
...
- `Pi=Pi_(n,N,epsilon_0,delta_m)` and \(B_\Pi\):
  ...
  `step_005` produces the prior; Proposition~\ref{prop:step-006-kernel}
  produces the kernel, and Proposition~\ref{prop:step-006-exact-dp}
  supplies its exact privacy certificate.
