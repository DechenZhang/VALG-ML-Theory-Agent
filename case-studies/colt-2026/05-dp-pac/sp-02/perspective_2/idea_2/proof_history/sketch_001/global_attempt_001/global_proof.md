...
- Identity check: the current sketch digest equals the digest recorded by the accepted review. The current `proof_tracker.md` contains the running global row for sketch attempt `1`, global attempt `1`.
- Run mode and budget: `interactive`; idea index `2 <= 30`, sketch attempt `1 <= 10`, global producer attempt `1 <= 100`.
...
and let the real sample come from coordinate \(J\). Conditional on this vector, generate an infinite iid pool from each \(Q_{e_j}\). Use the first \(n\) records of pool \(J\) as the external sample and use the next unused record from pool \(I_r\) at ideal position \(r\). The resulting ideal sample \(\bar S\) is iid from
...
so (G7.2) is realized by a concept in (C). The ideal and constructed learner inputs agree at all (m) positions on (R\le n).
...
For any zero-one loss, the constructed and ideal hidden-arm risks differ by at most (p_{\rm ov}), since they agree off the one overflow event.

### Block G8: exchangeability and the strict utility upper bound (`step_008`)

Conditional on the ordered iid vector \(\boldsymbol e\), the ideal sample, \(A\)'s coins, and the resulting \(h\sim A(\bar S)\) are generated without using \(J\). Thus \(h\) is conditionally independent of the hidden designation. For every fixed \(\boldsymbol e,h\),
...
Average (EX) over \(\boldsymbol e\) and the ideal run, and then charge the sole coupling failure from (OF). This gives the exact finite-game interface
...
For the accepted auxiliary output, on the ideal PAC-good event one has (L_{\bar Q_{\boldsymbol e}}(h)\le\alpha_0). Applying Markov to the nonnegative arm losses in (EX) gives

\[
\Pr_J\!\left[L_{Q_{e_J}}(h\circ\phi_J)>8\alpha_0
\mid \boldsymbol e,h,\ L_{\bar Q_{\boldsymbol e}}(h)\le\alpha_0\right]
\le\frac18.
\tag{G8.2}
\]
...
where the last inequality follows from (G9.4) and monotonicity of \(\log^*\). Thus all hypotheses of `(HP)` hold for the exactly private kernel \(B_\Pi\). The lower bound `(HP)` and upper bound `(UT)` give
...
At (k=1), (n=2m+12), every arm index equals the hidden arm, (R=m<n), and overflow is impossible. The ideal mixture is exactly (Q_e), (EX) is the identity on the sole arm, privacy remains one-record privacy, and the preceding arithmetic yields (m\ge c\log^*N). The original one-chain conclusion, rather than merely vanishing coupling defects or a stopped surrogate, is therefore preserved.
...
G1--G4 turn the negation of algorithm-wise finite-experiment hardness into the active empirical-threshold contradiction, G5 is the only quantifier-swap and prior-production point, G6--G8 construct an exactly private competitor with strictly smaller risk, and G9 verifies every rate condition before applying `(HP)`. No generated condition is consumed before production. The remaining work is to prove each accepted step at local-proof granularity, especially the full constants and finite indexing in G2--G4; this diagnostic is not itself proof evidence.
...
| G7: ideal coupling | Produces the exact realizable mixture and the sole overflow residual | `step_007` | G6 | `assump:minor-table` | Pool-law, off-overflow identity, and Bernstein calculation; `step-local` |
| G8: exchangeability/utility | Produces `(EX)`, `(UT)`, and the (7/8) auxiliary output | `step_008` | G6--G7 | PAC and fixed scales from the primitive assumptions | Conditional-independence, probability-mode, and coupling assembly; `step-local` |
| G9: specialization | Verifies `(HP)` admissibility and proves the main theorem and baseline | `step_009` | G5--G8 | Primitive delta scale | Constant choices, strict inequalities, and (k=1) trace; `step-local` |
...
| `step_007` | G7 | Consumes G6; feeds G8 and G9 | Iid pool construction, exact realizability, off-overflow equality, tail exponent | Sole coupling defect is overflow |
| `step_008` | G8 | Consumes G6--G7; feeds G9 | Iid-vector representation, conditional independence, `(EX)`, PAC-to-expectation, `(UT)`, Markov auxiliary | Permutation alone is not used as symmetry |
| `step_009` | G9 and final assembly | Consumes G5--G8 | Delta transfer, ceiling absorption, constants, contradiction, (k=1) recovery | Same (\delta_m), no relabeling |
...
| Ideal mixture, coupling, overflow | accepted-sketch derived target | G7 / `step_007` | G8--G9 | Realizability is proved from the primitive minor table |
| `(EX)`, `(UT)`, auxiliary statement | accepted-sketch derived target | G8 / `step_008` | G9 / final auxiliary use | Iid common experiments exclude hidden-designation dependence |
...
There is no missing derived-condition bridge. In particular, the hard prior, mixture realizability, exact privacy certificate, exchangeability identity, and rate admissibility are conclusions of named steps and are not promoted to theorem-facing assumptions.
...
| Bounded-loss conversion, conditional expectation, Markov | Probability-mode and arm-average conversions | Ideal mixture risk and nonnegative arm risks | PAC event, (0\le L\le1), conditional independence of (J) | Gives (G8.1), `(EX)`, `(UT)`, and (G8.2) | Direct current-notation derivations; no external theorem supplies exchangeability itself |
...
| Norm/metric mode | Zero-one empirical risk for G1--G4, expected finite-experiment zero-one risk for G5, population zero-one risk for G7--G8, and replacement-adjacency DP throughout. Every transfer states equality or an additive probability residual in that same risk. |
...
| Strict contradiction margin | (\alpha_0+\beta_0+p_{\rm ov}\le5/128+e^{-27/2}<1/20). PAC failure and overflow are separate defect classes and are not controlled by the delta threshold. |
| Baseline reduction | At (k=1), (n=2m+12), (R=m<n), overflow is zero, the mixture equals the sole experiment, and exact DP and `(EX)` remain unchanged. The output is the original (\Omega(\log^*N)) scale, not a weaker remainder statement. |
...
| Ideal mixture and overflow | Generated realizability/coupling certificate | Finite (m) positions, all (k\ge1) | (G7.1)--(G7.3), `(OF)` | Next-unused iid pools; (+12) occupancy reserve; minor table | Single overflow event | Positionwise equality is preserved off overflow; entire defect charged once by (1\{R>n\}) | algebraic coupling; reserve/ledger under declared scope | Primitive minor table; direct pool coupling; Bernstein | Minor table gives pointwise labels on the exact mixture support | Constructed and ideal samples are equal on (R\le n); loss residual (\le p_{\rm ov}) | step-local | G6 vector/kernel precedes pools; PAC utility not assumed | (R=0): simulated only; first hidden occurrence uses first pool record; (k=1): no overflow | Off-event equality plus (p_{\rm ov}\le e^{-27/2}) | Minor table; independent arm indices | Iid latent vector, pools | (R=n,R>n,\mu=6,k=1) traced | Without exact coupling, PAC risk would target the wrong sample law |
| Exchangeability `(EX)` | Same-target arm-to-mixture transfer | One finite uniform arm average | `(EX)` | Iid common experiment vector independent of (J) | Potential hidden-designation dependence; nonuniform arm risks | Exact finite average; no error multiplied by (k) | algebraic coupling | G6 iid vector and G7 ideal sample | Conditional independence supplies symmetry; permutation language alone is not used | Arm risks and mixture risk are exactly equal in zero-one population risk | step-local | Vector and ideal output law are produced without (J) before identity is consumed | (k=1) gives literal identity; highly nonuniform arm risks still average exactly | Pointwise equality in `(EX)` | Uniform independent (J) | Iid vector, ideal (h) | Deterministic (h), (k=1), nonuniform arms valid | Correlated latent arms could invalidate hidden-arm transfer |
| Utility `(UT)` and (7/8) output | Probability-mode conversion and upper side of contradiction | One PAC event, one overflow event, finite arm average | (G8.1), `(UT)`, (G8.2) | PAC guarantee; bounded loss; strict numerical slack | PAC failure (\beta_0); overflow (p_{\rm ov}); arm nonuniformity | Each event charged once; Markov handles finite arm average | structural upper comparison | Primitive PAC plus accepted G7/G8 controls | PAC supplies population-risk upper control, not structural lower content | Same experiment risk as `(HP)`; every residual has source and total (<1/20) | step-local | `(EX)` and `(OF)` precede `(UT)` | On PAC failure charge one; on good event Markov is active; (k=1) exact | (\alpha_0+\beta_0+p_{\rm ov}<1/20); (\Pr(L_J>8\alpha_0)\le1/8) | PAC constants | `(EX)`, `(OF)` | Zero overflow, deterministic output, (k=1) valid | Without strict slack, no contradiction to `(HP)` |
| Rate specialization and final contradiction | Quantitative bridge and theorem closure | Fixed sample, all (N\ge N_0) | (G9.1)--(G9.6) | Positive (a_{\rm th},a_\delta); strict `(HP)`/`(UT)` gap | Ceiling (13), delta scale conversion, small (N) | No recurrence; fixed arithmetic | structural lower/upper comparison | G5/G8 outputs plus primitive scale; boundary excluded only by allowed (N_0) | Available constants control exactly the claimed rate categories | Same (n,\delta_m) are consumed by `(HP)`; no relabeling or surrogate | step-local | All lower/upper interfaces precede G9 | At (N=N_0), additive (13) is already dominated; (m=1) satisfies comparisons | (G9.2), (G9.5), (G9.6), then strict contradiction | Primitive parameter scale | (a_{\rm th},a_\delta,(HP),(UT)) | (\varepsilon_0=1,\delta_m=0,m=1) valid | Missing comparison would make `(HP)` inapplicable |
| \(k=1\) baseline | Baseline invariance and exact one-chain recovery | Fixed \(k=1\) specialization | \(n=2m+12,R=m\), and `(EX)` is the identity | Sole-arm equality and exact privacy | Only fixed factor/additive sample loss; overflow vanishes | No arm accumulation | algebraic coupling; structural lower/upper comparison | Same active Alon mechanism and G5--G9 specialization | Mechanism supplies original \(\Omega(\log^*N)\) conclusion, not just zero defects | Sole produced and consumed threshold target are identical | step-local | One-arm prior/privacy/coupling precede final specialization | First position uses first real record once; stationary hidden designation \(J=1\) | \(R=m<n\), \(p_{\rm ov}=0\), mixture \(=Q_e\) | Primitive \(k=1\) minor and learner | One-arm prior/kernel | Exact/no-overflow boundary preserves full conclusion | A weaker stopped/remainder result would violate baseline invariance |
...
| Ideal iid realizable mixture and `(OF)` | G7 / `step_007` | Iid vector/pools, uniform indices, minor table, (+12) reserve | Sole event-level defect (R>n) controlled by Bernstein; no per-position residual | Exact sample equality off overflow; same target labels pointwise | (G7.2)--(G7.3), residual (\le p_{\rm ov}) | (27/2) exponent | G8--G9 | valid; coupling proof pending |
| `(EX)`, `(UT)`, and conditional (7/8) output | G8 / `step_008` | Iid common vector, ideal sample, PAC antecedent, `(OF)` | PAC failure and overflow separately bounded; nonuniform arms handled by exact averaging; no hidden fixed defect | Exact population-risk identity; constructed risk residual only (p_{\rm ov}) | (5/128+e^{-27/2}<1/20), Markov factor (8) | Iid symmetry and strict numerical slack | G9 / auxiliary use | valid; conditional assembly pending |
| Public (c,c_\delta,N_0) and final lower bound | G9 / `step_009` | G5 constants, exact privacy, `(UT)`, primitive scale | Ceiling, log conversion, delta transfer, small (N) each controlled | Same inherited (\delta_m), same (n), same theorem risk | (G9.1)--(G9.6) | (a_{\rm th},a_\delta) and strict risk gap | Final theorem | valid; arithmetic proof pending |
| (k=1) recovery | G6--G9 | Sole arm, one-arm prior, (R=m<n) | Overflow disappears; fixed wrapper/additive losses remain universal | Sole-arm experiment equals consumed target | (p_{\rm ov}=0), `(EX)` identity | Original one-chain hard value | Final baseline use | valid; specialization trace pending |

No exported interface combines unrelated defect classes under one parameter. In particular, the delta threshold controls the empirical DP and binary leakage terms, while PAC failure and overflow have separate fixed sources; neither is incorrectly absorbed by (c_\delta).
...
| Ideal mixture, coupling-success event, `(OF)` | G7 / `step_007` | G8--G9 | G6 -> pools/Bernstein -> G7 | derived | valid |
| `(EX)`, `(UT)`, auxiliary (7/8) certificate | G8 / `step_008` | G9 / final auxiliary statement | G6 + G7 + primitive PAC -> G8 | derived | valid |
| Delta/sample admissibility and public constants | G9 / `step_009` | Final theorem | G5 + G6 + G8 + primitive scale -> G9 | derived | valid |
| (k=1) baseline certificate | G6--G9 specialization | Final baseline conclusion | One-arm G5 prior -> G6 privacy -> G7 exact coupling -> G8 identity -> G9 | derived | valid |

Every consumer depends on an earlier producer. The produced empirical, mixture, and one-arm objects are either identical to their consumed target objects or connected by the explicit overflow residual in the theorem's zero-one-risk metric.
...
- Theorem-critical mechanism witness: passes at diagnostic granularity for every block. Positive/sign content comes from (g_{\rm gap}>0), the binary margin (a_0/M), the finite event-mass budget, exact one-use privacy, iid common experiments, and the strict (1/20) utility gap. No upper-bound-only source is used to assert a lower/sign claim.
...
- Residual-to-target adequacy: passes. G1 and G5 have zero residual; G2--G4 use the same restricted coordinates; G7 has the single explicit overflow residual; G8 compares the exact arm and mixture risks. There is no transformed, weighted, or reference-object surrogate lacking a same-target bridge.
- Exported-interface feasibility: passes at diagnostic granularity. All raw controls, margins, defect classes, dominance relations, and consumers are present. The local proofs must establish them, but no new bridge or changed output target is needed.
- Generated-condition provenance: passes. The prior, exact privacy certificate, mixture, coupling, exchangeability, utility, and admissibility inequalities all have legal named producers.
- Scope accumulation: passes. With-replacement multiplicities use the secrecy formula; product-DP leakage is (D\delta\le1/6); search failures are union-bounded by the choice of (D); coupling leakage is the one event (R>n). No persistent defect is asserted to vanish over an all-time or limiting scope.
- Noncircular closure: passes. The one-arm hard prior is produced only after learner-wise hardness; privacy is proved without utility; coupling is proved without exchangeability; `(EX)` precedes `(UT)`; G9 applies `(HP)` only after checking its conditions.
- Entry-state and boundary stress: passes. The diagnostic traces (\varepsilon=1), (\delta=0), odd (M), adjacent-rise endpoints, first binary probe, (R=0,R=n,R>n), unused changed records, PAC failure, (k=1), (m=1), and (N=N_0). No mechanism vanishes at an allowed boundary while its target remains false.
...
- Baseline invariance: passes. The first-transition trace at (k=1) has (R=m<n), exact sample equality, exact privacy, and `(EX)` as an identity; the full one-chain scale remains.
- Same-setting repair plausibility: the current global architecture needs no repair. If a local proof finds a numerical or indexing error inside the accepted interfaces, it should first be handled by the assigned `/proof-step`; if it changes a margin, dependency, output, mechanism, or boundary interface, it must return to `/proof-sketch`. No candidate `idea/theorem-contract defect` is currently supported.

## Global Gaps And Hard Steps

There is no unresolved global link, missing producer-consumer path, sketch/interface defect, or candidate idea/theorem-contract defect. The following local proof work remains. Each item is `step-local` under the unchanged accepted sketch.
...
7. **`step_007`: coupling and overflow.** Provisional units: conditional iid-pool lemma; pointwise realizability lemma; off-overflow equality; Bernstein application and derivative check. Mechanism witness: exact pool preservation plus the single finite-budget event `(OF)`.
8. **`step_008`: exchangeability and utility.** Provisional units: iid-vector representation; conditional-independence lemma; pointwise `(EX)`; PAC-to-expectation lemma; coupling-to-`(UT)` assembly; Markov auxiliary proposition. Mechanism witness: same-target equality `(EX)` and strict numerical slack in `(UT)`.
9. **`step_009`: public rate and baseline.** Provisional units: \(n\)-to-\(m\) delta comparison; ceiling/sample comparison; strict contradiction assembly; and \(k=1\) baseline proposition. Mechanism witness: (G9.1)--(G9.6) and exact one-arm entry trace.
