...
Under the definitions of \(Q_e\) and \(\mathcal R_n\) at the beginning of this subsection, let \(M=9n\), let \(B\) be any \(\{0,1\}^N\)-valued \(n\)-sample kernel, and let \(W_B\) be the wrapper of Proposition~\ref{prop:step-001-private-wrapper}. For every \(t \in [N]\) and every ordered tuple \(U=(u_1,\ldots,u_M) \in [N]^M\), define
\[
...
\]
The family \((\widetilde P_\ell)_{\ell\in[K]}\) remains pairwise exactly \((\varepsilon,\delta)\)-indistinguishable.
...
\]
Let \(s \sim s'\) mean that the ordered samples \(s,s' \in \mathsf S_{n,N}\) differ in at most one coordinate. Let
...
\(\widetilde B\) means that for every \(s \sim s'\) and every measurable
\(F\in\mathcal A\), both
\[
\widetilde B(F\mid s)
...
For completeness, construct the pools directly in the present finite
notation.  Put \(b:=9n\), so \(b\geq9\), and write
\(e_j=(t_j,U_j)\) with
\(U_j=(u_{j,1},\ldots,u_{j,b})\).  Because \(k\) is finite,
\([k]\times\mathbb N\) is countable; fix a bijection
\(\nu:[k]\times\mathbb N\to\mathbb N\).  On the Lebesgue probability
space \([0,1)\), write
\[
\omega=\sum_{r=1}^{\infty}D_r(\omega)b^{-r},
\qquad D_r(\omega)\in\{0,\ldots,b-1\},
\]
using the base-\(b\) expansion ending in zeros at the countable set of
points having two expansions.  If distinct digit positions
\(r_1,\ldots,r_d\) and digits \(a_1,\ldots,a_d\) are prescribed, then,
apart from those null endpoints, the corresponding event is a union of
\(b^{R-d}\) half-open base-\(b\) intervals of length \(b^{-R}\), where
\(R=\max_i r_i\).  Its probability is therefore \(b^{-d}\).  Hence
\((D_r)_{r\geq1}\) are mutually independent and uniform on
\(\{0,\ldots,b-1\}\).

Define, for \((j,s)\in[k]\times\mathbb N\),
\[
W_{j,s}(\omega)
:=\left(
u_{j,D_{\nu(j,s)}(\omega)+1},
\tau_{t_j}\!\left(u_{j,D_{\nu(j,s)}(\omega)+1}\right)
\right).
\]
Each \(W_{j,s}\) is uniform on the \(b\) labeled row occurrences of
\(U_j\), and hence has law \(Q_{e_j}\); duplicate row values merely
combine equal atoms.  Injectivity of \(\nu\) and the finite-cylinder
calculation show that every finite subfamily of the \(W_{j,s}\)
factorizes, which is exactly mutual independence for this countable
family.  This proves \eqref{eq:step-007-1} without an external product
citation.

Finally, \(\mathcal E_{n,N}^k\) is finite, so assigning this construction
on each latent fiber \(\mathbf e\) defines a conditional probability
kernel (with any version on a null fiber).  Its law depends only on
\(\mathbf e\).  Taking its product with the conditional law of \(J\)
and the finite law of the arm-index vector proves the asserted
conditional independence from \(J\) and \(\mathbf I\).  This is only
an enlargement of the probability space; every run reads at most \(m\)
cells from all pools combined.
