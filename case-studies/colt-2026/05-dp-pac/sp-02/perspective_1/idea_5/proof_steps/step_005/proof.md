...
- Assumptions used: Primitive \(L_\kappa\to\infty\); the setting-defined pointwise \(N_a\) formula and \(K_a\geq2\); accepted step_003 privacy identities.
...
- Primitive conditions:
  ...
  - The fixed privacy parameter satisfies \(\varepsilon_0\in(0,1)\). The setting definitions are
    ...
    \]
    These are setting-defined expressions, not additional assumptions. The setting also chooses
    \(K_a\geq2\), depending only on
    \(a,\alpha_0,\beta_0,C_{\mathrm{BLM}}\), as part of the pointwise definition (PS).
...
- Derived invariants supplied by accepted dependencies:
  - Accepted Lemma~\(\ref{lem:step-003-log-correction}\) and Proposition~\(\ref{prop:step-003-exact-privacy-schedule}\) supply \(u(L)\geq1\), \(u(L)\to\infty\) as \(L\to\infty\), and the exact identity
    ...
    =e^{-q^b-u(L)}.
    \]
  - Accepted step_004 remains a dependency because it is listed in the accepted sketch.
    Its conclusions are scoped to (SP), and none of those conclusions is used to prove (PN).
    In particular, this step does not use step_004 to obtain either the exact pointwise
    \(N_\kappa\) formula or the provenance of \(K_a\); both come directly from (PS) in the setting.
...
| ------------- | --------- | --------- | --------------------------- |
| unit_001 | lemma | Under the pointwise setting definition (PS), the setting domains, and accepted Lemma~\(\ref{lem:step-003-log-correction}\), for every \(\kappa\), with \(x_\kappa=q_\kappa^b\), \(u_\kappa=u(L_\kappa)\), and \(A_\kappa=K_a\varepsilon_0^{-2}x_\kappa(1+x_\kappa+u_\kappa)\), one has \(A_\kappa\geq6\), \(N_\kappa=\lceil A_\kappa\rceil\geq1\), and \(N_\kappa\leq2A_\kappa\). | Controls the setting-defined integer ceiling uniformly and fixes the exact scale used in every later inequality. |
...
### Direct setting definition: exact ceiling formula (PS)

The pointwise definition (PS) in the setting states that
\[
N_\kappa
=\left\lceil K_a\varepsilon_0^{-2}q_\kappa^b
  (1+T_a(q_\kappa,L_\kappa))\right\rceil,
\]
where \(K_a\geq2\) depends only on
\(a,\alpha_0,\beta_0,C_{\mathrm{BLM}}\), while
\(\varepsilon_0\in(0,1)\) is fixed. This definition is pointwise on the setting domain and does not require (SP).

### Accepted sketch dependency: step_004, not applied to (PN)

The accepted sketch lists step_004 as a dependency, so its accepted proof-review pair is retained in the dependency record. Its sample-subpower conclusions require (SP). No such conclusion is used in this step's proof of (PN), and the pointwise ceiling formula above is invoked directly from (PS), not from step_004.
...
Under the pointwise setting definition (PS), the setting domains, and accepted Lemma~\(\ref{lem:step-003-log-correction}\), for every \(\kappa\), define
\[
...
**Proof / justification.** Since \(a\geq1\), \(b=2a+2>0\). The basic setup gives \(q_\kappa\geq1\), hence \(x_\kappa=q_\kappa^b\geq1\). Accepted Lemma~\(\ref{lem:step-003-log-correction}\) gives \(u_\kappa\geq1\), so \(1+x_\kappa+u_\kappa\geq3\). The setting-defined constant satisfies \(K_a\geq2\), and \(\varepsilon_0\in(0,1)\) gives \(\varepsilon_0^{-2}\geq1\). Therefore
\[
A_\kappa
=K_a\varepsilon_0^{-2}x_\kappa(1+x_\kappa+u_\kappa)
\geq2\cdot1\cdot1\cdot3=6.
\]
Directly instantiating (PS), using
\(T_a(q_\kappa,L_\kappa)=x_\kappa+u_\kappa\), gives
\[
N_\kappa
=\left\lceil K_a\varepsilon_0^{-2}x_\kappa
  (1+x_\kappa+u_\kappa)\right\rceil
=\lceil A_\kappa\rceil.
\]
Thus \(N_\kappa\geq1\), and
\[
...
\(\delta_\kappa=e^{-x_\kappa-u_\kappa}\), while the pointwise setting definition (PS) supplies the exact integer \(N_\kappa=\lceil A_\kappa\rceil\) and the class-independent provenance of \(K_a\geq2\). Lemma~\(\ref{lem:step-005-ceiling}\) converts that exact ceiling into \(N_\kappa\leq2A_\kappa\) and \(N_\kappa\geq1\) with no asymptotic replacement. The accepted step_004 dependency is retained from the sketch, but none of its (SP)-scoped conclusions is used in this assembly of (PN).
...
- Admissibility conditions and auxiliary tolerances: \(a\geq1\), \(q_\kappa\geq1\), \(L_\kappa\geq0\), \(\lvert C_\kappa\rvert\to\infty\), \(\varepsilon_0\in(0,1)\), and setting-defined \(K_a\geq2\). The auxiliary \(\tau>0\) is only the definition of the limit, and \(c>0\) is only the source-allowance comparison parameter.
...
- \(A_\kappa\) is proof-local and is exactly the real quantity inside the setting-defined ceiling. \(N_\kappa=\lceil A_\kappa\rceil\) is public-facing and retains the setting's exact integer sample size.
...
- Assumption provenance is complete: the sequence size divergence and scalar ranges are primitive; the exact \(\delta\) and \(u\)-limit are supplied by accepted step_003; the exact pointwise \(N\) formula and \(K_a\geq2\) provenance come directly from (PS) in the setting; and the ceiling bound, factorization, finite supremum, \(u\)-decay, and allowance comparison are proved in named local units here. Accepted step_004 remains listed as a sketch dependency, but none of its (SP)-scoped conclusions is used for (PN). No generated event, learner state, stability invariant, or boundedness condition is assumed.
...
- This unit-attempt-2 repair used only the allowed evidence: the setting, the accepted sketch and sketch review, the accepted step_003 and step_004 proof-review pairs, and the triggering aggregate review. No proof history, global diagnostic, or other branch was consulted or used as proof evidence.
