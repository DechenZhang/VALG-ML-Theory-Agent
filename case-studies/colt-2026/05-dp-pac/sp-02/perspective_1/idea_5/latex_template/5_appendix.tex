...
Under Assumption~\ref{assump:polynomial-global-stability}
and Lemma~\ref{lem:step-003-log-correction}, let \(a\geq1\) be the
universal integer in the assumption, put \(b=2a+2\), fix
\(\varepsilon_0\in(0,1)\), and let \(K_a\geq2\) be the constant used
in the pointwise definition of \(N_a\).  Let
\(\{X_\kappa\}_{\kappa\in\mathbb N}\) be any sequence of domains and,
for every \(\kappa\), let
\(C_\kappa\subseteq\{0,1\}^{X_\kappa}\) be nonempty and finite.  Put
\(L_\kappa=\log|C_\kappa|\) and \(q_\kappa=q(C_\kappa)\), and define
\[
...
The pointwise definition has \(K_a\geq2\), and
\(\varepsilon_0^{-2}\geq1\).  Therefore
\[
A_\kappa
\geq2\cdot1\cdot1\cdot3=6.
\]
Indeed, the pointwise definition of \(N_a\) and
\(T_a(q_\kappa,L_\kappa)=x_\kappa+u_\kappa\) give
\[
N_\kappa
=\left\lceil
K_a\varepsilon_0^{-2}x_\kappa
(1+x_\kappa+u_\kappa)
\right\rceil
...
fix \(\varepsilon_0\in(0,1)\), let \(K_a\geq2\) be the constant used
in the pointwise definition of \(N_a\), let
\(\{X_\kappa\}_{\kappa\in\mathbb N}\) be any sequence of domains, and
let \(\{C_\kappa\}_{\kappa\in\mathbb N}\) satisfy
\(C_\kappa\subseteq\{0,1\}^{X_\kappa}\) nonempty and finite and
\(|C_\kappa|\to\infty\).  Put
\[
...
unbounded sequences.  No superpolynomial separation condition on
\(d_\kappa\) is required.
\end{proposition}
...
\label{eq:app-a121}
\end{equation}
Independently of those superpolynomial-separation consequences,
...
Its ceiling bound comes directly from the pointwise definition of
\(N_a\), and its limit uses only \(|C_\kappa|\to\infty\) to obtain
\(u(L_\kappa)\to\infty\); it does not consume any of the preceding
subpower conclusions.
...
\(\kappa\) and let \(\bar\delta\) be an allowed privacy schedule.  If
\begin{equation}
...
Moreover, under
Proposition~\ref{prop:step-005-source-allowance}, for every fixed
\(c>0\), the same learner is eventually
\begin{equation}
\left(
\varepsilon_0,
\frac{c}{N_\kappa^2\log(eN_\kappa)}
...
standard approximate-privacy comparison.  These privacy conclusions
derive their ceiling directly from the pointwise definition of
\(N_a\) and require only size divergence; the superpolynomial
condition is used separately above for the sample-rate conclusions.
