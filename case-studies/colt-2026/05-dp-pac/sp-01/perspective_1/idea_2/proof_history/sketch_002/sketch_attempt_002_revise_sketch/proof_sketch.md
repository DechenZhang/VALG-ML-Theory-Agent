...
- Sketch attempt: 2 (retry mode `revise_sketch`, `SEARCH_MODE=OPEN`)
- Binding setting: SHA-256 `8cdb060c6de66a9bdbf6a2ac920a48e2850550488e4e7e0926e61bd2a09ffe3c`.
- Triggering review: `proof_steps/step_002/review.md`, SHA-256 `375cb45b188861ffcbcefdd949c11a6562e8a000457ade261d1d000fde11a74c`, status `REVISE_SKETCH`, target `/proof-sketch`.

## Proof Roadmap

The selected framework is **Factorwise Yan learning with an ALMM expected-risk direct sum**, recorded in `technical_survey.md`. Its source provenance is Yan, arXiv:2505.06581v2, Algorithm `alg:improperlearner`, the labeled private-median/choosing/privacy/VC and tree-order components listed under `step_002`, together with Alon--Livni--Malliaris--Moran (ALMM), arXiv:1806.00949 / STOC 2019, `lem:bun`, `lem:reduction`, `thm:main`, `thm:shelah`, and `thm:ADPimpliesLD`. Yan's `thm:improper sample complexity` is audited as a non-output boundary, not used as quantitative proof authority.

The upper route first proves the product identities and then gives a new requested-confidence analysis of Yan's source algorithm at the exact displayed quota. It does **not** use the source's raw corollary with failure ((t+2)\beta_{\rm raw}) or its ensuing substitution (\beta_{\rm raw}=\beta_{\rm fac}/(t+2)). Instead, it fixes the raw confidence (b:=\beta_{\rm fac}/4), chooses a noncircular number (t_i) of independent blocks, and observes from the proof of Yan's `lem:accuracy h good` that the final deterministic risk comparison needs only one good block on the comparison side of the selected median layer. Private-median success gives at least (t_i/6) score-side blocks and at least (t_i/6) comparison-side blocks. A choosing-mechanism slack of at most (t_i/12) identifies the unique positive-score true-path point, while concentration of the independent blockwise VC-generalization failures makes fewer than (t_i/12) of all blocks bad except with probability (\beta_{\rm fac}/2). Hence at least one of the comparison-side blocks is good. Thus two private-subroutine failures of size (b) plus this concentration failure total at most (\beta_{\rm fac}), with no (t_i)-dependent confidence substitution and no outer (\log(t_i+2)) loss. The source proof parameter (TD(C_{i,f_i})+1) handles (d_i=1), and a uniform-permutation prefix wrapper fits the calibrated source input inside the exact quota (q_i).
...
- Required bridge or simplification obligations: `step_002` must (i) map Yan's (TD+1) proof parameter to (s_i), (ii) define a noncircular calibrated (t_i) whose private-median threshold, choosing slack, block-failure concentration, and total source prefix all fit inside (q_i), and (iii) prove requested confidence without the source's ((t+2)b) union bound; `step_004`--`step_005` must turn conditional factor guarantees into a global high-probability risk bound; `step_006` must prove every ceiling/logarithm domination in the public quota sum.
...
- Yan factor mechanism, confidence-calibration event, and quota certificate: derived from exact source components plus independent-block concentration in `step_002`; consumed by `step_003`, `step_005`, and `step_006`. Neither the source's raw ((t+2)b) corollary nor its final displayed big-O theorem is used as quantitative proof authority.
...
- Step ID: `step_002`
- Claim class: cited-tool wrapper, noncircular confidence calibration, and exact factor quota/utility/privacy interface.
- Theorem role: supplies every upper-bound factor mechanism.
- Mechanism source: Chao Yan, arXiv:2505.06581v2 (the arXiv source metadata records version 2), Algorithm `alg:improperlearner`; `thm:private median`; `lem:choosing mechanism`; `thm:privacy-OPTPrivateLearner`; `thm:learn vc`; `lem: must comparable`; `lem:determined point in c`; `lem:order in h`; `lem:order of y`; and the proof of `lem:accuracy h good`. The threshold-dimension comparison is the displayed theorem immediately preceding `cor: threshold dimension and littlestone dimension`. The raw corollary with failure ((t+2)\beta) and `thm:improper sample complexity` are recorded non-outputs for the exact quota because their stated substitution leaves the triggering review's unresolved fixed point; they are not used to certify the rate.
- Source-to-claim adequacy: fix (f_i\in C_i), put (U_i:=TD(X_i,(C_i)_{f_i})+1), and use source internal privacy (\varepsilon_p,\delta_p):=(\varepsilon_{\mathrm{fac}}/2,\delta_{\mathrm{fac}}/2) and fixed raw confidence (b:=\beta_{\mathrm{fac}}/4). The source objects ((\mathcal X,\mathcal C,U,N,\alpha,b,\varepsilon_p,\delta_p)) map to ((X_i,C_i,U_i,N_i,\alpha_{\mathrm{fac}},b,\varepsilon_p,\delta_p)). `assump:vc-one-factors` discharges VC one and finite LD; realizability and arbitrary (D_i) match the source PAC model; `assump:global-privacy-range` makes the parameters positive and (\varepsilon_p<2), as required by `lem:choosing mechanism`. Yan's distinguishability conventions are discharged by public recordwise quotienting of points with identical (C_i)-label vectors, deletion of duplicate function representations, and hard-coding coordinates on which all concepts agree; pulling the output back to (X_i) preserves risk exactly, and fixed recordwise preprocessing preserves replacement adjacency on arbitrary inputs. The source DP definition is one-entry replacement on all datasets, whereas utility is only for i.i.d. realizable data, exactly as consumed here. Its output is improper.

  The endpoint and calibration wrapper is a direct derivation from these source statements. Nonconstancy gives (U_i\ge2), while the source threshold-dimension comparison gives (U_i\le2^{d_i+1}+1); hence (1\le\log_2^*U_i\le c_s s_i) for a universal (c_s), including (d_i=1). Let
  \[
  L_i:=\log\frac{s_i}{\varepsilon_{\mathrm{fac}}\beta_{\mathrm{fac}}\delta_{\mathrm{fac}}},
  \qquad
  t_i:=\left\lceil K_t\frac{s_iL_i^2}{\varepsilon_{\mathrm{fac}}}\right\rceil
  \]
  for one universal (K_t). Choose (K_t) large enough that: (a) (t_i) exceeds `thm:private median`'s (U_i,1/3,b,\varepsilon_p,\delta_p) sample threshold; (b) the literal `lem:choosing mechanism` loss
  \[
  \Delta_i:=\frac{16}{\varepsilon_p}
  \log\frac{4t_i}{b\varepsilon_p\delta_p}
  \]
  satisfies (\Delta_i\le t_i/12); and (c), for independent block generalization failures (Z_{ij}) with (\Pr[Z_{ij}=1]\le b), Hoeffding's inequality gives
  \[
  \Pr\!\left[\sum_{j=1}^{t_i}Z_{ij}\ge t_i/12\right]
  \le \exp\{-2t_i(1/12-b)^2\}
  \le \beta_{\mathrm{fac}}/2.
  \]
  This choice is noncircular: (t_i) is explicit; (\log t_i+\log(1/(b\varepsilon_p\delta_p))\le c_tL_i) because (s_i\ge2), (\varepsilon_{\mathrm{fac}}\le1/20), (b=1/16384), and (\delta_{\mathrm{fac}}<1/2); and the same raw lower bound on (s_iL_i^2/\varepsilon_{\mathrm{fac}}) dominates the fixed Hoeffding threshold. No confidence parameter depends on (t_i).
  ...
  \qquad N_i:=t_im_i.
  \]
  The fixed ratios (b/\beta_{\mathrm{fac}}=1/4), (\varepsilon_p/\varepsilon_{\mathrm{fac}}=1/2), and (\delta_p/\delta_{\mathrm{fac}}=1/2), the endpoint bound above, and the displayed (t_i) envelope imply (N_i\le q_i) once the valid universal big-O envelope (K) in `setting.md` is chosen large enough. Every discarded term is explicit: (\log(s_i/(\varepsilon_pb\delta_p))=L_i+\log16\le2L_i), (\log(5/b)=\log(20/\beta_{\mathrm{fac}})\le2\log(5/\beta_{\mathrm{fac}})), and the two ceilings are dominated by the positive displayed quota factors. Thus no (\log(t_i+2)) or third logarithm is introduced.

  Run `alg:improperlearner` on a uniformly permuted size-(N_i) prefix of the (q_i)-multiset and ignore the remainder. Coupling the same permutation on adjacent quota inputs makes the prefixes identical or one-replacement adjacent; on i.i.d. data the prefix remains i.i.d. Source `thm:privacy-OPTPrivateLearner` therefore yields ((2\varepsilon_p,2\delta_p)=(\varepsilon_{\mathrm{fac}},\delta_{\mathrm{fac}}))-DP. For utility, condition on the data-independent permutation/partition, and write (B_{ij}) and (h_{ij}) for source block (j)'s deterministic-point set and induced hypothesis. The disjoint blocks are independent i.i.d. samples, and `thm:learn vc` makes each block bad with probability at most (b). On private-median success, `lem:determined point in c`, `lem:order of y`, and same-layer uniqueness from `lem: must comparable` give one true-path layer point with score at least (t_i/6) from the score-side blocks, and at least (t_i/6) blocks lie on the opposite comparison side. On choosing success, the selected point has score at least (t_i/6-\Delta_i\ge t_i/12)>0; since every other point at that layer has score zero, the selected point is the true-path point. If fewer than (t_i/12) blocks are bad, at least one of the (t_i/6) comparison-side blocks is good. For that block the inclusion relation in the final paragraph of the proof of `lem:accuracy h good` gives (B_{ij}\subseteq I_{\rm good}) and then (R_{D_i}(h_{\rm good},c_i)\le R_{D_i}(h_{ij},c_i)\le\alpha_{\rm fac}). Thus the deterministic comparison uses only this one good comparison witness, not the source corollary's stronger event that all (t_i) blocks are good. Averaging over the partition preserves the Hoeffding bound, and the three failure probabilities are at most (b+b+\beta_{\mathrm{fac}}/2=\beta_{\mathrm{fac}}). Known non-outputs: these sources do not prove the product theorem, padded-input utility, a cardinality bound, or the lower clause.
...
- Key positive/control term or structural source: Yan's VC-one tree path makes the positive-score layer candidate unique; the private median supplies at least (t_i/6) score-side blocks and (t_i/6) comparison-side blocks; the choosing guarantee selects the positive-score true-path point; and independent blockwise VC events ensure one comparison-side witness is good.
- Opposing defect terms: the raw ((t_i+2)b) union loss, dependence of source internal (t) on raw confidence, the outer (\log(t_i+2)) that a fixed-point substitution would create, implicit private-median constants, choosing slack, the printed (d_i=1) endpoint, input ordering, unused quota records, and padding. Fixed (b), concentration, the explicit (t_i), universal (K,K_t), the (TD+1) wrapper, symmetrized prefixing, and the downstream padding split control these classes separately.
- Closure/dominance/absorption relation: (\Delta_i\le t_i/12) makes the chosen score positive and therefore identifies the unique true-path point; fewer than (t_i/12) bad blocks leave a good witness among the at least (t_i/6) comparison-side blocks; (2b+\beta_{\mathrm{fac}}/2=\beta_{\mathrm{fac}}) closes confidence. The inequalities (N_i\le q_i) above close every inner and outer logarithm at squared-log scale. No utility is claimed when padding occurs.
- Accumulation behavior / scope compatibility: the only repeated scope is the finite family of (t_i) independent blocks. Bad indicators have adversarial sign but are controlled by the explicit Hoeffding tail; private-subroutine failures are unioned only twice. Privacy is not accumulated over the blocks because each source algorithm is one composed factor mechanism, and privacy is not accumulated over all factors here; affected-input composition is deferred to `step_003` with an at-most-two certificate.
- Obligation locality classification: `step-local`.
- Noncircular closure status: fixed (b) produces the explicit (t_i); source private median/choosing and independent block concentration produce the factor event before routing and utility consume it. No definition refers back to requested confidence through (t_i). Dependency path is `step_001 -> step_002 -> {step_003, step_005, step_006}`.
- Entry-state / first-update stress result: at (d_i=1), (2\le U_i\le5), so the private-median domain and (t_i) are positive and finite; if enough comparison-side blocks were bad the inclusion witness could fail, which is why the success event requires fewer than (t_i/12) bad blocks against a comparison set of size at least (t_i/6). At (q_i>N_i), the first transition is the coupled symmetrized prefix and changes at most one retained row.
- Baseline conclusion preserved: for (k=1), this is the same Yan improper algorithm with a source-valid fixed-confidence reanalysis and harmless symmetrized truncation, not a proper, finite-class, extra-logarithm, or weakened-confidence surrogate.
- Producer-consumer provenance: the (TD+1) envelope, explicit (t_i,N_i), two-sided median event, positive-score selection event, comparison-witness event, and factor DP/PAC interface are produced in `step_002`; consumers are `step_003`, `step_005`, `step_006`, and final upper assembly.
- Null or boundary regime tested: (d_i=1), (\delta\uparrow1), arbitrarily small positive (\delta), infinite (C_i), improper output, score-side/comparison-side orientation, exact bad-count threshold, unused prefix remainder, and nonrealizable padded inputs. Only DP, never utility, is used on padded inputs.
- Target conclusion false or theorem-critical obstruction present if source vanishes: if the median supplied only the score side and no linearly large comparison side, concentration would not replace the all-block union bound; if the final comparison required every block to be good, the fixed-point defect would persist. The two-sided median guarantee and one-good-comparison-witness inference exclude both regimes.
- Repair route if source is unsupported: None after the explicit source-component wrapper. If either the positive-score identification or the one-good-comparison-witness deterministic comparison is rejected on source inspection, the defect is a candidate `idea/theorem-contract defect`, because the unchanged log-squared quota cannot absorb the literal (\log(t_i+2)) term.
...
| Factor DP/PAC wrapper at quota (q_i) | `step_002`; Yan arXiv:2505.06581v2 `alg:improperlearner`, `thm:private median`, `lem:choosing mechanism`, `thm:privacy-OPTPrivateLearner`, `thm:learn vc`, `lem:order of y`, and proof of `lem:accuracy h good`; independent-block Hoeffding | `assump:vc-one-factors`; positive factor parameters; (2\le U_i=TD+1\le2^{d_i+1}+1); fixed (b=\beta_{\rm fac}/4); explicit (t_i=\lceil K_ts_iL_i^2/\varepsilon_{\rm fac}\rceil), (\Delta_i\le t_i/12), and (N_i=t_im_i\le q_i) | Raw ((t_i+2)b) union loss is bypassed by concentration; source-confidence fixed point and outer (\log(t_i+2)) are absent; endpoint, implicit constants, two ceilings, order/prefixing controlled explicitly; padding utility remains uncontrolled and is excluded | Produced and consumed factor use the same (C_i,D_i,c_i,R_{D_i}); a uniformly permuted i.i.d. prefix is still i.i.d., so no target or distribution residual | Median gives at least (t_i/6) score-side blocks and (t_i/6) comparison-side blocks; choosing loss at most (t_i/12) identifies the unique positive-score true-path point; fewer than (t_i/12) total bad blocks except with probability (\beta_{\rm fac}/2) leave one good comparison witness; (b+b+\beta_{\rm fac}/2=\beta_{\rm fac}); fixed-ratio log inequalities give (N_i\le q_i) | Unique true-path layer point, explicit choosing slack, two-sided median mass, Hoeffding margin (1/12-b>0), and sufficiently large valid universal (K,K_t) | `step_003`, `step_005`, `step_006` | None |
...
| Calibrated symmetrized Yan factor interface at exact (q_i), including (t_i,N_i), two-sided median mass, positive-score selection, comparison witness, and bad-block event | `step_002`; exact Yan source components and direct Hoeffding wrapper mapped in the mechanism witness; the raw ((t+2)b) corollary and final displayed-rate theorem are not proof authorities | `step_003`, `step_005`, `step_006` | Factor privacy, requested-confidence conditional utility, and exact squared-log quota for the global upper bound | `step_001 -> step_002 -> consumers -> step_015` | derived | None |
...
| `step_002` | For every factor, define the fixed-raw-confidence calibration (b=\beta_{\rm fac}/4), (U_i=TD(C_{i,f_i})+1), explicit (t_i), block size (m_i), and (N_i=t_im_i\le q_i); prove private-median feasibility and two-sided mass, choosing loss (\Delta_i\le t_i/12) and positive-score identification, independent bad-block tail at most (\beta_{\rm fac}/2), and one-good-comparison-witness accuracy; instantiate the uniformly permuted-prefix Yan mechanism, which is ((\varepsilon/2,\delta/2))-DP on every size-(q_i) dataset and has (R_{D_i}\le1/64) with probability (1-1/4096) on every unpadded realizable i.i.d. input. | `step_001` | Primitive: `assump:vc-one-factors`, `assump:global-privacy-range`; cited-source hypotheses mapped explicitly; independent block events are derived from the i.i.d. factor sample | Replace the invalid (\beta_{\rm raw}=\beta_{\rm fac}/(t+2)) fixed point by a noncircular two-sided-median-vs-bad-count argument while retaining the exact squared-log quota; also verify (d_i=1), replacement adjacency, unused-record prefixing, and multiset/order convention. | Yan arXiv:2505.06581v2 `alg:improperlearner`, `thm:private median`, `lem:choosing mechanism`, `thm:privacy-OPTPrivateLearner`, `thm:learn vc`, `lem:order of y`, proof of `lem:accuracy h good`, and the displayed (TD) comparison before `cor: threshold dimension and littlestone dimension`; Hoeffding for independent nonidentical Bernoulli failures; random-permutation prefix coupling. | Exact factor mechanism, confidence-calibration event, and quota certificate; no padded utility claim and no use of the raw ((t+2)b) corollary. | Upper objective: exposed (s_i,q_i,\varepsilon,\delta); all internal confidences and (t_i,N_i) explicit; universal (K,K_t); high-probability factor risk; fixed-sample, distributional-risk mode; prove (N_i\le q_i) without an extra logarithm. | PENDING |
...
None. The triggering source-interface defect is repaired in `step_002`: raw confidence is fixed at (\beta_{\rm fac}/4), the source's all-block union event is replaced by a two-sided-median-vs-bad-count concentration event, and the explicit (t_i) envelope closes private median, choosing slack, (d_i=1), and (N_i\le q_i) without (\log(t_i+2)). The expected-risk ALMM interface remains derived from exact labeled source-proof components in `step_008` before minimax or direct-sum work.
...
- Source check: Yan is identified as arXiv:2505.06581v2 with exact algorithm/lemma labels. Its raw ((t+2)b) corollary and `thm:improper sample complexity` are explicitly not used to certify the exact rate; `step_002` instead uses the labeled primitive mechanisms, source structural lemmas, both median sides, and the one-good-comparison-witness inference visible in the final paragraph of the proof of `lem:accuracy h good`. ALMM is identified as arXiv:1806.00949 / STOC 2019 with exact labels. No theorem number was invented for Yan's unnumbered introductory theorem or its unlabeled threshold-dimension comparison.
...
- Rate check: `step_002` exposes (b,t_i,m_i,N_i,\Delta_i), every fixed-ratio logarithm, the Hoeffding threshold, and (N_i\le q_i); no hidden (\log(t_i+2)) remains. Other ceilings, hidden-constant permissions, confidence conversion, overflow loss, and the final rational contradiction are assigned to explicit steps.
