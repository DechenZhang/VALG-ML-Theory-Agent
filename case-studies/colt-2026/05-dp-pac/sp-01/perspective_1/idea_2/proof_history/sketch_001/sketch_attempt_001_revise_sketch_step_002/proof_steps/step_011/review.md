...
- Proof validity: For $p=0$, $L=0$ almost surely and the quota is $8$, so the overflow event is empty.
...
- Proof validity: From $4\mu\le8$, the proof correctly obtains $m=8$ and the integer threshold $L\ge9$. Applying `unit_001` at $t=9$ is valid, and the derivative $g'(\mu)=9/\mu-1>0$ shows that the exponent is maximized at $\mu=2$.
...
- Proof validity: Integer-valuedness gives $\{L>m\}=\{L\ge\lceil4\mu\rceil+1\}\subseteq\{L\ge4\mu+1\}$. Substitution into `unit_001` yields the stated $f(\mu)$. The derivative $f'(\mu)=4\log(4+1/\mu)-1/\mu-3$ is correct, and $\log4>1$ plus $\mu\ge2$ gives $f'(\mu)>1/2$, so the minimum is attained at $\mu=2$.
