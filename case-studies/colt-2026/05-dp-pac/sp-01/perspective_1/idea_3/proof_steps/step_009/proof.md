...
| ------------- | --------- | --------- | --------------------------- |
| `unit_001` | lemma | Under Assumptions~\ref{assump:canonical-product} and \ref{assump:vc-one-factors}, accepted Lemma~\ref{lem:step-001-logstar}, accepted Proposition~\ref{prop:step-007-additive-vc}, and accepted Proposition~\ref{prop:step-008-factor-floor}, take \(a_{\rm VC}=1/2\), \(a_{\rm L}=\min\{1,a_0/20\}\in(0,1]\), \(R_0=\lceil32/a_{\rm L}\rceil\), \(c_{\rm low}=\min\{a_{\rm VC}/[128(R_0+2)],a_{\rm L}/64\}\), and \(H=\{i:r_i>R_0\}\). If a fixed candidate learner has the global PAC guarantee and \(n<c_{\rm low}M\), then \(w_L=\sum_{i\notin H}\pi_i<1/128\), and consequently \(H\ne\varnothing\). | Pays for every low factor, including \(r_i=R_0\), by the additive VC floor and handles the all-low branch. |
| `unit_002` | lemma | Under accepted Lemma~\ref{lem:step-001-logstar}, accepted Proposition~\ref{prop:step-008-factor-floor}, and Lemma~\ref{lem:step-009-low-mass}, take its exact constants and set \(a_{\rm VC}=1/2\), \(0<a_{\rm L}\le1\), \(R_0=\lceil32/a_{\rm L}\rceil\), \(c_{\rm low}=\min\{a_{\rm VC}/[128(R_0+2)],a_{\rm L}/64\}\le a_{\rm L}/64\), and \(H=\{i:r_i>R_0\}\). If \(n<c_{\rm low}M\) and \(i\in H\), then \(4n\pi_i<a_{\rm L}r_i/8\), \(\lceil4n\pi_i\rceil<a_{\rm L}r_i\), \(8<a_{\rm L}r_i\), and hence the exact setting budget satisfies \(m_{n,i}<a_{\rm L}r_i\). | Explicitly dominates the additive \(+2\), ceiling-one defect, floor eight, and every equality boundary. |
| `unit_003` | proposition | Under Assumption~\ref{assump:candidate-delta-budget}, accepted Proposition~\ref{prop:step-008-factor-floor}, and Lemma~\ref{lem:step-009-active-budget}, let \(R_0,c_{\rm low},H\) be exactly the constants and set fixed by Lemma~\ref{lem:step-009-low-mass}, fix the same one candidate \(n\), and assume \(n<c_{\rm low}M\). Then every \(i\in H\) satisfies \(m_{n,i}\ge8\), \(m_{n,i}<a_{\rm L}r_i\), and \(0<\delta\le c_\delta/[m_{n,i}^2\log(m_{n,i}+1)]\) at that same fixed candidate. | Supplies the exact numerical ALMM eligibility factor by factor, with equality at the delta cap allowed and no factor union bound. |
...
No external paper result is invoked in this step. The ALMM source analysis is encapsulated in accepted Proposition~\ref{prop:step-008-factor-floor}; this artifact uses only its reviewed current-notation interface.
...
| Lemma~\ref{lem:step-009-low-mass} | Under the fixed-candidate PAC and contradiction hypotheses, accepted Proposition~\ref{prop:step-007-additive-vc} fixes \(a_{\rm VC}=1/2\), accepted Proposition~\ref{prop:step-008-factor-floor} fixes \(a_{\rm L}=\min\{1,a_0/20\}\in(0,1]\), and the exact constants in (4)-(5) give \(w_L<1/128\) and \(H\ne\varnothing\). | Accepted structural, additive-VC, and factor-floor universal-constant results; proved in `unit_001`. | The low-complexity residual and all-low exclusion. |
| Lemma~\ref{lem:step-009-active-budget} | Under accepted Proposition~\ref{prop:step-008-factor-floor} and the exact constants and set fixed by Lemma~\ref{lem:step-009-low-mass}, namely \(a_{\rm VC}=1/2\), \(0<a_{\rm L}\le1\), \(R_0=\lceil32/a_{\rm L}\rceil\), \(c_{\rm low}=\min\{a_{\rm VC}/[128(R_0+2)],a_{\rm L}/64\}\le a_{\rm L}/64\), and \(H=\{i:r_i>R_0\}\), the conditions \(n<c_{\rm low}M\) and \(i\in H\) imply \(m_{n,i}<a_{\rm L}r_i\), while \(m_{n,i}\ge8\) by definition. | Accepted log-star comparison, accepted factor-floor constant range, the named low-mass interface, and direct arithmetic; proved in `unit_002`. | The strict ALMM subcritical-budget hypothesis. |
| Proposition~\ref{prop:step-009-almm-eligibility} | With \(R_0,c_{\rm low},H\) exactly as fixed by Lemma~\ref{lem:step-009-low-mass}, at the same fixed candidate satisfying Assumption~\ref{assump:candidate-delta-budget} and the local hypothesis \(n<c_{\rm low}M\), every active factor has the exact budget, strict subcriticality, and factorwise delta cap required by the accepted ALMM floor. | Assumption~\ref{assump:candidate-delta-budget}, the explicit fixed-candidate hypothesis \(n<c_{\rm low}M\), and Lemma~\ref{lem:step-009-active-budget}; proved in `unit_003`. | The complete hard-factor eligibility certificate exported downstream. |
...
Statement:
Under Assumptions~\ref{assump:canonical-product} and \ref{assump:vc-one-factors}, accepted Lemma~\ref{lem:step-001-logstar}, accepted Proposition~\ref{prop:step-007-additive-vc}, and accepted Proposition~\ref{prop:step-008-factor-floor}, fix a candidate \(n\in\mathbb N\) and a learner satisfying the global \((1/16,1/16)\) PAC guarantee. Let \(a_{\rm VC}=1/2\) be fixed by accepted Proposition~\ref{prop:step-007-additive-vc}, and let \(a_{\rm L}=\min\{1,a_0/20\}\in(0,1]\) be fixed by accepted Proposition~\ref{prop:step-008-factor-floor}. Define, before the class, learner, and candidate are chosen,
\[
...
Statement:
Under accepted Lemma~\ref{lem:step-001-logstar}, accepted Proposition~\ref{prop:step-008-factor-floor}, and Lemma~\ref{lem:step-009-low-mass}, take the exact theorem-style inputs \(a_{\rm VC}=1/2\), \(0<a_{\rm L}\le1\), \(R_0=\lceil32/a_{\rm L}\rceil\), \(c_{\rm low}=\min\{a_{\rm VC}/[128(R_0+2)],a_{\rm L}/64\}\le a_{\rm L}/64\), and \(H=\{i:r_i>R_0\}\) fixed there. Fix the same candidate \(n\) and assume \(n<c_{\rm low}M\). Then every \(i\in H\) satisfies
\[
...
Proof / justification:
Accepted Proposition~\ref{prop:step-008-factor-floor} supplies \(0<a_{\rm L}\le1\), while Lemma~\ref{lem:step-009-low-mass} fixes the exact \(R_0,c_{\rm low},H\) just stated. Fix \(i\in H\). Since \(r_i>R_0\), definition (4) gives
\[
a_{\rm L}r_i
>a_{\rm L}R_0
\ge32.
...
Statement:
Under Assumption~\ref{assump:candidate-delta-budget}, accepted Proposition~\ref{prop:step-008-factor-floor}, and Lemma~\ref{lem:step-009-active-budget}, let \(R_0,c_{\rm low},H\) be exactly the constants and set fixed by Lemma~\ref{lem:step-009-low-mass} and displayed in (4)-(6), fix the same one candidate \(n\), and assume the local hypothesis \(n<c_{\rm low}M\). Then, for every \(i\in H\),
\[
m_{n,i}\in\mathbb N,
\qquad
m_{n,i}\ge8,
...
Proof / justification:
The proposition hypothesis is exactly \(n<c_{\rm low}M\), and its \(R_0,c_{\rm low},H\) are the same objects fixed in Lemma~\ref{lem:step-009-low-mass} and consumed by Lemma~\ref{lem:step-009-active-budget}. Thus, for each \(i\in H\), the conditions \(n<c_{\rm low}M\) and \(i\in H\) are explicitly discharged before Lemma~\ref{lem:step-009-active-budget} is invoked. The budget definition (12) makes \(m_{n,i}\) an integer and gives \(m_{n,i}\ge8\); that lemma then gives the strict inequality \(m_{n,i}<a_{\rm L}r_i\), proving (20).
...
Accepted Proposition~\ref{prop:step-008-factor-floor} fixes \(a_{\rm L},c_\delta\) universally, and accepted Proposition~\ref{prop:step-007-additive-vc} fixes \(a_{\rm VC}=1/2\), all before the current class, learner, and candidate are considered. Definitions (4)-(5) therefore make \(R_0\) and \(c_{\rm low}\) universal constants with no class, candidate, privacy-parameter, or factor dependence.
...
\tag{23}
\]
The standing hypothesis \(n<c_{\rm low}M\) concerns this same candidate and the exact same \(R_0,c_{\rm low},H\), so it explicitly discharges the local antecedent of Lemma~\ref{lem:step-009-active-budget} before that lemma is invoked. The lemma then proves, for every active factor and at its exact setting-defined budget,
\[
...
Finally, retain the exact same \(R_0,c_{\rm low},H\) and the standing fixed-candidate hypothesis \(n<c_{\rm low}M\). This explicitly discharges the local antecedent required when Proposition~\ref{prop:step-009-almm-eligibility} invokes Lemma~\ref{lem:step-009-active-budget}. The proposition applies Assumption~\ref{assump:candidate-delta-budget} at this same \(n\), not at a changed or asymptotic candidate, and proves
\[
0<\delta\le
...
- Diagnostic boundary: no `global_proof.md`, `global_proof_review.md`, or `proof_history` artifact was read or used. The live same-step proof and triggering same-step review were read only for this scoped repair; the review supplied no new mathematical premise or authority to change the binding row.
