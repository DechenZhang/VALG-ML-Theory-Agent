...
\tag{4}
\]
let \(U_i=Q_i/{\sim_i}\), and write \(\tau_i:Q_i\to U_i\). Since \(Q_i\) is finite or countable, so is \(U_i\); every \(g\) descends uniquely to \(U_i\). Quotienting by (4) preserves every finite pattern and finite Littlestone tree by the same two-way representative argument as accepted Lemma~\ref{lem:step-001-quotient-invariance}. Thus (3) still holds on \(U_i\).
...
Statement:
Let \(F\) be finite or countable, equip \(\mathsf O:=F\cup\{\bot\}\) with its discrete sigma-field \(2^{\mathsf O}\), let \(t\in\mathbb N\), and let a database \(D=(v_1,\ldots,v_t)\) take values in \(\mathsf O\). Fix
\[
0<b<1,
\qquad
0<\varepsilon_c\le2,
...
\tag{12}
\]
Put \(\operatorname{OPT}(D):=\sup_{f\in F}q(D,f)\), with the convention \(\operatorname{OPT}(D)=0\) when \(F=\varnothing\); because the positive support is finite, this supremum is otherwise a maximum.
Run the exact BNSV thresholded kernel `alg:choosing` with parameters \((b,\varepsilon_c,\delta_c,k=1)\): form
\(\widetilde{\operatorname{OPT}}=\operatorname{OPT}+\operatorname{Lap}(4/\varepsilon_c)\), return \(\bot\) when
\[
\widetilde{\operatorname{OPT}}
<\frac8{\varepsilon_c}
  \log\!\left(\frac4{b\varepsilon_c\delta_c}\right),
\]
and otherwise run its \(\varepsilon_c/2\)-exponential-mechanism stage on the finite positive support
\(G(D)=\{f:q(D,f)>0\}\). Totalize the still-empty positive-support branch by returning \(\bot\) immediately when \(G(D)=\varnothing\), and call the resulting total rule \(A\). It is a Markov kernel from \((\mathsf O^t,2^{\mathsf O^t})\) to \((\mathsf O,2^{\mathsf O})\), and it is replacement-\((\varepsilon_c,\delta_c)\)-DP for every event in \(2^{\mathsf O}\). Moreover, defining \(q(D,\bot)=0\), with probability at least \(1-b\) its output \(\hat f\) satisfies
\[
q(D,\hat f)
...
For each fixed \(D\), the noisy threshold, finite-support exponential stage, and immediate-\(\bot\) branch define a probability measure on \((\mathsf O,2^{\mathsf O})\). Since \(\mathsf O^t\) is countable discrete, \(D\mapsto A(D,E)\) is measurable for every \(E\subseteq\mathsf O\). Thus the total rule is the Markov kernel asserted in the statement.

It remains to verify the new immediate-\(\bot\) branch rather than appeal to BNSV for an undefined empty exponential support. Let \(D_0,D_1\) be replacement neighbors with \(G(D_0)=\varnothing\). If \(G(D_1)=\varnothing\), both totalized laws are the point mass at \(\bot\). Otherwise (14) implies that \(G(D_1)=\{f_0\}\), \(q(D_1,f_0)=1\), and \(\operatorname{OPT}(D_1)=1\). The parameter ranges give
\[
1
<\frac4{\varepsilon_c}
  \log\!\left(\frac4{b\varepsilon_c\delta_c}\right),
\]
because \(4/(b\varepsilon_c\delta_c)>2\), \(4/\varepsilon_c\ge2\), and \(2\log2>1\). Hence \(D_1\) lies in Case (a) of the BNSV privacy proof. More explicitly, if \(p:=\Pr[A(D_1)\ne\bot]\), then passage of the noisy threshold requires
\[
\operatorname{Lap}(4/\varepsilon_c)
>\frac4{\varepsilon_c}
  \log\!\left(\frac4{b\varepsilon_c\delta_c}\right),
\]
and therefore
\[
p
\le\frac12\exp\!\left(-\log\frac4{b\varepsilon_c\delta_c}\right)
=\frac{b\varepsilon_c\delta_c}{8}
\le\delta_c.
\tag{14a}
\]
For every \(E\subseteq\mathsf O\), the two DP directions now follow by four explicit cases. If \(\bot\notin E\), then
\[
\Pr[A(D_1)\in E]\le p\le\delta_c
=e^{\varepsilon_c}\Pr[A(D_0)\in E]+\delta_c,
\qquad
\Pr[A(D_0)\in E]=0
...
\[
\Pr[A(D_1)\in E]\le1
\le e^{\varepsilon_c}\Pr[A(D_0)\in E]+\delta_c,
\]
while \(\Pr[A(D_1)\in E]\ge1-p\ge1-\delta_c\) and
\[
\Pr[A(D_0)\in E]=1
\le e^{\varepsilon_c}(1-\delta_c)+\delta_c
\le e^{\varepsilon_c}\Pr[A(D_1)\in E]+\delta_c.
\tag{14b}
\]
Relabeling \(D_0,D_1\) covers a transition into, rather than out of, empty support. Thus the exact privacy conclusion includes empty support and every sentinel transition in both directions.
...
Statement:
Under Assumptions~\ref{assump:vc-one-factors} and \ref{assump:global-privacy-range}, accepted Lemma~\ref{lem:step-001-logstar}, which gives \(s_i\ge2\), and Lemma~\ref{lem:step-002-order-geometry}, which gives \(1\le H_i\le2^{d_i+1}\), use the following fixed-accuracy instance of the checked Yan `thm:private median`: there is a universal \(C_{PM}>0\) such that, for every \(0<\eta<1\), \(\varepsilon_m>0\), and \(0<\delta_m<1\), on every finite ordered domain of size \(m\ge2\), the cited replacement-\((\varepsilon_m,\delta_m)\)-DP kernel returns a \(1/3\)-median except with probability at most \(\eta\) whenever its input count is strictly greater than
\[
C_{PM}\frac{\log_2^*m}{\varepsilon_m}
\log^2\!\left(\frac{e\log_2^*m}{\eta\delta_m}\right).
\]
Under these named inputs, there are frozen universal constants \(L_*\), \(C_T\), and \(K_*\) such that the following calibration works for every factor:
\[
...
Proof / justification:
Use the universal \(C_{PM}\) in the fixed-accuracy Yan interface named in the statement; it absorbs the fixed median-accuracy factor and any universal logarithm-base conversion. Put
\[
\Lambda_0:=\log(20e),
\qquad
...
\[
\mathbf1\{\bar f_i^\circ(q)\oplus h_{\rm out}^\circ(\tau_i(q))
       \ne\bar c_i(q)\}
=
\mathbf1\{h_{\rm out}^\circ(\tau_i(q))\ne g_*(\tau_i(q))\}.
\tag{43}
\]
The secondary quotient \(\tau_i\) also preserves this indicator exactly, so (42) is the quotient risk in (37). Accepted Lemma~\ref{lem:step-001-risk-pullback} would further pull it to raw factor risk with zero residual.
...
- `Appendix-local`: the transformed class \(\mathcal G_i\), secondary quotient \(U_i\), secondary quotient map \(\tau_i:Q_i\to U_i\), active set \(U_i^+\), implication order \(\preceq_i\), principal upset \(I_i(u)\), depth \(\ell_i(u)\), and height \(H_i\) package the source order geometry. They are all derived in Lemma~\ref{lem:step-002-order-geometry} from the setting reference and accepted quotient class; none is an assumption. The symbol \(\pi_i\) remains reserved for the accepted sketch's scalar factor weight and is not reused in this step.
