...
| unit_002 | lemma | Under Assumption~\ref{assump:finite-littlestone}, the accepted inequalities \(1\le v\le n_0\), and the Sauer--Shelah lemma, \(\Pi_C(n_0)\le\sum_{j=0}^{v}\binom{n_0}{j}\le(en_0/v)^v\). | Proves the second target inequality while retaining exponent \(v\) exactly. |
...
Under Assumption~\ref{assump:finite-littlestone}, the accepted dependency
conclusion \(1\le v\le n_0\), and the Sauer--Shelah lemma (SS), the growth
function obeys
\[
\Pi_C(n_0)
...
On the positive branch, the accepted dependency gives
