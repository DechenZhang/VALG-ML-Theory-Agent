...
- Proof roadmap:
  ...
  5. Goal: prove the selected actual item is exactly \(\operatorname{SOA}_{\mathcal G}\) for a \((d+1)\)-irreducible \(\mathcal G\subseteq C\), retain Lyu Lemma 4.2's structural conclusion, and replace the invalid generic uniform event by a measurable marked finite-reconstruction argument: one producer block fixes at most \((d+1)L\) candidates before the independent complement sample, a locally proved one-sided lower-tail Chernoff inequality evaluates them, and the accepted block constant absorbs \(k(d+1)L\). Then eliminate \(k,m,n_0,a,Q,p_j,L,B\), noise losses, and failure allocations in favor of the displayed public rate. Dependencies: items 1-4. Assumptions: all three primitive setting assumptions; no separability, permissibility, or countability assumption.
- Key lemmas / ingredients: Sauer--Shelah trace counting; the sample-without-replacement multiplicative/KL Chernoff bound; Lyu's fixed-block \(E_{\mathrm{good}}\) inclusion \(H_i^{r+1}\subseteq\bigcap_{i'}H_{i'}^r\); exact Lemma 4.3 pairing \((p_{r+1},p_r)=(2p_r,p_r)\); decomposition-dimension descent; Corollary 4.1's equal-DDim and zero-DDim conclusions; the \(p^d2^{d^2}\) essential-list envelope; one-list replacement privacy of Sparse Sample; sensitivity-one AboveThreshold queries and adaptive/concurrent composition; irreducibility-to-empirical-risk transfer; `lemma: Ldim-of-SOA` for the exact fixed improper output class; a finite-state occurrence-mark lift of Sparse Sample; producer-block/complement independence; Fubini applied only to the measurable marked failure event; a finite section union; and a locally proved one-sided Bernoulli lower-tail Chernoff bound.
- Main transfer challenges: The low-error upper tail needs the one-sided hypergeometric/KL form, not a misuse of a small-relative-deviation inequality when the population error is close to zero. The trace fixed point must account for the ceiling and universal \(C_{\mathrm{blk}}\). The integer teacher count, list envelope, Sparse Sample failure score, effective-domain loss, and AboveThreshold noise must be closed simultaneously with \(n_0=km\), exposing all logarithmic \(\alpha\)- and \(n_0\)-dependence. Privacy must be proved for a total procedure on arbitrary nonrealizable neighboring inputs. For PAC transfer, the marked occurrence must be a measurable coordinate of the exact finite mechanism state, every all-stage candidate union must depend only on its producer block, and the proof must avoid promoting pointwise finite-section bounds to product outer probability. The printed factor-one two-sided Chernoff statement in Lyu Proposition 1 is not a valid source; the needed one-sided lower tail is proved locally.
- Potential repair techniques and supporting references:
  ...
  - Technique: Lift an actual Sparse Sample output to an unreleased occurrence-marked pair, fix the marked producer block, union over that block's finite all-stage list family, and evaluate on the independent complement with a locally proved one-sided lower-tail Chernoff bound.
  - Supporting reference: Lyu, `algo:private sample`, `coro: essential hypotheses`, and the block-local list construction in the proof of `thm: DP-ERM-littlestone`; the concentration step is proved directly by the exponential-Markov lower-tail argument rather than imported from Lyu Proposition 1.
  - Why it may help: It preserves the exact output law and arbitrary-class scope while avoiding the invalid uncountable relative-VC supremum. Fubini is applied only to a measurable marked learner event; after fixing the producer block the candidate union is finite and fixed before the evaluation sample.
- Disposition: `selected`
- Branch notes: This is the only framework that changes an actual raw source control while preserving the source algorithmic path. The primary arXiv v1 source was checked directly: its technique overview attributes one factor of \(d\) in the block size to union bounding over at most \(n^d\) traces, and its formal \(E_{\mathrm{good}}\) quantifies over \(h\in C\). The earlier claim that every branch object was source-identical was too strong; sketch attempt 2 repaired the factor-two decomposition-scale residual, totalized omitted arbitrary-input failure paths, and fixed the exact Lemma 4.2 output class. The triggering step review then exposed a separate lesson: finite VC dimension and individual measurability do not imply the permissibility needed by a generic relative-VC theorem, and outer probability does not repair the product-conditioning step. Sketch attempt 3 therefore retains the structural VC conclusion but routes PAC utility through the measurable marked finite-reconstruction bridge already implicit in the exact block-local lists. The parent lineage and public learner are unchanged.
