...
We make the standard-Borel interfaces used below explicit. By
Proposition~\ref{prop:step-002-borel}, there are a Polish space \(P\), a
Borel set \(B\subseteq P\), and a Borel isomorphism
\(\phi:H_C\to B\). For every \(\ell\geq1\), the product map
\(\phi^\ell\) realizes \(H_C^\ell\) as the Borel subset \(B^\ell\) of the
Polish space \(P^\ell\); the zeroth power is a singleton. Thus every finite
power \(H_C^\ell\) is standard Borel.

For completeness, the product and tagged-union closure used here follows
directly from the same realization. If finitely many standard-Borel spaces
are realized as Borel sets \(B_i\) in Polish spaces \(P_i\), their product
is realized as the Borel set \(\prod_i B_i\) in the Polish space
\(\prod_i P_i\). For a finite or countable family, choose bounded complete
compatible metrics of diameter less than one on the \(P_i\)'s and give
\(\bigsqcup_iP_i\) the metric that equals the chosen metric within a
component and equals one between distinct components. This space is Polish:
a Cauchy sequence eventually stays in one complete component, and the union
of countable dense sets is countable and dense. Every component is clopen,
so \(\bigsqcup_iB_i\) is Borel in \(\bigsqcup_iP_i\). Hence finite products
and finite or countable tagged disjoint unions of the spaces used here are
standard Borel.

Applying this construction to the powers above proves that (2) is standard
Borel. Its component inclusions are measurable, the component
\(\ell=0\) is the unique empty list, and length is the measurable
disjoint-union tag. The same finite tagged-union construction makes (3)
standard Borel: it is the disjoint union of \(H_C\) and two singleton
spaces, with \(\perp\) and \(\dagger\) isolated. On every component of
length at least \(j\), the \(j\)-th item map is a measurable coordinate
projection; countable pasting over those components is measurable. Sending
lists of length below \(j\) to \(\dagger\) therefore gives a total
measurable coordinate
\(\mathsf{List}(H_C)\to H_C^\dagger\).

The diagonal needed for dynamic membership is also Borel directly from the
realization. The diagonal \(\Delta_P\) is closed in \(P\times P\), so
\[
 \Delta_B=(B\times B)\cap\Delta_P
\]
is Borel, and the Borel isomorphism \(\phi\times\phi\) transports it back
to
\(\Delta_{H_C}=\{(\bar h,\bar h):\bar h\in H_C\}\). The list-membership
relation
\[
...
finite product, or in a finite or countable tagged disjoint union of finite
products, of spaces of the following types:
\[
\mathfrak P_N,\quad \mathfrak A_N,\quad
\mathsf{List}(H_C),\quad \mathbb R,\quad [0,1],\quad H_C^\dagger.
\tag{8}
\]
The spaces \(\mathfrak P_N\) and \(\mathfrak A_N\), as well as any
additional finite or countable structural flag range, are countable
discrete and hence Polish under the discrete metric; \(\mathbb R\) and
\([0,1]\) are Polish,
and the remaining two factors were handled above. For each fixed structural
and stopping-time tag, the active transcript coordinates form a finite
product of these factors and are therefore standard Borel by the explicit
product realization above. The declared procedure has only finitely many
stopping-time tags, and adjoining any countable structural tags is covered
by the explicit tagged-union realization. Thus the resulting container,
denoted by \(\mathsf{Tr}_N\), is standard Borel. Its tag map and its
coordinate projections on every tagged component are measurable by the
construction. Variable stopping times are represented by these measurable
tags, not by an infinite transcript.
...
\(\mu_{\varnothing}=\delta_{\bar c_0}\), so (18) is exactly that Dirac law.
For an arbitrary supplied pointwise law, the universal kernel assertion
places no Dirac restriction on \(\mu_{\varnothing}\).
...
\(\mu_{\varnothing}\). Even when \(d=0\), an arbitrary Borel probability
measure on the improper output space \(H_C\) need not be Dirac. If the
supplied law is the null law of one of the three setting-defined learner
arms, Proposition~\ref{prop:step-001-zero} instead gives
\(\mu_{\varnothing}=\delta_{\bar c_0}\), yielding the exact deterministic
baseline. When \(d\ge1\), all quotient
...
For \(N=0\), universally
\(A_0(\varnothing,\cdot)=K_0(\varnothing,\cdot)\); this common law is Dirac
only under the arm-specific premise
\(K_0(\varnothing,\cdot)=\delta_{\bar c_0}\).
...
empty-tuple map, so for every \(E\in\mathcal H_C\),
\[
A_0(\varnothing,E)=K_0(T_0(\varnothing),E)=K_0(\varnothing,E).
\tag{22}
\]
Thus the universal empty-input conclusion is exactly \(A_0=K_0\).
If the supplied kernel is a setting-defined learner-arm null law,
Proposition~\ref{prop:step-001-zero} additionally gives
\(K_0=A_0=\delta_{\bar c_0}\); no Dirac conclusion follows for a generic
supplied law. For \(N\ge1\), raw duplicates, same-cell records, and arbitrary labels are all
...
every arbitrary-label input included. Only for the three setting-defined
learner-arm null laws does Proposition~\ref{prop:step-001-zero} identify the
\(d=0,N=0\) kernel as \(\delta_{\bar c_0}\); an arbitrary supplied law
remains arbitrary. Finally, Proposition~\ref{prop:step-002-record-map} makes
...
\(N\in\mathbb N_0\), with \(A_0=K_0\) universally and the exact Dirac
pullback only under the arm-specific premise. Together, these propositions
establish exactly the stated conclusion.
...
5. On \(d\ge1\), let \(N_*:=N_{j_*}\) be the actual sample size selected in
   ...
   comparison. If the standalone old arm's own sample size is being studied
   rather than the minimizing selected learner,
...
Proposition~\ref{prop:step-002-factorization} gives quotient factorization;
Lemmas~\ref{lem:step-002-vc} and \ref{lem:step-002-ld} give VC- and
Littlestone-dimension preservation; and
Propositions~\ref{prop:step-002-record-map},
\ref{prop:step-002-iid-pushforward}, and \ref{prop:step-002-risk} give
raw-neighbor transport, iid pushforward, and the exact risk identity. The
countable-domain kernel and raw pullback are
