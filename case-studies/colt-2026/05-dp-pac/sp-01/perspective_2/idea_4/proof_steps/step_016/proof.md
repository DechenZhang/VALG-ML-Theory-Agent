...
- Prior same-step proof/review provenance (producer Unit attempt 3; Review
  attempt 4):
  ...
  This prior pair is provenance only and is not proof evidence.
- Accepted dependency proof/review SHA-256 pairs:
  - `step_002` (producer Unit attempt 2; accepting Review attempt 2):
  ...
  - `step_003` (producer Unit attempt 3; accepting Review attempt 3):
  ...
  - `step_014` (producer Unit attempt 2; accepting Review attempt 2):
  ...
  - `step_015` (producer Unit attempt 3; accepting Review attempt 3):
...
Every listed dependency review has status `ACCEPTED`, matches the listed
proof artifact, and records the listed producer and review attempts. The
direct and transitive closure is current: `step_002` has no dependencies;
the accepting `step_003` review binds its current `step_001` pair and the
exact `step_002` pair above; the accepting `step_014` review binds its five
current direct pairs and certifies the accepted `step_011` and `step_013`
ancestor closure; and the accepting `step_015` review binds the exact current
`step_002`, `step_003`, and `step_004` pairs. Across the overlapping closure,
the `step_002` and `step_003` identities agree with the live direct pairs
above, and no stale pair is used as proof evidence. In particular, every use
below of
Propositions~\ref{prop:step-015-zero}, \ref{prop:step-015-kernel},
\ref{prop:step-015-dp}, \ref{prop:step-015-pac},
\ref{prop:step-015-rate}, and \ref{prop:step-015-boundaries} is bound to
the live accepted `step_015` producer-attempt-3 pair above; all six labels
and their stated conclusions were revalidated directly in that proof and
...
- Assumption~\ref{assump:finite-littlestone}: \(C\) is nonempty and
  ...
  Lemma~\ref{lem:step-002-ld} proves that \(d=0\) is exactly the singleton
  branch.
...
- Accepted Proposition~\ref{prop:step-014-vc-arm} supplies a complete
  ...
  the exact \(d=0\) law, and, on \(d\geq1\), after writing
  \(N_{\mathrm V}\) for its sample size and \(K_{\mathrm V}\) for its
  universal scalar \(K\) with \(q=4\),
...
- Against the live accepted `step_015` producer-attempt-3 pair recorded in
  Step Identity, revalidated Propositions~\ref{prop:step-015-zero},
  ...
  quotient/raw/marked-law tuple with accepted quotient kernel
  \(K_{\mathrm o}=K_C^{\mathrm{old\text{-}Lyu}}\), all-input
  \((\varepsilon,\delta)\)-DP, unconditional \((\alpha,\beta)\)-PAC
  utility, the shared \(d=0\) law, and
  \(N_{\mathrm o}=N_{\mathrm{old}}\le
  K_{\mathrm{old}}\Lambda^6R_{\mathrm{old}}\) for a universal scalar
  \(K_{\mathrm{old}}\geq1\). The old arm's own numerical limit uses the
  separate sufficient condition
  \(\delta K_{\mathrm{old}}\Lambda^6R_{\mathrm{old}}\to0\).

The accepted dependency artifacts are exactly the eight current proof/review
files listed in Step Identity. The six old-arm labels used in this allowed
interface were checked in, and are supplied by, the listed live `step_015`
pair.
...
| `unit_006` | proposition | Under Assumptions~\ref{assump:finite-littlestone}, \ref{assump:countable-evaluation-quotient}, \ref{assump:realizable-iid}, and \ref{assump:approximate-dp-regime}, accepted Proposition~\ref{prop:step-002-risk}, Propositions~\ref{prop:step-016-zero} and \ref{prop:step-016-finite-arm}, and accepted Propositions~\ref{prop:step-014-vc-arm}, \ref{prop:step-015-kernel}, \ref{prop:step-015-dp}, \ref{prop:step-015-pac}, and \ref{prop:step-015-rate}, the deterministic choice of the least certified complete-arm threshold gives, for every fixed allowed \(\delta\), \(m_C\le K_*\Lambda^6\min\{R_{\mathrm{fin}},R_{\mathrm{old}},R_{\mathrm{VC}}\}\), where \(K_*\) is universal; for infinite \(C\), only \(R_{\mathrm{fin}}\) is \(+\infty\). | Takes the minimum only after certifying a common kernel/output/decoder/DP/PAC/rate interface, and selects one law rather than mixing laws. |
| `unit_007` | proposition | Under Assumptions~\ref{assump:finite-littlestone}, \ref{assump:countable-evaluation-quotient}, \ref{assump:realizable-iid}, and \ref{assump:approximate-dp-regime}, Propositions~\ref{prop:step-016-zero}, \ref{prop:step-016-finite-arm}, and \ref{prop:step-016-minimum}, and accepted Propositions~\ref{prop:step-014-vc-arm}, \ref{prop:step-015-rate}, and \ref{prop:step-015-boundaries}, the exact \(d=0\), \(v=1\), \(v=d\), finite/infinite, fixed/scheduled \(\delta\), and source-scope baselines hold; at fixed \(\alpha,\beta,\varepsilon\) under the declared source-compatible schedule, \(m_C=\widetilde O(\min\{\log^+|C|,d^5,vd^4\})\), with \(vd^4=o(d^5)\) when \(v=o(d)\) and \(vd^4=d^5\) at \(v=d\). | Exports exactly the formalized frontier and strict structural specializations without claiming a characterization or unrestricted theorem. |
...
### Accepted quotient, transport, and risk results
...
- Assumption discharge: accepted
  Proposition~\ref{prop:step-002-factorization},
  Lemma~\ref{lem:step-002-ld}, and accepted
  ...
  \ref{assump:realizable-iid}; their accepted proof/review hashes are
  recorded above.
...
### Accepted quotient-kernel and raw-pullback results

- Restated result: Accepted
  Lemma~\ref{lem:step-003-countable-promotion} makes every pointwise Borel
  probability-law coordinate measurable on the countable-discrete input
  \(Z_Q^N\). Accepted
  Proposition~\ref{prop:step-003-quotient-kernel} therefore promotes a
  separately supplied everywhere-defined pointwise law on \(H_C\), including
  the finite-class law when defined, to a quotient kernel \(K_N\). Accepted
  Proposition~\ref{prop:step-003-raw-pullback} then applies to every such
...
- Assumption discharge: Assumption~\ref{assump:countable-evaluation-quotient},
  the setting's standard-Borel output conclusion, and accepted
  Proposition~\ref{prop:step-002-record-map} provide the countable-discrete
...
### Accepted normalized VC-arm proposition
...
### Accepted normalized old-Lyu propositions

- Current dependency validation: the live accepted `step_015`
  producer-attempt-3 proof/review pair recorded in Step Identity contains
  and accepts exactly the six labels
  \ref{prop:step-015-zero}, \ref{prop:step-015-kernel},
  \ref{prop:step-015-dp}, \ref{prop:step-015-pac},
  \ref{prop:step-015-rate}, and \ref{prop:step-015-boundaries}. Their
  statements respectively supply the exact null law, quotient/raw kernel,
  all-input privacy, unconditional PAC guarantee, explicit old-arm rate,
  and fixed-versus-scheduled boundary interface restated here.
- Restated result: Under all four setting assumptions, live accepted
  Proposition~\ref{prop:step-015-kernel} names the old quotient kernel
  \(K_{\mathrm o}=K_C^{\mathrm{old\text{-}Lyu}}\), and accepted
  ...
  \tag{C.O}
  \]
  for the universal scalar \(K_{\mathrm{old}}\ge1\). Accepted
...
\ref{prop:step-014-vc-arm}, and accepted
...
conclusion also gives \(v=0\). Accepted
Proposition~\ref{prop:step-002-factorization} gives the corresponding
...
and hence has risk zero for every \(D\). Accepted
Proposition~\ref{prop:step-014-vc-arm} and the live accepted
Proposition~\ref{prop:step-015-zero}, revalidated against the `step_015`
pair in Step Identity, define their null branches by this same unique
...
Lemma~\ref{lem:step-003-countable-promotion} makes every event-mass coordinate
...
to which accepted Proposition~\ref{prop:step-003-quotient-kernel} applies;
in particular, \(K^{\mathrm{fin}}_{C,N}\) is a quotient kernel produced by
that proposition. Accepted Proposition~\ref{prop:step-002-record-map} makes
\(T_N\) measurable, so the exact premise of accepted
Proposition~\ref{prop:step-003-raw-pullback} is met and that proposition gives
...
accepted Proposition~\ref{prop:step-002-risk},
Propositions~\ref{prop:step-016-zero} and
\ref{prop:step-016-finite-arm}, accepted
...
The completed arm certificates are as follows. The old-arm row is reissued
against the live accepted `step_015` producer-attempt-3 pair in Step
Identity: its kernel, privacy, PAC, and rate entries are exactly
Propositions~\ref{prop:step-015-kernel}, \ref{prop:step-015-dp},
\ref{prop:step-015-pac}, and \ref{prop:step-015-rate}, whose statements and
acceptance were revalidated in the current artifacts.
...
| old | accepted \(K_{\mathrm o}=K_C^{\mathrm{old\text{-}Lyu}}\) and its raw pullback through \(T_{N_{\mathrm o}}\) | \(H_C\), then \(\operatorname{Dec}_C\) | \((\varepsilon,\delta)\)-DP | the same raw decoded-risk failure at most \(\beta\) | \(B_{\mathrm o}:=K_{\mathrm{old}}\Lambda^6R_{\mathrm{old}}\) |
...
Accepted Proposition~\ref{prop:step-002-risk}, recorded as (D.1), is the
...
\ref{prop:step-016-minimum}, accepted
Proposition~\ref{prop:step-014-vc-arm}, and accepted
...
5. On \(d\ge1\), let \(N_*:=N_{j_*}\) be the actual sample size selected in
   ...
   the minimizing selected learner, accepted
   Proposition~\ref{prop:step-015-boundaries} supplies only
   \[
   \delta K_{\mathrm{old}}\Lambda^6R_{\mathrm{old}}\to0
   \quad\Longrightarrow\quad
   N_{\mathrm o}\delta=N_{\mathrm{old}}\delta\to0;
...
accepted Proposition~\ref{prop:step-014-vc-arm}, and the fixed-parameter
statement of the live accepted
Proposition~\ref{prop:step-015-boundaries}, revalidated against the current
`step_015` pair, proves
Item 4.
...
setting schedule. Equation (7.3) is the separate live accepted old-arm
statement,
...
Proposition~\ref{prop:step-014-vc-arm} proves that the lower-order displayed
term introduces no larger positive power. The live accepted and revalidated
Proposition~\ref{prop:step-015-rate} exposes exactly \(d^5\), and
...
VC-arm tuple. The live accepted Propositions~\ref{prop:step-015-kernel},
\ref{prop:step-015-dp}, \ref{prop:step-015-pac}, and
\ref{prop:step-015-rate} supply the independently complete old-arm tuple.
Together with the live accepted Propositions~\ref{prop:step-015-zero} and
\ref{prop:step-015-boundaries}, these are exactly the six `step_015` labels
revalidated against the current producer-attempt-3 proof/review pair in Step
Identity; their null, kernel, privacy, PAC, rate, and boundary interfaces are
unchanged.
...
- Hidden constants may depend on:
  ...
  \(K_{\mathrm V}\) and \(K_{\mathrm{old}}\) are universal scalars, and
  \(K_*=\max\{8,K_{\mathrm{old}},K_{\mathrm V}\}\) is universal. The common
...
- Probability conversion:
  accepted Propositions~\ref{prop:step-002-iid-pushforward} and
  \ref{prop:step-002-risk} give exact sample pushforward and exact risk equality,
...
| Standalone old arm schedule | Accepted Proposition~\ref{prop:step-015-boundaries} gives (7.3). The setting's VC schedule is not used as a substitute when the old arm's own \(N_{\mathrm o}\delta\) is the quantity under study. |
...
- Public-facing outputs proved here:
  ...
  common \(K_*\) and exponent \(6\). The minimal selected-learner interface
  in Proposition~\ref{prop:step-016-minimum}, including its actual sample
  size \(N_*\) and common DP/PAC/rate conclusions, is public-facing; its tie
  index and tie rule are not.
- Appendix-local objects:
  the generic finite-kernel families \(K^{\mathrm{fin}}_{C,N}\) and
  \(A_N^{\mathrm{fin}}\), the recurring shorthand \(M:=|C|\), the empirical
  count \(L_{\bar s}\), weights \(w_{\bar s}\), normalizer \(Z_{\bar s}\),
  numerator \(W_{\bar s}(E)\), the three certified thresholds
  \(B_{\mathrm{fin}},B_{\mathrm o},B_{\mathrm V}\), and the exact ceiling
  expression (5.1). They expose the proof bridge but are not new theorem
  assumptions. Only the generic families' specialization at
  \(N_{\mathrm{fin}}\) is exported to the public finite-arm interface, and
  the final frontier writes \(|C|\) rather than exporting \(M\).
- Proof-local objects:
...
- Constant provenance:
  ...
  and the paid ceiling. The scalar \(K_{\mathrm V}\) is the explicitly
  translated universal \(K\) from accepted
  Proposition~\ref{prop:step-014-vc-arm}; the scalar
  \(K_{\mathrm{old}}\) is inherited from accepted
  Proposition~\ref{prop:step-015-rate}. The symbol \(K_{\mathrm o}\) remains
  reserved for the old quotient kernel from accepted
  Proposition~\ref{prop:step-015-kernel}. No free finite constant is
  introduced.
...
- Schedule note:
  ...
  explicit VC schedule. Equation (7.3) preserves accepted
  Proposition~\ref{prop:step-015-boundaries}'s separate sufficient condition
  for the standalone old arm. Pure privacy of the
  finite arm is not confused with a numerical \(N_{\mathrm{fin}}\delta\)
  limit.
- Diagnostic boundary:
  no global diagnostic was read or used for this repair attempt. Every
  mathematical line is supported by the current setting, accepted
  dependencies, and named local units; no diagnostic is evidence, a cited
