...
- Unit attempt: 3
- Repair provenance:
  - latest same-step proof/review pair before this refresh: unit attempt 2
    proof SHA-256
    bcb840332dff8fe4f0c2028744e46eff951fb4e7aa6e47f74fd4c3bcfdc7d492 /
    stale review SHA-256
    ee3c367e9ab057d098023701bd7b10469eb3b5381f6c3cc3ff1cc7c1952e93bd;
  - refresh trigger: accepted step_003 unit/review attempt 3 and accepted
    step_004 unit/review attempt 2 replaced the dependency identities below.
    This transitive refresh changes no target, algorithm, assumption, scalar
    dictionary, local theorem label, or local mathematical conclusion.
...
- Accepted dependency proof/review SHA-256 pairs:
  ...
  - step_003 (unit attempt 3, review attempt 3):
  ...
  - step_004 (unit attempt 2, review attempt 2):
...
- Accepted Lemma~\ref{lem:step-003-countable-promotion} and
  ...
  pullbacks through \(T_N\) are raw learner kernels. Universally, this gives
  \(A_0=K_0\) but does not force an arbitrary empty-input law to be Dirac.
  When the supplied law is the setting-defined old-Lyu arm, and only for
  that arm here, accepted Proposition~\ref{prop:step-003-quotient-kernel}
  carries the arm-specific Step 001 null-law authority and gives
  \(K_0=A_0=\delta_{\bar c_0}\). This is an accepted step_003 conclusion,
  not a new direct dependency on step_001.
...
| --- | --- | --- | --- |
| unit_001 | proposition | Under Assumptions~\ref{assump:finite-littlestone} and \ref{assump:countable-evaluation-quotient}, accepted Lemma~\ref{lem:step-002-ld}, and accepted Propositions~\ref{prop:step-002-factorization}, \ref{prop:step-002-risk}, \ref{prop:step-003-quotient-kernel}, and \ref{prop:step-003-raw-pullback}, if \(d=0\), the setting-defined old-Lyu \(N_{\mathrm{old}}=0\) law has \(K_0=A_0=\delta_{\bar c_0}\), zero risk, and \((0,0)\)-DP. | Closes the exact arm-specific null branch while retaining the universal \(A_0=K_0\) boundary. |
...
| unit_006 | proposition | Under accepted step_002-step_003 interfaces and Proposition~\ref{prop:step-015-total}, the totalized old quotient law and raw pullback are Markov kernels with exact risk transport; the kernel promotion is universal and not a generic Dirac claim. | Produces the common measurable output interface. |
...
- Accepted Lemma~\ref{lem:step-003-countable-promotion} and
  ...
  \ref{prop:step-003-raw-pullback}: an arbitrary pointwise family of
  probability laws indexed by \(Z_Q^N\) is a quotient Markov kernel, and
  composition with \(T_N\) is a raw Markov kernel. Universally
  \(A_0=K_0\). Only when the supplied law is a setting-defined arm does the
  arm-specific Step 001 premise exposed by accepted step_003 identify that
  common law as \(\delta_{\bar c_0}\); this proof invokes that stronger
  conclusion only for \(K_C^{\mathrm{old\text{-}Lyu}}\). These results are
  used in Propositions~\ref{prop:step-015-zero} and
...
### Finite combinatorics, inline sampling derivations, and privacy facts
...
- Finite symmetric-mean interface, proved inline below: if \(N\ge1\),
  \(0\le m\le N\), and \(w_1,\ldots,w_N\ge0\), then, with
  \(e_m\) denoting the degree-\(m\) elementary symmetric polynomial and
  the degree-zero powers interpreted as one,
  \[
  e_m(w_1,\ldots,w_N)
  \le\binom Nm\left(\frac1N\sum_{j=1}^Nw_j\right)^m.
  \tag{C6}
  \]
  Lemma~\ref{lem:step-015-tails} proves (C6) directly by pairwise averaging,
  including its boundary cases, and then applies it to
  \(w_j=e^{\lambda x_j}\). Thus (C6) is a local derivation, not an imported
  named inequality.
- Bernoulli lower-tail interface, proved inline below: if
  \(M\in\mathbb N_0\), \(X_1,\ldots,X_M\) are independent Bernoulli
  variables of common mean \(p\in[0,1]\),
  \(S=\sum_{j=1}^M X_j\), and \(0<\theta<1\), then
  \[
  \Pr[S\le M(1-\theta)p]
  \le\exp(-\theta^2pM/2).
  \tag{C7}
  \]
  Lemma~\ref{lem:step-015-lower-tail} derives (C7) by exponential Markov
  with \(\lambda=-\log(1-\theta)\), proves the needed scalar inequality,
  checks the degenerate cases, verifies iid independence after conditioning
  on the producer block, and instantiates \(\theta=3/4\). No named lower-tail
  theorem is used as authority.
...
\ref{prop:step-002-risk}, \ref{prop:step-003-quotient-kernel}, and
\ref{prop:step-003-raw-pullback}, if \(d=0\), then \(\bar C\)
has a unique element \(\bar c_0\). The setting-defined old-Lyu arm with
\(N_{\mathrm{old}}=0\), quotient law
\[
K_{\mathrm o}(\varnothing,E)=\mathbf1_E(\bar c_0),
\tag{1.1}
\]
has \(K_0=A_0=\delta_{\bar c_0}\); its raw pullback along \(T_0\) is a
measurable \((0,0)\)-DP Markov kernel.
...
Dirac probability law on the standard-Borel output space. Accepted
Proposition~\ref{prop:step-003-quotient-kernel} invokes the arm-specific
Step 001 null-law authority only because the supplied law is the
setting-defined old-Lyu arm, and therefore identifies its quotient null law
with (1.1). Accepted Proposition~\ref{prop:step-003-raw-pullback} gives the
universal identity \(A_0=K_0\), so the old arm's raw null law is the same
Dirac measure. A constant law on the unique empty input is \((0,0)\)-DP.
Accepted Proposition~\ref{prop:step-002-factorization}
...
condition is present, and no arbitrary empty-input law is asserted to be
Dirac. \(\square\)
...
and \ref{prop:step-003-raw-pullback}, accepted
...
pointwise family to a Markov kernel, and accepted
Proposition~\ref{prop:step-003-quotient-kernel} instantiates that universal
conclusion as (6.1); in particular, data-dependent finite lists and fallback
choices do not require a measurable global selector or a Dirac output law.
Accepted Proposition~\ref{prop:step-002-record-map} makes
...
symmetric polynomial of a nonnegative vector \(w\in[0,\infty)^N\). The
...
coordinate, and let
\(\bar w=N^{-1}\sum_jw_j\). The sum and hence \(\bar w\) stay fixed. If
\[
...
\tag{8.3d}
\]
which proves (C6), including \(m=N\); the already separated
\(m=0,1\) cases cover the other endpoints. In the live application,
\(N=n_{\mathrm o}=k_{\mathrm o}m_{\mathrm o}\),
\(m=m_{\mathrm o}\), \(m_{\mathrm o}\ge1\), and
\(k_{\mathrm o}\ge2\), so \(1\le m<N\).
...
these mistake indicators. In the live application,
...
\(g(\theta)\ge0\), and (20.2) proves (C7) without invoking an external
lower-tail theorem. Taking \(\theta=3/4\), then using \(p>\alpha\), yields
\[
\exp[-(3/4)^2pM/2]
=\exp(-9pM/32)
\le\exp(-9\alpha M/32),
...
Proposition~\ref{prop:step-015-zero} closes the exact \(d=0\) branch. Its
universal dependency interface is kernel promotion with \(A_0=K_0\); its
stronger Dirac conclusion is invoked only for the setting-defined old-Lyu
arm through the arm-specific Step 001 authority exposed by accepted
Proposition~\ref{prop:step-003-quotient-kernel}. On \(d\ge1\),
Lemma~\ref{lem:step-015-dictionary},
...
Lemma~\ref{lem:step-015-tails} proves both fixed-trace branches, including
the pairwise-smoothing derivation of (C6); and
...
\ref{lem:step-015-lower-tail}, the latter deriving (C7) by exponential
Markov and a scalar inequality, reduce the adaptive population-error event
to a finite producer-local union, and Proposition
...
- Proof-local sampling variables:
  ...
  interfaces. Equations (C6) and (C7) are proved inside
  Lemmas~\ref{lem:step-015-tails} and
  \ref{lem:step-015-lower-tail}, respectively; neither is used as an
  external named authority.
- Additional proof-local inline-derivation helpers:
  the generic integers \(N,m\), vector \(w\), remaining-coordinate vector
  \(z\), pair \(a,b\), coefficients
  \(A_{\mathrm{sm}},B_{\mathrm{sm}},C_{\mathrm{sm}}\), mean \(\bar w\),
  smoothing sequence \(w^{(s)}\), and variance \(V_s\) occur only in the
  proof of (C6). The Bernoulli sum \(S\), threshold \(q\), tilt
  \(\lambda\), and scalar functions \(g,u\) occur only in the proof of
  (C7). Each is defined from the local vectors or Bernoulli variables before
  use and exports no new theorem-facing object, assumption, constant, or
  rate.
...
- Null-law provenance:
  accepted step_003 promotes every totalized pointwise law to a quotient
  kernel and every raw pullback satisfies \(A_0=K_0\) universally. The
  stronger \(K_0=A_0=\delta_{\bar c_0}\) identity used in
  Proposition~\ref{prop:step-015-zero} is invoked only for the
  setting-defined old-Lyu arm through the arm-specific Step 001 authority
  already exposed by accepted step_003; it is not asserted for an arbitrary
  law on \(H_C\) and does not add step_001 as a direct dependency.
...
- Scope boundary:
  every conclusion is explicitly conditional on the finite-or-countable
  measurable evaluation quotient in
  Assumption~\ref{assump:countable-evaluation-quotient}. Uncountable
  evaluation quotients remain outside the claim, and this old-arm step does
  not solve the unrestricted DP-PAC open problem.
- Repair-context boundary:
  only the live unit-attempt-2 proof and its now-stale live review were read
  as same-step repair context. No global diagnostic or proof-history
  artifact was read or used in this transitive refresh. Every claim above
  is proved from the setting, accepted sketch row, current accepted
  dependency pairs, checked frozen source, or earlier local units.
