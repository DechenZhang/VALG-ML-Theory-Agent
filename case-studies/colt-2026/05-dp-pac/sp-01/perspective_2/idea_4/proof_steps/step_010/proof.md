...
This step remains inside the setting's explicitly conditional
finite-or-countable measurable evaluation-quotient regime. It makes no claim
for uncountable evaluation quotients and does not solve the unrestricted
DP-PAC open problem.
...
- Derived invariants and interfaces supplied by accepted dependencies:
  ...
  - Accepted Lemma~\ref{lem:step-004-occurrence} and
    ...
    \ref{prop:step-004-projection} use the accepted actual/fallback
    transcript status to supply, on the declared \(d\geq1\) VC-Lyu branch,
    the finite positive occurrence set on an actual path, the marked law,
    and its exact projection to the released \(K_C\) law.
...
These three marked results are consumed only on the declared \(d\geq1\)
VC-Lyu branch. At \(d=0,N=0\), the accepted stronger Dirac conclusion is
arm-specific through accepted Proposition~\ref{prop:step-001-zero}; the
universal promotion of an arbitrary empty-input pointwise law supplies only
the kernel and raw-pullback identity, not a generic Dirac law. No null-law
claim is used in the positive-dimensional mechanism derivation below.
...
- Baseline-reduction check: The \(d=0\) branch bypasses all mechanisms and
  retains the accepted arm-specific exact no-data output from
  Proposition~\ref{prop:step-001-zero}; it is not inferred from generic
  empty-input kernel promotion. On \(d\geq1\), first-stage and
