...
- Accepted global-proof SHA-256:
  `bbb47a7e0753c47fc073d71b9e8e5b0a74d40d79aad7e22c1e8d7bf44c687784`
- Accepted global-proof-review SHA-256:
...
- Prior same-step proof SHA-256:
  `092c5ba976f43a4b7d1bf746aba05c1caa0d49f2bd3de55a254f58dc6e4c4a7f`
- Prior same-step review SHA-256:
  `5ba08c887b44fc901e74bd3bc2793a06e3accf2a7d999f62532ec042c211eb87`
- Controller archive of the prior pair:
  `perspective_2/idea_4/proof_history/sketch_001/step_step_003_attempt_002_proof_step_flaw/proof_steps/step_003/`.
  This archive is provenance only and is not proof evidence.
...
- Derived invariants and interfaces supplied by accepted dependencies:
  ...
  - Accepted Proposition~\ref{prop:step-001-zero}, **Exact
    zero-dimensional law**, supplies the setting-defined \(N=0\), \(d=0\)
    Dirac law for each learner arm; it imposes no restriction on an
    arbitrary separately supplied pointwise law.
...
- Local conditional hypotheses: None. The arm-specific identity
  (K_0=\delta_{\bar c_0}) is an accepted dependency conclusion used only
  when a supplied law is instantiated as a setting-defined learner arm; it
  is not assumed for the universal kernel or pullback conclusions. In
  particular, no realizability, good event, source-valid path, privacy
  inequality, PAC conclusion, measurable selector, or propriety condition
  is assumed.
...
| `unit_004` | proposition | Under Assumption~\ref{assump:countable-evaluation-quotient}, accepted Propositions~\ref{prop:step-001-totalization} and~\ref{prop:step-001-zero}, Lemma~\ref{lem:step-003-countable-promotion}, and Proposition~\ref{prop:step-003-coding}, every totalized pointwise \(H_C\)-valued output law on \(Z_Q^N\) defines a Markov kernel \(K_N:Z_Q^N\leadsto H_C\), including all fallback paths; only when the supplied \(d=0,N=0\) law is a setting-defined arm law furnished by Proposition~\ref{prop:step-001-zero} is \(K_0=\delta_{\bar c_0}\). | Constructs the required quotient learner kernel for every totalized pointwise law while preserving the arm-specific null baseline. |
| `unit_005` | proposition | Under Assumption~\ref{assump:countable-evaluation-quotient}, accepted Proposition~\ref{prop:step-002-record-map}, and Proposition~\ref{prop:step-003-quotient-kernel}, \(A_N(s,E):=K_N(T_N(s),E)\) is a Markov kernel \(Z_X^N\leadsto H_C\) for every \(N\in\mathbb N_0\); universally \(A_0=K_0\), and the pullback is Dirac only under the arm-specific premise \(K_0=\delta_{\bar c_0}\). | Produces the exact raw-input pullback kernel with unchanged output codomain. |
...
- Restated result: If \(d=0\), then \(C\) and \(\bar C\) are singletons and,
  at \(N=0\), the setting-defined no-data quotient output law is the Dirac
  measure at the unique \(\bar c_0\in\bar C\subseteq H_C\).
...
- Conclusion used: Each setting-defined learner arm's null branch is a
  pointwise Borel probability law equal to \(\delta_{\bar c_0}\). This is
  the exact \(d=0,N=0\) authority in
  Proposition~\ref{prop:step-003-quotient-kernel} and the target-step
  assembly; it is not authority to identify an arbitrary supplied law on
  \(H_C\) as Dirac.
...
| Proposition~\ref{prop:step-003-quotient-kernel} | Any pointwise \(H_C\)-law supplied by a totalized procedure; additionally, accepted Proposition~\ref{prop:step-001-zero} when that law is a setting-defined \(d=0,N=0\) arm law | Quotient Markov kernel \(K_N\) universally; \(K_0=\delta_{\bar c_0}\) only for the arm-specific null laws | Lemma~\ref{lem:step-003-countable-promotion} for every supplied law, accepted Proposition~\ref{prop:step-001-totalization} on the positive branch, and accepted Proposition~\ref{prop:step-001-zero} only for the setting-defined null arms; used by Proposition~\ref{prop:step-003-raw-pullback}. |
| Proposition~\ref{prop:step-003-raw-pullback} | Quotient kernel \(K_N\) and measurable \(T_N\) | Raw Markov kernel \(A_N=K_N\circ T_N\), with \(A_0=K_0\) universally and an arm-specific Dirac pullback when \(K_0=\delta_{\bar c_0}\) | Accepted Proposition~\ref{prop:step-002-record-map}; used in target assembly. |
...
\(N=0\), and the supplied law is one of these setting-defined arm laws,
accepted Proposition~\ref{prop:step-001-zero} gives
\(\mu_{\varnothing}=\delta_{\bar c_0}\), so (18) is exactly that Dirac
law. The universal kernel assertion alone places no such restriction on an
arbitrary supplied \(\mu_{\varnothing}\).
...
When \(N=0\), \(Z_Q^0=\{\varnothing\}\), so the universal conclusion says
only that (18) is the constant-input kernel with output measure
\(\mu_{\varnothing}\). Even when \(d=0\), this arbitrary Borel probability
measure need not be Dirac because the improper output space \(H_C\) need not
be a singleton. If the supplied law is instead the null law of any of the
three setting-defined learner arms, the accepted dependency explicitly
named in this proposition, Proposition~\ref{prop:step-001-zero}, gives
\(\mu_{\varnothing}=\delta_{\bar c_0}\); substituting this arm-specific
identity into (18) yields the exact deterministic baseline. When \(d\ge1\),
all quotient datasets, including duplicate records and
arbitrary/nonrealizable labels, are atoms of \(Z_Q^N\) on which the accepted
procedure is total. Therefore none is removed from the kernel domain.
\(\square\)
...
For \(N=0\), the universal identity is
\(A_0(\varnothing,\cdot)=K_0(\varnothing,\cdot)\). Consequently the raw
pullback is \(\delta_{\bar c_0}\) only when the preceding proposition's
arm-specific premise \(K_0(\varnothing,\cdot)=\delta_{\bar c_0}\) holds.
...
empty-tuple map, so (20) gives, for every \(E\in\mathcal H_C\),
\[
A_0(\varnothing,E)=K_0(T_0(\varnothing),E)
=K_0(\varnothing,E).
\tag{22}
\]
Thus the unique empty-input raw pullback always satisfies \(A_0=K_0\).
Under the arm-specific premise
\(K_0(\varnothing,\cdot)=\delta_{\bar c_0}\) furnished in
Proposition~\ref{prop:step-003-quotient-kernel}, (22) additionally gives the
exact Dirac pullback; without that premise no Dirac conclusion follows. For
...
\(H_C\). Accepted Proposition~\ref{prop:step-001-zero} separately supplies
the exact \(d=0,N=0\) Dirac law at the unique \(\bar c_0\) for each
setting-defined learner arm. Accepted
...
every arbitrary-label input included. For the three setting-defined arms,
and only under that arm-specific instantiation, accepted
Proposition~\ref{prop:step-001-zero} identifies the \(d=0,N=0\) kernel as
\(\delta_{\bar c_0}\); an arbitrary supplied empty-input law remains an
arbitrary Borel probability measure on \(H_C\). Finally, accepted
Proposition~\ref{prop:step-002-record-map} and
Proposition~\ref{prop:step-003-raw-pullback} prove that
\(A_N=K_N\circ T_N\) is a raw-input learner kernel for every
\(N\in\mathbb N_0\), with \(A_0=K_0\) universally and therefore the exact
Dirac pullback for each setting-defined null arm. These named results
establish exactly the target row.
...
every supplied \(N=0\) law is proved to be a genuine singleton-input kernel.
The stronger Dirac baseline is retained exactly for the setting-defined arms
through Proposition~\ref{prop:step-001-zero}, rather than inferred from the
singleton input or asymptotically.
...
- Setting-defined, public-facing objects:
  ...
  the three exact pointwise learner-arm laws, and the two error functionals
  retain their meanings from `setting.md`.
- Accepted dependency objects: The total procedure and zero-dimensional law
  are used only through
  Propositions~\ref{prop:step-001-totalization} and
  \ref{prop:step-001-zero}. The latter identifies only the setting-defined
  arm laws at \(d=0,N=0\), not arbitrary pointwise laws on \(H_C\); the
  quotient Borel structure, raw record map, and measurable risk are used only
  through
...
- Proof-local objects: \(\mathsf U_N,\mathfrak P_N,\mathfrak A_N\), each
  ...
  the transcript kernel \(\Gamma_N\). The arbitrary supplied family
  \((\mu_{\bar s})_{\bar s\in Z_Q^N}\) is also proof-local until it is
  instantiated as one of the setting-defined arm laws. These are coding or
  proof-interface devices only.
...
- Boundary provenance: For an arbitrary supplied pointwise law, \(N=0\)
  yields the constant-input kernel with output measure
  \(\mu_{\varnothing}\), and its raw pullback satisfies \(A_0=K_0\). For
  each setting-defined learner arm, accepted
  Proposition~\ref{prop:step-001-zero} additionally gives
  \(K_0=A_0=\delta_{\bar c_0}\). Empty restrictions and invalid lists are
  sanitized to the empty list; invalid, no-success, and mechanism-failure
  paths return \(\bar c_0\). Duplicate records and arbitrary labels remain
  input atoms. Improper outputs remain legitimate elements of the
  standard-Borel space \(H_C\), which need not be a singleton when \(d=0\).
...
- Assumption provenance: Countability and measurable quotient cells are the
  ...
  hypotheses.
- Scope boundary: The branch remains strictly `conditional` under
  Assumption~\ref{assump:countable-evaluation-quotient}; this artifact makes
  no assertion for uncountable evaluation quotients and does not solve the
  unrestricted DP-PAC open problem. The occurrence mark belongs to the next
  proof step. This artifact proves neither marked projection nor any
  privacy, neighbor-use, PAC, concentration, or rate claim.
- Global diagnostic boundary: The optional `global_proof.md`, SHA-256
  `bbb47a7e0753c47fc073d71b9e8e5b0a74d40d79aad7e22c1e8d7bf44c687784`,
  was read only after its paired `global_proof_review.md`, SHA-256
  `490e6b89ecd229a41dac2b24c82bb277cbacfd66acff00f4f5c82fc58ac7cf09`,
  was verified current and `ACCEPTED`. Its step-relevant planning idea was
  the countable-input promotion followed by measurable composition through
  \(T_N\), both independently proved above. No diagnostic statement is
  evidence, an assumption source, a cited result, or authority for the
  target claim.
