...
Proposition~\ref{prop:step-003-quotient-kernel} later give its measurable
kernel realization on the countable quotient input; measurability is not
assumed here.
...
Proposition~\ref{prop:step-010-mechanism-good} gives (9)--(10) and, using
Proposition~\ref{prop:step-006-good-event}, defines
\(E_{\rm core}=E_{\rm good}\cap E_{\rm mech}\). Finally,
Lemma~\ref{lem:step-012-empirical} gives (12). Together, the cited results
prove the displayed identities and inequalities without requiring an
additional event, tail bound, or independence assertion. \(\square\)
\end{proof}
...
\ref{lem:step-011-adaptive-composition}, and
...
\[
 ...
 \tag{C5}
\]
for a suitable universal \(c_{\mathrm{AT}}\ge1\). With at most \(d+1\)
independent \(\operatorname{Lap}(1/\eta)\) query noises, their simultaneous
absolute-value threshold is
\(\eta^{-1}\log((d+1)/\beta_a)\), except with probability \(\beta_a\).
...
The displayed trace bound is Sauer--Shelah~\citep{sauer1972}; Maclaurin's
inequality and the Bernoulli lower tail give (C6)--(C7). Finally,
Lemma~\ref{lem:step-011-adaptive-composition} supplies the stated adaptive
composition and postprocessing interface. Combining these identities and
bounds gives the stated old-arm interface without adding a further claim.
\(\square\)
\end{proof}
...
Under Assumptions~\ref{assump:finite-littlestone},
\ref{assump:countable-evaluation-quotient}, and
\ref{assump:approximate-dp-regime}, the Lyu~\citep{lyu2025} decomposition results,
...
\tag{5.1}
\]
For every nonempty restriction, the existence result supplies a
...
Lemma~\ref{lem:step-015-source-map}, and the Lyu~\citep{lyu2025} decomposition and
...
Littlestone dimension at most \(d\). The source existence result
...
of the essential-hypothesis corollary says its essential list is
...
Lemmas~\ref{lem:step-011-adaptive-composition} and
\ref{lem:step-015-locality} and
Propositions~\ref{prop:step-002-record-map},
...
Proposition~\ref{prop:step-016-minimum} choose one entire learner by a
