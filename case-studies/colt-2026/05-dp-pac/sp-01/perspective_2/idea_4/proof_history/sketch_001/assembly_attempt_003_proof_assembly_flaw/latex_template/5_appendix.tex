...
\paragraph{Essential-hypothesis bound.}
...
\noindent \  The conditional list bound in
  ...
  not supplied by Lyu~\citep{lyu2025}'s lemma and is not claimed in this argument.

\paragraph{AboveThreshold law.}
...
\noindent \  With
  ...
  \]
  makes the corresponding privacy cost at most
  \((\varepsilon/4,\delta/2)\), conditional on the later proof that the
  current queries have sensitivity one. This fixes the allocation;
...
No external theorem is needed here. The standard-Borel assertion is proved
directly by a
...
No external theorem is needed. The proof uses the kernel-measurability
propositions and finite-measure arguments.
...
No external paper theorem is used directly in this argument.  The 
parameter calibration above has already discharged its source-side premises.
...
Apply the union bound to (23), retain the four-term envelope from
...
The finite-population estimate in (5) is the only probabilistic input needed
for the fixed-point calculation below; all remaining steps are algebraic.
...
\paragraph{Valid decompositions and DDim.}
...
\paragraph{Existence of finite decompositions.}
...
\paragraph{Factor-two comparison.}
...
\paragraph{Essential hypotheses.}
...
\paragraph{Essential-list cardinality.}
...
Proposition~\ref{prop:step-008-essential-lists}. The present argument
establishes the pointwise structural and size conclusions; the DDim descent
and common-support implications are proved in the subsequent propositions.
...
These statements establish the restriction-list conclusions. They export
...
irreducibility scales to the DDim-descent, mechanism-utility, SOA-utility,
and PAC-conversion arguments. They
do not prove the DDim-descent argument, a common-score stage, or any later
...
branches upstream. The next argument uses only (3). It does not repeat a stage
union, make a block-independence claim, or change the event's probability.

\paragraph{Current-notation factor-two comparison.}

Lyu~\citep{lyu2025}'s Lemma 4.3 states the
...
from that result; and \(p_{r+1}=2p_r\) is exact. Thus (7)-(8)
...
\paragraph{Essentiality and common support.}
...
4. On nonrealizable arbitrary inputs, some restrictions may be empty; the
    the totalization assigns them empty lists without evaluating DDim or
...
These statements establish the DDim-descent conclusions: finite DDim
...
interface. They export only the common-stage producer to the
mechanism-utility argument. They do
...
The following two mechanism laws are used in the forms given by
Lyu~\citep{lyu2025}; their current-notation instantiations are recorded
explicitly before the mechanism-utility propositions.

\paragraph{Sparse-sampling interface.}
...
\noindent \ The sampled sparse law is as follows. Given an arbitrary domain
  ...
  The exponent has no factor \(1/2\), and \(\perp\) is a separate
...
\noindent \  Lemma~\ref{lem:step-001-calibration}
  ...
  the calibration in Lemma~\ref{lem:step-001-calibration}. Lemma~\ref{lem:step-010-score-support} below
  proves the effective support bound. Lemma 3.1's privacy conclusion is not
  used to claim raw privacy in this argument: raw-record-to-one-list adjacency
  remains for the raw-record privacy argument below.
\noindent \  Only the exact categorical law (5), its distinguished
  ...
  calculation below; no further utility assertion about the mechanism is
  needed.

\paragraph{AboveThreshold interface.}
...
\noindent \ Its privacy guarantee is the following: for sensitivity-one adaptive queries and at most
  ...
  The same construction permits the stated Above/Below symmetry and concurrent
  composition.
...
\noindent \  Lemma~\ref{lem:step-010-score-support} proves
  ...
  allocation. This is a list-coordinate sensitivity statement; raw-input
  privacy follows only after the record-locality argument below.
\noindent \  The exact independent Laplace tests in (6) are used for
  mechanism utility. The raw privacy conclusion is proved separately.


\noindent \ Lemma~\ref{lem:step-001-calibration}, recorded in (1), supplies the
...
\noindent \ Proposition~\ref{prop:step-009-termination}, Finite DDim
  ...
  realizable event in question. It incurs no mechanism randomness,
...
\noindent \ Proposition~\ref{prop:step-004-projection}, says that summing out
  the mark returns exactly \(K_C\). It does not assert that the marked pair
  is private; that property is handled in the privacy subsection.
...
the list-tuple interface; the raw-record implication is proved in
Proposition~\ref{prop:step-011-list-locality}.
...
\noindent Propositions
  ...
  for \(d=0,N=0\) the law is exactly \(\delta_{\bar c_0}\). We use these
  kernels and event sections below without introducing a selector or
  reopening a measurability argument. All probability inequalities below are
  kernel inequalities for every \(E\in\mathcal H_C\), and the final quotient
  inequality pulls back through (2).
...
\noindent For the positive branch and every indexed block \(i\),
  ...
  an empty restriction, while Proposition
  ~\ref{prop:step-008-list-envelope} gives (3) unconditionally. Identical
  ordered block data give identical restrictions, decomposition choices,
  ordered lists, and sanitation outcomes; hence one record replacement can
  change only the list coordinate of its containing block. No
  event-conditioned cross-block inclusion, DDim descent, common item, or
  utility event is used here.

\noindent Lemma~\ref{lem:step-010-score-support} states that for finite sets
  \(L_1,\ldots,L_k\subseteq H_C\), each of size at most \(L\),
  ...
  \tag{5}
  \]
  changes by at most one when one entire list coordinate is replaced. Its
  support \(\bigcup_iL_i\cup\{\perp\}\) is finite of size at most \(kL+1\),
  has a positive finite normalizer, and has score zero for an all-empty tuple.
  The sanitized tuple
  \((\mathcal L_1^r,\ldots,\mathcal L_k^r)\) at each stage, including tuples
  with some or all lists empty, is covered by
  Proposition~\ref{prop:step-011-list-locality}. This is the
  raw/quotient-record-to-one-list implication; no mechanism-good or
  realizability event is needed. Thus (5) is a sensitivity-one query at every
  common prefix, and the selected-stage Sparse Sample law has finite support
  on every input.

\paragraph{Sparse-sampling privacy.}
...
\paragraph{AboveThreshold privacy.}
...
\noindent \ With \(K=1\), \(\delta_a=\delta_{\rm AT}:=\delta/2\), and the
  a constant \(c_{\rm AT}\geq1\) is fixed large enough that
...
\paragraph{Actual essential functions.}
...
optimal decomposition and dimension premises are already discharged by the
restriction-list propositions. Equation (14) supplies literal function identity, not
equality only on \(\bar S\).

\paragraph{Empirical restriction contradiction.}

The printed statement of Lyu~\citep{lyu2025} Theorem 3 contains the malformed
phrase \(\operatorname{SOA}_{\mathcal G}(h)\). That phrase is not used. The
...
\paragraph{Irreducible SOA family.}
...
Lemma 4.2, applied in (16)--(17), is now discharged with
...
functions pointwise. Item 6 follows from (11) and the status argument already
...
used in this argument. The Bernoulli lower tail is derived directly in
...
representatives. This argument uses only (7)--(8) and the block-local definition
...
This interface gives a deterministic full-master empirical certificate on
...
\(\bar H\in\mathcal G_i(\bar S_i)\) when \(J=i\), and Proposition
...
No paper theorem is invoked directly in this argument. All source-theorem
applications were checked inside the preceding result; the following argument uses
...
restriction and the arms outside this result. \(\square\)
...
\(K=\max\{1,4K_{\rm fp}\}\) and \(q=4\), and checks \(v=1\), \(v=d\), and
every allowed fixed \(\delta\). Proposition~\ref{prop:step-014-zero}
separately preserves the exact \(d=0\) baseline.

These named results imply Proposition~\ref{prop:step-014-vc-arm}, the
...
We apply the structural and mechanism statements of Lyu~\citep{lyu2025} to
the quotient class \(\bar C\), with the source domain identified with \(Q_C\).
Lemma~\ref{lem:step-002-ld} gives
\(\operatorname{LD}(\bar C)=d\), so every application below has the required
dimension premise. No cited result is applied directly on the raw space \(X\).

\paragraph{Quotient interfaces.}

\noindent Proposition~\ref{prop:step-002-record-map} says that \(T_N\) is
measurable and preserves equality or one-record replacement. Proposition
\ref{prop:step-002-iid-pushforward} gives
\(T_N(S)\sim P_{\bar D,\bar c}^N\) for realizable iid samples, while
Proposition~\ref{prop:step-002-risk} gives the exact risk identity (D1) for
every \(\bar h\in H_C\), including improper outputs. Lemma
\ref{lem:step-003-countable-promotion} promotes pointwise laws on the
countable quotient input to Markov kernels. Finally, Lemmas
\ref{lem:step-004-occurrence} and \ref{lem:step-004-mark-kernel} together
with Propositions~\ref{prop:step-004-lift} and
\ref{prop:step-004-projection} provide the finite occurrence mark and its
exact output projection; Proposition~\ref{prop:step-015-mark} verifies the
same construction for the old lists.

\paragraph{Without-replacement tail.}
...
\noindent For a fixed binary population of size \(N\), mean
  ...
  \tag{C1}
  \]
\noindent With \(N=n_{\mathrm o}\), \(m=m_{\mathrm o}\), and
\(\xi=1/(5d)\), each partition block is marginally uniform without
replacement conditional on the indexed master sample. This bound is used
only for \(\mu>\gamma_{\mathrm o}/3\) in
Lemma~\ref{lem:step-015-tails}; the near-zero branch is handled separately.

\paragraph{Decomposition and essentiality facts.}
...
\noindent In the old-arm application,
\(\mathcal G=H_{\mathrm o,i_*}^{r+1}\),
...
\noindent The threshold (C4) is built into Lemma~\ref{lem:step-015-dictionary};
  ...
  directly from the displayed weights.
...
\paragraph{Utility implication of the structural construction.}

\noindent Lyu~\citep{lyu2025}'s structural construction specifies the
...
\noindent We use the empirical contradiction and stage conventions only;
the printed expression \(\operatorname{SOA}_{\mathcal G}(h)\) and the
probability-one amplification sentence are not invoked. Kernel,
high-confidence, marked-reconstruction, and raw-DP claims are proved in the
current notation below. The stage conventions and empirical contradiction are
...
\paragraph{Finite combinatorial and composition facts.}
...
\ref{lem:step-015-locality}, the privacy bounds (C4)--(C5),
...
the exact common-interface result and export the common-interface
\[
\boxed{
...
5. On \(d\ge1\), let \(N_*:=N_{j_*}\) be the actual sample size selected in
   ...
   is minimal,
   Define \(q_*:=6\). Since \(\Lambda\ge1\) and
   \(K_*=\max\{8,K_{\mathrm{old}},K_{\mathrm V}\}\ge K_{\mathrm V}\),
