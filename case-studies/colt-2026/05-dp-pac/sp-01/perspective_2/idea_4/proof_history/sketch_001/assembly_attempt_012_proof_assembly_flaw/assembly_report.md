...
- Sketch attempt: `1`.
- Assembly attempt: `12`.
- Retry context: aggregate-routed repair of the Step 016 public schedule
  synchronization defect after assembly attempt 11 was archived.
- Repair trigger: `proof_review.md`, SHA-256
  `7aa04a19af69a282b16bf1ebaf9dc276d72a29c13eb125d1395f36f47d2173e5`
...
- The public bundle is the nine files under `latex_template/` plus this
  controller-private report. Attempt 12 modified only
...
Attempt 12 synchronizes Proposition `prop:step-016-frontier`, Item 5, with
the accepted Step 016 selected-learner bridge and the already-correct
Corollary `cor:frontier`. Equation (7.2) gives
`N_* delta <= delta K_V Lambda^4 R_VC`, so the public selected-learner
schedule is exactly `delta K_V Lambda^4 R_VC -> 0`; the stale disclaimer
about a weaker selected-learner premise was removed. Equation (7.3) remains
the separate standalone old-arm implication
`delta K_O Lambda^6 R_old -> 0 => N_old delta -> 0`, with no claim that the
selected-learner schedule implies it. The repair did not change an accepted
claim, assumption, dependency, constant, rate, privacy mode, PAC mode,
horizon, boundary case, baseline arm, or conditional theorem scope.
...
`TECTONIC_CACHE_DIR=<TEMP_ROOT>/tectonic-cache-final3 <TECTONIC_BIN> --bundle <TEMP_ROOT>/tectonic-bundle.6ZSA5R --only-cached --keep-logs --outdir <TEMP_ROOT>/p2-assembly12-final.kaQ7In main.tex`

The resulting PDF is
`<TEMP_ROOT>/p2-assembly12-final.kaQ7In/main.pdf` (716,602 bytes). The final log
...
or citation diagnostics. The 93 existing explicit `\square` commands remain
...
- `latex_template/5_appendix.tex`: `c5e86ad10c022193807886f84d629e98af60cba8616b8f7641c23beff80e26c0`
