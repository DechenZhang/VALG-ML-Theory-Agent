...
\noindent Prove the \(d=0\) branch, define the exact positive-branch   allocations and least feasible teacher count, prove the \(\bar k\) witness   and teacher margin, and totalize every source failure/fallback path.

\paragraph{Supporting facts}



\paragraph{Lyu~\citep{lyu2025} Corollary 4.1, \texttt{coro: essential hypotheses}}

\noindent \ The cited result is applied under: Lyu~\citep{lyu2025} Definition 4.2 first fixes
...
\noindent \  Returning to the candidate teacher-count variable
...
\noindent \  On the positive branch, \(d\) is a positive integer,
...
\noindent \ The cited construction is: Given an arbitrary domain \(\mathcal U\), an
  ...
  \(B\).
\noindent \ The cited privacy statement is: If every list has size at most \(L\) and
  \[
...
\noindent \  \(\mathcal U=H_C\), \(t\) is a candidate teacher count,
...
\noindent \  The conditional list bound in
...
\noindent \ The cited statement is: For sensitivity-one adaptive queries, the source
...
\noindent \  The current process is stopped at its first Above report,
...
\noindent Prove bijective quotient factorization,   \(\operatorname{VC}(\bar C)=v\), \(\operatorname{LD}(\bar C)=d\),   \(T_N\) measurability and neighbor preservation, pushforward sampling, and   the exact decoder-risk identity.
...
The named propositions below give the measurable quotient, product-law, and
risk interfaces used in the proof.
...
including improper outputs and arbitrary fibers. Together, these propositions
prove every clause of the stated claim and the full target. The preceding
results are used only through their displayed statements.



\subsection{Countable-Domain Kernel Construction}\label{app:step_003}

\noindent Construct the measurable quotient Markov kernel for every   totalized pointwise law and prove its raw pullback is a kernel on   \(Z_X^N\).

\paragraph{Supporting facts}


No paper result is used in this step. The preceding kernel construction and
...
\noindent \  Under the primitive conditions discharged in its 
...
\noindent \  The exact quotient data spaces and exact output
  space appearing in the stated conclusion.
\noindent \  Countability and measurable quotient cells are
  exactly the primitive condition allowed in this step.
\noindent \  The input has the full power-set sigma-field, output
...
\noindent \  For every \(N\in\mathbb N_0\), the recordwise quotient
  map
  \[
...
\noindent \  The exact \(T_N\) in the raw pullback
  \(A_N(s,E)=K_N(T_N(s),E)\).
\noindent \  Its  proof derives measurability atom by
  atom from the measurable quotient cells. Its separate neighbor conclusion
  is not used in this step.
\noindent \  Composition of a quotient-kernel event coordinate with
  \(T_N\) is raw-input measurable.

\paragraph{Proposition~\ref{prop:step-002-risk}: measurable risk}
functional

\noindent \  For every fixed probability measure \(D\), target
...
\noindent \  Population-error events of the form
  \(\{\bar h:\operatorname{err}_{\bar D}(\bar h,\bar c)>a\}\).
\noindent \  The dependency proves the claim by finite sums or a
  monotone limit of countably many evaluation-coordinate sums.
\noindent \  Population-error sections are Borel output events.

\paragraph{Standard-Borel closure and diagonal facts}

\noindent \  Finite products and countable disjoint unions of standard
...
\noindent \  \(Y=H_C\), the finite-list space
...
\noindent \  Proposition~\ref{prop:step-002-borel} supplies that \(H_C\) is standard
  ...
  Polish space.
\noindent \  The list-membership relation and all transcript
  coordinate projections used below are measurable.

The countable-domain lemma, coding proposition, event proposition, quotient
kernel proposition, and raw-pullback proposition below provide the measurable
interfaces used in this subsection.
...
\noindent Lift the exact output law to   \(\widetilde K_C\) with a finite occurrence mark and prove the exact   projection identity.
...
\noindent \  For every \(N\in\mathbb N_0\), every family of
...
\noindent \  The exact totalized procedure has a standard-Borel
...
\noindent \  The list coordinates
  \(\lambda_{i,r}\), selected-stage coordinate, actual-output event
  \(\mathsf{Act}\), terminal output \(\bar H\), and transcript kernel
  \(\Gamma_N\) below.
\noindent \  The preceding kernel construction verifies the complete
  transcript and all boundary states. No separate dependency is imported here.
\noindent \  On an actual-output transcript, \(\bar H\) occurs in at
...
\noindent \  The relation between any generated finite list and an
...
\noindent \  Membership of \(\bar H(\tau)\) in each all-stage
  producer list and the status split \(\mathsf{Act}\sqcup
  \mathsf{Act}^c\).
\noindent \  Measurability is an  derived
  conclusion of kernel-measurability result; no truth of a utility event is assumed.
\noindent \  Every occurrence indicator and every mark weight below is
  measurable.

\paragraph{Proposition~\ref{prop:step-003-quotient-kernel}:}
Exact released marginal

\noindent \  For the exact totalized positive-dimensional procedure,
  ...
  \tag{1}
  \]
\noindent \  The exact \(K_C\), not an arbitrary coupling or a
  new algorithm, and the same terminal-output coordinate used in the marked
  lift.
\noindent \  The dependency proof is
    and covers every totalized input.
\noindent \  Equation (1) is the last equality in the projection
  proof.

The occurrence, mark-kernel, lift, and projection results below establish the
finite marked kernel and its exact released-output marginal.
...
privacy residual. These statements establish the exact sketch-row claim and
...
\noindent For a fixed quotient master sample   \(\bar S=((q_r,y_r))_{r=1}^{n_0}\), prove   \[   |\mathcal E_{\bar C}(\bar S)|\le (e n_0/v)^v   \]   on the positive branch.
...
The following statements provide the needed interfaces:
...
\noindent Prove the fixed-trace high-error and low-error block tails   and define the exact simultaneous \texttt{E\_good} event.
...
\noindent Solve the \(n_0=km\) fixed point, prove   \(\log(en_0/v)=O(\log Q)\), and charge \texttt{E\_good} failure to   \(\beta_{\mathrm{tr}}\).
...
These statements establish exactly the confidence-budget result sketch row: the fixed point,
...
\noindent Prove the exact source/current stage map, factor-two   Lemma 4.3 pairing, essential-list envelope, actual SOA identities, and   irreducibility scales on the quotient.
...
\noindent \ Lemma~\ref{lem:step-008-stage-map} supplies the exact endpoint and
  factor-two parameter identities and the \(\operatorname{LD}\le d\)
  premise for every later source application.
\noindent \ Proposition~\ref{prop:step-008-inclusion} supplies the sole class
  inclusion used in Proposition~\ref{prop:step-008-transition}.
\noindent \ Proposition~\ref{prop:step-008-decompositions} supplies optimal trees,
  finite leaves, and valid leaf irreducibility for the essential-list and
  leaf-scale propositions.
\noindent \ Proposition~\ref{prop:step-008-transition} supplies only the DDim
  comparison and equal-DDim actual-function inheritance exported to the
  later structural consumer.
\noindent \ Proposition~\ref{prop:step-008-essential-lists} supplies the list-size
...
\noindent \ Propositions~\ref{prop:step-008-list-envelope} and
...
These statements establish exactly the restriction-list result claim. They export
...
\noindent Prove finite DDim descent and a score-\(k\) common actual   quotient function by stage \(d\).
...
\noindent \ Lemma~\ref{lem:step-009-legal-state} derives nonemptiness before defining
  \(D_{i,r}\) and supplies attained integer potentials and scores.
\noindent \ Proposition~\ref{prop:step-009-equality-common} uses (7)-(8) to show that
  equality of consecutive maximum potentials forces \(q_r=k\).
\noindent \ Proposition~\ref{prop:step-009-unit-drop} takes the contrapositive of
...
\noindent \ Proposition~\ref{prop:step-009-zero-support} uses Corollary 4.1 Item 4
  and the realizable target, rather than nonnegativity alone, to produce
  common support.
\noindent \ Proposition~\ref{prop:step-009-termination} composes the one-unit drops
...
These statements establish exactly the DDim-descent result claim: finite DDim
...
\noindent Instantiate AboveThreshold and Sparse Sample, prove the   mechanism-good event, and show the output is an actual list item rather   than fallback.
...
\noindent \ The sampled sparse law is as follows. Given an arbitrary domain
  \(\mathcal U\), lists
...
\noindent \ Its privacy guarantee is the following: if every list has size at most \(L\) and
...
\noindent \  Lemma~\ref{lem:step-001-calibration}
...
\noindent \  Only the exact categorical law (5), its distinguished
...
\noindent \ The threshold process is as follows. On each received query \(q_r\),
...
\noindent \ Its privacy guarantee is the following: for sensitivity-one adaptive queries and at most
  ...
  composition.
\noindent \  One first-crossing process, \(K=1\), threshold
  \(k/2\), noise parameter \(\eta\), and the at most \(d+1\) queries in
  (3).
\noindent \  Lemma~\ref{lem:step-010-score-support} proves
...
\noindent \  The exact independent Laplace tests in (6) are used for
  mechanism utility. No privacy conclusion is exported from this result.

\paragraph{Mathematical dependencies}

\noindent \ Lemma~\ref{lem:step-001-calibration}, Legal positive-branch
  ...
  \[
    ...
    \tag{7}
  \]
\noindent \ Proposition~\ref{prop:step-001-teacher}, Finite public teacher
  witness and least feasible count, supplies exactly (2), with every
  ceiling retained.
\noindent \ Proposition~\ref{prop:step-001-totalization}, Pre-sampling
...
\noindent \ Lemma~\ref{lem:step-009-legal-state}, Legal realizable stage
  ...
  \(q_r=0\).
\noindent \ Proposition~\ref{prop:step-009-termination}, Finite DDim
...
\noindent \ Lemma~\ref{lem:step-004-occurrence}, Measurable finite
...
\noindent \ Proposition~\ref{prop:step-004-lift}, Marked lift of the exact
  quotient output law, supplies the analysis-only marked kernel.
\noindent \ Proposition~\ref{prop:step-004-projection}, Exact released
...
These statements establish exactly the mechanism guarantee: one adaptive
...
\noindent Prove all-input quotient and raw replacement   \((\varepsilon,\delta)\)-DP for the released output.
...
\noindent \  The current VC-sensitive quotient learner at its
...
\noindent \  The preceding measurable-kernel propositions establish the
...
\noindent \  All probability inequalities below are legitimate kernel
...
\noindent \  For the positive branch and every indexed block \(i\),
...
\noindent \  The same data-independent partition and the same
  \(d+1\) block-local stage lists consumed by AboveThreshold and Sparse
  Sample.
\noindent \  The dependency's finite-Littlestone and source
...
\noindent \  Identical ordered block data give identical restrictions,
...
\noindent \  Lemma~\ref{lem:step-010-score-support} states
...
\noindent \  Equation (5) is a legal sensitivity-one query at every
...
\noindent The cited Lyu~\citep{lyu2025} source is identified by v1 labels
\texttt{algo:private sample} and
  \texttt{lemma: privacy of sparse sampling}, at the source statement in
\noindent \  Given an arbitrary domain
...
\noindent \ With \(\mathcal U=H_C\),
  ...
  \[
    ...
    \tag{7}
  \]
\noindent \  Equation (3) supplies the cap, (7) implies (6), and
  ...
  \[
    ...
    \tag{8}
  \]
\noindent \  The selected-stage conditional kernel in Proposition
...
\noindent The cited Lyu~\citep{lyu2025} source is identified by v1 labels
\texttt{algo: abovethreshold} and
  \texttt{lemma: privacy of abovethreshold}, at the source statement in
\noindent \  For sensitivity-one adaptive
...
\noindent \ With \(K=1\), \(\delta_a=\delta_{\rm AT}:=\delta/2\), and the
  ...
  \(\varepsilon/4\).
\noindent \  Proposition~\ref{prop:step-011-list-locality}
...
\noindent \  Its finite first-crossing transcript is
...
\(d=0,N=0\) branch is \((0,0)\)-DP. These statements establish precisely the
 privacy guarantee. They prove no empirical utility, population
...
\noindent Prove exact SOA membership and   \(e_{\bar S}(\bar H)\leq\alpha/8\) for actual outputs on the source-good   path.
...
If \(S=((x_j,y_j))_{j=1}^n\) is indexed, \(\varnothing\ne\mathcal G\subseteq
\{0,1\}^{\mathcal X}\) is \(n\)-irreducible,
\(h=\operatorname{SOA}_{\mathcal G}\), and every \(g\in\mathcal G\) has
\(e_S(g)\le2u\), then \(e_S(h)\le2u\).
...
\noindent \ Proposition~\ref{prop:step-012-selected-leaf} supplies the exact
...
\noindent \ Lemma~\ref{lem:step-012-leaf-error} supplies the all-\(g\) empirical
  premise to Lemma~\ref{lem:step-012-empirical}.
\noindent \ Lemma~\ref{lem:step-012-empirical} and
  Proposition~\ref{prop:step-012-fixed-family} supply the two exported
  conclusions assembled below.
\noindent \ Proposition~\ref{prop:step-012-boundaries} verifies that the same named
...
regime. These statements establish exactly the SOA utility guarantee and
supply the empirical-error/actual-SOA interface to the PAC conversion. They prove
...
\noindent Prove measurable marked producer-block reconstruction,   (Holdout), the one-sided lower-tail Chernoff bound, finite multiplicity   domination, and quotient population error at most \(\alpha\).
...
Lemma~\ref{lem:step-013-lower-tail}. The preceding propositions are stated
below in current notation.

\paragraph{Mathematical dependencies}

\noindent \ Lemma~\ref{lem:step-004-occurrence} states that the all-stage
  ...
  \(0\).
\noindent \ Proposition~\ref{prop:step-004-lift} constructs the marked
...
\noindent \ Proposition~\ref{prop:step-004-projection} gives, for every
  \(B\in\mathcal H_C\),
  \[
...
\paragraph{Finite list envelope}
...
\paragraph{Empirical SOA certificate}
...
\noindent \ Lemma~\ref{lem:step-013-block-family} supplies the measurable finite
...
\noindent \ Proposition~\ref{prop:step-013-pathwise} supplies the mandatory
  full-path inclusion, before any conditioning, to
  Proposition~\ref{prop:step-013-finite-integration}.
\noindent \ Lemma~\ref{lem:step-013-lower-tail} supplies the fixed-candidate iid
  complement estimate to
  Proposition~\ref{prop:step-013-finite-integration}.
\noindent \ Proposition~\ref{prop:step-013-finite-integration} and
  Lemma~\ref{lem:step-013-beta-gen} supply the core population-failure
  charge to Proposition~\ref{prop:step-013-pac}.
\noindent \ Proposition~\ref{prop:step-013-pac} combines that charge with (1), (6),
...
bound exactly to the released quotient law. These statements establish the
 exact PAC conversion. They do not condition on \(J\) for independence,
...
\noindent Eliminate all VC-arm auxiliaries and prove the displayed   conditional \(R_{\mathrm{VC}}\) rate with no hidden positive power.
...
The interface, elimination, rate, and zero-dimensional propositions below are
the only facts needed for the VC-arm specialization.
...
\noindent Independently instantiate the quotient-first old-Lyu~\citep{lyu2025}   construction with coarse \(d\)-trace calibration and prove its own   DP/PAC/rate bound.
...
\noindent \ Proposition~\ref{prop:step-002-record-map}: for every finite
...
\noindent \ Proposition~\ref{prop:step-002-iid-pushforward}: if
  \(S\sim P_{D,c}^N\), then
  \(T_N(S)\sim P_{\bar D,\bar c}^N\). It is used in
  Proposition~\ref{prop:step-015-pac}.
\noindent \ Proposition~\ref{prop:step-002-risk}: identity (D1) holds for
...
\noindent \ Lemma~\ref{lem:step-003-countable-promotion}: an arbitrary
...
\noindent \ The cited source is identified by prop:chernoff-sample-without-replacement.
\noindent \  for a fixed binary population of size \(N\), mean
  ...
  \tag{C1}
  \]
\noindent \ With \(N=n_{\mathrm o}\), \(m=m_{\mathrm o}\),
  \(\xi=1/(5d)\), and one fixed quotient error trace.
\noindent \  conditional on the complete indexed master sample, each
...
\noindent \ The cited source is identified by def: irreducible, def: p-decomposition,
...
\noindent \  if \(\mathcal H\) has Littlestone dimension at most
...
\noindent \ With \(\mathcal G=H_{\mathrm o,i_*}^{r+1}\),
  \(\mathcal H=H_{\mathrm o,i}^{r}\), and
  \(2p=p_{\mathrm o,r+1}\), \(p=p_{\mathrm o,r}\).
\noindent \  Lemma~\ref{lem:step-015-source-map} proves the class inclusion
...
\noindent \ The cited source is identified by lemma: Ldim-of-SOA.
\noindent \  for
  \[
  \widehat{\bar C}_{d+1}
...
\noindent \  Lemma~\ref{lem:step-002-ld} gives
...
\noindent \ The cited source is identified by algo:private sample and
  lemma: privacy of sparse sampling.
\noindent \  given \(k\) lists \(\mathcal L_i\), define
...
\noindent \ With the old selected-stage lists, \(L=L_{\mathrm o}\),
  \(B=B_{\mathrm o}\), \(\varepsilon_{\mathrm{SS}}=\varepsilon/8\), and
  \(\delta_{\mathrm{SS}}=\delta/2\).
\noindent \  (C4) is built into Lemma~\ref{lem:step-015-dictionary};
...
\noindent \ The cited source is identified by algo: abovethreshold and
  lemma: privacy of abovethreshold.
\noindent \  a stopped AboveThreshold process receiving adaptive
...
\noindent \ The cited source is identified by thm: DP-ERM-littlestone and coro: PAC learning.
\noindent \  the proof of Theorem 3 specifies the
...
\noindent \ The following source statements are not used: the malformed theorem display
  ...
  proof.
\noindent \ We use only the stage conventions and empirical contradiction are
...
\noindent \ Sauer--Shelah~\citep{sauer1972}: if a binary class has VC dimension at most \(d\) and
...
\noindent \ Maclaurin's inequality: for nonnegative \(w_1,\ldots,w_N\),
  \[
  ...
  nonnegative.
\noindent \ Bernoulli lower-tail Chernoff bound: if \(X_1,\ldots,X_M\) are
  ...
  \(\theta=3/4\).
\noindent \ Adaptive sequential composition and postprocessing: if the first
...
confidence, and \(\delta\)-schedule boundaries. These statements establish
...
\noindent Prove the finite-class arm, take the minimum only after all   arms share interfaces, and state \texttt{d=0}, \texttt{v=d}, finite/infinite, and   small-\texttt{delta} baselines.
...
\noindent \  Under the primitive assumptions discharged in its 
...
\noindent \  the fixed finite set \(\bar C\subseteq H_C\), the
  empirical mistake-count law on \(Z_Q^N\), its raw pullback, and the raw and
  quotient bad-risk events.
\noindent \  Proposition~\ref{prop:step-002-factorization},
  ...
  \ref{assump:countable-evaluation-quotient}, and
  recorded above.
\noindent \  \(|\bar C|=|C|\), exact raw adjacency transport, exact iid
  probability transport, and zero-residual decoded-risk transport.

\paragraph{Quotient-kernel and raw-pullback results}

\noindent \  Lemma~\ref{lem:step-003-countable-promotion} makes every pointwise Borel
...
\noindent \  the finite exponential-weight law (2.2) below and its
  pullback \(A_N^{\mathrm{fin}}\).
\noindent \  Assumption~\ref{assump:countable-evaluation-quotient},
...
\noindent \  measurable quotient and raw finite-arm kernels, including
  arbitrary labels and \(N=0\).

\paragraph{Normalized VC-arm proposition}

\noindent \  Under all four setting assumptions, there is a complete
...
\noindent \  all kernel, raw-neighbor, event, privacy, PAC, and rate
  interfaces were derived in the preceding result and assembled by Proposition~\ref{prop:step-014-vc-arm}.
\noindent \  one complete learner threshold in the deterministic
  minimum, plus its exact null and schedule boundaries.

\paragraph{Normalized old-Lyu~\citep{lyu2025} propositions}

\noindent \  Under all four setting assumptions, Proposition~\ref{prop:step-015-kernel} names the old quotient kernel
  \(K_{\mathrm o}=K_C^{\mathrm{old\text{-}Lyu}}\), and 
...
\noindent \  the 
  ...
  and \(N_{\mathrm o}\).
\noindent \  Propositions~\ref{prop:step-015-kernel}, \ref{prop:step-015-dp},
...
\noindent \  the second complete threshold in the deterministic minimum
  and the old arm's separate numerical schedule.

\paragraph{Interface consequences}
...
has cost \(B_{\mathrm o}:=K_{\mathrm{old}}\Lambda^6R_{\mathrm{old}}\). The
...
these seven propositions prove every clause of the stated claim and the full
target. The argument uses only the displayed results.
