...
- Derived invariants and interfaces supplied by accepted dependencies:
  - Accepted Lemma~\ref{lem:step-004-occurrence} and
    ...
    \ref{prop:step-004-projection} supply the measurable finite occurrence
    mark, positive-mark support on actual paths, the marked Markov kernel,
    and the exact released-output projection.
  ...
  - Accepted Proposition~\ref{prop:step-010-mechanism-good} supplies the
    measurable mechanism event, its conditional failure charge
    \(\beta_{\mathrm{AT}}+\beta_{\mathrm{SS}}\), actual rather than fallback
    status on that event, selected-list membership, and positive occurrence
    marking.
...
status `ACCEPTED` for sketch attempt 1.
...
item, not fallback; its accepted marked occurrence is positive and obeys
...
Under accepted Lemma~\ref{lem:step-004-occurrence} and accepted
...
- Constant provenance:
  ...
  introduced.
- Assumption provenance: iid realizable quotient sampling is the sole
  primitive condition used. Candidate lists, events, actual output,
  empirical error, mark support, list cap, fixed point, and projection are
  accepted derived outputs. Block-local measurability, Holdout, pathwise
  inclusion, the Bernoulli tail, finite integration, multiplicity
  domination, and the final ledger are proved by the six named local
  results.
...
- Global diagnostic boundary: the paired global-proof review is `ACCEPTED`
  at the hashes in `## Step Identity`. Its Step 013 planning warning to
  prove the pathwise finite inclusion before conditioning was followed, but
  every inclusion, tail, scalar domination, and probability conversion
  above was independently proved from the setting and accepted dependency
  artifacts. Neither diagnostic artifact is proof evidence, a cited
