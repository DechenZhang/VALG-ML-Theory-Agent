...
most \(2u\); the contradiction is reproduced in
Lemma~\ref{lem:step-012-empirical}.  Finally, Lyu's Lemma~4.2 defines
\[
...
The following two finite inequalities are proved inline below rather than
imported from an external source. If \(N\ge1\), \(0\le m\le N\), and
\(w_1,\ldots,w_N\ge0\), then, with \(e_m\) denoting the degree-\(m\)
elementary symmetric polynomial, \(e_0=1\), and a zeroth power interpreted
as one,
\[
 e_m(w_1,\ldots,w_N)
 ...
 \tag{C6}
\]
If \(M\in\mathbb N_0\), \(X_1,\ldots,X_M\) are independent Bernoulli
variables of common mean \(p\in[0,1]\), \(S=\sum_{j=1}^M X_j\), and
\(0<\theta<1\), then
\[
 \Pr[S\le M(1-\theta)p]
 \le\exp(-\theta^2pM/2).
...
pairwise-smoothing proof of (C6) and the exponential-Markov proof of (C7)
are given in Lemmas~\ref{lem:step-015-tails} and
\ref{lem:step-015-lower-tail}, respectively; neither inequality is used as
