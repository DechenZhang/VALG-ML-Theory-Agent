...
Under Assumption~\ref{assump:approximate-dp-regime}, Lemma~\ref{lem:step-001-calibration}, Proposition~\ref{prop:step-001-teacher}, Proposition~\ref{prop:step-001-totalization}, Lemma~\ref{lem:step-009-legal-state}, Proposition~\ref{prop:step-009-termination}, and the finite-mark results of Step~004, the positive-dimensional quotient construction has the following two mechanism interfaces.
...
Lemma~\ref{lem:step-010-score-support} proves sensitivity one for a list
replacement and finiteness of the selected support.  Lemma~\ref{lem:step-009-legal-state}
...
Under Assumptions~\ref{assump:finite-littlestone},
\ref{assump:countable-evaluation-quotient}, and
\ref{assump:approximate-dp-regime}, suppose \(d\ge1\).  The old-arm
construction on \(Q_C\) satisfies the following interfaces.
...
kernel.  The finite occurrence construction supplies an analysis-only mark,
and the old marked law projects exactly to its released quotient law.
...
\[
 ...
 \tag{C1}
\]
In the old construction \(N=n_{\mathrm o}\), \(m=m_{\mathrm o}\), and
\(\xi=1/(5d)\); conditional on the indexed master sample, each partition
block is marginally uniform without replacement.  The bound is used for
\(\mu>\gamma_{\mathrm o}/3\), with the near-zero branch handled separately.
...
\(p^d2^{d^2}\) essential hypotheses (all members at DDim zero).  The
old-arm substitution is
\(\mathcal G=H_{\mathrm o,i_*}^{r+1}\),
\(\mathcal H=H_{\mathrm o,i}^{r}\),
\(2p=p_{\mathrm o,r+1}\), and \(p=p_{\mathrm o,r}\).
...
\[
 ...
 \tag{C3}
\]
has Littlestone dimension at most \(d\).  For old selected-stage lists,
the Sparse Sample law uses score \(s(h)=|\{i:h\in\mathcal L_i\}|\), failure
score \(B_{\mathrm o}\), list cap \(L_{\mathrm o}\),
\(\varepsilon_{\mathrm{SS}}=\varepsilon/8\), and
\(\delta_{\mathrm{SS}}=\delta/2\), and satisfies
\[
 B_{\mathrm o}\geq
 \frac{10\log(L_{\mathrm o}/\delta_{\mathrm{SS}})}
      {\varepsilon_{\mathrm{SS}}}
 \quad\Longrightarrow\quad
 (2\varepsilon_{\mathrm{SS}},\delta_{\mathrm{SS}})\text{-DP}
 \tag{C4}
\]
under one-list replacement.  The stopped AboveThreshold interface has
\[
 \bigl(c_{\mathrm{AT}}\eta_{\mathrm o}
 [\sqrt{\log(1/\delta_{\mathrm{AT}})}+\log(1/\delta_{\mathrm{AT}})],
 \delta_{\mathrm{AT}}\bigr)\text{-DP},
 \tag{C5}
\]
with \(\delta_{\mathrm{AT}}=\delta/2\), threshold \(k_{\mathrm o}/2\), and
at most \(d+1\) queries.  Its simultaneous noise threshold is
\(\eta_{\mathrm o}^{-1}\log((d+1)/\beta_{\mathrm{AT,o}})\), except with
probability \(\beta_{\mathrm{AT,o}}\).

The usable structural content of Lyu~\citep{lyu2025}, Theorem 3 and
Corollary 5.1, is the \(d+1\)-stage restriction/list/AboveThreshold/
Sparse-Sample construction and the implication that an actual essential
output \(\operatorname{SOA}_{\mathcal G}\), when the members of a sufficiently
irreducible \(\mathcal G\) are empirically accurate, is empirically accurate.
The current proof rederives the stage map and empirical implication in
Lemma~\ref{lem:step-015-source-map} and Proposition~\ref{prop:step-015-soa};
it does not use the malformed displayed expression or an unsupported
probability-one amplification.
...
\[
 ...
 \tag{C6}
\]
and the Bernoulli lower-tail bound gives
\[
 ...
 \tag{C7}
\]
Adaptive sequential composition adds the privacy pairs and measurable
postprocessing or common-randomness mixing does not increase them.
\end{proposition}

\begin{proof}
The first paragraph is the combination of Propositions~\ref{prop:step-002-record-map},
\ref{prop:step-002-iid-pushforward}, and \ref{prop:step-002-risk},
Lemma~\ref{lem:step-003-countable-promotion}, and the finite-mark lemmas;
Proposition~\ref{prop:step-015-mark} verifies the same mark construction for
the old lists.  The without-replacement inequality is Lyu~\citep{lyu2025}'s
Proposition~2,
and the stated values of \(N,m,\xi\) follow from the old partition.

The decomposition, leaf, irreducibility, and DDim assertions are Lyu~\citep{lyu2025}'s
Definitions~4.2--4.3, Corollary~4.1, and the associated decomposition lemmas,
after substituting the endpoint map proved in
Lemma~\ref{lem:step-015-source-map}; all restrictions are subsets of \(\bar C\),
and empty restrictions are handled by the totalization.  Lemma~\ref{lem:step-015-lists}
proves the essential-list cap and irreducible witness, while
Proposition~\ref{prop:step-015-descent} proves the finite descent and common
score.  The family bound (C3) follows from Lyu~\citep{lyu2025}'s Lemma~4.2 and
\(\operatorname{LD}(\bar C)=d\) from Lemma~\ref{lem:step-002-ld}.

Lemma~\ref{lem:step-015-dictionary} supplies the threshold in (C4), and
Lemmas~\ref{lem:step-015-lists} and \ref{lem:step-015-locality} supply its
list cap and adjacency.  The old AboveThreshold parameters and sensitivity
are those in Lemma~\ref{lem:step-015-at} and
\ref{lem:step-015-locality}; its tail follows by the Laplace density.
The trace, Maclaurin, Bernoulli, composition, and postprocessing statements
are applied exactly in Lemmas~\ref{lem:step-015-traces},
\ref{lem:step-015-tails}, \ref{lem:step-015-lower-tail}, and
Proposition~\ref{prop:step-015-dp}.  Thus every displayed interface is a
direct current-notation instantiation of the cited result or a named local
proof used below.
\end{proof}
