...
\[
 m_C(\alpha,\beta;\varepsilon,\delta)=\inf\left\{N:
 \sup_D\sup_{c\in C}\Pr_{S\sim P_{D,c}^{N},\,\bar H\sim A_N(S,\cdot)}
 [\operatorname{err}_D(\operatorname{Dec}_C(\bar H),c)>\alpha]\le\beta\right\},
\]
where the first supremum ranges over all probability measures on $(X,\Sigma)$.
Learners may be computationally unbounded and need not be proper.
