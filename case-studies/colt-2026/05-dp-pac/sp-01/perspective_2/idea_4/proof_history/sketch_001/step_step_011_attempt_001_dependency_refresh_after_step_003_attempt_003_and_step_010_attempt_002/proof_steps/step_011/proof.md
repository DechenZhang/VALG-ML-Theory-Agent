...
- Derived invariants and interfaces supplied by accepted dependencies:
  ...
  - Accepted Proposition~\ref{prop:step-003-quotient-kernel}, **Quotient
    ...
    labels, duplicates, invalid-source sanitation, fallback, and the exact
    \(d=0,N=0\) Dirac law.
...
| `unit_007` | proposition | Under accepted Proposition~\ref{prop:step-003-raw-pullback}, Lemma~\ref{lem:step-011-record-locality}, and Proposition~\ref{prop:step-011-quotient-dp}, the raw kernel in (2) is \((\varepsilon/2,\delta)\)-DP and therefore \((\varepsilon,\delta)\)-DP; the \(d=0,N=0\) law is \((0,0)\)-DP. | Gives the exact quotient-to-raw pullback and closes all requested null and boundary cases. |
...
Under Assumption~\ref{assump:approximate-dp-regime}, accepted Proposition
...
For \(d=0\), accepted Proposition
~\ref{prop:step-003-quotient-kernel} supplies \(N=0\) and the exact law
\(\delta_{\bar c_0}\) on the unique empty quotient input. Accepted
Proposition~\ref{prop:step-003-raw-pullback} gives the same Dirac law on the
...
Finally, accepted Proposition~\ref{prop:step-003-raw-pullback} and
...
- Baseline-reduction check: At \(d=0,N=0\), the mechanism reduces exactly to
  the accepted no-data Dirac law and is \((0,0)\)-DP. At formal \(k=2\), all
...
- Accepted dependency objects:
  ...
  ~\ref{prop:step-003-quotient-kernel}, and
  ~\ref{prop:step-003-raw-pullback}. The block restrictions and lists are
...
- Boundary discipline:
  \(d=0,N=0\) is the exact no-mechanism Dirac branch. On the positive branch,
