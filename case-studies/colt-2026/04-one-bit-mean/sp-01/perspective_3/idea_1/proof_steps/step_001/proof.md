...
### Minkowski and reverse triangle inequalities in \(L_k(D)\)

For \(k\geq1\) and finite \(L_k(D)\) functions \(U,V\), Minkowski's
inequality and its reverse-triangle consequence state
\[
  \|U+V\|_{L_k(D)}\leq\|U\|_{L_k(D)}+\|V\|_{L_k(D)},
  \qquad
  \bigl|\|U\|_{L_k(D)}-\|V\|_{L_k(D)}\bigr|
  \leq\|U-V\|_{L_k(D)}.
\]
The second inequality follows by applying the first to
\(U=(U-V)+V\), and then interchanging \(U\) and \(V\).
Assumption~\(\ref{assump:parameter-domain}\) gives \(k>1\), while
Assumption~\(\ref{assump:moment-class}\) gives
\(\|X-\mu\|_{L_k(D)}\leq\sigma\); constant functions also belong to
\(L_k(D)\). Thus the inequalities apply to \(X-\mu\), \(\mu-m\),
\(X-m\), and \(X-m'\) for every fixed \(m,m'\in\mathbb R\). Their
conclusion is used in
Proposition~\(\ref{prop:step-001-independence}\) to prove that the shifted
moment is a Borel function of its center.
...
\]
If in addition \(|c-\mu|\leq50\sigma\) and \(a_k\geq200\), then
\[
...
\(|c-\mu|\leq50\sigma\) and \(a_k\geq200\), then the setting-derived
quantity
\[
...
**Proof / justification.** Fix any realized scalar \(c\) on the derived event
\(\mathcal E_{\rm loc}\); then \(m_0(c)\) is deterministic in the integral
defining \(M_k(c)\). For every
\(x\in\mathbb R\),
\[
  |x-m_0|^k
  =|(x-\mu)+(\mu-m_0)|^k
...
For this fixed \(D\), define the shifted \(L_k\)-norm map
\[
  \Phi_D(m):=\left(\int_{\mathbb R}|x-m|^kD(dx)\right)^{1/k},
  \qquad m\in\mathbb R.
\]
The restated Minkowski inequality and
Assumption~\(\ref{assump:moment-class}\) give
\[
  \Phi_D(m)
  \leq \|X-\mu\|_{L_k(D)}+|m-\mu|
  \leq \sigma+|m-\mu|<\infty.
\]
Thus \(\Phi_D\) is finite everywhere. By the restated reverse triangle
inequality, for every \(m,m'\in\mathbb R\),
\[
  |\Phi_D(m)-\Phi_D(m')|
  \leq\|(X-m)-(X-m')\|_{L_k(D)}
  =|m-m'|.
\]
Hence \(\Phi_D\) is 1-Lipschitz and therefore Borel measurable. The map
\(c\mapsto m_0(c)\) is Borel by
Lemma~\(\ref{lem:step-001-core}\), so
\[
  M_k(c)=\Phi_D(m_0(c))^k
\]
is a Borel function of \(c\). Since \(c\) is
\(\mathscr L_{\rm loc}\)-measurable, \(M_k(c)\) is consequently
\(\mathscr L_{\rm loc}\)-measurable.
...
- **Proof-local objects.** The constant seed value \(r_\star\), the scalar
  ...
  proof-local and are not exported. The function
  \(\Phi_D(m)=(\int|x-m|^kD(dx))^{1/k}\) is likewise proof-local: it is
  introduced only to prove Borel measurability of the public-facing
  \(M_k(c)\) and is not exported.
