...
- Intended claim: Conditional on the localization transcript with
  \(c\in\mathcal E_{\rm loc}\), prove the \(W_i(c)\) are iid with mean
...
| unit_003 | lemma | Under Assumption~\(\ref{assump:parameter-domain}\), Lemma~\(\ref{lem:step-007-block}\), and conditional independence of the fixed disjoint groups, choose \(\alpha_k=4\) and \(G_\delta=2\lceil\alpha_k\log(8/\delta)\rceil+1\). With the median defined as the \((G_\delta+1)/2\)-th order statistic, on \(\mathcal E_{\rm loc}\) its conditional probability of missing \(\theta(c)\) by more than \(\epsilon/2\) is at most \(\exp(-G_\delta/8)\leq\delta/2\). | Proves the exact majority implication, independent-indicator Hoeffding constant, odd ceiling, tie convention, and confidence choice. |
...
Lemma~\(\ref{lem:step-007-block}\), choose the universal, hence k-only,
constant
\[
  \alpha_k=4,
  \qquad
