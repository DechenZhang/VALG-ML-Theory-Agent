...
outside a $\nu_i$-null set.  Write $S_i$ for the conditioning space, namely
$\R^{d-1}$ when $d\geq2$ and $\R^0$ when $d=1$.  The marginal $\nu_i$ is a
probability measure on $S_i$, and $Q^i$ is a probability kernel from $S_i$
to $\R$.  The measure--kernel product theorem
\citep[Chapter~1]{Kallenberg2002}, applied to these objects, defines on
$S_i\times\R$ the measure
\[
\gamma_i(C):=\int Q^i_y(C_y)\,\nu_i(\dd y),
\qquad C_y:=\{x:(y,x)\in C\}.
\]
It supplies the measurability of the section integral and countable
additivity.  Moreover,
\[
\gamma_i(S_i\times\R)
=\int Q^i_y(\R)\,\nu_i(\dd y)=\int1\,\dd\nu_i=1,
\]
so $\gamma_i$ is a probability before the Radon--Nikodym theorem is invoked.
For every Borel rectangle $D\times A$, regular conditionality gives
\[
\gamma_i(D\times A)
=\int_D Q^i_y(A)\,\nu_i(\dd y)
=\mu(Y_i\in D,X_i\in A).
\]
Rectangles form a $\pi$-system containing the whole product and generate its
product sigma-algebra.  The finite measures $\gamma_i$ and
$\mathcal L_\mu(Y_i,X_i)$ have the same whole-space mass and agree on every
rectangle.  Finite-measure uniqueness therefore identifies the current
objects exactly:
\[
\gamma_i=\mathcal L_\mu(Y_i,X_i).
\]

If $(\nu_i\otimes\lambda)(C)=0$, Tonelli's theorem
\citep[Chapter~2]{Folland1999} gives
$\lambda(C_y)=0$ for $\nu_i$-almost every $y$.  Conditional absolute
continuity then gives $Q^i_y(C_y)=0$, and hence
$\gamma_i\ll\nu_i\otimes\lambda$.  The ordinary product measure
$\beta_i:=\nu_i\otimes\lambda$ is sigma-finite before Radon--Nikodym is
used: the sets $S_i\times[-n,n]$, $n\geq1$, cover the whole product and
\[
\beta_i(S_i\times[-n,n])
=\nu_i(S_i)\lambda([-n,n])=2n<\infty.
\]
This remains true when $S_i=\R^0$ and requires no density of the possibly
singular marginal $\nu_i$.  Since $\gamma_i$ is finite and
$\gamma_i\ll\beta_i$, the Radon--Nikodym theorem
\citep[Chapter~3]{Folland1999} supplies a
nonnegative jointly measurable $q_i(y,x)$ such that
\[
\gamma_i(\dd y,\dd x)=q_i(y,x)\nu_i(\dd y)\lambda(\dd x).
\]
The whole-space calculation also gives
$\int q_i\,\dd(\nu_i\otimes\lambda)=1$.
Choose the explicit countable family
\[
\mathscr P_{\mathbb Q}:=\{\varnothing,\R\}
 \cup\{(a,b]:a,b\in\mathbb Q,\ a<b\}
\subseteq\mathcal B(\R),
\]
which is a $\pi$-system, contains the whole pivot space, and generates
$\mathcal B(\R)$.  Intersections remain in the family or become empty, and
every open interval is a countable union of rational half-open members.  For
every $B\in\mathscr P_{\mathbb Q}$ and Borel $D\subseteq S_i$, comparison on
$D\times B$ gives
\[
\int_DQ^i_y(B)\,\nu_i(\dd y)
=\int_D\!\left(\int_Bq_i(y,x)\dd x\right)\nu_i(\dd y).
\]
Both integrands are nonnegative and integrable.  Equality on every $D$
implies equality almost everywhere.  Intersect the resulting full-measure
sets over the countable $\mathscr P_{\mathbb Q}$.  On that common set define
\[
\widetilde Q^i_y(B):=\int_Bq_i(y,x)\dd x.
\]
The member $B=\R$ gives
$Q^i_y(\R)=\widetilde Q^i_y(\R)=1$.  Thus the two fiber measures are finite,
have equal whole-space mass, and agree on the generating $\pi$-system.
The $\pi$--$\lambda$ theorem and its finite-measure uniqueness consequence
\citep[Chapter~1]{Folland1999} extend their equality to every Borel set.
Hence $q_i(y,\cdot)$ is a probability density of $Q^i_y$ on one
$\nu_i$-full set.
...
\]
so $\widehat K_i(y)\leq m$.  If $m=0$, nonnegativity gives equality.
Otherwise take a finite nonnegative $a<m$.  The set $\{x:q(x)>a\}$ has
positive measure and therefore contains a Lebesgue point of $q$ by the
Lebesgue differentiation theorem \citep[Chapter~3]{Folland1999}.  Some
bounded open interval $L$ around that point satisfies
\[
\frac1{|L|}\int_Lq(t)\dd t>a.
\]
Put
\[
\delta:=\int_Lq\dd\lambda-a|L|>0,
\qquad
\rho(E):=\int_E|q|\dd\lambda.
\]
The measure $\rho$ is finite because $q$ is a probability density.  Its
continuity on sets of small Lebesgue measure follows from the explicit
truncation inequality: for every measurable $E$ and every $N\geq1$,
\[
\rho(E)
\leq N\lambda(E)+\int_{\{|q|>N\}}|q|\dd\lambda.
\]
Indeed, split $E$ between $\{|q|\leq N\}$ and its complement.  Moreover,
$|q|\1_{\{|q|\leq N\}}\uparrow|q|$, so monotone convergence
\citep[Chapter~2]{Folland1999} makes the tail integral tend to zero.  Given
$\varepsilon>0$, choose $N$ so that the tail is below $\varepsilon/2$; then
$\lambda(E)<\varepsilon/(2N)$ implies $\rho(E)<\varepsilon$.

Choose rational endpoints $r_n,s_n$ tending to the endpoints of $L$, with
$r_n<s_n$, and set $J_n=(r_n,s_n)$.  Then
$\lambda(L\mathbin\triangle J_n)\to0$ and $|J_n|\to|L|$.  The preceding
continuity and the definition of $\delta$ give
\[
\begin{aligned}
\int_{J_n}q\dd\lambda-a|J_n|
&\geq \int_Lq\dd\lambda-a|L|
 -\rho(L\mathbin\triangle J_n)-a\bigl||J_n|-|L|\bigr|\\
&=\delta-\rho(L\mathbin\triangle J_n)
 -a\bigl||J_n|-|L|\bigr|.
\end{aligned}
\]
The last two terms tend to zero.  For all sufficiently large $n$, the
right-hand side is positive, so some rational interval has average strictly
larger than $a$ and $\widehat K_i(y)>a$.  Letting $a\uparrow m$, or taking
arbitrarily large finite $a$ when $m=+\infty$, gives
\begin{equation}
...
Two densities of the same fiber law agree $\lambda$-almost everywhere by
Radon--Nikodym uniqueness \citep[Chapter~3]{Folland1999}, so their essential
suprema agree.  If $Q^i$ and $\widetilde Q^i$ are two regular conditional
kernels for the same pair $(X_i,Y_i)$, then for every Borel
$D\subseteq S_i$ and every $J\in\mathscr J_{\mathbb Q}$,
\[
\int_DQ^i_y(J)\,\nu_i(\dd y)
=\mu(Y_i\in D,X_i\in J)
=\int_D\widetilde Q^i_y(J)\,\nu_i(\dd y).
\]
Apply this equality to the measurable set on which one of the two bounded
integrands is strictly larger than the other, and then reverse their roles.
It follows that $Q^i_y(J)=\widetilde Q^i_y(J)$ almost everywhere.  Countability
gives one common full-measure set for all rational intervals, and the defining
supremum gives the same cap there.  This proves both density-version and
kernel-version independence without selecting values on null fibers.
...
\phi_\alpha(\theta)=0\}.
\]
The polynomial evaluation map is continuous.  The finite-product,
closed-subset, and continuous-image compactness facts
\citep[Sections~24 and~26]{Munkres2000} show that $Z_n$ is compact and that
its coefficient projection is compact and Borel.  Exact existential
quantification gives
\[
...
\begin{proof}
We first prove parameterized measurability and disintegration in the exact
product order used by the chart events.  Fix a probability kernel $Q_y$ from
$S$ to $\R$, where $S=\R^{d-1}$ or, for $d=1$, $S=\R^0$.  Put
$W:=\R\times S$ and
\[
\mathscr R:=\{A\times B:A\in\mathcal B(\R),\ B\in\mathcal B(S)\}.
\]
This is a $\pi$-system, explicitly contains $W$, and generates the Borel
sigma-algebra of $W$.  Indeed, Borel rectangles are Borel, and every open
subset of $W$ is a countable union of open rectangles from countable bases
of $\R$ and the Euclidean space $S$; the one-point case is immediate.

Let $\mathcal M$ be the class of Borel sets $E\subseteq W$ for which
$y\mapsto Q_y(E_y)$ is measurable.  The whole product belongs to
$\mathcal M$ because $Q_y(W_y)=Q_y(\R)=1$.  Rectangles belong to
$\mathcal M$ because
\[
Q_y((A\times B)_y)=\1_B(y)Q_y(A).
\]
The class is closed under complements relative to $W$, since
$Q_y((E^c)_y)=1-Q_y(E_y)$, and under countable disjoint unions, since kernel
countable additivity gives
\[
Q_y\left(\left(\bigcup_nE_n\right)_y\right)
=\sum_nQ_y((E_n)_y),
\]
whose partial sums are measurable and converge pointwise.  Thus $\mathcal M$
is a $\lambda$-system containing $\mathscr R$, including its whole-space
member.  The $\pi$--$\lambda$ theorem \citep[Chapter~1]{Folland1999} gives
$\mathcal M=\mathcal B(\R)\otimes\mathcal B(S)$.

Now suppose $Q_y$ is a regular conditional law of $X$ given $Y$, and let
$\nu=\mathcal L(Y)$.  Let $\mathcal D$ be the class of Borel
$E\subseteq W$ for which
\[
\mathbb P((X,Y)\in E)=\int Q_y(E_y)\,\nu(\dd y).
\]
The preceding argument makes the integrand legal for every Borel $E$.  The
whole product belongs to $\mathcal D$, because both sides equal one.  Every
...
=\int Q_y((A\times B)_y)\,\nu(\dd y).
\]
Complements remain in $\mathcal D$ by subtraction from the equal
whole-space mass.  If $(E_n)$ are pairwise disjoint members of $\mathcal D$,
their sections are pairwise disjoint.  Kernel countable additivity and
monotone convergence \citep[Chapter~2]{Folland1999} give
\[
\begin{aligned}
...
\end{aligned}
\]
Thus $\mathcal D$ is a $\lambda$-system containing $\mathscr R$, again with
the whole product explicitly verified.  A second $\pi$--$\lambda$ argument
...
\]
Consequently, pointwise on $G_0$,
\[
Q^0_y((E^0_{I_0})_y)
\leq\widehat K_0(y)\lambda(s_0(I_0;y))
\leq\widehat K_0(y)B_0(d,R)|I_0|.
\]
The rightmost function is measurable because $\widehat K_0$ is measurable
and $B_0(d,R)|I_0|$ is a fixed finite scalar.  We therefore integrate only
this measurable majorant, rather than requiring a separate measurability
claim for the conditioning-dependent image length:
\[
\begin{aligned}
\mu(H_{d,I_0})
&\leq\int_{G_0}\widehat K_0(y)B_0(d,R)|I_0|\nu_0(\dd y)\\
&= B_0(d,R)|I_0|\int_{G_0}\widehat K_0(y)\nu_0(\dd y)\\
...
\end{aligned}
\]
For $d\geq2$, $y\in[-R,R]^{d-1}$ on $G_\infty$; appending the pivot value
zero gives a support-compatible coefficient vector, so the deterministic
outer certificate applies.  For $d=1$ the empty-tuple specialization applies
directly.  Thus
Proposition~\ref{prop:positive-outer-certificate} gives
\[
\lambda(s_\infty(I_+;y))\leq B_\infty(d,R)|I_+|.
\]
Combining this bound with finite-cap domination gives, pointwise on
$G_\infty$,
\[
Q^\infty_y((E^\infty_{I_+})_y)
\leq\widehat K_\infty(y)\lambda(s_\infty(I_+;y))
\leq\widehat K_\infty(y)B_\infty(d,R)|I_+|.
\]
The rightmost expression is a measurable cap times a fixed finite scalar.
Integrating only that measurable majorant and then using the mean bound gives
\[
\begin{aligned}
\mu(H_{d,I_+})
&\leq\int_{G_\infty}\widehat K_\infty(y)B_\infty(d,R)|I_+|
       \nu_\infty(\dd y)\\
&= B_\infty(d,R)|I_+|
...
\end{aligned}
\]
For $d\geq2$, appending pivot value zero to the support-compatible $y$ gives
a coefficient vector in the cube; for $d=1$, use the exact empty-tuple
specialization.  The specifically negative certificate gives
\[
\lambda(s_\infty(I_-;y))\leq B_\infty(d,R)|I_-|,
\]
and finite-cap domination gives, pointwise on $G_\infty$,
\[
Q^\infty_y((E^\infty_{I_-})_y)
\leq\widehat K_\infty(y)\lambda(s_\infty(I_-;y))
\leq\widehat K_\infty(y)B_\infty(d,R)|I_-|.
\]
The rightmost expression is measurable because it is the measurable cap
times a fixed finite scalar.  Integrating only this majorant gives
\[
\begin{aligned}
\mu(H_{d,I_-})
&\leq\int_{G_\infty}\widehat K_\infty(y)B_\infty(d,R)|I_-|
...
full conditioning-marginal set \cite[Theorem~6.3]{Kallenberg2002};
Radon--Nikodym uniqueness \cite[Theorem~3.8]{Folland1999} then makes the
...
\cite[Theorem~6.3]{Kallenberg2002} and of Radon--Nikodym densities
\cite[Theorem~3.8]{Folland1999} shows that every permissible version has the
...
Product measure on the two product rectangles comprising $A_{d,R}$
\cite[Theorem~2.37]{Folland1999} gives
\[
\lambda_d(A_{d,R})=0.
\]
