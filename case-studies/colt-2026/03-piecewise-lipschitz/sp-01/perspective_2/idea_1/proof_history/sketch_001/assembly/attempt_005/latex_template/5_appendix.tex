...
to $\R$.  Therefore the kernel-product construction
\cite[Lemma~1.41(iii)]{Kallenberg2002}, instantiated with $\nu_i$ and $Q^i$,
defines a probability measure $\gamma_i=\nu_i\otimes Q^i$ on
$S_i\times\R$ and gives, for every product-Borel set $C$,
\[
\gamma_i(C):=\int Q^i_y(C_y)\,\nu_i(\dd y),
\qquad C_y:=\{x:(y,x)\in C\}.
\]
In particular, the cited kernel-product lemma supplies the measurability of
$y\mapsto Q^i_y(C_y)$ and the countable additivity of $\gamma_i$; moreover,
$\gamma_i(S_i\times\R)=1$.  Thus $\gamma_i$ is a finite measure before the
Radon--Nikodym theorem is invoked.
...
$\lambda$ is sigma-finite.  The Radon--Nikodym theorem
\cite[Theorem~3.8]{Folland1999} supplies a
nonnegative jointly measurable $q_i(y,x)$ such that
\[
\gamma_i(\dd y,\dd x)=q_i(y,x)\nu_i(\dd y)\lambda(\dd x).
\]
Choose the countable generating $\pi$-system
\[
\mathcal P:=\{\varnothing,\R\}
 \cup\{(a,b]:a,b\in\mathbb Q,\ a<b\}
\subseteq\mathcal B(\R),
\]
which contains the whole pivot space.  For every $B\in\mathcal P$,
comparison on all rectangles $D\times B$ gives
\[
Q^i_y(B)=\int_Bq_i(y,x)\dd x
\quad\text{for $\nu_i$-almost every $y$}.
\]
Intersect the countably many full-measure sets.  On the resulting common set,
the measures $B\mapsto Q^i_y(B)$ and
$B\mapsto\int_Bq_i(y,x)\dd x$ agree on $\mathcal P$.  Because
$\R\in\mathcal P$, the second measure has the same total mass one as the
probability measure $Q^i_y$.  Thus both measures are finite, with equal
whole-space mass.  The
bounded-measure uniqueness theorem \cite[Lemma~1.17]{Kallenberg2002}, with
this whole-space condition and the generating $\pi$-system $\mathcal P$,
therefore extends the identity to every Borel $B$ on that same full-measure
...
of $q$ by the Lebesgue differentiation theorem
\cite[Theorem~3.21]{Folland1999}.  Some bounded open interval $L$ around that point satisfies
\[
\frac1{|L|}\int_Lq(t)\dd t>a.
\]
Choose $J_n=(a_n,b_n)\in\mathscr J_{\mathbb Q}$ whose rational endpoints
converge to those of $L$.  Then $|J_n\mathbin{\triangle}L|\to0$ and
$|J_n|\to|L|>0$.  To justify
the integral limit directly, define the finite measure
\[
\rho(E):=\int_E|q|\dd\lambda,
\qquad \rho(\R)=\|q\|_{L^1}<\infty.
\]
Given $\varepsilon>0$, choose $0<M<\infty$ so that
$\int_{\{|q|>M\}}|q|\dd\lambda<\varepsilon/2$.  Whenever
$\lambda(E)<\varepsilon/(2M)$,
\[
\rho(E)
\leq M\lambda(E)+\int_{\{|q|>M\}}|q|\dd\lambda
<\varepsilon.
\]
Hence $\rho(J_n\mathbin{\triangle}L)\to0$, and therefore
\[
\left|\int_{J_n}q\dd\lambda-\int_Lq\dd\lambda\right|
\leq\rho(J_n\mathbin{\triangle}L)\longrightarrow0.
\]
Together with $|J_n|\to|L|$, this shows that the average of $q$ over $J_n$
converges to its average over $L$, which is strictly larger than $a$.  For
all sufficiently large $n$, therefore,
$|J_n|^{-1}\int_{J_n}q\dd\lambda>a$, so $\widehat K_i(y)>a$.  Letting
...
Radon--Nikodym uniqueness \cite[Theorem~3.8]{Folland1999}, so
their essential suprema agree.  If $Q^i$ and $\widetilde Q^i$ are two regular
conditional kernels, regular-conditional-distribution uniqueness
\cite[Theorem~6.3]{Kallenberg2002} gives
...
measurable rectangles, including the whole product $\R\times S$.  The
$\pi$--$\lambda$ theorem \cite[Theorem~1.1]{Kallenberg2002} proves the assertion
...
holds form another $\lambda$-system.  It contains $\R\times S$, since the
probability on the left and the integral of $Q_y(\R)=1$ on the right both
equal one.  Complements therefore follow by subtraction from this equal
whole-space mass, while disjoint unions follow by countable additivity and
monotone convergence \cite[Theorem~2.14]{Folland1999}.  A second
$\pi$--$\lambda$ argument \cite[Theorem~1.1]{Kallenberg2002} gives, for every Borel
$E\subseteq\R\times S$,
\begin{equation}
