...
=\sum_{\substack{\nu\in\mathcal A\\ \nu_r\geq1}}
  \nu_r a_\nu z^{\nu-e_r},
\]
where $e_r$ is the $r$th coordinate vector. Since
$\lvert z^{\nu-e_r}\rvert\leq1$ and
$\nu_r\leq\lvert\nu\rvert_1\leq d$,
\[
\begin{aligned}
\lvert\partial_{z_r}S(z)\rvert
...
**Proof / justification.** Fix $x\in[-1,1]$ and $1\leq i\leq N$. For $q\geq1$
write
\[
z(x):=(x,\eta_1(x),\ldots,\eta_q(x)),
\qquad
z_j(x):=(x,\eta_1(x),\ldots,\eta_j(x)).
\]
...
z_j(x)\in[-1,1]^{j+1}.
\]
For $q=0$, use only $z(x)=(x)\in[-1,1]$; there are no truncated vectors
$z_j$.
...
- $z(x)=(x,\eta_1(x),\ldots,\eta_q(x))$ and
  $z_j(x)=(x,\eta_1(x),\ldots,\eta_j(x))$ are `proof-local` abbreviations for
  setting-defined expressions. Their unit-box membership is proved in
