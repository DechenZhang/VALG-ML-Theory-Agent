...
\]
The regular-level-set theorem \citep{lee2013smooth} therefore applies at
every incidence, without a root-transversality premise, and gives the stated
hypersurface and normal.  Tangent roots $u=0$ remain regular because the
coefficient gradient is the unit vector $\gamma$.
...
$|u|$ proves (A.1) without division by $u$ or selection of a root branch.
\end{proof}
...
$F=\|F\|_2\gamma$ with $\|F\|_2>0$, proving (A.2).
...
Lemma~\ref{lem:central-null-classes}.  The left side of (A.3) therefore has
...
\tag{A.4}
\]
Using (A.1) and then the coarea formula on the rectifiable hypersurface
...
The coarea hypotheses are met because (A.1) gives the measurable
nonnegative integrand and $J_{\mathcal S_J}\tau>0$.

Replacing the open cube section in (A.5) by the closed one changes no
...
$\gamma_F'=0$.  Thus (A.8) is exactly $0\leq0$, including all interval
conventions, and Ball's positive-dimensional theorem is not invoked.

Assume $N\geq2$.  Equations (A.3) and (A.9) give
\[
\begin{aligned}
&\Pr_{\alpha\sim\mu}
[\exists\theta\in I:\langle\alpha,F(\theta)\rangle=0]\\
...
\tag{A.12}
\]
Only a central linear section is used.  Substitution into (A.10) gives the
coefficient
\[
\begin{aligned}
...
proves (A.8).  The upper bound need not be at most one and is not clipped.
...
\tag{A.20}
\]
No term is dropped.  Equations (A.18)--(A.20) prove (A.15).  If $I$ is
...
Now restrict to $|I|>0$ and divide (A.15) by this positive length:
\[
...
$C^{\mathrm{Pf}}_{\mathcal D}(F;\Theta)$, proving (A.16).

Substituting $A=(2R)^N\kappa$ gives (A.17) by equality.  The parameter $M$
...
\tag{A.24}
\]
If $\Gamma_{\mathrm{proj}}(F)=0$, (A.18) gives zero probability for every
...
remaining terms are setting quantities or the bound (A.26).  Define
\[
H_j(\theta,\beta)
:=F_0(\theta)+\sum_{i\neq j}\beta_iF_i(\theta),
\qquad
...
The quotient identity, (A.26), and (A.28) yield
\[
\begin{aligned}
...
\end{pmatrix}.
\]
The coordinate permutation has determinant of magnitude one, proving (A.32).
...
=-F_j(\theta)\partial_\theta T_j(\theta,\beta),
\]
which is (A.33).  Since the pivot is nonzero, tangency is equivalent to a
...
\begin{proof}
If $I$ is empty, every set and integral in (A.34) is empty, so assume $I$ is
...
roots can only increase the right side of (A.35).  By
...
\tag{A.36}
\]
Combining (A.35)--(A.36) proves the first line of (A.34); removing the
indicator proves the second by nonnegativity.  The finite-level derivative
bounds (A.30) and the finite measure of
...
At the least level (A.38), put $\beta=\alpha_{-j}$.  The converse part of
Lemma~\ref{lem:affine-chart-jacobian} gives
$(\theta,\beta)\in D_{j,m_*(\theta)}$ and
$\Psi_j(\theta,\beta)=\alpha$.  Thus
$\alpha\in\mathcal R_{m_*(\theta)}$, proving (A.37).
...
degenerate classes are not removed from (A.37); their nullity is a separate
...
inequality in (A.39).  The density cap was used only on the full coefficient
...
\]
This proves (A.42)--(A.43).  In particular
...
\]
Substitution gives (A.44).  The leading coefficient occurs only in the
...
\]
Solving gives (A.47), and direct differentiation gives
\[
\partial_\theta T_1
...
0=\theta^d+\sum_{k=0}^{d-2}\beta_k\theta^k
 +\alpha_{d-1}\theta^{d-1}
\]
can be divided by $\theta^{d-1}$, giving (A.49).  Differentiating on the
...
\end{aligned}
\]
Since $d\geq2$, $1\leq d$, proving (A.50).
...
proposition gives (A.51).  No chart union bound, coordinate conditioning,
...
\tag{A.59}
\]
Substituting (A.59) in (A.58) proves (A.53).  If either cell is empty, its
...
(A.60), and $1+R\cdot1\cdot0/2=1$ gives the equality.  There is no high
...
sum by the right side of (A.53).  Its calculation first uses the sharper
high-chart speed, then (A.57), and only then the disjoint length ledger
(A.59), so it introduces no factor for the number of charts.  Multiplication
by the literal affine coefficient $\kappa$ proves (A.62).
...
the first identity in (A.64), and direct differentiation gives the second.
Consequently,
\[
\begin{aligned}
\|\gamma_F'(\theta)\|_2
...
The coefficient axes make (A.68) exact, not only almost everywhere.  If
...
\tag{A.73}
\]
Multiplication by the constant density $1/4$ proves (A.69).
...
\tag{A.78}
\]
Combining (A.76)--(A.78) proves (A.74).  At $\delta=1$, the three scales
...
(A.79), including the endpoint and degree-zero branches.
...
remaining projective-speed assertions in (A.79).

Proposition~\ref{prop:central-rate} gives (A.80)--(A.81) with the same
deterministic feature, ordinary-probability mode, literal constants, and
ordered suprema.  Proposition~\ref{prop:affine-bound} gives (A.82) in the
...
(A.83).  Lemma~\ref{lem:monic-pivot-partition} gives the prescribed cells,
and Lemmas~\ref{lem:monic-low-chart} and
\ref{lem:monic-high-chart} give (A.84)--(A.85), including the transition
...
(A.86), on the same monic object and lower-coefficient law.

Proposition~\ref{prop:counter-presentation} gives the tuple and speed in
(A.87), Lemma~\ref{lem:counter-wedges} gives (A.88) with all closed
boundaries, and Proposition~\ref{prop:counter-scale} gives (A.89).  Each
