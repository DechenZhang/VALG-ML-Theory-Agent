...
- Sketch attempt: 2
- Attempt type: `revise_sketch` dependency-interface repair
- Search mode: `OPEN`
- Run mode: `interactive`
- Target mode: Exact-goal, unconditional, full-progress theorem

This is a complete attempt-2 sketch. Attempt-1 proofs and reviews are stale diagnostic material only and are not proof evidence for this attempt; every step below starts with `Review status = PENDING` and must be reproved and freshly reviewed under this sketch.
...
\]
the area and coarea formulas expose the local factor (|\langle\gamma_F'(\theta),a\rangle|), and Ball plus the density cap gives the central probability bound. Third, each affine root event is covered by the images of the exhausted pivot maps (\Psi_{j,m}(\theta,\beta)), whose determinant is (|\partial_\theta T_j|); monotone exhaustion gives the general affine inequality. Fourth, the monic presentation is inserted into that same affine result, its two velocities are calculated directly, and Counter-example 1 is calculated directly. The final step combines only earlier interfaces into the exact theorem. To keep every public clause on a direct legal producer path, `step_010` consumes the primitive derivative output of `step_001`, the normalized-projective bridge of `step_002`, and the exact monic presentation/pivot/velocity certificate of `step_007` directly, in addition to the central, general-affine, monic-probability, and scale-audit outputs of `step_005`, `step_006`, `step_008`, and `step_009`.
...
| --------------- | ---------------- | ------------- | --------------------------------- |
| (D_*=\Delta B_Q(1+qB_P)) and (\lvert G_i'\rvert\leq D_*) | `assump:balcan-common-chain`, `assump:anchored-unit-range` | `step_001` | `step_002`, `step_006`, `step_010` |
| Global nonvanishing and (\Gamma_{\mathrm{proj}}(F)\leq\sqrt N D_*/h) | `assump:parameter-regime`, `assump:anchored-unit-range`, `step_001` | `step_002` | `step_003`, `step_005`, `step_010` |
...
| Central all-law/all-interval probability interface | `assump:cube-density-laws`, `step_003` | `step_004` | `step_005` directly; `step_010` only through the exported `step_005` central clause |
...
| Exact monic presentation, pivot partition, and velocity certificates | Primitive scalar choices (d,R,J,\Theta) together with the q=0 specialization of the setting | `step_007` | `step_008`, `step_010` |
...
- Noncircular closure status: The derivative bound is produced directly from primitive data before every consumer; it does not assume projective speed or the final anti-concentration result. Dependency path: primitive assumptions -> `step_001` -> (`step_002`, `step_006`, `step_010`).
...
- Producer-consumer provenance: Producer `step_001`; direct consumers `step_002`, `step_006` (regularity interface), and `step_010` (primitive-presentation clause).
...
- Obligation locality classification: `step-local`.
- Noncircular closure status: Nonvanishing is produced by the primitive anchor before normalization; projective speed is then produced from `step_001`. Direct consumers `step_003`, `step_005`, and `step_010` do not feed back into the bridge.
...
- Producer-consumer provenance: Primitive anchor plus `step_001` -> `step_002`; direct consumers `step_003`, `step_005`, and `step_010`; `step_004` sees only the incidence interface exported by `step_003`.
...
- Producer-consumer provenance: Ball plus `assump:cube-density-laws` and `step_003` -> `step_004`; direct consumer `step_005`, with final assembly only through the central clause exported by `step_005`.
...
- Mechanism source: Planned earlier-step outputs `step_002` and `step_004`, followed by exact algebra and the definition of (C^{\mathrm{Pf}}_{\mathcal D}). No old proof artifact, new theorem, or hidden regularity source is introduced.
...
- Target conclusion false or theorem-critical obstruction present if source vanishes: If either earlier output were missing, the explicit target would not close. Both are legal earlier dependencies in the roadmap and require fresh acceptance under attempt 2.
...
- Obligation locality classification: `step-local`.
- Noncircular closure status: Pivot nonvanishing and velocity control follow directly from the explicit monomial presentation before either consumer invokes them. Dependency path: primitive monic data -> `step_007` -> (`step_008`, `step_010`).
...
- Producer-consumer provenance: Producer `step_007`; direct consumers `step_008` and `step_010`; final uses are the exact monic presentation/pivot/velocity certificates and the baseline specialization clause.
...
- Mechanism source: Planned earlier-step affine inequality `step_006` and exact velocity/presentation certificate `step_007`. No old proof artifact or independent polynomial-root citation is used.
...
- Mechanism source: Planned earlier-step outputs only. The direct inputs are the primitive derivative envelope from `step_001`, the normalized-projective bridge from `step_002`, the central outputs from `step_005`, the general affine output from `step_006`, the exact monic presentation, prescribed pivots, chart formulas, velocity certificates, and (d=1) convention from `step_007`, the exact monic probability bound from `step_008`, and the scale audit from `step_009`.
- Source-to-claim adequacy: Each public final clause is identical to an output of one of those seven direct producers in the same convention. In particular, assembly does not promote the primitive bridge through `step_005` or the appendix-local monic chart objects through `step_008`. It introduces no new cited result, lemma, assumption, simplification, or mechanism source.
...
- Closure/dominance/absorption relation: Conjoin, without further algebra, the direct outputs of `step_001`, `step_002`, `step_005`, `step_006`, `step_007`, `step_008`, and `step_009`.
...
- Producer-consumer provenance: `step_001`, `step_002`, `step_005`, `step_006`, `step_007`, `step_008`, `step_009` -> `step_010` -> final theorem. Each public primitive-bridge and monic-certificate clause therefore has its exact producer as a direct dependency.
...
| Full exact-goal theorem | `step_010` | Direct outputs of `step_001`, `step_002`, `step_005`, `step_006`, `step_007`, `step_008`, and `step_009` | Every defect class is allocated to its exact producer; no new or uncontrolled class enters assembly, and no appendix-local object is promoted through a nonexporting step | Same objects and modes clause by clause; all transfer residuals already zero | Direct conjunction of the seven listed producer interfaces, with no assembly algebra | Earlier anchor, section, pivot, and velocity sources; no assembly-only slack | Final theorem | None |

## Generated Output Flow

| Generated output or control | Producer step or source | Consumers | Final theorem use | Dependency path | Provenance class | Missing-flow blocker |
| --------------------------- | ----------------------- | --------- | ----------------- | --------------- | ---------------- | -------------------- |
| Pfaffian derivative envelope (D_*) | `step_001`; direct from `assump:balcan-common-chain`, `assump:anchored-unit-range` in the exact (x)-coordinate convention | Direct: `step_002`; regularity interface of `step_006`; primitive-bridge clause of `step_010` | Primitive-presentation clause | primitives -> `step_001` -> (`step_002`, `step_006`, `step_010`) | derived | None |
| Global nonvanishing and projective-speed certificate | `step_002`; exact object relation (\gamma_F=\gamma_G\circ x), source-compatible with Euclidean normalization | Direct: `step_003`, `step_005`, `step_010` | Conditioning and central explicit rate | `step_001` -> `step_002` -> (`step_003`, `step_005`, `step_010`) | derived | None |
...
| Exact monic object/pivot/velocity certificate | `step_007`; direct monomial calculation, same lower-coefficient target, residual zero | Direct: `step_008`, detailed monic-certificate clause of `step_010` | Baseline specialization conditions and public chart certificates | primitive monic data -> `step_007` -> (`step_008`, `step_010`) | derived | None |
...
| Full assembled theorem | `step_010` | Final theorem statement | Every inseparable formalized-goal clause | (`step_001`, `step_002`, `step_005`, `step_006`, `step_007`, `step_008`, `step_009`) -> `step_010` | derived | None |
...
&\{\texttt{step\_001},\texttt{step\_002},\texttt{step\_005},\texttt{step\_006},\texttt{step\_007},\texttt{step\_008},\texttt{step\_009}\}\to\texttt{step\_010}.
\end{aligned}
\]
`step_007` is an independent direct specialization unit and therefore has no dependency; it is placed before both consumers `step_008` and `step_010`. If freshly accepted under attempt 2, the central analytic branch (`step_001`--`step_005`), the general affine branch (`step_006`), the direct monic certificate (`step_007`), and the scale audit (`step_009`) expose separate review boundaries. `step_008` may consume the affine theorem only after both the general chart and exact monic certificate are accepted. `step_010` introduces no mathematical content: it consumes `step_001` and `step_002` directly for the primitive bridge, `step_007` directly for the detailed monic presentation and chart certificates, and `step_005`, `step_006`, `step_008`, and `step_009` for their stated public conclusions. Thus it does not treat transitive proof ancestry or appendix-local inputs as public exports, and it retains the exact formalized goal.
