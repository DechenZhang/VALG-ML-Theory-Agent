...
- Unit attempt: 1
- Progress type: `full`
- Materiality boundary: This is the exact structural R4 producer required by the branch's full-progress theorem. It proves the monic presentation, pivots, and chart velocities, but it does not claim the affine probability inequality assigned to `step_008` or the full theorem assigned to `step_010`.

## Binding Artifacts

- Binding `setting.md` SHA-256: `f77d11e1a559e22a96b14de1936d50c691c79d5a4a15e5806ddf4e6d119ada83`
- Binding `proof_sketch.md` SHA-256: `2fa48ee16cca9a8d0379071f448d1e72ce98ae46d624482d772e74b97649c74a`
...
- Dependency proof/review hashes: None; the accepted target row has no dependencies.
...
- `assump:balcan-common-chain`: use its \(q=0\) branch, so the chain is empty, \(M=0\), \(B_P=0\), and every random-feature output \(G_i\) is its polynomial \(Q_i\) evaluated directly at \(x\).
- `assump:anchored-unit-range`: use the literal anchor \(Q_1\equiv 1\). The chain-range requirement is vacuous when \(q=0\).
- `assump:affine-chart-data`: use only its setting-defined affine-chart formula and its stated admissibility criterion. Satisfaction by the displayed monic construction is not assumed: Proposition~\ref{prop:step-007-monic-presentation} proves \(F_0\in C^1(\Theta)\), and Lemma~\ref{lem:step-007-pivot-partition} proves measurability, coverage, and pivot nonvanishing for the prescribed cells.
...
| ------------- | --------- | --------- | --------------------------- |
| `unit_001` | proposition | Under Assumptions~\ref{assump:parameter-regime}, \ref{assump:balcan-common-chain}, and \ref{assump:anchored-unit-range}, if \(d\geq1\) and \(J\subset\mathbb R\) is bounded, then some nondegenerate \(\Theta=[c-h,c+h]\supseteq J\) has \(Q_0(x)=(c+hx)^d\), \(Q_{k+1}(x)=(c+hx)^k\), \(F_0(\theta)=\theta^d\), \(F_{k+1}(\theta)=\theta^k\), and \(F_0+\langle\alpha,F\rangle=p_\alpha\) pointwise, satisfies the \(C^1\) requirement of Assumption~\ref{assump:affine-chart-data}, and has \(q=0\), \(M=0\), \(B_P=0\), \(N=d\), \(A=(2R)^d\kappa\), \(\Delta_{\mathrm{aug}}=d\), and deterministic monic coefficient outside \(\alpha\in\mathbb R^d\). | Establishes the exact object mapping, affine regularity, lower-coefficient object, and parameter tuple. |
| `unit_002` | lemma | Under Assumption~\ref{assump:parameter-regime} and Proposition~\ref{prop:step-007-monic-presentation}, if \(d\geq2\), then \(E_1=J\cap\{\lvert\theta\rvert\leq1\}\), \(E_d=J\cap\{\lvert\theta\rvert>1\}\), and \(E_j=\varnothing\) otherwise form a measurable partition with \(F_1=1\neq0\) on \(E_1\) and \(F_d=\theta^{d-1}\neq0\) on \(E_d\), thereby satisfying the pivot-cover requirement of Assumption~\ref{assump:affine-chart-data}; if \(d=1\), then \(E_1=J\) and \(F_1=1\neq0\). This includes \(\theta=0\), \(\lvert\theta\rvert=1\), and empty cells. | Verifies rather than assumes the exact pivot partition required by the affine interface. |
| `unit_003` | lemma | Under Assumption~\ref{assump:parameter-regime}, Proposition~\ref{prop:step-007-monic-presentation}, and Lemma~\ref{lem:step-007-pivot-partition}, if \(d\geq2\), \(\theta\in E_1\), and \(\beta\in[-R,R]^{d-1}\), then the chart definition in Assumption~\ref{assump:affine-chart-data} gives \(T_1=-\theta^d-\sum_{k=1}^{d-1}\beta_k\theta^k\) and \(\lvert\partial_\theta T_1\rvert\leq d+R\sum_{k=1}^{d-1}k=V_d\); if \(d=1\), then the empty-sum chart is \(T_1=-\theta\) with \(\lvert T_1'\rvert=1=V_1\). | Supplies the low-pivot velocity certificate and empty-sum branch. |
| `unit_004` | lemma | Under Assumption~\ref{assump:parameter-regime}, Proposition~\ref{prop:step-007-monic-presentation}, and Lemma~\ref{lem:step-007-pivot-partition}, if \(d\geq2\), \(\theta\in E_d\), and \(\beta\in[-R,R]^{d-1}\), then the chart definition in Assumption~\ref{assump:affine-chart-data} gives \(T_d=-\theta-\sum_{k=0}^{d-2}\beta_k\theta^{k-d+1}\) and \(\lvert\partial_\theta T_d\rvert\leq1+R\sum_{k=0}^{d-2}(d-1-k)=1+R d(d-1)/2\leq V_d\), with every negative-power estimate restricted to \(\lvert\theta\rvert>1\). | Supplies the high-pivot velocity certificate without a singular leading coordinate. |
...
Statement: Under Assumptions~\ref{assump:parameter-regime}, \ref{assump:balcan-common-chain}, and \ref{assump:anchored-unit-range}, if \(d\geq1\) and \(J\subset\mathbb R\) is a bounded interval, then there is a nondegenerate interval \(\Theta=[c-h,c+h]\supseteq J\) for which
...
Set \(q=0\). The chain tuple is empty, so the definitions in Assumptions~\ref{assump:parameter-regime} and \ref{assump:balcan-common-chain} give \(M=0\), \(B_P=0\), and \(G_i(x)=Q_i(x)\). For \(x=x(\theta)=(\theta-c)/h\),
...
Statement: Under Assumption~\ref{assump:parameter-regime} and Proposition~\ref{prop:step-007-monic-presentation}, if \(d\geq2\), then the exact sets
...
form a measurable partition of \(J\), with \(F_1\neq0\) on \(E_1\) and \(F_d\neq0\) on \(E_d\), and therefore satisfy the concrete pivot-cover requirement in Assumption~\ref{assump:affine-chart-data}. If \(d=1\), the exact choice \(E_1=J\) is measurable and has \(F_1\neq0\) everywhere. These conclusions remain valid at \(\theta=0\), at \(\lvert\theta\rvert=1\), and when any displayed cell is empty.
...
Statement: Under Assumption~\ref{assump:parameter-regime}, Proposition~\ref{prop:step-007-monic-presentation}, and Lemma~\ref{lem:step-007-pivot-partition}, if \(d\geq2\), \(\theta\in E_1\), and \(\beta=(\beta_1,\ldots,\beta_{d-1})\in[-R,R]^{d-1}\), then the chart definition in Assumption~\ref{assump:affine-chart-data}, with the nonpivot coordinates indexed by exponent, gives exactly
...
Statement: Under Assumption~\ref{assump:parameter-regime}, Proposition~\ref{prop:step-007-monic-presentation}, and Lemma~\ref{lem:step-007-pivot-partition}, if \(d\geq2\), \(\theta\in E_d\), and \(\beta=(\beta_0,\ldots,\beta_{d-2})\in[-R,R]^{d-1}\), then the chart definition in Assumption~\ref{assump:affine-chart-data}, with the nonpivot coordinates indexed by exponent, gives exactly
...
For the requested \(d=2\) audit, Lemma~\ref{lem:step-007-low-pivot-chart} and the high-pivot calculation just proved reduce without any hidden sum to
...
and exactly \(q=0\), \(M=0\), \(B_P=0\), \(N=d\), \(A=(2R)^d\kappa\), and \(\Delta_{\mathrm{aug}}=d\). It also proves that the monic coefficient is the deterministic offset and that \(\alpha=(\alpha_0,\ldots,\alpha_{d-1})\) contains exactly the \(d\) lower coefficients, in the original order and with no additional coordinate.
...
- Admissibility conditions and auxiliary tolerances: \(d\geq1\), \(R>0\), \(0<\kappa<\infty\), nonempty \(\mathcal D_{d,R,\kappa}\), \(h>0\), bounded \(J\subseteq\Theta\), and \(\beta\in[-R,R]^{d-1}\). The high-pivot certificate is restricted exactly to \(d\geq2\) and \(\lvert\theta\rvert>1\). There are no auxiliary tolerances.
...
- Contribution to any Rate Specialization Bridge: The exact tuple, unchanged \(d\)-dimensional lower-coefficient object, prescribed partition, and literal two-chart cap are the structural inputs required by `step_008`. This step introduces no auxiliary loss, probability conversion, or random leading coordinate; the probability conclusion remains assigned to `step_008`.
- Baseline-reduction check, if applicable: At \(d=1\), the high-pivot branch disappears, the nonpivot tuple is the unique element of \([-R,R]^0\), the sum is empty, and \(T_1=-\theta\) has exact speed \(1=V_1\). For every \(d\geq1\), the identity \(F_0+\langle\alpha,F\rangle=p_\alpha\), the same lower-coefficient vector, and the literal cap \(V_d\) preserve the exact affine-monic baseline path for `step_008`; there is no conservative loss or surrogate object in this producer.
...
- **Public-facing objects:** \(F_0,F,p_\alpha,Q_0,Q_1,\ldots,Q_d\); the exact tuple \(q=0\), \(M=0\), \(B_P=0\), \(N=d\), \(A=(2R)^d\kappa\), \(\Delta_{\mathrm{aug}}=d\); the prescribed \(E_j\); the chart coordinates \(T_1,T_d\); and the literal reused cap \(V_d=d+R d(d-1)/2\). These are exactly the direct objects consumed by `step_008` and the detailed monic-certificate clause of `step_010`.
...
- **Constant and parameter provenance:** \(R,\kappa\), and \(A=(2R)^N\kappa\) come directly from Assumption~\ref{assump:parameter-regime}; \(d\) and bounded \(J\) are target quantifiers; \(c,h,\Theta\) are constructed in Proposition~\ref{prop:step-007-monic-presentation}; \(q,M,B_P,N,\Delta_{\mathrm{aug}}\) are computed there from the displayed specialization; and \(V_d\) is proved by Lemmas~\ref{lem:step-007-low-pivot-chart} and \ref{lem:step-007-high-pivot-chart}. No free bounded quantity is introduced.
...
- **Diagnostic boundary:** The paired `global_proof.md` and `global_proof_review.md` have the hashes recorded above, and the review status is `ACCEPTED`. The diagnostic's two-pivot decomposition was used only to plan the local units. It is not proof evidence, a cited result, an assumption source, or authority to change the accepted row. The live attempt-1 same-step proof was consulted only as diagnostic history; no statement from it is used as evidence here.
- **Public-surface restriction:** No probability estimate, cube-volume factor, root-counting statement, or additional helper dictionary is exported. The public-facing output is limited to the exact object mapping, specialization tuple, pivot partition, and two chart-velocity certificates stated in the target-step assembly, with direct consumer paths to `step_008` and `step_010`.
