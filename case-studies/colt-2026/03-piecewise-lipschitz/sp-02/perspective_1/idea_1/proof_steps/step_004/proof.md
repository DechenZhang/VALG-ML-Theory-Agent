...
- Derived invariants supplied by accepted dependencies:
  - Proposition~\ref{prop:step-003-central-volume} from the accepted
    ...
    `648841deed8e205ff9dcbf7b74267a77bdbb7d519a7ce1f9833de0d88f503cf5`.
    Its review is
    `perspective_1/idea_1/proof_steps/step_003/review.md`, status `ACCEPTED`,
    SHA-256
    `456aa120c700edc7e711f09f283845c3e7eeee8036951fafdd1212e5682b2ec0`.
...
1. **Ball's central cube-section theorem (paper result, restated in the
   ...
   \tag{4.3}
   \]
   Here \(\mathcal H^{n-1}\) has the standard Euclidean normalization, so
   its restriction to the \((n-1)\)-plane \(H\) is exactly the source's
   intrinsic \((n-1)\)-dimensional volume. The source cube is closed, has
   side length one, volume one, and center zero; its faces are part of the
   section in (4.3).

   In Lemma~\ref{lem:step-004-scaled-ball}, the source objects are mapped by
   \(n=N\), \(H=v^\perp\), and the similarity \(y\mapsto2Ry\).
   Assumption~\ref{assump:parameter-regime} and the local branch give
   \(N\geq2\) and
   \(R>0\); \(\|v\|_2=1\) makes \(v^\perp\) a Euclidean codimension-one
   linear hyperplane through zero. Since \(v\) is arbitrary, this discharges
   orientation compatibility without rotating the coefficient vector or its
   law. Thus every source hypothesis is discharged. The conclusion used is
   only (4.3). Ball is not used for an affine translate of \(v^\perp\), for
   hyperplane motion, for \(N=1\), or for any probability, density, or
...
\end{aligned}
\]
This proves (4.8). Both source and target cubes are closed, and (4.10) maps
their faces exactly; no boundary face is discarded or charged by a separate
factor. Every hyperplane used here contains the origin, and \(v\) was an
arbitrary unit vector, so the conclusion is orientation-uniform without a
WLOG rotation. In particular, the proof neither invokes nor derives a bound
for a translated section \((b+v^\perp)\cap[-R,R]^N\). \(\square\)
...
The assumed nonemptiness of the law class also implies, for this fixed
probability density,
\[
...
\tag{4.14}
\]
Thus \(A\geq1\). This observation is not used in any subsequent inequality
or in the constant algebra; in particular, it supplies no hidden relaxation.

First suppose \(N=1\). The accepted dependency supplies the same \(C^1\)
unit normalized curve used in (4.2). In one coefficient dimension,
\[
...
right side of (4.11) is nonnegative. In fact, the dependency's unit scalar
curve also satisfies \(\gamma_F(\theta)^2=1\), so differentiation gives
\(\gamma_F'=0\) and (4.11) is exactly \(0\leq0\). This is a direct
zero-dimensional-section argument; Ball's positive-dimensional theorem is
not invoked. It covers every interval convention, including empty and
singleton cases.
...
\tag{4.17}
\]
The coefficient in (4.17) simplifies with no inequality and no hidden
constant. Indeed,
\[
\begin{aligned}
\kappa R\sqrt N\,\sqrt2(2R)^{N-1}
...
\(A=(2R)^N\kappa\). Equations (4.17)--(4.18) prove (4.11). The resulting
right-hand side is allowed to exceed one; an upper bound on a probability
need not itself be at most one. No truncation by \(1\), and hence no hidden
case split or coefficient change, is used.
...
infinite root fibers, and identically-zero combinations by null coefficient
sets or projection multiplicity. A stationary \(\gamma_F\) on all of \(I\)
makes the accepted incidence integral zero and hence makes the event
probability zero. Applying the single full-density bound (4.13) preserves
every dependency-null coefficient class under every admissible correlated
law.

All pointwise estimates above act on the original setting coefficient
vector \(a=(a_1,\ldots,a_N)\). The source dilation in
Lemma~\ref{lem:step-004-scaled-ball} computes only deterministic section
measure; it neither rotates nor transforms \(a\), \(f_\mu\), or \(\mu\).
The closed section in (4.2), including all cube-face points, is exactly the
closed section bounded in Lemma~\ref{lem:step-004-scaled-ball}. Since
\(\mu\) and \(I\) were arbitrary after the deterministic presentation was
fixed, (4.11) has the claimed all-law, all-interval quantifier order. No
events are union-bounded over laws, intervals, roots, or orientations.
\(\square\)
...
inputs on the exact original coefficient coordinates and closed cube, and
...
weaker statement. Tangent and multiple roots, infinite fibers,
identically-zero combinations, endpoints, stationary curves, and zero-length
intervals enter only through accepted
Proposition~\ref{prop:step-003-central-volume}; no such class is excluded or
recounted here. The proof uses neither a union bound nor a confidence-mode
conversion, and its central conclusion is not asserted to replace the
separate affine-monic route.
...
- Term absorption or simplification inequalities: The sole pointwise
  ...
  =A\sqrt{N/2}.
  \]
  No term is dropped, absorbed, or declared lower order. Nonemptiness of the
  law class separately gives
  \(1\leq\kappa(2R)^N=A\), but this inequality is not used in the displayed
  coefficient identity. The final upper bound may be larger than one and is
  not clipped.
...
- Baseline-reduction check, if applicable: For \(N=1\), the central section
  ...
  nor altered by this central step; a homogeneous central section theorem
  does not replace the affine chart argument with its deterministic monic
  offset.
...
- \(H=v^\perp\) and \(D_R(y)=2Ry\) are `proof-local` cited-result mapping
  ...
  radius is introduced. The local normal \(v\) is `proof-local` and
  arbitrary; it is never used to rotate or reparameterize the setting law.
...
- Constant provenance: \(\sqrt2\) is the checked literal conclusion of
  ...
  introduced.
- Surface classification: Lemma~\ref{lem:step-004-scaled-ball} is
  `appendix-local`; it packages the cited-result convention translation.
  Proposition~\ref{prop:step-004-correlated-central-sweep} and only its
  displayed all-law probability inequality are `public-facing`. All other
  symbols introduced in this artifact are `proof-local`.
...
- The pre-overwrite sketch-1 same-step proof and review, SHA-256
  ...
  were consulted only as the permitted latest same-step diagnostics. Their
  byte-identical archived copies were not consumed as current evidence or as
  acceptance. Stable local-unit IDs were retained, and every statement,
  source mapping, hypothesis discharge, constant, and boundary case was
  checked anew against the attempt-2 sketch and current accepted dependency.
