...
comparisons in \textup{(A1)} are Borel, as are $K_R$ and
$\{V_j<+\infty\}$.
...
$E_j$.  Conversely, \textup{(A1)} makes $j$ the least minimizer.  This
...
$U_j\times\mathbb R^{J_j}$, rewrite \textup{(A4)} as
\[
...
$\mathbb R^0$, both sums are empty, and \textup{(A5)} reads
...
finite affine expression \textup{(A4)} at fixed $\beta$ gives
\textup{(A6)} without interchanging any limit or infinite sum.  At an
...
bound in \textup{(A7)}.  There is one literal factor $R$ per nonpivot term
...
original-coordinate chart \textup{(A4)} satisfies \textup{(A5)} and
\textup{(A7)}.  These conclusions include ties, zero unselected features,
...
strict-before and weak-after comparisons in \textup{(A1)}, resolving all
ties by the least index.  Lemma~\ref{lem:denominator-exhaustion} proves both
inclusions in \textup{(A3)} and finite-level entry.  Finally,
...
\tag{A12}
\]
This set is open.  Replacing the functions in \textup{(A4)} by their
extensions defines a $C^1$ function $\widetilde T_j$ on $O_j$, and insertion
as in \textup{(A8)} defines a $C^1$ map
...
\textup{(A9)} is a Borel subset of $O_j$.  The final condition in
\textup{(A9)} is the inverse image of $[-R,R]$ under the continuous ambient
map $\widetilde T_j$.  Thus $D_{j,m}$ is Borel and the restricted map is
Borel.

Kechris's Borel-image result states that a Borel image of an analytic set
between standard Borel spaces is analytic
\citep[Section~14, Proposition~14.4, p.~86]{Kechris1995}.  Euclidean spaces
are standard Borel, and a Borel domain is analytic, so each
$\Psi_j(D_{j,m})$ is analytic.
...
precisely $S_I$.  The same cited proposition therefore makes $S_I$
analytic without relying on the graph-coverage identity proved below.

Every set just shown analytic lies in $C=[-R,R]^N$: this is explicit in
\textup{(A10)}, and \textup{(A9)} puts the beta coordinates and the
...
\textup{(A9)}.  For $N=1$, $J_1=\varnothing$ and
...
condition is imposed.  For $N=1$, \textup{(A15)} is scalar and the
zero-dimensional beta measure in \textup{(A16)} is one.
\end{proposition}
...
\]
Restriction to the original chart proves \textup{(A15)}.  The beta
...
Applying \textup{(A18)} coordinatewise gives a global Lipschitz map
...
setting.  Both sides of \textup{(A19)} are nonnegative extended integrals.
Summing over $\ell$, exchanging the sum with the nonnegative integral, and
using disjointness of \textup{(A17)} yields
\[
\begin{aligned}
...
\end{aligned}
\]
which proves \textup{(A14)} with no localization count or constant.

Lemma~\ref{lem:analytic-chart-events} makes the image measurable.  The
multiplicity in \textup{(A14)} is at least one on that image, even when it
is infinite.  Thus \textup{(A14)} and \textup{(A15)} give the first
inequality in \textup{(A16)}.  For the second, nonnegative product
...
\textup{(A14)}.  Distinct or repeated roots add preimages; the proof never
...
\begin{proof}
Only the condition $\theta\in E_{j,m}$ in \textup{(A9)} depends on $m$.
...
$D_{j,m}$, and \textup{(A8)} yields $\Psi_j(\theta,\beta)=a$.  This proves
one inclusion in \textup{(A23)}.
...
limit in their nonnegative integrals turns \textup{(A26)} into
\[
\lambda_N(A_j)
...
By \textup{(A23)}, finite subadditivity, and \textup{(A27)},
\[
\lambda_N(S_I)
...
\tag{A29}
\]
Equations \textup{(A28)}--\textup{(A29)} prove \textup{(A25)}.  Chart
...
If $I$ is empty or a singleton, $|I|=0$ and \textup{(A29)} makes all chart
...
original-coordinate root set $S_I$ in \textup{(A10)} is analytic and
Lebesgue measurable and satisfies \textup{(A25)}.
\end{proposition}
...
multiplicity identity, determinant \textup{(A15)}, and beta-cube factor.
Lemma~\ref{lem:exact-root-event-coverage} proves both inclusions in
\textup{(A23)} in the original coefficient coordinates.
Proposition~\ref{prop:root-event-volume} then performs the increasing limit
and uses the disjoint-cell identity \textup{(A29)}.  The separate strict
...
Substituting it into \textup{(A33)} gives the first two lines of
\textup{(A30)}.  Since $R>0$,
\[
\kappa(2R)^{N-1}
=\frac{\kappa(2R)^N}{2R}
...
Equation \textup{(A32)} also gives $\mu(C^c)=0$.  The unrestricted root
event in \textup{(A31)} differs from $S_I$ only by a subset of $C^c$; that
subset belongs to the completed law and has probability zero.  This proves
\textup{(A31)} without changing the event on the law's support.
...
$\lambda_N(S_I)=0$, and \textup{(A33)} forces zero probability.  For
...
density integration.  The right side of \textup{(A30)} may exceed one; it
...
\tag{A37}
\]
The cube has volume $(2R)^N$, so \textup{(A37)} integrates to one and
...
\tag{A38}
\]
Thus it defines an admissible law.  At $A=1$, \textup{(A38)} is equality,
...
\tag{A41}
\]
The value in \textup{(A41)} is a vacuous extended-real consequence, not a
...
\textup{(A37)} exists, so the boundary belongs to \textup{(A40)} whenever
...
admissible law and positive-length interval satisfy \textup{(A30)}--
\textup{(A31)}, while the ordered law-then-interval expression has the
semantics in \textup{(A40)}--\textup{(A41)}, with the interval supremum
...
with the algebraic identity \textup{(A34)}.  The sharp index classifications
...
The coefficient in \textup{(A46)} is exactly one.  This is a deterministic
...
\]
and adding $b_d(\theta)=\theta^d$ proves \textup{(A51)}.  This is equality
...
Substituting $N=d$ into $A=(2R)^N\kappa$ completes \textup{(A52)}.  No
...
monomial appears only in the first term.  For $d=1$, \textup{(A55)} is
\[
|\theta'|+R\sum_{k=1}^0(\cdots)=1,
\]
which proves \textup{(A53)}.
...
\]
for both signs of $\theta$, proving equality in \textup{(A54)}.  At
...
\tag{A56}
\]
This proves \textup{(A54)}.  Both points $\theta=1$ and $\theta=-1$ stay
...
\tag{A62}
\]
Substituting \textup{(A60)}--\textup{(A62)} into the definition of $V_j$
gives equality in \textup{(A57)} for both signs of $\theta$.  Since
$|\theta|>1$, each $m/|\theta|^{m+1}\leq m$; equation \textup{(A56)}
...
=\sum_{k=0}^{d-1}|\theta|^k,
\]
which is \textup{(A58)}.  At every point of this set take the minimum over
...
\tag{A65}
\]
At $|\theta|=1$, only \textup{(A64)} is used, so there is no duplicated
boundary or chart-count cost.

If $K_R$ is nonempty, taking its defining supremum in
\textup{(A63)}--\textup{(A65)} proves \textup{(A59)}.  If it is empty,
...
\textup{(A59)} depends only on $d$ and $R$, not the location, diameter, or
endpoints of $\Theta$.
\end{proof}
...
\leq d+R\frac{d(d-1)}2,
\]
independently of the chosen localization.  Finally, \textup{(A51)}
identifies the affine event with the polynomial event for the same
$\alpha,\theta,I$, and $\mu$.

Applying \textup{(A30)} with $N=d$ therefore gives
\[
\begin{aligned}
...
\end{aligned}
\]
which is \textup{(A66)}.  Because $R>0$,
\[
\kappa(2R)^{d-1}
=\frac{\kappa(2R)^d}{2R}=\frac A{2R},
\]
proving \textup{(A67)} while preserving the requested $\kappa$ form.
...
condition entered the argument.  The right side of \textup{(A66)} may
...
Suppose $I=\{\theta_0\}$.  Equation \textup{(A51)} gives the coefficient
root set
\[
\begin{aligned}
H_{\theta_0}
...
\textup{(A69)} is the graph of the continuous affine function
\[
...
continuous.  The right side of \textup{(A68)} is also zero when
...
the monic family in \textup{(A50)} has the original-space identity and
descriptors \textup{(A51)}--\textup{(A52)}, obeys the conditioning bound
\textup{(A59)} for every compact restriction, and satisfies the all-bounded-
interval probability bound \textup{(A68)}.  The coefficient is literally
\[
...
top-pivot speed with the reindexing \textup{(A61)}, takes the pointwise
minimum over legal pivots, and then takes the supremum over the exact
$K_R$, proving \textup{(A59)} with no localization dependence.
...
uses the same objects and same law to obtain \textup{(A66)} and the exact
algebra \textup{(A67)}.  Proposition~\ref{prop:complete-monic-baseline}
adds empty and singleton intervals by the explicit nullity calculation
\textup{(A69)}--\textup{(A71)}.  No ordered-supremum or capacity branch is
...
=\frac{A\Gamma_{\rm piv}(b,F;R)}{2R}|I|,
\]
which is \textup{(T1)}.  Its sharp law- and interval-index classifications
then give \textup{(T2)} with a finite nonnegative value exactly when
...
proves \textup{(T3)} for the entire range $0<\delta\leq1$.
...
bound \textup{(T4)}, preserves the deterministic leading coefficient, and
derives \textup{(T5)} for positive-length intervals before completing empty
