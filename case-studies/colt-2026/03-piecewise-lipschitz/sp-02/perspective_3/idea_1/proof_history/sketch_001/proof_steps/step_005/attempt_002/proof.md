...
plane and have \(\lambda_2\)-measure zero. Under the vertical-section
convention \(B_y=\{x:(x,y)\in B\}\), the vertical axis has singleton sections
for every \(y\), whereas the horizontal axis has section \([-1,1]\) only at
\(y=0\) and empty sections otherwise. Its sole nonzero section occurs on a
null set of slice parameters, so its area is
