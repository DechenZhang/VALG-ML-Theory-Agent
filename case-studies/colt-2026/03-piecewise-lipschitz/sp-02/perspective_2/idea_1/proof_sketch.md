...
- Sketch attempt: 2
- Attempt type: Target-preserving interface repair
- Retry mode: `revise_sketch`
- Trigger: attempt-1 `step_012` review status `REVISE_SKETCH`; the complete monic presentation/certificate package lacked a legal named producer-consumer path to final assembly
- Search mode: `OPEN`
...
The roadmap has a common static/sweep trunk and three specialization branches. `step_001` derives the coefficient-height and homogeneous projective certificates. `step_002` proves the persistent-root coefficient locus is null. `step_003` proves the pivot-chart area inequality on finite exhaustion levels and passes to arbitrary measurable pivot partitions. `step_004` converts the chart integrand exactly into the coordinate-free affine normal-velocity integral. Independently, `step_005` proves translated cube sections are no larger than central ones and invokes Ball's sharp bound, while `step_006` bounds affine normal velocity using derivative closure and the root-section identity. `step_007` assembles the general affine rate. The homogeneous branch `step_008` sharpens it through the projective identity. The monic branch `step_009` verifies the augmented monomial presentation, derivative-shift certificate, two pivots, and velocity caps. `step_010` then proves a named current-notation wrapper that explicitly re-exports that complete `step_009` package together with the exact probability inequality obtained from `step_003`. `step_011` audits Counter-example 1. `step_012` performs dependency-only theorem assembly from named theorem-style outputs, including the `step_007` zero-certificate result, the `step_008` radial-cancellation result, and the complete `step_010` monic wrapper; bare step IDs may appear only for audit traceability, not as proof authority.

This attempt retains the twelve stable Step IDs and every unaffected attempt-1 claim, source, constant, interface, and boundary treatment. All attempt-2 step proofs and reviews must be fresh; archived or attempt-1 step artifacts are diagnostic repair context only and are not current evidence.
...
- Required bridge or simplification obligations: Verify the constant shift matrix, both pivot formulas and derivatives, negative-power bounds on \(|\theta|>1\), the \(d=1\) branch, \((2R)^{d-1}\) beta-volume, and disjoint partition-length identity. `step_010` must state a named current-notation theorem-style wrapper whose conclusion includes the exact monic object, \(q=M=m=0\), \(\Delta=N=d\), \(A=(2R)^d\kappa\), the constant derivative-shift matrix with nonzero entries \(B_{0,d}=d\) and \(B_{k+1,k}=k\) for \(1\leq k\leq d-1\), \(\widehat\Lambda_{B,T}=(\sum_{k=1}^d k^2)^{1/2}\), the legal pivot/velocity package, and the exact probability inequality.
- Baseline invariance obligations: Recover exactly \(\kappa(2R)^{d-1}(d+Rd(d-1)/2)|J|\) from `step_003`, with no independent root theorem or weakened remainder, and carry both the presentation/certificate and probability conclusions to `step_012` through the named `step_010` wrapper.
...
| Exact monomial instance, shift matrix, certificate, pivots, and velocity caps | Direct \(q=0\) specialization of the primitive setting | `step_009` | `step_010`, which must re-export the complete package in a named theorem-style wrapper consumed by `step_012` |
| Complete affine-monic wrapper: presentation/certificate package plus exact probability bound | `step_003`, `step_009` | `step_010` | `step_012` |
...
- Theorem role: Supplies every deterministic specialization input needed for `step_010` to recover the source baseline from the common affine sweep and to state a named complete monic wrapper for final assembly.
...
- Obligation locality classification: `step-local`.
- Noncircular closure status: The explicit monomial tuple independently produces its certificate, legal pivots, and velocities before the affine chart theorem is specialized. Dependency path: primitive monic data -> `step_009` -> named complete wrapper in `step_010` -> `step_012`.
...
- Producer-consumer provenance: Producer `step_009`; direct consumer `step_010`, which must restate the complete presentation/certificate/pivot/velocity package in a named current-notation theorem-style result; `step_012` consumes that named `step_010` wrapper rather than bare transitive `step_009` ancestry.
...
- Step ID: `step_010`
- Claim class: Exact affine-monic Rate Specialization Bridge and current-notation presentation/certificate wrapper.
- Theorem role: Recovers the mandatory polynomial baseline from `step_003` itself with literal constants and exports one named theorem-style interface carrying every monic presentation/certificate clause needed by `step_012`.
- Mechanism source: Accepted-derived chart inequality `step_003` and the complete exact monic presentation, derivative-shift certificate, pivot, and velocity package from `step_009`. No independent polynomial-root theorem is used. The wrapper is a current-notation consequence of those two dependencies, not an assertion based on their bare step IDs.
- Source-to-claim adequacy: `step_003` applies with \(N=d\) to the actual \(d\)-dimensional lower-coefficient law, and `step_009` supplies and proves the exact tuple \(\widetilde F=(\theta^d,1,\theta,\ldots,\theta^{d-1})\), \(q=M=m=0\), \(\Delta=N=d\), \(A=(2R)^d\kappa\), its constant derivative-shift matrix with nonzero entries \(B_{0,d}=d\) and \(B_{k+1,k}=k\) for \(1\leq k\leq d-1\), \(\widehat\Lambda_{B,T}=(\sum_{k=1}^d k^2)^{1/2}\), and the measurable legal pivot/velocity package. The named `step_010` wrapper must restate those exact conclusions and cite their named `step_009` theorem-style producers. Dropping the chart indicator is allowed by `step_003`; integrating each remaining beta cube gives exactly \((2R)^{d-1}\).
...
- Key positive/control term or structural source: The explicit derivative-shift identity and certificate, uniform velocity cap \(V_d=d+Rd(d-1)/2\), beta-volume \((2R)^{d-1}\), and disjoint partition length.
...
- Closure/dominance/absorption relation: The named wrapper first transfers the complete `step_009` package by exact current-notation restatement with no changed object, metric, or constant, and then proves
...
- Obligation locality classification: `step-local`.
- Noncircular closure status: The general chart theorem is proved before specialization, and the monic calculations are independent of the desired baseline probability. Dependency paths merge only at `step_010`; the named wrapper then legally carries both the deterministic monic package and probability conclusion to `step_012`.
...
- Baseline conclusion preserved: Exactly \(\kappa(2R)^{d-1}(d+Rd(d-1)/2)|J|\) for every \(d\geq1\), bounded \(J\), and arbitrary correlated lower-coefficient law, together with the exact deterministic-leading-coefficient presentation and certificate. No mode, dimension, constant, interface clause, or conclusion is weakened.
- Producer-consumer provenance: Named theorem-style results from (`step_003`, `step_009`) -> named complete current-notation wrapper in `step_010` -> `step_012`. The wrapper conclusion, not transitive ancestry or a bare step ID, is the final assembly authority for the full monic package.
...
- Mechanism source: Named theorem-style outputs of the direct dependencies `step_001`, `step_003`, `step_007`, `step_008`, `step_010`, and `step_011` only. In particular, the theorem basis and proof must name the `step_007` zero-certificate result, the `step_008` exact radial-cancellation result, the `step_010` complete monic presentation/certificate-and-probability wrapper, and every other result whose conclusion is consumed. Bare step IDs may be recorded for audit traceability only and are not mathematical proof authority.
- Source-to-claim adequacy: Each final clause is identical to a named earlier theorem-style producer output in the same Balcan convention, coefficient dimension, probability mode, interval mode, and norm. The complete monic package is a direct conclusion of the named `step_010` wrapper, so final assembly no longer relies on transitive `step_009` ancestry. Assembly introduces no external theorem or hidden simplification.
...
- Key positive/control term or structural source: The dependency-closed set of named static-certificate, sweep, zero-certificate, radial-cancellation, specialization-wrapper, and exact-audit theorem-style outputs.
- Opposing defect terms: Clause loss, hidden \(q,M,\Delta\) dependence, changed law or interval scope, a singular leading-coordinate representation, unhandled tangent/multiple roots, suppressed \(1/\delta\) scale, missing named authority, or transitive consumption beyond an exported interface. Each is excluded by a named direct producer.
- Closure/dominance/absorption relation: Conjoin the named theorem-style outputs from `step_001`, `step_003`, `step_007`, `step_008`, `step_010`, and `step_011` verbatim, including all stated parameter and mode audits. The proposition statement and proof must cite every consumed named result, including zero-certificate and radial-cancellation, rather than citing a step ID as proof authority.
...
- Obligation locality classification: `step-local`.
- Noncircular closure status: Every assembly input has an earlier named theorem-style producer, and no producer assumes the final theorem. The complete monic package reaches assembly through the explicit `step_009` -> named `step_010` wrapper -> `step_012` path. The dependency graph below is acyclic.
...
- Baseline conclusion preserved: The exact affine-monic presentation, shift-matrix certificate, constant, deterministic-leading-coefficient dimension, and arbitrary-correlated-law scope from the named `step_010` wrapper appear verbatim, not as a homogeneous or coarser affine surrogate. Counter-example 1 retains the exact upper \(1/\delta\) and lower \(1/(4\delta)\) scales.
- Producer-consumer provenance: Named theorem-style outputs from all six direct dependencies -> `step_012` assembly proposition -> final theorem statement. The named complete monic wrapper supplies the repaired path; zero-certificate and radial-cancellation are named explicitly at consumption.
...
| Affine velocity \(\leq(1+NR^2)\widehat\Lambda_{B,T}\) on each root section | `step_006` | Root equation; cube support; closure identity; `step_001` | \(F_0\) controlled only on the root section; \(a\) by cube support; \(B\) by certificate; no unrelated persistent term | Produced quantity is exactly the coordinate-free integrand numerator divided by \(\lVert F\rVert\); residual zero | \(\lVert(1,a)\rVert\lVert B\rVert\lVert\widetilde F\rVert/\lVert F\rVert\leq(1+NR^2)\widehat\Lambda\) | Root-section equality and anchor-backed nonzero denominator | `step_007` | None |
...
| Exact monic presentation, derivative-shift certificate, legal pivots, and \(V_d=d+Rd(d-1)/2\) velocity cap | `step_009`, direct \(q=0\) derivation | Explicit augmented tuple; coefficient-by-coefficient constant shift identity; constant inner pivot; outer \(\theta^{d-1}\) pivot; coefficient cube | Presentation degrees and dimension are fixed directly; inner monomial derivatives, outer negative powers, transition, and \(d=1\) are separately controlled; no location or hidden-coordinate residual | Produced tuple gives exactly \(p_\alpha\) on the same \(d\) lower coefficients; deterministic leading coefficient stays external; same Euclidean/Frobenius convention; residual zero | Direct matrix multiplication gives \(q=M=m=0\), \(\Delta=N=d\), \(A=(2R)^d\kappa\), and \(\widehat\Lambda^2=\sum_{k=1}^d k^2\); direct derivatives and \(\lvert\theta\rvert^{k-d}\leq1\) give the velocity caps | Literal anchor one or \(\lvert\theta\rvert^{d-1}>1\); no auxiliary threshold | Named complete current-notation wrapper in `step_010` only | None |
| Complete named affine-monic wrapper: exact presentation/certificate/pivots plus \(\kappa(2R)^{d-1}V_d\lvert J\rvert\) | `step_010`, current-notation theorem-style wrapper of `step_003` and `step_009` | Named `step_009` results give the exact tuple, \(q=M=m=0\), \(\Delta=N=d\), \(A=(2R)^d\kappa\), nonzero shift entries \(B_{0,d}=d\) and \(B_{k+1,k}=k\), height, pivots, and caps; named `step_003` result gives the chart inequality; beta-volume and partition length are exact | Correlation is controlled by the full density cap; smaller outer rate is explicitly dominated; \(d=1\) uses the zero-dimensional convention; no presentation, certificate, interval-location, or leading-coordinate defect is uncontrolled | Wrapper restates the same augmented monomial tuple, matrix, height, lower-coefficient vector, law, interval, and scalar/Euclidean metrics; \(F_0+\langle\alpha,F\rangle=p_\alpha\); every transfer residual is zero | Exact current-notation restatement of the deterministic package, followed by \(\kappa(2R)^{d-1}V_d(\lvert E_1\rvert+\lvert E_d\rvert)=\kappa(2R)^{d-1}V_d\lvert J\rvert\) | Shift identity and Frobenius equality from `step_009`; legal-pivot source from the explicit split; no auxiliary margin | `step_012`, which consumes the named wrapper as direct theorem-style authority | None |
...
| Full exact-goal theorem | `step_012` | Named theorem-style outputs from direct dependencies `step_001`, `step_003`, `step_007`, `step_008`, `step_010`, and `step_011`, including named zero-certificate, radial-cancellation, and complete monic-wrapper results | Every defect class is assigned to a named producer; the complete monic package is exported by `step_010`; assembly introduces none | Clause-by-clause same-object consumption with zero residuals; no transitive proof-history or bare-step authority | Conjunction of literal named producer outputs, with no new absorption, proof, or interface enlargement | Earlier anchor, area, section, velocity, pivot, shift-matrix, and exact-audit sources | Final theorem | None |
...
| Exact monic presentation, derivative-shift certificate, pivots, and velocity caps | `step_009`; direct monomial calculation with named theorem-style outputs | Named complete wrapper in `step_010` | Exact baseline presentation/certificate inputs | primitive monic data -> named `step_009` results -> `step_010` wrapper | derived | None |
| Complete named affine-monic presentation/certificate-and-probability wrapper | `step_010`; current-notation theorem-style wrapper proved from named `step_003` and `step_009` results | `step_012` | Baseline invariance: exact presentation, derivative-shift certificate, deterministic-leading-coefficient dimension, and exact probability conclusion | (`step_003`, `step_009`) -> named complete wrapper in `step_010` -> `step_012` | derived | None |
...
| Full assembled theorem | `step_012`; dependency-only conjunction of named theorem-style results | Final theorem statement | Every inseparable formalized-goal clause | named outputs of `step_001`, `step_003`, `step_007` (including zero-certificate), `step_008` (including radial cancellation), `step_010` (complete monic wrapper), and `step_011` -> `step_012` | derived | None |
...
| `step_004` | Prove the fixed-section identity \(\lvert T_j'\rvert d\beta=\lvert F_0'+\langle a,F'\rangle\rvert d\mathcal H^{N-1}/\lVert F\rVert\), sum it over the partition, and derive the coordinate-free first affine swept-area inequality. | `step_003` | Derived: `step_003`; primitive feature definitions from `assump:anchored-derivative-closure` | Match the chart and Hausdorff Jacobians exactly so pivot denominators cancel and no second root theorem is introduced. | Direct differentiation of the chart equation and Euclidean graph/section Jacobian calculation. | Coordinate-free affine normal-velocity interface. | R2 and R3: exact first sweep integral. | PENDING |
...
| `step_012` | Assemble one unconditional theorem containing the static bridge, equivalent chart form, coordinate-free affine rate and \(C^{\mathrm{aff}}\), sharper homogeneous rate and \(C^{\mathrm{Pf}}\), the complete exact monic presentation/certificate and probability recovery from the named `step_010` wrapper, Counter-example audit, and all dependence/mode statements, with no new claim. Its proposition basis and proof must name every consumed theorem-style producer, including the `step_007` zero-certificate result and `step_008` radial-cancellation result; bare step IDs are audit metadata only. | `step_001`, `step_003`, `step_007`, `step_008`, `step_010`, `step_011` | Primitive assumptions exactly as allocated; derived named theorem-style outputs from every dependency | Prevent clause loss, target switching, hidden dependence, mode changes, untracked generated outputs, transitive-interface consumption, or use of step IDs as proof authority at theorem assembly. | Dependency-checked conjunction of explicitly named theorem-style producer conclusions only. | Full exact formalized goal with a closed named producer-consumer basis. | R1--R5, including both baseline invariance obligations and the full monic certificate path. | PENDING |
...
`step_005` and `step_009` are independent direct units and therefore have no dependencies; each is placed before all consumers. The common affine-sweep branch ends at `step_007`, the sharper homogeneous branch ends at `step_008`, and the exact baseline branch ends at `step_010`. The repaired monic path is primitive monic data -> named theorem-style `step_009` presentation/certificate results -> named complete current-notation wrapper in `step_010` -> `step_012`. Thus the six direct dependencies of `step_012` may remain unchanged: `step_010` now legally exports the full package rather than only the probability inequality. `step_012` introduces no mathematical content and may consume only fresh attempt-2 accepted named theorem-style outputs; it must name each consumed result, including zero-certificate and radial-cancellation, while step IDs serve only as traceability metadata. This decomposition localizes the area-formula, translated-section, derivative-closure, monic-pivot, and scale-audit obligations without allowing a later specialization or assembly label to act as a missing producer.
