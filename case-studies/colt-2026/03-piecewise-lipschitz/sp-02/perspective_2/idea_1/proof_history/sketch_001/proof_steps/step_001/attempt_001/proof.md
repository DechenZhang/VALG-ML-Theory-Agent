...
- Intended claim: Prove \(F_{j_*}=1\Rightarrow F\neq0\),
  \(\sup_\Theta\|B\|_{\mathrm{op}}\leq\widehat\Lambda_{B,T}\), and, when
  \(F_0=0\),
  \(F'=B_FF\),
...
| `unit_005` | proposition | Under Assumptions~\ref{assump:parameter-regime}, \ref{assump:balcan-common-chain}, and \ref{assump:anchored-derivative-closure}, Lemmas~\ref{lem:step-001-anchor}, \ref{lem:step-001-height}, and \ref{lem:step-001-homogeneous-block}, and Proposition~\ref{prop:step-001-projective}, if \(q=0\), \(m=0\), \(B\) is constant, \(N=1\), \(\theta\) is an endpoint of \(\Theta\), or \(\widehat\Lambda_{B,T}=0\), then \(F_{j_*}=1\), \(F\neq0\), and \(\sup_\Theta\lVert B\rVert_{\mathrm{op}}\leq\widehat\Lambda_{B,T}\) still hold and, whenever \(F_0\equiv0\), so do \(F'=B_FF\), \(\gamma_F'=(I_N-\gamma_F\gamma_F^{\mathsf T})B_F\gamma_F\), and \(\Gamma_{\mathrm{proj}}(F)\leq\widehat\Lambda_{B,T}\); moreover the literal height reduces to \((\sum_{k=1}^d k^2)^{1/2}\) for the displayed constant monomial shift and to \(1/\delta\) for the displayed one-entry shear. | Discharges the accepted boundary and baseline-scale obligations without a hidden constant. |
...
| Proposition~\ref{prop:step-001-boundary} (Boundary and baseline consistency) | In each declared degenerate/boundary regime, anchor nonvanishing and the matrix bound remain valid and, when \(F_0=0\), the block identity, normalized-vector identity, and projective bound remain valid; the two required constant-matrix heights are \((\sum_{k=1}^d k^2)^{1/2}\) and \(1/\delta\). | The general certificate and the two explicitly stated constant matrices. | Direct specialization of Lemmas~\ref{lem:step-001-anchor}, \ref{lem:step-001-height}, and \ref{lem:step-001-homogeneous-block} and Proposition~\ref{prop:step-001-projective}; no later step is used. | Exact-rate audit and target-step assembly. |
...
Proposition~\ref{prop:step-001-projective}, if \(q=0\), \(m=0\), \(B\) is constant, \(N=1\), \(\theta\) is
an endpoint of \(\Theta\), or \(\widehat\Lambda_{B,T}=0\), then
...
If \(\widehat\Lambda_{B,T}=0\), every nonnegative summand
\(\sum_{\ell=0}^m|b_{rs,\ell}|T_*^\ell\) is zero. Since \(T_*>0\), every coefficient \(b_{rs,\ell}\) is zero,
and hence \(B\equiv0\). The closure identity gives \(\widetilde F'=0\) on the interval \(U\), so the tuple is
...
For the constant augmented monomial shift in item 1, the nonzero coefficient magnitudes are exactly
\(d,1,2,\ldots,d-1\). The literal \(m=0\) formula just proved gives
...
The sum over \(1\leq k\leq d-1\) is empty when \(d=1\), so that boundary case gives height one. For the
constant one-entry shear in item 2, the same literal formula gives
...
do not change these conclusions or introduce a hidden constant. These named results jointly establish every
clause of the exact `step_001` row. No dependency result or global diagnostic statement is used as mathematical
authority.
...
- Admissibility conditions and auxiliary tolerances: Exactly \(N\geq1\), \(m\geq0\), \(T>0\),
...
- Baseline-reduction check: For the constant augmented monomial shift, the certificate is exactly
  ...
  \(1/\delta\). Both follow from the same literal coefficient formula with \(m=0\); no conservative loss,
  asymptotic notation, or hidden factor is introduced.
...
- `appendix-local`: the two explicit constant-matrix patterns in
  ...
  assumption or helper dictionary.
...
- Assumption provenance: \(F_0\equiv0\) is a static local specialization stated in the accepted target. Anchor
  ...
  this step. There is no generated event, recurrence, stability condition, boundedness premise, or accepted
