...
- Binding controller-open tracker SHA-256: `b0d2751d6237dd2f5461fc0e5edaa18144df068ebfc4fcfd157bbfd75f8d5c9e`
...
| `unit_006` | proposition | **Global Linear-Case Chart** (`prop:step-009-linear-case`). Under Assumptions~\ref{assump:parameter-regime}, \ref{assump:balcan-common-chain}, and \ref{assump:anchored-derivative-closure}, if \(d=1\) and \(J\) is bounded, then the sole pivot \(\alpha_0\) is legal on all of \(J\), the zero-dimensional-beta chart is \(T_1(\theta)=-\theta\), and \(\lvert\partial_\theta T_1\rvert=1\). | Handles the empty-sum and zero-dimensional chart branch exactly. |
...
- Proposition~\ref{prop:step-009-linear-case} gives the complete \(d=1\) pivot/chart/velocity branch under the zero-dimensional-beta convention.
...
Statement:
Under Assumptions~\ref{assump:parameter-regime}, \ref{assump:balcan-common-chain}, and \ref{assump:anchored-derivative-closure}, let \(d=1\), let \(J\subset\mathbb R\) be bounded, and let \(\alpha_0\in[-R,R]\). Then
\[
