...
- Primitive conditions:
  ...
  - Assumption~\ref{assump:rank_window} gives \(r<k\), hence \(k\ge r\).
    This is used only to ensure that the first \(r\) columns exist and to
    include the boundary \(k=r+1\). The construction uses no upper bound on
    \(k\).
...
| --- | --- | --- | --- |
| `unit_001` | lemma | Under Assumptions~\ref{assump:independent_initialization} and \ref{assump:rank_window}, where the latter gives \(r<k\), hence \(k\ge r\), and the accepted Gaussianization interface, the positive-diagonal QR decomposition of the first \(r\) columns of one standard Gaussian \(r\times k\) array produces a measurable Haar \(O(r)\) orientation independent of the complete rotated shape, including the rotated remaining \(k-r\) columns; the singular first-block branch is null and admits an exact measurable extension. | Gives an explicit orientation/shape decomposition without an abstract orbit section. |
...
| `unit_003` | proposition | Under Assumptions~\ref{assump:independent_initialization} and \ref{assump:rank_window}, where the latter gives \(r<k\), hence \(k\ge r\), the accepted Gaussianization interface, and Proposition~\ref{prop:step-006-reflection-absorption}, applying the construction independently in the three modes gives independent product-Haar \(SO(r)\) orientations jointly independent of a measurable internal tangent subspace \(E\), and \(\dim(E)\le3kr\). | Produces the exact random object consumed by the fixed-target Haar step. |
...
- Proposition~\ref{prop:step-006-exact-equivariance} combines the accepted
  dependency range in equation (1) with the three exact matrix factorizations
  to prove \(\mathscr S_0=LQE\).
...
and the accepted Gaussianization interface, let
...
**Proof / justification.** Since \(J^2=I_r\), equation (12) gives
\[
...
Under Assumptions~\ref{assump:independent_initialization} and
\ref{assump:rank_window}, where the latter gives \(r<k\), hence \(k\ge r\),
the accepted Gaussianization interface, and
...
**Proof / justification.** By equation (16), for every component and mode,
\[
...
accepted exact range formula (1) proves equation (23).
...
in equation (23). The accepted `step_005` propositions then identify this
...
- Admissibility conditions and auxiliary tolerances: only \(k\ge r\), already
  implied by Assumption~\ref{assump:rank_window}, which gives \(r<k\); no
  tolerance or large-\(r\) threshold is introduced.
...
- Baseline-reduction check: if \(H_a=H_b=H_c=I_r\), equation (23) reduces exactly to
...
- The internal synthesis map \(\mathcal T_R\), its standard-basis matrix
  \(V_R\), pseudoinverse regularization in equation (21), and component
  subspaces \(E_i^m\) are `proof-local`. They prove measurability and the dimension bound
