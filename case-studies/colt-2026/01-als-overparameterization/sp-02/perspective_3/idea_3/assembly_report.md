...
- Assembly identity: sketch attempt 10, assembly attempt 4.
- Assembly attempt 4 repairs the public Step 008--010 probability interface
  without changing a theorem unit or probability budget. Proposition
  `prop:step-009-raw-witness` retains the deterministic raw-leverage-to-witness
  implication, names Proposition `prop:step-008-raw-leverage` and Lemma
  `lem:step-008-tail-conversion`, states the exact threshold
  `r >= r_{0,LEV}(kappa)`, and exports the pointwise conditional bound
  `P_init(E_deficit^c | A,B,C) <= r^-20` for every realized triple in
  `E_cond`. Its proof displays the inherited exponential failure and its
  polynomial conversion. Proposition `prop:step-010-conditional-union` now
  consumes that complete conclusion while retaining the exact four-event
  union `r^-20 + 3r^-20 = 4r^-20`, with no independence shortcut or added
  failure term.
...
- Assembly-attempt-4 static gates passed: all 15 accepted step reviews, all 45
  accepted source theorem units, all 45 public theorem units, all 15 appendix
  subsections, and all 15 subsection assemblies match. The 253 labels are
  unique, all 185 internal reference targets and all seven citation keys
  resolve, theorem/proof/assumption/display environments balance, and the six
  stable assumption labels match the setting. Every surviving cross-used
  display uses a stable ASCII label and `\eqref`; there are no manual tags,
  source aliases, malformed references, workflow artifact paths, audit
  scaffolding, template placeholders, duplicate labels, unresolved
  references, or layout warnings.
...
- Assembly attempt 4 completed two network-enabled Tectonic stabilization
  passes. Each ran TeX, BibTeX, the required TeX rerun, and `xdvipdfmx`,
  regenerated the 51-page, 353011-byte `main.pdf`, and exited successfully.
  `main.log` and `main.blg` contain no unresolved reference, undefined
  citation, multiply-defined label, overfull-box, underfull-box, LaTeX/package
  warning, or BibTeX warning.
