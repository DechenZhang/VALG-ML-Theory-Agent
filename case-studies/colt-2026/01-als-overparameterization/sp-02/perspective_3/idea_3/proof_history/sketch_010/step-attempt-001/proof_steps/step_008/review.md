...
1. Location: `unit_003`, equation (16), at the mean-threshold comparison. Defect: the display reads `3r^{-3/4}le\frac1{4\kappa_1^{12}}` with bare `le` instead of the valid relation command `\le`. Downstream effect: as written, the key domination of the Haar mean by `\tau_\kappa` is not a valid LaTeX mathematical inequality, so the exact artifact-syntax gate fails even though the intended arithmetic is correct. Smallest repair direction: replace only `le` by `\le`, preserve the unchanged claim and derivation, and obtain a fresh step review.
...
- Proof validity: The mathematical argument is correct. Since `\kappa_1\ge1`, `\tau_\kappa=(4\kappa_1^{12})^{-1}` lies in `(0,1]`. Assumption~\ref{assump:rank_window} and `d\le3kr` give `d/r^3\le3k/r^2\le3r^{-3/4}`. The threshold `r\ge\lceil(12\kappa_1^{12})^{4/3}\rceil` is exactly equivalent to `3r^{-3/4}\le\tau_\kappa`. Conditioning on the factors and shapes fixes `L,E,d,X=L^{\mathsf T}D_r`, while `Q` remains product Haar. Thus the accepted tail with `t=\tau_\kappa` gives `h\le2\tau_\kappa`; the elliptic transfer and `\|D_r\|_F^2=r` give (LEV). However, equation (16) does not encode its intended comparison because `\le` is misspelled as bare `le`.
...
- Rigor checklist: The maximal case `k=\lfloor r^{5/4}\rfloor`, `d=0`, nonzero `X`, tolerance range, uniform integration over shapes, exponent `c_Hr/(16\kappa_1^{24})`, and orthonormality calculation `\|D_r\|_F^2=r` all pass. Exact display syntax fails at equation (16).
...
- Contribution to target step: Mathematically derives the exact raw leverage certificate and its exponential conditional failure, but the malformed key display prevents certification of the submitted artifact.
- Verdict: REVISE_STEP
- Repair direction: In equation (16), replace bare `le` by `\le` and make no mathematical or interface change.
...
No hidden independent mathematical subclaim was found. The full-column projector identity, inverse Gram bound, tensor-product singular values, exact `\kappa_1^{12}` loss, conditioning/disintegration, rank-window mean domination, `\|D_r\|_F^2=r`, shape integration, and exponential tail conversion all appear in named units or checked dependency restatements. The only defect is the malformed relation token in equation (16).

## Target Claim Audit

The derivation targets the exact sketch-row output: conditional on every realized factor triple in `\mathcal E_{\rm cond}`, the raw coefficient projection `\|P_{\mathscr S_0}D_r\|_F^2` is at most `r/2` with failure at most `r^{-20}` for the full window `r<k\le\lfloor r^{5/4}\rfloor` and sufficiently large `r` depending only on `\kappa`. The target `D_r`, span `\mathscr S_0`, Frobenius norm, and probability mode are unchanged. No retired `TOP`, `INC`, pointwise `P3`, or `21z` mechanism, normalized/surrogate target, operator supremum, residual gap, or theorem drift appears. The submitted artifact nevertheless fails certification because its key mean-threshold display is syntactically invalid.

## Explicit Rate Audit

The exposed variables `r,k,d,\kappa,\kappa_1,\tau_\kappa,c_H,a_\kappa` are complete. Constants are universal or depend only on `\kappa` through displayed formulas and thresholds, not on `n,r,k`, the base triple, shapes, target direction, time, or horizon. The probability mode is conditional high probability after fixing factors, followed by a shape-uniform integration with no union loss; the horizon is initialization-only; the norm is raw coefficient Frobenius projection energy. The admissibility checks `\tau_\kappa\in(0,1]`, `d/r^3\le3k/r^2\le3r^{-3/4}\le\tau_\kappa`, `\kappa_1^{12}(2\tau_\kappa)=1/2`, and `8e^{-a_\kappa r}\le r^{-20}` are mathematically correct under the explicit threshold. The malformed `le` token must be repaired before the mean-absorption bridge is artifact-valid. The identity case `L=I` reduces to the exact direct product-Haar bound, preserving the raw-target baseline.

## Notation Surface Audit

The proof exports only (LEV). The accepted objects `L,Q,E,d,h` retain their meanings; `U,b` and the oblique Gram matrix are proof-local; `X=L^{\mathsf T}D_r`, `\tau_\kappa`, `a_\kappa`, and `r_{0,\rm LEV}` are appropriately appendix-local with explicit provenance. Delimiter counts are balanced, and no bare reference or citation command was found. The sole notation/syntax failure is bare `le` in equation (16).

## Target-Step Assembly Audit

The four named local results, together with accepted steps 001, 006, and 007, mathematically imply the exact raw leverage claim and conditional `r^{-20}` failure budget. The assembly uses theorem-style labels, retains `D_r` and `\mathscr S_0` exactly, and introduces no new bridge or lemma. As submitted, it cannot be accepted because Proposition~\ref{prop:step-008-raw-leverage} contains the malformed equation (16); after that isolated correction, no assembly or sketch change is indicated.

## Review Rationale

All requested mathematical checks pass, including the `d=0` branch, exact tensor-product singular losses, conditioning order, tolerance range, maximal-rank mean threshold, raw-target norm, shape-uniform probability mode, and explicit tail conversion. The accepted sketch interface is sound and no proof decomposition change is needed. The smallest defect is local to one malformed relation command in `unit_003`, so the controlled status is `REVISE_STEP` with producer retry target `/proof-step step_008`.
