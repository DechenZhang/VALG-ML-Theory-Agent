...
- Tectonic completed two TeX and cross-reference passes with zero warnings,
  followed by a successful `xdvipdfmx` pass that regenerated `main.pdf`. The
  controller's network-enabled compile supplied the standard `rsfs10.pfb` and
  `cmex8.pfb` resources missing from the initial stripped offline cache; the
  post-repair cached rerun also exited successfully. Standard `mathrsfs`
  typography is retained, with no source-level font substitution.
