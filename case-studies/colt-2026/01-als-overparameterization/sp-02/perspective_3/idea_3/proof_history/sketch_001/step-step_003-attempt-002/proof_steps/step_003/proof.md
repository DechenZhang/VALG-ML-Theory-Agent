...
| `unit_005` | lemma | Required but unresolved under Assumptions~\(\ref{assump:rank_window}\) and \(\ref{assump:independent_initialization}\), accepted Proposition~\(\ref{prop:step-001-realized-conditioning}\), accepted Lemma~\(\ref{lem:step-002-balanced-gaussianization}\), Proposition~\(\ref{prop:step-003-quotient}\), Lemma~\(\ref{lem:step-003-angular-moments}\), and Proposition~\(\ref{prop:step-003-offblock-split}\): uniformly conditional on a fixed realized triple in \(\mathcal E_{\rm cond}\), a current-notation repeated-block Gaussian-chaos theorem must bound the full projected off-component operator by \(C_{\rm TF}(\kappa)(\sqrt{k}/r+\sqrt{t/r})\) with failure at most \(C_{\rm dec}(\kappa)e^{-t}\). | This is the exact missing interface for (TF), including the same-mode deviations and all six cross-mode arrays. |
...
What is still missing is a theorem whose conclusion is uniform over
...
factorization of every cross-mode block. In the first stage,
\(Y\mapsto((B_p^{\mathsf T}Y)\circ(C_p^{\mathsf T}C_p-I_k))^{\mathsf T}\)
has mean-square scale \(\sqrt{k}/r\) for each fixed \(Y\). In the second
stage, the independent direction matrix \(A_p\) acts on the resulting random
...
1. it takes the supremum over \(\|Y\|_F=1\), rather than proving a fixed-
   \(Y\) moment bound;
