...
| `unit_011` | lemma | Under the assumptions and accepted dependencies of Lemma~\(\ref{lem:step-003-radial-transfer}\), for every integer \(p\ge1\), the moment \(\mathbb E\operatorname{tr}|\widetilde E_G|^{2p}\) has the exact colored cyclic-word expansion (31). Summing tangent-coordinate indices collapses each physical mode to the Gram cycle on the subsequence of positions whose tangent color is not that mode. Wick pairings give covariance loops with weight bounded by \(\kappa_1^{16p}r^{F-4p}\), and every diagram with \(v\) distinct component labels satisfies \(F+2v\le4p+3\). | Gives a model-specific trace/colored-graph reduction and proves the loop-versus-free-label invariant needed to audit repeated-component contractions. |
| `unit_012` | proposition | Under Assumptions~\(\ref{assump:rank_window}\) and \(\ref{assump:independent_initialization}\), accepted Proposition~\(\ref{prop:step-001-realized-conditioning}\), accepted Lemma~\(\ref{lem:step-002-balanced-gaussianization}\), Lemma~\(\ref{lem:step-003-radial-transfer}\), and Lemma~\(\ref{lem:step-003-colored-trace}\), conditional on a fixed realized triple in \(\mathcal E_{\rm cond}\), writing \(\chi_M=\operatorname{tr}(\Omega_M^2)\), the exact quadratic moment is (34) and is at most \(9\kappa_1^8k(k-1)/r\). For general \(p\), the total contribution of visit-local Wick diagrams is at most \(kr[3\kappa_1^8\sqrt{k}/r]^{2p}\). All-distinct component words are contained in this closed class. | Proves the target leading scale for the quadratic moment and closes the complete visit-local/all-distinct sector without a dimension net or logarithmic leading loss. |
...
assembly are independent nontrivial obligations. The colored trace reduction
and visit-local moment closure are additional independent units preserved from
attempt 8.
...
### Wick--Isserlis formula with covariance-loop contraction

- Source or name: the finite-dimensional Wick--Isserlis formula.
- Restated statement: if \(g\) is a centered Gaussian vector with covariance
  \(\Omega\), then the expectation of a product of an odd number of linear
  coordinates is zero, while for \(2s\) coordinates it is the sum over all
  perfect matchings of the product of the matched covariance entries. If the
  coordinate indices are themselves summed along a finite tensor network,
  each alternating cycle of network edges and Wick covariance pairs contracts
  to \(\operatorname{tr}(\Omega^\ell)\), where \(\ell\) is the number of
  covariance pairs in that cycle.
- Instantiated objects: independently for each component \(i\) and physical
  mode \(M\), the Gaussian vector \(u_i^M\) with covariance \(\Omega_M\)
  from Lemma~\(\ref{lem:step-003-radial-transfer}\).
- Assumption discharge: the lemma proves centered Gaussianity and independence
  across all \((i,M)\); finite dimension makes every moment and finite sum
  integrable. Since \(\operatorname{tr}\Omega_M=1\) and
  \(\|\Omega_M\|_{\rm op}\le\kappa_1^4/r\),
  \[
    \operatorname{tr}(\Omega_M^\ell)
    \le(\kappa_1^4/r)^{\ell-1}\qquad(\ell\ge1).
  \]
- Conclusion used: Lemma~\(\ref{lem:step-003-colored-trace}\) and
  Proposition~\(\ref{prop:step-003-visit-local}\).
...
- Lemma~\(\ref{lem:step-003-colored-trace}\) supplies the exact full-matrix
  colored cyclic-word expansion, covariance-loop formula, and
  \(F+2v\le4p+3\) invariant used to localize the remaining diagrams.
- Proposition~\(\ref{prop:step-003-visit-local}\) supplies the exact quadratic
  moment and the complete visit-local Wick contribution at the required
  logarithm-free leading scale.
...
Unit attempt 7 removed the column-normalization issue rather than treating it
...
Unit attempt 8 proved a model-specific part of that enumeration. Lemma~
\(\ref{lem:step-003-colored-trace}\) gives the exact colored cyclic-word and
Wick-loop formula and proves \(F+2v\le4p+3\), where \(v\) is the number of
distinct component labels and \(F\) the total covariance-coordinate loops.
Proposition~\(\ref{prop:step-003-visit-local}\) proves the exact quadratic
moment and sums every visit-local Wick diagram at the target leading scale.
The remaining diagrams are precisely those for which, at some repeated
component/mode group, a Wick pair joins half-edges belonging to distinct
visits. The loop invariant bounds each such diagram but does not by itself
bound the aggregate multiplicity of these cross-visit matchings at the
\(\sqrt{p/r}+p/r\) scale. The one-component event (21l) and cap events
(21m)--(21t) show why a variance-only all-confidence replacement is false,
but they do not refute the fixed \(p_0=\Theta(\log r)\) moment. Consequently
the cross-visit Wick sum, rather than ACG normalization, adaptive-supremum
measurability, or visit-local contractions, is the precise remaining part of
(21z).
...
**Lemma (colored cyclic trace and covariance-loop invariant).**
\(\label{lem:step-003-colored-trace}\)
Under Assumptions~\(\ref{assump:rank_window}\) and
\(\ref{assump:independent_initialization}\), accepted
Proposition~\(\ref{prop:step-001-realized-conditioning}\), accepted
Lemma~\(\ref{lem:step-002-balanced-gaussianization}\), and Lemma~
\(\ref{lem:step-003-radial-transfer}\), conditional on every fixed realized
triple in \(\mathcal E_{\rm cond}\), the following holds for every integer
\(p\ge1\). For tangent color \(m\in\{a,b,c\}\), component \(i\), and
coordinate \(\alpha\in[r]\), define the Gaussian tangent feature
\[
 V_{i,m,\alpha}
 =e_\alpha^{(m)}\otimes\!\bigotimes_{M\ne m}u_i^M.
\tag{28}
\]
In the natural column ordering,
\[
 (\widetilde E_G)_{(i,m,\alpha),(j,n,\beta)}
 =\mathbf1_{\{i\ne j\}}
 \langle V_{i,m,\alpha},V_{j,n,\beta}\rangle.
\tag{29}
\]
Let \(L=2p\), with all position indices read modulo \(L\). For a cyclic tangent-color word
\(\boldsymbol m=(m_0,\ldots,m_{L-1})\), put
\(S_M=\{\ell:m_\ell\ne M\}\). If \(S_M\ne\varnothing\), list its elements
in cyclic order and join consecutive elements. For a component word
\(\boldsymbol i=(i_0,\ldots,i_{L-1})\) satisfying
\(i_\ell\ne i_{\ell+1}\), define
\[
 \mathcal W_M(\boldsymbol i,\boldsymbol m)
 =\mathbb E_M\!\prod_{s\to t\ \text{consecutive in }\ S_M}
   \langle u_{i_s}^M,u_{i_t}^M\rangle,
\tag{30}
\]
where \(\mathbb E_M\) integrates only the \(M\)-mode Gaussian array, and set
\(\mathcal W_M=r\) when \(S_M=\varnothing\). Then exactly
\[
 \mathbb E[\operatorname{tr}|\widetilde E_G|^{2p}\mid A,B,C]
 =\sum_{\boldsymbol m}\ \sum_{\substack{\boldsymbol i:\
 i_\ell\ne i_{\ell+1}}}
 \prod_{M\in\{a,b,c\}}\mathcal W_M(\boldsymbol i,\boldsymbol m).
\tag{31}
\]

For each nonempty \(S_M\), expand (30) by Wick pairings at every
component/mode group. If \(F_M\) is the number of resulting alternating
coordinate loops and \(F\) is their total over the three modes, counting the
factor \(r\) from an empty \(S_M\) as one identity loop, then every individual
colored Wick diagram obeys
\[
 |\operatorname{weight}(\mathcal D)|
 \le \kappa_1^{16p}r^{F-4p}.
\tag{32}
\]
If \(v\) is the number of distinct component labels in
\(\boldsymbol i\), then
\[
 F+2v\le4p+3.
\tag{33}
...
**Proof / justification.** Formula (29) is the definition of the Gaussian
Terracini Gram with its component diagonal removed. Since the matrix is
self-adjoint,
\(\operatorname{tr}|\widetilde E_G|^{2p}
=\operatorname{tr}(\widetilde E_G^{2p})\). Expanding this trace gives a
cyclic product of the entries (29). In physical mode \(M\), the vector at
position \(\ell\) is \(e_{\alpha_\ell}\) when \(m_\ell=M\), and is
\(u_{i_\ell}^M\) otherwise. Summing \(\alpha_\ell\) over every position with
\(m_\ell=M\) inserts resolutions of the identity and contracts each maximal
run of such positions. If at least one position remains, this leaves exactly
the Gram cycle on the cyclic subsequence \(S_M\); if no position remains, it
leaves \(\operatorname{tr}I_r=r\). The three physical-mode arrays are
independent, proving (31).

Apply the restated Wick--Isserlis formula separately at every independent
pair \((i,M)\). Each alternating coordinate loop with \(\ell\) covariance
pairs contributes \(\operatorname{tr}(\Omega_M^\ell)\). Lemma~
\(\ref{lem:step-003-radial-transfer}\) gives
\[
 \operatorname{tr}(\Omega_M^\ell)
 \le(\kappa_1^4/r)^{\ell-1}.
\]
Across the three nonidentity mode networks, the total number of covariance
pairs is
\(\sum_M|S_M|=2L=4p\). Multiplying the loop bounds, and treating an empty
mode as one identity loop, proves (32).

It remains to prove (33). At every visit position in a nonempty \(S_M\),
there are two incident half-edges. The visit-local reference pairing joins
those two half-edges. It produces one coordinate loop for each nonempty
physical mode; together with a possible identity loop, its total is exactly
three. Let
\[
 s_{i,M}=|\{\ell\in S_M:i_\ell=i\}|.
\]
On the \(2s_{i,M}\) half-edges at one group, the union of the reference
matching and an arbitrary Wick matching is a disjoint union of even cycles.
The arbitrary matching can be obtained from the reference one using at most
\(s_{i,M}-1\) two-pair switches. Each switch reconnects two covariance
pairs and can increase the global coordinate-loop count by at most one.
Therefore
\[
 F\le3+\sum_{i,M:s_{i,M}>0}(s_{i,M}-1).
\]
The sum of all \(s_{i,M}\) is \(4p\). If the component word uses \(v\)
distinct labels, every such label occurs in at least two nonempty
component/mode groups: one occurrence of tangent color \(m\) belongs to the
other two physical-mode subsequences. Hence the number of nonempty groups is
at least \(2v\), and (33) follows. \(\square\)

### unit_012: proposition

**Proposition (quadratic moment and visit-local trace sector).**
\(\label{prop:step-003-visit-local}\)
Under Assumptions~\(\ref{assump:rank_window}\) and
\(\ref{assump:independent_initialization}\), accepted
Proposition~\(\ref{prop:step-001-realized-conditioning}\), accepted
Lemma~\(\ref{lem:step-002-balanced-gaussianization}\), Lemma~
\(\ref{lem:step-003-radial-transfer}\), and Lemma~
\(\ref{lem:step-003-colored-trace}\), conditional on every fixed realized
triple in \(\mathcal E_{\rm cond}\), write
\(\chi_M=\operatorname{tr}(\Omega_M^2)\le\kappa_1^4/r\). Then
\[
 \begin{aligned}
 \mathbb E[\operatorname{tr}|\widetilde E_G|^2\mid A,B,C]
 =k(k-1)\bigl[&r(\chi_b\chi_c+\chi_a\chi_c+\chi_a\chi_b)\\
 &+2(\chi_a+\chi_b+\chi_c)\bigr]
 \le9\kappa_1^8\frac{k(k-1)}r.
 \end{aligned}
\tag{34}
\]
For any integer \(p\ge1\), call a colored Wick diagram in (31)
`visit-local` when, at every component/mode group, it pairs the two
half-edges belonging to the same visit position. The sum of the absolute
weights of all visit-local diagrams satisfies
\[
 \mathcal T_p^{\rm loc}
 \le kr\left(3\kappa_1^8\frac{\sqrt k}{r}\right)^{2p}.
\tag{35}
\]
In particular, every component word with all labels distinct is included in
this closed sector.

**Proof / justification.** For \(p=1\), sum squared Frobenius norms of the
nine blocks for each ordered pair \(i\ne j\). A same-mode block, for example
the \(aa\) block, contributes
\(r\chi_b\chi_c\). A cross-mode block, for example \(ab\), contributes
\[
 \mathbb E\!\left[\langle u_i^c,u_j^c\rangle^2
   \|u_j^a\|_2^2\|u_i^b\|_2^2\right]=\chi_c.
\]
There are three same-mode blocks and two ordered cross-mode blocks for each
remaining physical mode, which proves the equality in (34). The covariance
bound from Lemma~\(\ref{lem:step-003-radial-transfer}\) gives
\(\chi_M\le\kappa_1^4/r\); since \(\kappa_1\ge1\), the displayed upper bound
follows.

For a fixed tangent-color and component word, the visit-local matching is
unique. If every tangent color is the same mode \(M\), its weight is
\[
 r\prod_{N\ne M}\operatorname{tr}(\Omega_N^{2p});
\]
otherwise all three subsequences are nonempty and its weight is
\[
 \prod_M\operatorname{tr}(\Omega_M^{|S_M|}).
\]
In either case, because \(\sum_M|S_M|=4p\), the absolute weight is at most
\(\kappa_1^{16p}r^{3-4p}\). There are at most \(3^{2p}\) tangent-color
words and \(k^{2p}\) component words. Consequently
\[
 \mathcal T_p^{\rm loc}
 \le3^{2p}\kappa_1^{16p}k^{2p}r^{3-4p}
 =kr\left(3\kappa_1^8\frac{\sqrt k}{r}\right)^{2p}
   \left(\frac{k}{r^2}\right)^{p-1}.
\]
Assumption~\(\ref{assump:rank_window}\) gives
\(k/r^2\le r^{-3/4}\le1\), proving (35). If every component label is
distinct, each nonempty component/mode group has one visit and hence only the
visit-local Wick matching, proving the final statement. \(\square\)
...
Lemma~\(\ref{lem:step-003-colored-trace}\) then rewrites the full moment as an
exact colored cyclic sum, proves the covariance-loop formula, and establishes
\(F+2v\le4p+3\). Proposition~
\(\ref{prop:step-003-visit-local}\) proves the exact quadratic moment and
sums every visit-local diagram, including every all-distinct component word,
at the target \(\sqrt{k}/r\) leading scale. The remaining portion of (21z) is
the aggregate of cross-visit Wick pairings at repeated component/mode groups.

The exact step does not close because Lemma~
\(\ref{lem:step-003-colored-trace}\) and Proposition~
\(\ref{prop:step-003-visit-local}\) do not bound that cross-visit aggregate
at \(\sqrt{p/r}+p/r\), and hence do not prove all of (21z). The all-\(t\)
sufficient wrapper cannot be retained as written. If the remaining sector of
(21z) were proved, Markov's
inequality, (21x-c), and the explicit specialization above would give (22)
with the safe choice
\[
  C_{\rm TF}(\kappa)=32e\sqrt{32/30}\,C_G(\kappa),
\]
which would establish (TF) on \(\mathcal E_{\rm TF}(30\log r)\), and Proposition~
...
- Probability mode: initialization-high-probability, uniformly conditional on
  ...
  while the visit-local part of the Gaussian trace moment is proved and its
  cross-visit Wick sector remains unresolved.
...
- Term absorption or simplification inequalities:
  ...
  \]
  \[
    p_0\le32\log r,\qquad
    \frac{p_0}{r}\le\sqrt{\frac{p_0}{r}},
  \]
  and hence, with
  \(b_r=\sqrt{k}/r+\sqrt{30\log r/r}\),
  \[
   \frac{\sqrt k}{r}+\sqrt{\frac{p_0}{r}}+\frac{p_0}{r}
   \le2\sqrt{32/30}\,b_r.
  \]
  Thus a proved (21z), Markov, and (21x-c) would permit the safe choice
  \[
    C_{\rm TF}(\kappa)=32e\sqrt{32/30}\,C_G(\kappa).
  \]
  The remaining deterministic absorption
  \[
...
- Full-operator covariance reduction: equation (21d) gives
  ...
  failure event of probability at most \(r^{-30}\). Lemma~
  \(\ref{lem:step-003-colored-trace}\) proves the exact full trace-word and
  loop invariant, while Proposition~\(\ref{prop:step-003-visit-local}\)
  proves (34)--(35). Therefore only the repeated-component cross-visit sector
  of (21z) remains; its candidate \(p_0/r\) term is absorbed by the displayed
...
- Probability conversion: the three (TC) pairs cost
  ...
  \(3kr e^{-2p_0}\le r^{-40}\), hence at most \(r^{-30}\). Equation (36)
...
1. **Failed local assembly of (21z):** Lemma~
   \(\ref{lem:step-003-colored-trace}\) (`unit_011`) proves the exact colored
   cyclic-word expansion, the covariance-loop formula, and
   \(F+2v\le4p+3\). Proposition~
   \(\ref{prop:step-003-visit-local}\) (`unit_012`) proves the exact
   quadratic moment and sums every visit-local Wick diagram at the target
   leading scale. What remains in (21z), at
   \(p_0=\lceil31\log r\rceil\), is the sum of diagrams for which some
   repeated component/mode group pairs half-edges from distinct visit
   positions. Bounding this cross-visit sum by
   \[
    3kr\left[C_\kappa\left(
      \frac{\sqrt k}{r}+\sqrt{\frac{p_0}{r}}+\frac{p_0}{r}
    \right)\right]^{2p_0}
   \]
   is the sole missing theorem-range estimate.
2. **First independent hard obligation: the four-colored face/circuit
   inequality remains unresolved.** For each individual cross-visit diagram,
   (32)--(33) give
   \(|\operatorname{weight}(\mathcal D)|
   \le\kappa_1^{16p}r^{F-4p}\) and
   \(F+2v\le4p+3\). These bounds expose the correct tradeoff between free
   component labels and covariance loops, but they do not yet control the
   topology of the complete four-colored contraction.

   The exact diagnostic formulation is as follows. Contract the color-0 trace
   edges and let \(K\) be the resulting connected loopless Eulerian
   multigraph with \(E=2p\) edges and \(B\) vertices. The local tangent
   matching defines a distinguished one-circuit Euler-tour transition system,
   and each tour visit carries its tangent color. For each physical mode
   \(M\), the selected Wick pairing induces a circuit partition \(H_M\) of
   the same graph that agrees with the Euler-tour pairing at the \(M\)-colored
   visits; write \(F_M\) for its number of circuits and
   \(F=F_a+F_b+F_c\). The needed topological inequality is
   \[
      B-2\le\sum_{M\in\{a,b,c\}}(E/2-F_M),
      \qquad\text{equivalently}\qquad F+B\le3p+2.
   \tag{37}
   \]
   In the equivalent jacket notation requested for this audit, with
   incidence-graph cycle rank \(\beta\) and jacket Euler deficits
   \(\Delta_M\), the same obligation is
   \(\sum_M\Delta_M\ge\beta-E/2+1\). No proof and no valid diagram
   counterexample to (37) is established in this attempt. In particular,
   degree-two suppression, Cohn--Lempel/interlace-rank reasoning, and jacket
   Euler characteristics are only candidate proof routes and are not used as
   authority anywhere above.
3. **Second independent hard obligation: excess-sensitive multiplicity
   summation remains unresolved even if (37) holds.** A face bound controls
   the power of \(r\) of one diagram, but (21z) additionally requires a
   canonical count of component identifications and cross-visit Wick pairings
   at each topological excess. That count must offset its own multiplicity and
   produce the exact
   \(\sqrt{p/r}+p/r\) correction without multiplying the leading
   \(\sqrt{k}/r\) term by powers of \(\log(kr)\). No such enumeration is
   proved here. The component lower event (21l) and cap events (21m)--(21t)
   rule out crude norm-first and variance-only all-confidence substitutes,
   but they do not refute the fixed \(p_0=\Theta(\log r)\) moment.
4. **Sketch decomposition is now the smallest sound workflow repair.** The
   current monolithic `step_003` bundles at least the two independent hard
   obligations in items 2--3: first establish or refute the exact
   face/jacket inequality (37), then, conditional on the valid topology,
   prove the excess-sensitive diagram count. After the explicit controller
   feedback requesting this separation, a `/proof-sketch` decomposition is
   warranted. This is an interface decomposition only: it must preserve the
   exact (TF) claim, primitive assumptions, accepted dependencies, rate,
   probability mode, quotient convention, and raw-deficit output. It does not
   authorize weakening the theorem or changing the sketch's mathematical
   target.
5. **Failed target assembly:** without (21z), (TF) and the lower bound on
   ...
   The event intersection and prefactor-aware conversion (36) are now
   explicit; only the cross-visit sector of the Gaussian trace-moment producer
   of \(\mathcal E_{\rm TF}\) is missing.
...
- \(V_{i,m,\alpha}\), the subsequences \(S_M\), the mode moments
  \(\mathcal W_M\), the loop counts \(F_M,F\), the distinct-label count
  \(v\), the visit counts \(s_{i,M}\), and \(\mathcal T_p^{\rm loc}\) are
  `proof-local` colored-trace objects defined and bounded in (28)--(35).
  Lemma~\(\ref{lem:step-003-colored-trace}\) proves their exact source and
  loop invariant; Proposition~\(\ref{prop:step-003-visit-local}\) proves the
  visit-local sum. The Gaussian covariance energies
  \(\chi_M=\operatorname{tr}(\Omega_M^2)\) are `proof-local` proved
  quantities bounded by \(\kappa_1^4/r\).
- The four-colored half-edge encoding, the contracted Eulerian multigraph
  \(K\), its edge and vertex counts \(E=2p,B\), the circuit partitions
  \(H_M\), their circuit counts \(F_M\), the incidence cycle rank \(\beta\),
  and the jacket deficits \(\Delta_M\) described in `Blockers` are
  `proof-local` diagnostics only. The identification of \(F_M\) with a
  color-\(0M\) alternating-circuit count is bookkeeping for the already proved
  Wick loops. Inequality (37), its jacket reformulation, and the
  excess-sensitive multiplicity estimate are explicitly unproved and are not
  used in any local result or target assembly.
...
- \(\mathcal E_{\rm TF}(t)\) is an `appendix-local` proposed generated event
  ...
  to (21z), while Lemma~\(\ref{lem:step-003-colored-trace}\) and Proposition~
  \(\ref{prop:step-003-visit-local}\) prove its trace reduction and
  visit-local sector. The cross-visit sector remains unproved; the event is used
...
- Assumption provenance: conditioning and Gaussianization are accepted derived
  ...
  `unit_010`; the colored trace formula, loop invariant, quadratic moment, and
  visit-local sector are proved in `unit_011`--`unit_012`. The cross-visit
  sector of (21z), and therefore (TF), remains a named blocker and is not
...
- Attempt 10 preserves all twelve reviewer-approved local units from attempt
  9. It adds no mathematical unit because the topology audit did not produce
  a proof or a valid counterexample; it only records the exact two-stage
  combinatorial obstruction and the resulting need for sketch decomposition.
- Attempt 10 read `global_proof.md` only after confirming that the paired
  ...
  sketch, accepted dependencies, the triggering attempt-9 proof/review,
