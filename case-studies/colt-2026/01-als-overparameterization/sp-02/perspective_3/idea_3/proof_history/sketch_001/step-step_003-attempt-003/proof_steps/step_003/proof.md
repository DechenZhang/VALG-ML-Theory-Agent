...
- Conclusion used: the constant-probability sharpness statement (20e).
...
**Lemma (fixed-input cross-mode mean square and sharpness).**
...
\tag{20e}
\]
Consequently the \(\sqrt{k}/r\) term required by (TF) is already necessary
for a fixed input; (20e) is a sharpness check, not a contradiction to (TF).
...
\(\kappa_1^4\sqrt{k-1}\,\|Y\|_F/r\). Its isotropic one-column stress test also
shows that this leading order is necessary with constant probability. In the
uniform problem, however, the independent direction matrix \(A_p\) acts on
the data-dependent random class \(\mathcal Q_{B,C}\), and the maximizing
\(Y\) may itself depend on \(A_p,B_p,C_p\).
...
does not prove the displayed \(\sqrt{k}/r\) rate. Thus neither available route
discharges (21).
...
normalization through already proved moments, and shows by an isotropic
one-column lower bound that \(\sqrt{k}/r\) is the sharp leading scale.
...
- Fixed-input scale and sharpness:
  ...
  \(1/36\). This validates the leading scale but does not interchange
  expectation and the data-dependent supremum.
...
1. **Failed local unit:** Lemma~
   ...
   fixed-input mean-square premise and shows the leading scale is sharp; the
   unresolved operation is precisely the uniform, data-dependent upgrade.
2. **Why current inputs do not discharge it:** `step_001` supplies
   ...
   interface, variance scale, nine-array assembly, and conditional tail.
...
- Assumption provenance: conditioning and Gaussianization are accepted derived
  ...
  the fixed-input cross-mode mean square are proved here; (TF) remains the
