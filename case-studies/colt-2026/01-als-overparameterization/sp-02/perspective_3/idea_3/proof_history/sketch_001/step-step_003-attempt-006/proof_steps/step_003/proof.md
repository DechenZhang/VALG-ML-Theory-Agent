...
- Local conditional hypotheses:
  ...
  - The isotropic angular specialization \(H_A=H_B=H_C=I_r\) is used only
    ...
    (21l) and all-confidence obstruction (21m)--(21r) of Lemma~
...
required polynomial-confidence tail. Moreover, Lemma~
...
product-direction Gram tail is proved here.
...
\(k/r^2\). Equations (21m)--(21r) then prove a new obstruction: the proposed
...
\(\ref{lem:step-003-terracini-chaos}\) does not prove the target-range
analogues of (21j)--(21k), and hence does not prove a valid replacement for
(21). The all-\(t\) sufficient wrapper cannot be retained as written; it must
either be restricted to a range containing \(30\log r\) or include the
large-deviation corrections in (21u). If the target-range tails were proved,
(22)
...
\(r^{10}\ge2C_{\rm dec}(\kappa)\) and \(r^5\ge2\). Without that target-range
tail bound and its finite prefactor, neither (TF) nor the deficit event is
exported.
...
- Full-operator covariance reduction: equation (21d) gives
  ...
  are at most \(\kappa_1^8(k-1)/r^2\). Equations (21m)--(21r) prove that the
  ...
  quantifier must be restricted. At the consumed value \(t=30\log r\), those
  extra terms are absorbed by the original target terms, so the theorem-facing
  (TF) rate itself is not weakened by this obstruction.
- Probability conversion: the three (TC) pairs cost
  \(3r^{-30}\le r^{-25}\). A valid target-range replacement for (21) at
  \(t=30\log r\) would cost
...
1. **Failed local unit:** Lemma~
   ...
   identities (21g)--(21h). It also proves that the proposed all-\(t\)
   full quotient tail (21), weighted ACG covariance tail (21j), and same-mode
   product-direction Gram tail (21k) are false: the isotropic cap events
   (21o), (21q), and (21s) contradict their \(e^{-t}\) bounds at the explicit
   confidence choices (21p), (21r), and (21t). What remains is a target-range
   tail, uniformly for all nine mode
   arrays at least at \(t=30\log r\), preserving the logarithm-free leading
   scale and a finite \(C(\kappa)r^{-30}\) prefactor. Equivalently, an
   all-confidence theorem may include the large-deviation corrections in
   (21u), which are absorbed at the consumed confidence.
   This obstruction changes the proof-local all-confidence wrapper, not the
   accepted sketch's theorem-facing rate, because the sketch consumes only
   \(t=30\log r\). A same-setting target-range proof therefore remains a
   `/proof-step step_003` repair; `/proof-sketch` is required only if that
   target-range interface itself fails.
2. **Why current inputs do not discharge it:** `step_001` supplies
   ...
   of order \(\sqrt{k}/r\) with constant probability, so a bounded-increment
   route pays the entire leading scale before polynomial-confidence
   amplification. Equations (21m)--(21r) now rule out the stronger pure
   subgaussian all-confidence interface itself, not merely that crude proof
   route. No checked cited theorem supplies the exact anisotropic
   weighted-covariance law, same-mode product Gram, nine-array assembly,
   quotient interface, and target-range conditional tail.
3. **Failed target assembly:** without a valid target-range replacement for
   (21), (TF) and the lower bound on
...
- \(\mathcal E_{\rm TF}(t)\) is an `appendix-local` proposed generated event
  ...
  sufficient tails (21j)--(21k), but does not prove their target-range
  replacements; it is used
...
- Assumption provenance: conditioning and Gaussianization are accepted derived
  ...
  `unit_005`. The target-range tails replacing (21j)--(21k), and therefore
  (TF), remain named blockers and are not promoted to assumptions.
- Boundary cases: Gaussian coefficient images are nonzero almost surely. The
  ...
  large-deviation scale, but this does not affect \(t=30\log r\).
- Attempt 6 did not read or use `global_proof.md`. All preserved and new
  claims use only the setting, accepted sketch, accepted dependencies, the
  triggering attempt-5 proof/review, restated standard facts, and local
  derivations.
