...
2. **Exact combinatorial residual:** for each individual cross-visit diagram,
   (32)--(33) give
   \(|\operatorname{weight}(\mathcal D)|
   ...
   aggregate number of nonlocal matchings with a given collision profile.
   A valid completion must sum those profiles without replacing the model by
   a generic strong noncommutative-Khintchine bound that multiplies the leading
   \(\sqrt{k}/r\) term by powers of \(\log(kr)\).

   A concrete diagnostic route is now localized but remains unproved. Split
   each trace position into incoming and outgoing half-edge nodes, use a
   color-0 matching for the cyclic trace edges, and use one matching for each
   physical mode, taking the tangent-coordinate identity matching at positions
   of that tangent color and the selected Wick matching elsewhere. Then
   \(F_M\) is the number of alternating color-\(0M\) circuits. The
   three-physical-color subgraph can split one component label into several
   connected bubbles, so its bubble count cannot silently be identified with
   \(v\). A possible completion would first prove a face/excess inequality
   such as
   \[
     F+v\le3p+2,
   \]
   define \(e=(3p+2-v)-F\), and then count canonical component
   identifications and nonlocal pairings with enough loss in \(e\) to offset
   their multiplicity. Neither this stronger face inequality nor the required
   excess-sensitive multiplicity sum has been proved, and neither is used as
   mathematical authority in this artifact.

   The component lower event
   (21l) and cap events (21m)--(21t) rule out crude norm-first and
   variance-only all-confidence substitutes, but no fixed-\(p_0\)
   counterexample is proved. This remains a same-setting
   `/proof-step step_003` repair; `/proof-sketch` is required only if the
   theorem-range event itself is shown false or needs an interface change.
...
- The four-colored half-edge encoding, physical-color bubble count, and
  candidate excess \(e=(3p+2-v)-F\) described in `Blockers` are
  ...
  Wick loops. The stronger face inequality and excess-sensitive multiplicity
  estimate are explicitly unproved and are not used in any local result or
  target assembly.
...
- Attempt 9 read `global_proof.md` only after confirming that the paired
  ...
  sketch, accepted dependencies, the triggering attempt-7 proof/review,
