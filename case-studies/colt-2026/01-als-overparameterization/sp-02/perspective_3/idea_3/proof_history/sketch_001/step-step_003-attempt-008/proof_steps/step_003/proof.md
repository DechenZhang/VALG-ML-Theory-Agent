...
and visit-local moment closure are additional independent units in attempt 8.
...
Unit attempt 8 proves a model-specific part of that enumeration. Lemma~
...
Under the assumptions and accepted dependencies of Lemma~
\(\ref{lem:step-003-colored-trace}\), write
...
- Term absorption or simplification inequalities:
  ...
  \[
    C_{\rm TF}(\kappa)=32e\sqrt{32/30}\,C_G(\kappa).
  \]
  The remaining deterministic absorption is
  \[
...
2. **Exact combinatorial residual:** for each individual cross-visit diagram,
   ...
   \(\sqrt{k}/r\) term by powers of \(\log(kr)\). The component lower event
...
- Attempt 8 read `global_proof.md` only after confirming that the paired
