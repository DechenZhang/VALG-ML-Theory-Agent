...
- Local conditional hypotheses:
  ...
  - The isotropic angular specialization \(H_A=H_B=H_C=I_r\) is used only
    ...
    (21l) of Lemma~\(\ref{lem:step-003-terracini-chaos}\), as a diagnostic
    lower-bound and alignment stress test. It is not assumed in the
...
| `unit_005` | lemma | Under Assumptions~\(\ref{assump:rank_window}\) and \(\ref{assump:independent_initialization}\), accepted Proposition~\(\ref{prop:step-001-realized-conditioning}\), accepted Lemma~\(\ref{lem:step-002-balanced-gaussianization}\), Proposition~\(\ref{prop:step-003-quotient}\), Lemma~\(\ref{lem:step-003-angular-moments}\), Proposition~\(\ref{prop:step-003-offblock-split}\), and Lemma~\(\ref{lem:step-003-fixed-input-crossmode}\), conditional on a fixed realized triple in \(\mathcal E_{\rm cond}\), every cross-mode array has an exact full-multi-column representation \(E^{ab}(E^{ab})^*=\sum_jR_j^{bc}\otimes p_j^a(p_j^a)^{\mathsf T}\); its two full-angular-law operator-variance orientations are bounded by \(\kappa_1^8(k-1)/r^2\). The desired tail (21) follows from the displayed weighted ACG covariance tail for these sums and the analogous product-direction Gram tail, but those two uniform tails remain unresolved. | Replaces the fixed-input random-class gap by an exact positive-semidefinite covariance identity that already includes data-dependent multi-column inputs, proves both variance orientations at the target scale, and isolates the precise missing sharp tail for all nine arrays. |
...
required to complete Lemma~\(\ref{lem:step-003-terracini-chaos}\). Proposition~
...
The unresolved result is therefore the following sharper current-notation
wrapper. Uniformly over the conditioned ACG laws, it must give an
\(e^{-t}\) upper tail at scale \(k/r^2+t/r\) for the positive-semidefinite
sum in (20aa), for all six mode permutations, and an upper tail at scale
...
statement has been restated and discharged for the weighted covariance sum
(20aa), the ACG column normalization, all six coupled cross-mode arrays, the
same-mode deviations, the fixed-factor conditional probability, and the
required \(C_{\rm dec}(\kappa)e^{-t}\) tail. Treating such a theorem as
available would violate the source-to-claim contract.
...
**Lemma (full multi-column covariance reduction and remaining sharp tail).**
...
The unresolved statement is that these exact variance identities admit the
following sharp uniform tail, jointly with the same-mode arrays.  There must
exist finite constants \(C_{\rm TF}(\kappa)\),
\(C_{\rm dec}(\kappa)\), and \(r_{\rm TF}(\kappa)\) such that, for every
...
It remains to upgrade (21f)--(21h) to a tail for the random weighted
covariance itself.  A sufficient cross-mode statement is
\[
 \mathbb P\left(
...
\tag{21j}
\]
with the five permuted analogues.  Indeed,
\(\sqrt{x+y}\le\sqrt x+\sqrt y\), so (21d) and (21j) give the required
cross-mode norm.  The three same-mode arrays require
\[
 \mathbb P\left(
...
\tag{21k}
\]
and its two permutations.  A union bound over nine arrays and the operator
norm bound for a \(3\times3\) block matrix would then prove (21), with finite
constants depending only on \(\kappa\). Explicitly, if every one of the nine
mode-block norms is at most \(u\), then for
\(x=(x_a,x_b,x_c)\), each output-mode norm is at most
\(u(\|x_a\|+\|x_b\|+\|x_c\|)\), so the full norm is at most
\(3u\|x\|\). Quotient projection is contractive.
...
This is a rigorous obstruction to that proof route, not a counterexample to
(21).  No multi-column quotient/full-operator counterexample to (21) is
proved here.
...
Lemma~\(\ref{lem:step-003-terracini-chaos}\) now removes that random-class
...
\(k/r^2\). The only unproved part of that lemma is the sharp weighted ACG
covariance tail (21j)
and product-direction Gram tail (21k), including their mode permutations.

The exact step does not close because Lemma~
\(\ref{lem:step-003-terracini-chaos}\) does not prove (21j)--(21k), and hence
does not prove (21). If those tails were proved, (22)
...
- Full-operator covariance reduction: equation (21d) gives
  ...
  are at most \(\kappa_1^8(k-1)/r^2\). The missing tail (21j) is exactly at squared
  scale \(k/r^2+t/r\), whose square root is bounded by
  \(\sqrt{k}/r+\sqrt{t/r}\). The same-mode tail (21k) is required at the
  unsquared target scale.
...
1. **Failed local unit:** Lemma~
   \(\ref{lem:step-003-terracini-chaos}\) (`unit_005`) proves the exact
   multi-column covariance reduction (21d) and the two target-scale variance
   identities (21g)--(21h), but it does not prove the weighted ACG covariance
   tail (21j) or the same-mode product-direction Gram tail (21k). These tails,
   including all mode permutations, are exactly what remains to prove (21).
   They must preserve the logarithm-free leading scale \(\sqrt{k}/r\), add
   only \(\sqrt{t/r}\), and retain a finite
   \(C_{\rm dec}(\kappa)e^{-t}\) prefactor uniformly over fixed conditioned
   factors.
2. **Why current inputs do not discharge it:** `step_001` supplies
   ...
   Equation (21d) eliminates the former uncertainty about the adaptive input:
   it is exactly a positive-semidefinite weighted covariance sum. No checked
   result controls that sum when the weights \(R_j^{bc}\) are generated by
   two ACG arrays and the rank-one factors use the third column-normalized ACG
   array. Replacing \(R_j^{bc}\) by its operator norm discards its internal
   direction structure; (21l) proves that one such component already has norm
   of order \(\sqrt{k}/r\) with constant probability, so a bounded-increment route
   pays the entire leading scale before polynomial-confidence amplification.
   This rules out that proof route but does not contradict (21). No checked
   cited theorem has the exact anisotropic weighted-covariance law, same-mode
   product Gram, nine-array assembly, quotient interface, or conditional tail.
...
- \(\mathcal E_{\rm TF}(t)\) is an `appendix-local` proposed generated event
  ...
  the exact covariance reduction but not the required tails (21j)--(21k); it
  is used only as a local conditional premise in Proposition~
...
- \(C_{\rm TC}(\kappa)=6\kappa_1^8+75\) is an `appendix-local` proved
  ...
  \(C_{\rm sm,dec}\) occur only in the unresolved sufficient tails
  (21j)--(21k). None is used as an established bounded quantity.
...
- Assumption provenance: conditioning and Gaussianization are accepted derived
  ...
  assumptions. The tails (21j)--(21k), and therefore (TF), remain named
  blockers and are not promoted to assumptions.
...
- Attempt 5 did not read or use `global_proof.md`. All preserved and new
  claims use only the setting, accepted sketch, accepted dependencies, the
  triggering attempt-4 proof/review, restated standard facts, and local
  derivations.
