...
| `unit_002` | proposition | Under Assumptions~\ref{assump:dimension}, \ref{assump:rank_window}, \ref{assump:base_conditioning}, \ref{assump:gaussian_smoothing}, and \ref{assump:independent_initialization}, accepted Proposition~\ref{prop:step-003-ambient-floor}, accepted Lemma~\ref{lem:step-004-finite-variation}, accepted Proposition~\ref{prop:step-004-objective-limit}, and membership in \(\mathsf C_2\), define \(m=\delta-L_P-\zeta\) and \(\epsilon=(m/(\kappa^6C_T))^2\). Then \(m>\delta/2\), \(\epsilon>0\), the objective has a finite limit, and that limit is at least \(\epsilon\|T\|_F^2\). | Performs the nonnegative squaring and scalar limit passage with all dependence retained. |
...
| `unit_004` | proposition | Under accepted Propositions~\ref{prop:step-003-baseline-floor} and \ref{prop:step-004-objective-limit}, accepted Lemma~\ref{lem:step-004-finite-variation}, conditional clause 3, \(\delta>0\), \(L_P<\delta/4\), and the recorded specialization \(Q=I\), \(E_\rho=0\), and \(T=D_r\), the stronger limiting floor \((\delta-L_P)^2\|T\|_F^2\) holds. | Verifies the exact/noiseless baseline invariance obligation. |
...
Lemma~\ref{lem:step-004-finite-variation}, and accepted
Proposition~\ref{prop:step-004-objective-limit}, suppose a realized
...
\ref{assump:rank_window}, \ref{assump:base_conditioning},
\ref{assump:gaussian_smoothing}, and
\ref{assump:independent_initialization}, and with the setting-defined ALS
trajectory and positive parameters satisfying
...
Lemma~\ref{lem:step-004-finite-variation}, and accepted
Proposition~\ref{prop:step-004-objective-limit}, suppose the recorded
exact/noiseless coordinate-orthonormal specialization
\[
Q=I,
\qquad E_\rho=0,
\qquad T=D_r
...
\quad\text{for every }t\ge0.
\]
The accepted `step_004` results give the displayed finite objective limit
under clause 3. Lemma~\ref{lem:step-005-limit-order} passes the pointwise
baseline floor to that same limit. Thus removing the smoothing residual and
coordinate distortion preserves the stronger constant relative floor; it
does not replace the baseline conclusion by an absolute remainder, stopped
claim, or vanishing-defect statement. \(\square\)
