...
1. The required compile verification is pending. The installed Tectonic
   executable exited with code 101 before TeX processing because its macOS
   `system-configuration` proxy initialization panicked with `Attempted to
   create a NULL object`; the same panic occurred with `--only-cached`.
   Consequently there is no TeX log from which to certify zero errors,
   unresolved references, or warnings. Smallest repair target:
   `/proof-assembly` after controller-side compilation in an environment
   where Tectonic can initialize.
...
- `arxiv.sty` and `ims.bst` are byte-identical to the canonical shared support
  files. `reference.bib` remains present, and the empty bibliography is
  suppressed because the accepted proofs invoke no external source theorem.
