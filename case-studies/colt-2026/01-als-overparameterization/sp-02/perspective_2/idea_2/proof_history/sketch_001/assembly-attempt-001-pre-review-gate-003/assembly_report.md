...
1. The required post-repair compile verification is pending. After the
   `subfigure` conflict was removed, a controller-side Tectonic run completed
   but reported invalid-UTF-8 warnings from the unused `algorithm` and
   `algorithmic` packages and one 17.07095pt overfull box in the tensor-map
   premise. The unused package loads have now been removed, and that premise
   has been reflowed by displaying the unchanged definition of \(Q\). This
   sandbox's Tectonic still exits with code 101 before TeX processing because
   its macOS `system-configuration` proxy initialization panics with
   `Attempted to create a NULL object`. Consequently the revised bundle needs
   one further controller-side compile to certify zero errors, unresolved
   references, and warnings. Smallest repair target: `/proof-assembly`
