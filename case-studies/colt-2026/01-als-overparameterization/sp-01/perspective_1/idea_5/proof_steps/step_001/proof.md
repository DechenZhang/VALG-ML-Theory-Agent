...
- Step ID: `step_001`
- Unit attempt: 2
- Repair trigger: `proof_steps/step_001/review.md` (unit attempt 1)
...
For a unified mode index `M in {U,V,W}`, define the setting-level raw
columns and normalized columns by
\[
\begin{array}{c|ccc}
M&U&V&W\\
\hline
\bar f_{Mj}&\bar a_j&\bar b_j&\bar c_j\\
\bar e_{Mj}&\bar u_j&\bar v_j&\bar w_j
\end{array}
\qquad
\alpha_{Mj}:=\|\bar f_{Mj}\|_2.
\]
Thus `bar e_(Mj)=bar f_(Mj)/alpha_(Mj)`. Let
`g_(Uj),g_(Vj),g_(Wj)` denote respectively the setting perturbations
`g_j^(A),g_j^(B),g_j^(C)`, and define the relative perturbation
\[
h_{Mj}:={g_{Mj}\over\alpha_{Mj}},\qquad
s_{Mj}:={\rho\over\alpha_{Mj}}\le s:=\kappa _0\rho,qquad
d_{Mj}:=\|\bar e_{Mj}+h_{Mj}\|_2.
\]
The realized unit direction is
`e_(Mj)=(bar e_(Mj)+h_(Mj))/d_(Mj)`. Define the realized factor matrices
\[
E_M:=[e_{M1},\ldots,e_{Mr}],\qquad E_U:=U,\quad E_V:=V,\quad E_W:=W,
\]
so the local matrix symbol is exactly the setting matrix in every mode. The
realized raw column is
`bar f_(Mj)+g_(Mj)=alpha_(Mj)(bar e_(Mj)+h_(Mj))`.
...
The numerical normalization absorption is explicit: since `q_*=1/4096`
and `h_*<q_*/40`,
\[
\nu_*
 \le { (q_*/40)^2\over 2(1-q_*/40)}
 = {q_*^2\over 3200(1-q_*/40)}
 < {q_*^2\over3000}.                                                           \tag{11a}
\]
Equation (2) of the Gaussian event gives
...
\sum_{\ell\ne j}|\langle e_{Mj},e_{M\ell}\rangle|
 \le {B_*\over(1-d_*)^2}
 \le {q_*/2+q_*^2/768\over(1-q_*/32)^2}.                                      \tag{20}
\]
With `q_*=1/4096`, the required numerical absorption is displayed as
\[
{q_*/2+q_*^2/768\over(1-q_*/32)^2}
 =q_*\,{1/2+1/(768\cdot4096)\over(1-1/(32\cdot4096))^2}
 <0.501q_*<q_*.                                                                \tag{20a}
\]
Thus the inequality holds for every row and mode,
proving (14). \(\square\)
...
- Term absorption or simplification inequalities:
  ...
  \[
  h_*<q_*/40,\quad
  \nu_*\le {q_*^2\over3200(1-q_*/40)}< {q_*^2\over3000},\quad
  d_*\le q_*/32,
  \]
  \[
  B_*\le q_*/2+q_*^2/768,qquad
  {B_*\over(1-d_*)^2}\le0.501q_*<q_*.
  \]
...
- `E_G` is `proof-local`. It is generated and proved in
  ...
  its raw clauses.
- The raw-column symbols `bar f_(Mj)` and the normalized symbols
  `bar e_(Mj),alpha_(Mj),h_(Mj),s_(Mj),d_(Mj)` are `appendix-local`
  translations of the original setting objects. The mapping to
  `bar a,bar b,bar c`, `bar u,bar v,bar w`, the three raw base norms, and the
  three perturbation families is stated before the derivation. The matrix
  alias `E_M` is also `appendix-local` and is explicitly identified with
  `U,V,W`; none of these aliases introduces a new condition.
