...
This is proof-sketch attempt 10 for `perspective_1/idea_5`, rerun in
`revise_sketch` mode after the triggering global-proof review at
...
- Attempt-10 repair summary: preserve the entire `step_007b` first-sweep bridge,
  but put the `step_008` coordinate maps on the scaling quotient.  A canonical
  product-preserving equal-norm representative is applied after each block;
  the only amplitude coordinate is the gauge-invariant represented product
  `s_j=log(gamma_j/lambda_j)`, so raw held-mode scales cannot masquerade as a
  contracting output.  The maps `U_M^q` take the two held quotient coordinates
  and return the newly solved direction plus the common product coordinate.
  With `ell=8q_*+32rho_ALS<1/16`, the sequential U/V/W estimates are
  `d_U^+<=ell max(d_V,d_W)`, `d_V^+<=ell max(d_U^+,d_W)`, and
  `d_W^+<=ell max(d_U^+,d_V^+)`.  The explicit `G_eq` assembly is justified by
  ALS scale equivariance and acts only on quotient coordinates.  The resulting
  full-sweep map `Psi^q` satisfies
  `d_Q(Psi^q(e),Psi^q(e'))<=3ell d_Q(e,e')<=(1/4)d_Q(e,e')` on the produced
  target-span tube.  No full-state or raw per-mode amplitude contraction is
  claimed, and the stopping/rate flow uses this quotient full-sweep recurrence.
...
quotient basin -> cyclic ALS -> restart and rate specialization`.

Attempt 10 retains the attempt-9 restriction: it does not claim that the
...
The attempt-10 local refinement is a coordinate-to-cycle construction.  A
...
explicit inputs.  The three coordinate outputs are composed in the prescribed
U-to-V-to-W order, and only the completed triple is passed through the final
product-one/equal-norm gauge.  This is the object identity used by
`E_basin_parallel` and `E_stop`.
...
3. **Selected-state bridge and local refinement.**
   ...
   - Required bridge or simplification obligations: retain the full residual
     ...
     input dependence, compose the sequential U/V/W recurrence, prove the
     final product-one/equal-norm gauge transfer into the full quotient norm,
     prove exact target-span entry and preservation, and state the auxiliary
     stopping choice and probability conversion.
...
- `E_basin_parallel` and the active quotient contraction are derived in
  ...
  coordinate-output maps and their held-input recurrence, and then assembles
  the full-sweep map; `step_009` consumes only that quotient full-sweep output.
  No generated basin condition is placed in the theorem assumptions.
...
- Gauge-fixed coordinates and seminorm: on `T_parallel`, apply `B` and write
  the unit direction matrices as `P_M=G_M(I+C_M)` with
  `C_M=L_M^T P_M-I`.  The only amplitude coordinate retained is the
  gauge-invariant product coordinate `s_j=log(gamma_j/lambda_j)`; per-mode
  raw amplitudes are not coordinates of the quotient map.  For
  `xi_M=(C_M,s)` and `tilde_xi_M=(tilde C_M,tilde s)`, set
  `d_M^q(xi_M,tilde_xi_M):=max{||C_M-tilde C_M||_(row,1),
  ||C_M-tilde C_M||_(col,1), max_j||G_M(C_M-tilde C_M)(:,j)||_2,
  ||s-tilde s||_infty}`.  The scalar `s` is common to the three mode
  coordinates, not three independent held amplitudes.  Thus a pure
  componentwise rescaling has zero distance, as required by the target
  quotient.  The sign chart and target permutation are fixed by `step_005`,
  and the exact target is `(C_U,C_V,C_W,s)=(0,0,0,0)`.
- Coordinate-output maps and scale cancellation: let `X_M(rho_ALS)` be the
  canonical quotient coordinates with `d_M^q(xi_M,0)<=rho_ALS`.  For each
  cyclic mode define
  `U_U^q: X_V x X_W -> X_U`,
  `U_V^q: X_U x X_W -> X_V`, and
  `U_W^q: X_U x X_V -> X_W` as follows: choose canonical representatives of
  the two held quotient inputs, perform the exact setting normal-equation
  solve, then apply `B` to the resulting quotient state and return only the
  newly solved direction coordinate together with the common product scalar
  `s^out`.  The old mode coordinate is not an argument.  In the exact
  orthogonal target, the held scale factors cancel in the normal equations:
  `gamma_j^out=lambda_j` for every block, so no uncontracted per-mode scale
  direction is hidden in the map.  Away from the target, scale dependence is
  multiplied by a direction/pair-gram defect and is charged in the displayed
  tube term below.
  ...
  direction is independent of `D_gamma`.  Thus the quotient maps have no
  hidden derivative in a held per-mode scale; only the direction/pair-Gram
  comparison contributes to `d_M^q`.
- Source-to-claim adequacy: exact target-span unfolding, the `31/32` pair-Gram
  ...
  calculation give, for two valid held-input pairs and
  `D_h=max{d_{M'}^q,d_{M''}^q}`,
  ...
  then imply
  `d_M^q(U_M^q(xi_(M'),xi_(M'')),
       U_M^q(tilde_xi_(M'),tilde_xi_(M'')))
   <= ell D_h`,
...
- Sequential Gauss--Seidel recurrence: for two quotient input triples `e` and
  `tilde e`, set `Delta_M=d_M^q(xi_M,tilde xi_M)` and
  `D=max{Delta_U,Delta_V,Delta_W}` (the common product-coordinate difference
  is included once in each `Delta_M`).  Use the actual setting order with the
  canonical representative after each block:
  `xi_U^+=U_U^q(xi_V,xi_W)`,
  `xi_V^+=U_V^q(xi_U^+,xi_W)`, and
  `xi_W^+=U_W^q(xi_U^+,xi_V^+)`.  The held-input bounds give
  `Delta_U^+<=ell max{Delta_V,Delta_W}<=ell D`,
  `Delta_V^+<=ell max{Delta_U^+,Delta_W}<=ell D`, and
  `Delta_W^+<=ell max{Delta_U^+,Delta_V^+}<=ell^2 D`.
  Because every intermediate state is already in the canonical quotient
  chart, no unchanged raw held-amplitude difference is counted as a
  contracting coordinate.
- Full-sweep map and explicit gauge assembly: define `G_eq` on any raw
  completed triple by the same product-one equal-norm rule `B`; for positive
  columns this is the unique representative with all three norms equal to
  `gamma_j^(1/3)`.  The scale-equivariance identity proves that inserting
  `G_eq` after each block gives a quotient-equivalent sequence to the literal
  cyclic ALS sequence.  On the tube, the log product coordinate and the
  direction coordinates obey the explicit transfer
  `d_Q(G_eq(xi_U^+,xi_V^+,xi_W^+),
  G_eq(tilde xi_U^+,tilde xi_V^+,tilde xi_W^+))
  <=3 max_M Delta_M^+`; this factor acts only on gauge-invariant coordinates,
  not on discarded per-mode scales.  Define
  `Psi^q(e):=G_eq(U_U^q(xi_V,xi_W),
  U_V^q(U_U^q(xi_V,xi_W),xi_W),
  U_W^q(U_U^q(xi_V,xi_W),U_V^q(U_U^q(xi_V,xi_W),xi_W)))`.
  Therefore `d_Q(Psi^q(e),Psi^q(tilde e))<=3ell D<=(3/16)D<
  (1/4)d_Q(e,tilde e)`.  This is the only contraction exported to
  `E_basin_parallel`; raw per-mode amplitude maps are deliberately not
  assigned a contraction constant.
- Residual-to-target adequacy: `E_parallel_entry` and `T_parallel` use the
  ...
  removed by `G_eq`; only `gamma_j/lambda_j` remains, so no unlisted
  condition number is introduced.  The quotient seminorm includes the
  target direction, dual row/column, and represented-product fields, while
  `G_eq` transfers to the full post-sweep quotient state.  Thus the full-sweep
  target-span contraction controls the original relative Frobenius residual
  with no surrogate.
- Key positive/control term or structural source: target pair-Gram positivity,
  ...
  displayed per-mode estimate control all of them.  Perpendicular leakage is
...
- Closure/dominance/absorption relation: `E_parallel_entry` gives
  ...
  `norm(e^(s+1))_Q<=(3/16)norm(e^s)_Q<=(1/4)norm(e^s)_Q` for every `s>=1`.
...
- Accumulation behavior / scope compatibility: the all-sweep forcing term is
  ...
  `sum_(s>=1)norm(e^s)_Q<=(4/3)norm(e^1)_Q`; exact unfolding range containment
  preserves `N_M^s=0` at every subupdate.  No adversarial additive defect or
...
- Obligation locality classification: `step-local`; the prior
  ...
  held-input recurrence, and final gauge transfer, while `step_007b` supplies
  the generated target-span domain.
...
- Entry-state / first-update stress result: start from the actual state `e^1`
  ...
  `rho_ALS/3`, so all three quotient coordinate maps and the canonical gauge
  are active on the first contracted sweep.  At the exact CP point, each
  quotient output equals its target coordinate and `G_eq` is stationary.  A
  ...
  consumed.
- Baseline conclusion preserved: exact orthogonal factors are fixed and arbitrary
  accuracy remains possible with no additive floor.
- Producer-consumer provenance:
...
- Mechanism source: the finite `E_parallel_entry` bridge, the quotient
  coordinate maps and sequential recurrence from `step_008`, the final-gauge full-sweep
  factor `3ell<=3/16<1/4`, and relative residual equivalence in the balanced
  chart.
- Source-to-claim adequacy: `step_008` produces the actual full-sweep map
  `Psi^q` and the recurrence `norm(e^(s+1))_Q<=(3/16)norm(e^s)_Q`; use the
  conservative inequality `3/16<1/4` only for the public stopping choice.
...
- Source-to-claim adequacy: `Pr(all J runs fail | E_sm) <= (1-p_0)^J`; choose
  ...
  r(n^2+r^2)(1+ceil(log(8 kappa_0^2/epsilon)/log4))]`, so
...
- Source-to-claim adequacy: the map gives `(A+,B+,C+)=(BC,AC,AB)`, and the
  ...
  recurrence remains zero, and `G_eq` is the identity on the equal target
  amplitudes.  Thus arbitrary positive `epsilon` is allowed.
...
- Entry/first-update stress: the exact fixed point is unchanged by the first
  coordinate output, all later coordinate outputs, and the final gauge;
...
| `E_basin_parallel`: invariant target-span quotient tube and full-sweep map `Psi^q` | `step_008` | `E_parallel_entry`, Khatri-Rao floor, dual kernel, target-span normal-equation derivatives, gauge-fixed coordinate seminorms `d_M^q`, held-input maps `U_M^q`, scale-equivariant canonical gauge `G_eq` | coefficient, inverse, product-coordinate, and direction defects scale with tube radius; perpendicular defect is identically zero by exact range preservation; raw held scales are quotiented before output comparison | exact CP quotient and original loss; target-span condition is produced by the algorithm and uses the same realized target | coordinate `ell=8q_*+32rho_ALS<1/16`; sequential U/V/W recurrence; quotient gauge transfer `<=3`; full-sweep `3ell<=3/16<1/4`; exact target-span preservation | `rho_ALS=1/1024`, entry `<rho_ALS/3` | `step_009` | None |
| `E_stop`: arbitrary accuracy | `step_009` | first-sweep entry, contraction, residual equivalence, active cost | numerical residual controlled geometrically after one finite entry sweep; no confidence term in rank | residual is original relative Frobenius residual | `m>=1+ceil(log(8 kappa_0^2/epsilon)/log4)` | `C_res=8 kappa_0^2` | `step_010`, final theorem | None |
...
| `E_basin_parallel`, quotient coordinate maps, and full-sweep map `Psi^q` | `step_008` | `step_009` | arbitrary-accuracy active refinement on the produced invariant domain | `E_parallel_entry,E_sm` -> canonical quotient gauge `B` -> target-span normal equations -> `d_M^q` codomains and held-input `U_M^q` maps -> sequential U/V/W recurrence -> `G_eq` transfer -> `Psi^q` -> `step_009` | derived | None |
| `E_stop` | `step_009` | final assembly | public accuracy and stopping theorem | `E_parallel_entry,E_basin_parallel` -> `step_009` -> `E_stop` | derived | None |
...
| `step_008` | On the generated invariant domain `T_parallel(rho_ALS)`, quotient active factors by the positive product-one scaling action, define the canonical equal-norm representative, define gauge-fixed coordinate codomains/seminorms and held-input maps `U_U^q,U_V^q,U_W^q`, prove the sequential U/V/W Gauss--Seidel recurrence, explicitly assemble `G_eq` and the quotient full-sweep map `Psi^q`, and establish `d_Q(Psi^q(e),Psi^q(e'))<=3ell d_Q(e,e')<=(3/16)d_Q(e,e')<(1/4)d_Q(e,e')`; make no raw per-mode amplitude or full-state one-block contraction claim and no claim on the false unrestricted tube. | `step_001`, `step_007b` | `E_sm`, `E_parallel_entry` (derived), `assump:base-scale` (primitive) | quotient object identity, scale-equivariance, target-span Hadamard control, held-input dependence, represented-product cancellation, sequential composition, and explicit gauge assembly | dual-basis kernel proof; target-span normal-equation inverse perturbation; exact unfolding invariance; scale-equivariant canonical gauge; explicit quotient coordinate maps and recurrence; Uschmajew DOI `10.1137/110843587`, Assumption 1, Lemma 3.2, Theorems 3.3 and 3.5 only for quotient interpretation | `E_basin_parallel`, quotient coordinate contraction and full-sweep map `Psi^q` | Terminal structural/regularity objective; `ell=8q_*+32rho_ALS<1/16`, quotient gauge factor `3`, full-sweep factor `3ell<=3/16<1/4`, explicit `rho_ALS`, inverse, and zero-forcing constants | PENDING |
| `step_009` | Including the one entry sweep, iterate active ALS to relative residual `epsilon` in `m=1+O(log(1/epsilon))` sweeps, using the produced quotient full-sweep map `Psi^q` and its `3ell<=3/16<1/4` recurrence, while inactive columns remain zero and the residual metric is the original rank-`k` Frobenius metric. | `step_007b`, `step_008` | `E_parallel_entry`, `E_basin_parallel`, `assump:accuracy-confidence` (primitive) | first-sweep accounting, quotient coordinate-to-full-sweep transfer, residual equivalence, and stopping-time accounting | quotient full-sweep recurrence and balanced-chart norm comparison | `E_stop`, per-run runtime | Numerical-error explicit objective with `m>=1+ceil(log(8 kappa_0^2/epsilon)/log4)` using the conservative `3/16<1/4` inequality | PENDING |
...
`E_parallel_entry -> B -> (d_M^q,U_U^q,U_V^q,U_W^q) -> sequential recurrence
-> G_eq -> Psi^q -> E_basin_parallel`; the scale-equivariant canonical gauge
removes raw held amplitudes before each quotient output is compared.  No held
block is treated as part of a contracting one-block codomain.  The full-sweep
quotient recurrence, rather than a per-block full-state estimate, is the sole
source consumed by `step_009`.
...
claim is deferred to `step_011` under the explicit deterministic limiting
  clause.  The new `step_008` quotient coordinate maps, held-input recurrence,
  canonical gauge transfer, and `Psi^q` composition are explicit sketch
  obligations; no raw per-mode amplitude or full-state one-block contraction
  is hidden in the `step-local` label.
