...
- Goal statement:
  ...
  basin.  Active cyclic CP-ALS then has a nonsingular Hessian modulo scaling,
  contracts linearly to arbitrary relative Frobenius accuracy, and has a
  polynomial stopping time.  With probability at least \(1-\delta_{\rm init}\)
  ...
  have zero same-state residual, and the final residual can tend to zero as
  \(\epsilon\downarrow0\).
...
- Rigor:
  ...
  The only semantic delta from perspective_1/idea_4 is the
  stationarity-certified same-state carry-over and the two-stage
  \(L_{\rm burn},L_{\rm cert}=O(\log r)\) proposal horizon; no displacement
  filter or look-ahead representative is retained.
  ...
  *Local Convergence of the Alternating Least Squares Algorithm for Canonical
  ...
  four families.  Realized geometry, coverage, recurrence closure,
  certification success, score/cluster correctness, same-state row/column
  control, basin entry, contraction, and stopping remain proof obligations,
  not assumptions.
