...
- Survey pass: attempt 2, revised after `proof_history/sketch_002/proof_sketch_review.md`
...
- Proof roadmap:
  ...
  4. Classify all certified states by support size, use the explicit
  ...
  5. In dual coordinates, close the explicit small-root column inequality and
     ...
     forcing is quadratic in Gram mass and the certificate forcing is at most
     `O(r tau_r)` in both induced `l1` norms.
  6. Transfer certified directions and observable best scalars into a
     numerical quotient tube (`rho_ALS=1/1024`), prove active rank-r/zero
     padding equivalence, and prove local cyclic-ALS contraction.
...
- Main transfer challenges: Certification must be reached from every covered
  ...
  uniform radius and exact zero-padding bridge are required.
...
- Branch notes: This is not the idea-4 last-update coefficient framework.
  There is no pre-state pair matrix.  Attempt 2 inserts the legal
  pre-cluster producer, separates trajectory chart error from selected dual
  coefficient error, and fixes numerical constants (`s_comp=1/128`,
  `C_best=512`, `rho_ALS=1/1024`).  At `q_real=0`, a finitely certified
  transient may have nonzero error, and that error remains in `R_M`; only an
  exact fixed point has zero certificate residual.  The rho-zero case is an
  explicit deterministic limiting specialization, not a new primitive
  probability domain.
