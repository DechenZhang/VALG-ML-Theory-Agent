...
This is proof-sketch attempt 3 for `perspective_1/idea_5`, rerun in
`revise_sketch` mode after `proof_history/sketch_004/proof_sketch_review.md`.
...
- Triggering review: `proof_history/sketch_004/proof_sketch_review.md`
...
- Attempt-3 repair summary: the all-certified-state classifier uses an
  l2/support-size alternative rather than a false two-coordinate dichotomy;
  `a_t` and an r-uniform ratio-l1 chart are explicit; right-side certificate
  signs and a product-preserving theta gauge are fixed; per-entry dual residual
  bounds prevent an `r^2 tau_r` row charge; and the quotient norm uses relative
...
all-certified-state l2/support-size score/graph classification -> selected
...
- `E_cluster` (one data-only cluster per target and a per-mode chart radius,
  denominator, and score margin) and `E_gauge` (post-score oriented columns
  and positive scalar) are derived from all certified states in `step_005`
  using `E_cert_eq` and the l2/support-size lemma.
...
- Source-to-claim adequacy: `K_M` is formed from the same stored columns in
  ...
  classification the matrix form is
  `P_M D_M = G_M K_M Sigma_M + R_M`, where
  ...
  Redefining `K_M` from the `bar P` columns absorbs the right sign matrices and
  gives `bar P_M D_M=G_M bar K_M+bar R_M` with no hidden sign convention.
...
- Mechanism source: the pre-cluster equation `E_cert_eq`, the chart l1 mass
  `a_t` from `step_004`, near-orthogonal Gram rows, and the exact orthogonal
  support calculation.  Let `c_M=L_M^T p_M` be the dual coefficient vector
  after the sign diagonal is retained, and normalize it in l2.  The three
  certificate equations imply the approximate pair-product relations
  `c_U = sign(theta)(c_V circ c_W)/||c_V circ c_W||_2 + e_U`, cyclically,
  with `max_M ||e_M||_2 <= 12(q_*+tau_r)`.
- Source-to-claim adequacy: use the following l2/support-size stability lemma,
  proved by substituting the three pair-product relations and applying
  Cauchy-Schwarz.  For every certified state, either (I) there is a unique
  index j with `min_M |c_{M,j}| >= 1-64(q_*+tau_r)` and
  `sum_{ell != j}|c_{M,ell}| <= 2 a_chart` in every mode, or (II) there is an
  integer `m>=2` and a common support set I of size m such that each mode's
  squared coefficient profile is within `32(q_*+tau_r)` in l1 of the uniform
  profile `1_I/m`.  In branch (II), the score satisfies
  `|theta|/lambda_max <= 1/sqrt(m)+32(q_*+tau_r) <=
  1/sqrt(2)+32(q_*+tau_r)`.
  This covers the exact m-support orthogonal fixed points for every m,
  including m much larger than 4; it does not assert that two coordinates are
  individually large.  In branch (I), the chart and denominator bounds give
  `|theta| >= (1-40q_*-8tau_r) lambda_j`, per-mode ambient error at most
  `1/128`, and `31/32 <= d_M/lambda_j <= 33/32`.  Numerically,
  `(1/sqrt(2)+32q_*+32tau_r)1.01 < 0.73` while
  `0.85(1-40q_*-8tau_r)>0.84`, so the score filter rejects every branch-II
  state once a covered component exists.  The coverage event supplies one
  branch-I certified witness for every target, and weight balance gives
  `min_j(1-40q_*-8tau_r)lambda_j >
  0.85 max_j(1+40q_*+8tau_r)lambda_j`; hence every target witness passes the
  `0.85 sigma_max` filter.  Therefore the retained graph has at least one
  branch-I vertex per target and no branch-II vertex, so its strict graph
  margins force exactly `r` connected components.
- Graph and target interface: branch-I states assigned to the same target have
  ...
  `E_cluster` therefore includes the proof-only permutation, the per-mode
  dual l1 bound `sum_{ell != j}|c_{M,ell}|<=2 a_chart`, the ambient radius
  `1/128`, and the denominator margin.  These are r-uniform interfaces, not
  max-entry surrogates.
...
- Residual-to-target adequacy: every retained state is compared in modewise
  ...
  edges alone.
- Key control term: the l2 pair-product profile relation and the component
  score margin; opposing terms are Gram leakage, weight imbalance, certificate
  l2 error, and the support-size profile defect.  Each is bounded numerically
  above.
...
- Entry/first-update stress: an exact component fixed point passes the filter;
  every exact m-support fixed point has score `lambda/sqrt(m)` and is rejected
  for m>=2 by the same bound.  At q=0 negative component scores are handled
  by the product-preserving orientation below; a zero-score state is rejected.
...
- Closed column bootstrap: let
  `chi=max_{M,j} sum_{ell != j}|(E_M)_{ell j}|`.  The `E_cluster` chart
  interface gives the initial small branch `chi<=2 a_chart=1/256`, uniformly
  ...
  `|(L_M^T R_M)_{ell j}/lambda_j| <= 2 tau_r`; consequently its column l1
...
- Exported residual interface `E_row_residual`: for every mode, target row
  ...
  `|(L_M^T R_M)_{ell j}/lambda_j| <= 2 tau_r`; hence
...
| `E_cert`: stored state with `zeta <= tau_r` | `step_004` | chart contraction and finite certification horizon | successive displacement is the only residual; geometric decay controls it | stored state is itself compared with its own same-state contraction | `zeta_t<=3(1/4)^(t-tb)<=tau_r` | `step_004b`, `step_005` | None |
| `E_cert_eq`: oriented per-state certificate equation | `step_004b` | `E_cert`, nonzero contractions, projective signs | ambient l2 residual has unknown sign but exact per-column budget `d tau_r` | compares state only to its same-state contraction; no target claim yet | multilinearity and signed residual identity | `d_M>0`, `norm(R(:,a))_2<=d_M tau_r` | `step_005`, `step_006` | None |
| `E_gauge`: post-score oriented columns and positive scalar | `step_005` | theta lower bound, certificate signs, algorithm z-sign placement | sign is discrete; theta-negative branch is handled before product-one gauge | oriented columns represent exactly the initialized tensor and redefine `bar theta=abs(theta)` | absorb right sign matrices into K after `bar P_W=sign(theta)P_W`; then product-one gamma | `abs(theta)>0.8 lambda_min` | `step_006`, `step_007` | None |
| `E_cluster`: one observable component cluster per target | `step_005` | all certified states, `E_cert_eq`, l2 support-size lemma, scores, modewise correlations | Gram, weight, certificate, and support-profile errors controlled | graph uses the same direction metric as the target chart | `m>=2` score bound `1/sqrt(m)+32(q+tau)`, `0.85` gap, `1-64 q_*` edge threshold | `step_006` | None |
...
| `E_gauge` | `step_005` | `step_006`, `step_007` | post-score product-preserving orientation and positive scalar | `E_cert_eq` -> support/score branch -> `E_gauge` -> consumers | derived | None |
...
| `step_004b` | For every certified slot, before labels or clustering, derive the right-sided column sign equation `P_M D_M=G_M K_M Sigma_M+R_M` with `norm(R_M(:,a))_2 <= d_M(a) tau_r`, including the nonzero-contraction branch and the later `bar P` convention. | `step_004` | `E_cert` (derived), `assump:random-initialization` (primitive procedure) | signed equation and residual projection without assuming target distance | exact multilinearity and definition of `zeta`; signed normal-equation wrapper | `E_cert_eq` | Certificate objective; finite per-column residual and no target-bias claim | PENDING |
| `step_005` | Using `E_cert_eq`, classify every certified state with the l2/support-size stability lemma; prove the `m>=2` score bound, theta lower bound, sign/gauge consistency, component/mixture gap, graph separation, and exactly `r` observable clusters with per-mode dual l1 chart and denominator margins. | `step_004`, `step_004b` | `E_sm`, `E_chart`, `E_chart_l1`, `E_cert_eq` (derived) | all-certified-state support classification, large-support fixed points, and negative/zero scores | signed pair-product equations, l2 profile stability, explicit perturbation inequalities | `E_cluster`, `E_gauge`, target permutation, `s_comp`, denominator margins | Structural explicit objective; constants `32,40,8,0.85,1/128,64` and graph threshold | PENDING |
| `step_006` | From the selected-state equation, close the dual-coordinate column bootstrap by the explicit quadratic small-root inequality; derive per-entry projected residual `<=2 tau_r`, induced row/column residual masses `<=2r tau_r`, then close induced coefficient row and column l1 masses `O(q_*^2+r tau_r)`. | `step_004`, `step_005` | `E_sm`, `E_cluster`, `E_cert_eq`, `E_chart_l1` (derived) | diagonal normalization, dual projection, small-root selection, per-entry residual conversion, spatial row closure | exact dual-basis algebra, unit-norm identity, Hadamard l1 inequalities, root/absorption arithmetic | `E_col`, `E_row_residual`, `E_row` | Selected-state structural objective; explicit `omega`, `chi_-`, per-entry `tau_r`, and `eta_row` scales | PENDING |
...
`R_M` is formed from that stored state, and `E_col` is closed before the
