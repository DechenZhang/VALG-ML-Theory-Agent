...
- Survey pass: attempt 8, revised after `proof_history/sketch_014/proof_sketch_review.md`
...
- Main transfer challenges: Certification must be reached from every covered
  ...
  uniform radius and exact zero-padding bridge are required.  Attempt 8
  makes the threshold/tail ledger global over certified slots, exposes the
  coefficient-vector Gram expansion, exact normalization cancellation, and
  denominator arithmetic (`104eta_0` ratio,
...
- Branch notes: This is not the idea-4 last-update coefficient framework.
  There is no pre-state pair matrix.  Attempt 8 inserts the global
