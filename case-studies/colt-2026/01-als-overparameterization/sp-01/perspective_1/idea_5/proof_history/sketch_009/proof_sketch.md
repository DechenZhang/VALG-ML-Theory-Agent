...
This is proof-sketch attempt 9 for `perspective_1/idea_5`, rerun in
`revise_sketch` mode after the triggering attempt-8 `step_008` review at
`proof_history/sketch_008/proof_steps/step_008/attempt_001/review.md`.
...
- Attempt-9 repair summary: accepted `step_007` supplies more than membership
...
Attempt 9 does not claim that the whole ball defined only by `norm(e)_Q` is a
...
3. **Selected-state bridge and local refinement.**
   ...
   - Fixed quantities: `q_*`, the target chart thresholds, relative amplitude
     coordinates, the entry field `beta_perp`, and
   ...
   - Required bridge or simplification obligations: retain the full residual
     ...
     Khatri-Rao inverse, prove exact target-span entry and preservation, and
     state the auxiliary stopping choice and probability conversion.
...
- `E_basin_parallel` and the active quotient contraction are derived in
  `step_008` on the target-span tube produced by `step_007b`; `step_009`
  consumes that output.  No generated basin condition is placed in the
  theorem assumptions.
...
- Mechanism source: `E_parallel_entry` from `step_007b`, the realized
  ...
  unrestricted `norm(e)_Q` ball.
- Source-to-claim adequacy: on `T_parallel`, write `P_M=G_M B_M`; then every
  current same-mode Gram is exactly `B_M^T H_M B_M`.  The dual row and column
  fields in `norm(e)_Q` therefore control the complete Khatri-Rao pair Gram;
  there is no unobserved `F_M=N_M^T N_M` term.  For two states in the tube,
  the direct inverse identity and Hadamard expansion give, for each active
  mode update,
  `||Phi_M(e)-Phi_M(e')||_Q <=
  [8q_*+16(norm(e)_Q+norm(e')_Q)] norm(e-e')_Q` after the stated quotient
  rebalance.  At `rho_ALS=1/1024` the bracket is below `1/16`; the fixed
  `U,V,W` Gauss--Seidel trace, including the final equal-norm gauge, gives
  `||Phi_cyc(e)-Phi_cyc(e')||_Q<=(3/16)||e-e'||_Q<
  (1/4)||e-e'||_Q`.
...
- Residual-to-target adequacy: `E_parallel_entry` and `T_parallel` use the
  ...
  relative amplitude coordinates and has constant at most
  `8 Gamma^2(1+q_*^2)/(1-q_*^2)<9`.  Absolute raw scales cancel against
  `lambda_j^(1/3)`; `kappa_0` remains only in arithmetic and stopping-cost
  bounds.  Thus target-span quotient contraction controls the original
  relative Frobenius residual with no surrogate or unlisted condition number.
...
- Closure/dominance/absorption relation: `E_parallel_entry` gives
  `norm(e^1)_Q<rho_ALS/3`; the exact target is a fixed point and the cyclic
  factor is below `1/4`, so
  `norm(e^(s+1))_Q<=(1/4)norm(e^s)_Q` for every `s>=1`.  This both preserves
...
- Obligation locality classification: `step-local`; the prior
  sketch/interface defect has
  been removed by `step_007b` and the contraction claim is now restricted
  only to a legally produced, invariant algorithmic domain.
- Noncircular closure status: `step_007b` proves entry and target-span membership
  before use; `step_008` proves its own inverse and contraction recurrence;
  `step_009` consumes only these derived outputs.  Neither entry nor
  invariance assumes convergence.
- Entry-state / first-update stress result: start from the actual state `e^1` produced after
  the first cyclic sweep, not an arbitrary point in the old full tube.  It
  has every perpendicular part exactly zero and norm below `rho_ALS/3`, so
  the first contracted sweep is active immediately.  At the exact CP point,
  the first sweep and every later sweep are stationary.  If the target pair
  floor vanished, the quotient Hessian could be singular beyond scaling;
  `E_sm` excludes that boundary before entry is consumed.
...
- Mechanism source: the finite `E_parallel_entry` bridge, the `1/4`
  target-span quotient contraction, and relative residual equivalence in the
  balanced chart.
...
- Obligation locality: `step-local`.
- Noncircular closure: `step_007b` produces entry, `step_008` produces
  contraction, and both are available before stopping; no restart event is
  used by this step.
...
| `E_quotient`: relative-amplitude quotient norm and accepted entry decomposition | `step_007` | product-one equal-norm gauge, relative amplitudes `alpha`, `E_col`, `E_row`, `bar P_M=G_M B_M+N_M` with `G_M^T N_M=0` and columnwise `tau_r` | absolute `kappa_0` scale cancels after division by `lambda_j^(1/3)`; collective perpendicular interaction is not yet discarded | exact CP scaling quotient and actual selected directions share the realized target; `N_M` is the exact orthogonal residual | relative amplitude coordinate, `Gamma<=1.01`, and accepted `48omega/tau_r` entry bounds | `norm(e)_Q` definition and target projector, no hidden condition number | `step_007b` | None |
| `E_pair_entry`: first-sweep collective perpendicular and pair-Gram control | `step_007b` | `E_sm`, `E_best`, `E_quotient`, `F_M=N_M^T N_M`, dual row/column bounds, relative amplitudes | mixed target-span/perpendicular terms are at most `2(1+q_*)(1+48omega)^2 tau_r^2`; coherent shared-perpendicular term is exactly charged by `r tau_r^4`; no uncontrolled class | exact held Khatri-Rao pair Gram and cross Gram of the setting update; no surrogate | `beta_perp<10^(-8)omega`, normalized cross deviation `<=96omega`, pair deviation `<=192omega+2beta_perp<1/512`, inverse floor `31/32` | target pair floor `1-q_*^2` and `tau_r=q_*^2/(10^4r)` | first `U,V,W` subupdates and `E_parallel_entry` | None |
...
| `E_basin_parallel`: invariant target-span quotient tube | `step_008` | `E_parallel_entry`, Khatri-Rao floor, dual kernel, target-span normal-equation derivatives | coefficient, inverse, gauge, and Taylor defects scale with tube radius; perpendicular defect is identically zero by exact range preservation | exact CP quotient and original loss; target-span condition is produced by the algorithm and uses the same realized target | target-span per-mode `<1/16`, cyclic `<1/4`, and exact target-span preservation | `rho_ALS=1/1024`, entry `<rho_ALS/3` | `step_009` | None |
...
| `E_basin_parallel` and quotient contraction | `step_008` | `step_009` | arbitrary-accuracy active refinement on the produced invariant domain | `E_parallel_entry,E_sm` -> target-span normal equations -> `step_008` -> `step_009` | derived | None |
...
| `step_008` | On the generated invariant domain `T_parallel(rho_ALS)`, establish the quotient Hessian kernel, normalized inverse floor `31/32`, exact target-span preservation, and `<1/4` cyclic CP-ALS contraction; make no contraction claim on the false unrestricted `norm(e)_Q` tube. | `step_001`, `step_007b` | `E_sm`, `E_parallel_entry` (derived), `assump:base-scale` (primitive) | source convention mapping, target-span Hadamard control, quotient kernel, per-mode and cyclic composition | dual-basis kernel proof; target-span normal-equation inverse perturbation; exact unfolding invariance; Uschmajew DOI `10.1137/110843587`, Assumption 1, Lemma 3.2, Theorems 3.3 and 3.5 only for quotient interpretation | `E_basin_parallel`, quotient contraction `nu<=1/4` | Terminal structural/regularity objective; explicit `rho_ALS`, target-span inverse, per-mode, cyclic, gauge, and zero-forcing constants | PENDING |
| `step_009` | Including the one entry sweep, iterate active ALS to relative residual `epsilon` in `m=1+O(log(1/epsilon))` sweeps, while inactive columns remain zero and the residual metric is the original rank-`k` Frobenius metric. | `step_007b`, `step_008` | `E_parallel_entry`, `E_basin_parallel`, `assump:accuracy-confidence` (primitive) | first-sweep accounting, residual equivalence, and stopping-time accounting | contraction recurrence and balanced-chart norm comparison | `E_stop`, per-run runtime | Numerical-error explicit objective with `m>=1+ceil(log(8 kappa_0^2/epsilon)/log4)` | PENDING |
