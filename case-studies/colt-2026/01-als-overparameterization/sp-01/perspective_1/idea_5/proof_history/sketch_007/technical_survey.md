...
- Survey pass: attempt 4, revised after `proof_history/sketch_006/proof_sketch_review.md`
...
- Applicability to current branch: The NeurIPS paper supplies the simultaneous
  ...
  `P_M D_M=G_M K_M^raw Sigma_M+R_M^raw`, with the weight diagonal retained in
...
- Proof roadmap:
  ...
  4. Classify all certified states with a weighted support/score lemma that
     retains `diag(lambda)`, covers arbitrary m-support orthogonal fixed
     points, uses the exact score `S_I=(sum lambda_i^(-2))^(-1/2)` at the
     unequal-weight boundary, handles negative and zero scores, and proves
     observable cluster correctness with chart and denominator margins.
...
- Main transfer challenges: Certification must be reached from every covered
  ...
  uniform radius and exact zero-padding bridge are required.  Attempt 4 also
  has to preserve the weight diagonal in the support equations and recheck the
  numerical score gap at `Gamma=1.01`; the right-side sign-matrix convention
  and product-preserving gauge used when `theta<0` are binding interfaces.
- Potential repair techniques and supporting references:
  ...
  - Technique: Use a weighted support/score lemma rather than perturbing an
    unweighted uniform-profile statement.  In an exact orthogonal support `I`,
    the stationary coefficients satisfy `c_i^2 proportional to lambda_i^(-2)`
    and the score is `S_I=(sum_{i in I}lambda_i^(-2))^(-1/2)`.
  - Supporting reference: the same-state weighted normal equations in
    `perspective_1/idea_5/setting.md` and the counterexample recorded in
    `proof_history/sketch_006/proof_sketch_review.md`.
  - Why it may help: The direct bound
    `|theta| <= lambda_max(1/sqrt(2)+48(q_*+tau_r))` keeps the `0.85` filter
    separated under `Gamma<=1.01` without charging `Gamma-1` to Gram error.
- Disposition: `selected`
- Branch notes: This is not the idea-4 last-update coefficient framework.
  There is no pre-state pair matrix.  Attempt 4 inserts the weighted
  `E_support_wt` producer between the raw certificate and clustering, replaces
  the invalid unweighted uniform-profile claim by a direct weighted score
  bound, exports ratio-l1 and per-entry residual interfaces, and fixes the
  right-sided raw/barred sign convention.  Relative amplitude coordinates
  make `C_best=512` independent of `kappa_0`; the numerical tube remains
