...
This is proof-sketch attempt 7 for `perspective_1/idea_5`, rerun in
`revise_sketch` mode after `proof_history/sketch_012/proof_sketch_review.md`.
...
- Triggering review: `proof_history/sketch_012/proof_sketch_review.md`
...
- Attempt-7 repair summary: every certified slot now receives a global
  ...
  two first-order Gram-factor leakages explicitly:
  `eta_G=2q_*+q_*^2`.  Together with the Neumann/normalization defect
  `eta_N=4q_*^2` and finite-support certificate charge
  `eta_C=2sqrt(r)tau_r`, set `eta_0=eta_G+eta_N+eta_C` and
  `eta_rel=8eta_0`.  The retained floor is still `vartheta_*>1/16`; the
  denominator-aware ratio chain gives a product factor `68eta_0` and the
...
- Attempt-4 repair summary retained: the pair equations retain exact
  ...
  conventions are stated.  The attempt-7 denominator-aware bound is
...
- Global all-certified tail producer `E_cert_tail`: this ledger is evaluated
  ...
  `eta_G:=2q_*+q_*^2`, `eta_N:=4q_*^2`, and
  ...
  the two Gram-factor leakages expand as
  `(I+E_{M'})circ(I+E_{M''})-I=E_{M'}circ I+Icirc E_{M''}
  +E_{M'}circ E_{M''}`; `||E_{M'}||_{row,1},||E_{M''}||_{row,1}<=q_*`,
  and each entry is at most `q_*`, so
  `||E_{M'}circ E_{M''}||_{row,1}<=q_*^2`.  Thus
  `sum_i|e^G_{M,i}|<=eta_G=2q_*+q_*^2`.  Neumann inversion and
  ...
  `g_{M,i}>=vartheta_*-eta_0>0`.  Since
  `eta_C<=2q_*^2/10^4`, the declared worst case obeys
  `eta_0<=2q_*+5.0002q_*^2<4.89e-4` and `vartheta_*>0.0937`, so the
  denominator calculation is
  explicit:
  `d_0:=eta_0/(vartheta_*-eta_0)<=11eta_0<1/128`, and
  `|e_{M,i}/g_{M,i}|<=d_0`.
  ...
  `(2d_0)/(1-d_0)<=22.3eta_0`.  Multiplying the three mode factors and
  retaining the quadratic terms gives a product factor `1+Delta_{i,l}` with
  `|Delta_{i,l}|<=68eta_0` (the bound is `65.4eta_0` at the extremal
  ...
  `(1+22.3eta_0)^2-1<46eta_0`.  Substitution into the unit-norm identity yields
  a common scale
  ...
  loss with the `<46eta_0` mode-square loss is below `118eta_0`; we export
  ...
  <=1.42e-3<a_cert:=7q_*+8tau_r<1/512` at the extremal reserve.
  The first term uses
  ...
  =8eta_0+2eta_rel^2/vartheta_*<=4.24e-3<16eta_0`; on `I_vartheta` the
...
- Attempt-7 threshold constant rerun: the two-factor Gram budget and the
  denominator-aware profile change the score margins but not the singleton
  tail input: `a_cert+4q_*<11q_*+8tau_r<1/256`, strictly below the old
  small-root cutoff.  At `r=1`, `eta_0<4.89e-4`,
  `tail_M<1.42e-3<a_cert`, `s_mix*1.01<0.794`, and
...
| `E_cert_tail` | `step_004b` | `step_005` | global no-label low/singleton/multi-support tail ledger for every certified slot, including finite-support residual conversion, two-term Gram budget, and denominator-aware profile | `E_cert_eq,E_sm` -> `eta_G=2q_*+q_*^2`, `eta_N`, `eta_C` -> `sqrt(r)tau_r` conversion -> ratio product `68eta_0` -> `E_cert_tail` -> `step_005` | derived | None |
...
| `step_005` | Using `E_cert_eq` and the global `E_cert_tail` ledger, retain `diag(lambda)` in the pair equations and prove the weighted support/score classifier, the direct `m>=2` score bound, theta lower bound, sign/gauge consistency, component/mixture gap, graph separation, and exactly `r` observable clusters with per-mode dual l1 chart and denominator margins. | `step_004`, `step_004b` | `E_sm`, `E_chart`, `E_cert_tail`, `E_cert_eq` (derived); `E_chart_l1` only as covered-slot tightening | all-certified-state support classification, threshold-straddling states, unequal-weight stationary profiles, and negative/zero scores | explicit `vartheta` threshold, `vartheta_*` floor, `a_cert` tail, two-term Gram budget, denominator profile `144eta_0`, exact `S_I`, and perturbation inequalities | `E_support_wt`, `E_cluster`, `E_gauge`, target permutation, `s_comp`, denominator margins | Structural explicit objective; constants `1/8,1/16,7,16,64,68,96,144,160,0.85`, `s_mix*1.01<0.794`, and graph threshold | PENDING |
