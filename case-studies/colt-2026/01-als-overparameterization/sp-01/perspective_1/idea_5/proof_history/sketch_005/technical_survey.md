...
- Survey pass: attempt 3, revised after `proof_history/sketch_004/proof_sketch_review.md`
...
- Proof roadmap:
  ...
  4. Classify all certified states with an l2/support-size stability lemma,
     which covers arbitrary m-support orthogonal fixed points, use the
     component-versus-mixture score gap, handle negative and zero scores, and
     prove observable cluster correctness with chart and denominator margins.
...
- Main transfer challenges: Certification must be reached from every covered
  ...
  uniform radius and exact zero-padding bridge are required.  Attempt 3 also
  fixes the right-side sign-matrix convention and the product-preserving gauge
  used when `theta<0`.
...
- Branch notes: This is not the idea-4 last-update coefficient framework.
  There is no pre-state pair matrix.  Attempt 3 inserts the legal pre-cluster
  producer, replaces the invalid two-coordinate support dichotomy by a
  support-size/l2 lemma, exports ratio-l1 and per-entry residual interfaces,
  and fixes the right-sided sign/gauge convention.  Relative amplitude
  coordinates make `C_best=512` independent of `kappa_0`; the numerical tube
  remains `rho_ALS=1/1024`.  At `q_real=0`, a finitely certified transient may
  have nonzero error, and that error remains in `R_M`; only an exact fixed
  point has zero certificate residual.  The rho-zero case is an explicit
  deterministic limiting specialization, not a new primitive probability
  domain.
