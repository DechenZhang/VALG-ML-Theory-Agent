...
- Survey pass: attempt 6, revised after `proof_history/sketch_010/proof_sketch_review.md`
...
- Proof roadmap:
  ...
  4. Emit a global no-label `E_cert_tail` ledger for every certified state:
     ...
     `mu_i=lambda_i/lambda_min`, and separate the raw defects
     `eta_G=q_*`, `eta_N=4q_*^2`, and `eta_C=2sqrt(r)tau_r`.  The finite
     conversion is `sqrt(r)tau_r=q_*^2/(10^4sqrt(r))`, which is absorbed
     without changing rank or probability.  With
     `eta_0=eta_G+eta_N+eta_C` and `eta_rel=16eta_0`, threshold at
     `vartheta=1/8`, prove `vartheta_*>1/16`, derive the retained-coordinate
     denominator/ratio chain and `|delta_{M,i}|<=32eta_0`, charge singleton
...
- Main transfer challenges: Certification must be reached from every covered
  ...
  uniform radius and exact zero-padding bridge are required.  Attempt 6
  makes the threshold/tail ledger global over certified slots, exposes the
  `sqrt(r)tau_r` finite-support charge and denominator arithmetic, and binds
  the raw/post-score notation; the right-side sign-matrix convention and
  product-preserving gauge used when `theta<0` remain binding interfaces.
- Potential repair techniques and supporting references:
  ...
  - Supporting reference: the same-state weighted normal equations in
    ...
    `proof_history/sketch_010/proof_sketch_review.md`.
...
- Branch notes: This is not the idea-4 last-update coefficient framework.
  There is no pre-state pair matrix.  Attempt 6 inserts the global
  ...
  ledger records `eta_G`, `eta_N`, `eta_C`, the exact finite-support
  `sqrt(r)tau_r` conversion, the retained-floor denominator chain, and the
  `32eta_0` profile bound.  Relative amplitude coordinates make
  `C_best=512` independent of `kappa_0`; the numerical tube remains
