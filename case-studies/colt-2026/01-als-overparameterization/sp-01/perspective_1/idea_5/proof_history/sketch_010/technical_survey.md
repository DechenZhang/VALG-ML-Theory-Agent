...
- Survey pass: attempt 5, revised after `proof_history/sketch_008/proof_sketch_review.md`
...
- Applicability to current branch: The NeurIPS paper supplies the simultaneous
  ...
  idea 4.  The proof sketch binds the shared notation explicitly: raw
  `P^raw,K^raw,Sigma,R^raw,theta` are used only for certificate production;
  after sign absorption, the setting-level `P,K,R` are aliases for
  `bar P,bar K,bar R`, and all theorem-facing scalar bounds use
  `bar theta=abs(theta)`.
- Proof roadmap:
  ...
  4. Emit a global no-label `E_cert_tail` ledger for every certified state:
     ...
     `mu_i=lambda_i/lambda_min`, threshold at `vartheta=1/8`, prove
     the retained floor `vartheta_*>1/16`, charge singleton l1 tails by
     `a_cert=7q_*+8tau_r`, and charge multi-support score tails by
     `16(q_*+tau_r)`.  Feed that ledger into a weighted support/score lemma
     that retains `diag(lambda)`, covers arbitrary m-support orthogonal fixed
...
- Main transfer challenges: Certification must be reached from every covered
  ...
  uniform radius and exact zero-padding bridge are required.  Attempt 5 also
  has to make the threshold/tail ledger global over certified slots and bind
  the raw/post-score notation; the right-side sign-matrix convention and
  product-preserving gauge used when `theta<0` are binding interfaces.
- Potential repair techniques and supporting references:
  ...
  - Supporting reference: the same-state weighted normal equations in
    ...
    `proof_history/sketch_008/proof_sketch_review.md`.
  - Why it may help: Every certified state is classified without target labels
    ...
    `|theta| <= lambda_max(1/sqrt(2)+48(q_*+tau_r))` then keeps the `0.85`
    filter separated under `Gamma<=1.01`.
- Disposition: `selected`
- Branch notes: This is not the idea-4 last-update coefficient framework.
  There is no pre-state pair matrix.  Attempt 5 inserts the global
  ...
  ratio-l1 and per-entry residual interfaces, and binds the raw/barred
  convention: setting-level `P,K,R` after retention mean the barred objects,
  while `theta` is raw and `bar theta=abs(theta)`.  Relative amplitude
  ...
  have nonzero error, and that error remains in `R_M^raw` (or `bar R_M` after
