...
- Applicability to current branch: The NeurIPS paper supplies the simultaneous
  ...
  contraction norm gives the exact same-state equation
  `P_M D_M = G_M K_M + R_M`; both `K_M` and `P_M` are formed from that stored
  state.  Passing to the true-factor dual coordinates therefore avoids every
  pre-update/post-update identification that blocked idea 4.
- Proof roadmap:
  1. Derive the realized norm, Gram-row, weight, and Khatri-Rao events and the
     rare three-mode Gaussian window coverage.
  2. Prove the displayed simultaneous `R,S` recurrences, enter a contracting
     target chart, and make consecutive projective displacements fall below
     `tau_r` in the certification window.
  3. Classify approximate projective fixed points by support size, use the
     component-versus-mixture score gap, and prove observable cluster
     correctness.
  4. In dual coordinates, first close each selected column and then close the
     incoming row mass from the exact same-state equation.  The static forcing
     is quadratic in Gram mass and the certificate forcing is at most
     `O(r tau_r)` in both induced `l1` norms.
  5. Transfer the certified directions and observable best scalars into a
     quantitative quotient chart and prove local cyclic-ALS contraction.
- Key lemmas / ingredients: normalized Gaussian Gram perturbation; correlated
  ...
  pair-mass recurrences; contraction-mapping displacement decay; perturbative
  classification of Jacobi fixed points; true-factor dual bases; induced
  row/column Hadamard inequalities; and a quantitative block-normal-equation
  perturbation bound.
- Main transfer challenges: Certification must be reached from every covered
  window without assuming convergence; all high-score certified states, not
  only the covered witnesses, must be classified; the dual certificate
  residual must be normalized by the target weight without losing more than
  `r tau_r`; and the local ALS source has an existential neighborhood, so a
  current-notation uniform radius is still required.
...
- Branch notes: This is not the idea-4 last-update coefficient framework.
  There is no pre-state pair matrix.  At `q_real = 0`, a finitely certified
  transient may have nonzero error, and that error remains in `R_M`; only an
  exact fixed point has zero certificate residual.  This preserves the exact
  orthogonal baseline without falsely asserting that every finite transient
  forcing term vanishes.
