...
- Triggering review SHA-256: `fb245bd8db9d518b74f77682d3fbe5a382a4ab0877d4d08557290d9757b150f3`
...
| --- | --- | --- | --- |
| `unit_001` | lemma | Under the three accepted dependencies, an a priori fixed-point envelope `bar beta` is defined without any generated call output; its deterministic U/V/W budgets stay in the affine-ledger domain and dominate every staged perpendicular ledger. | Produces a noncircular continuation envelope before the first call. |
...
| `unit_003` | lemma | Under the a priori envelope and entry records, the U call has `h_U=0`; its row/column defects, `d_U`, normalizer positivity, and inverse floor are proved before use; after the exact solve it emits the actual/comparator predecessor certificate retaining current `F_U`. | First chronological call and pre-V producer. |
| `unit_004` | lemma | Under the U predecessor certificate, the V call is inside the a priori domain; its row/column defects, `d_V`, positivity, and inverse floor are proved before use; after the exact solve it emits the corresponding V certificate retaining current `F_V`. | Second chronological call and pre-W producer. |
| `unit_005` | lemma | Under the U/V certificates, the W call is inside the a priori domain with all margins proved before use; the completed trajectory defines the actual `beta_FS`, sharpens the recurrence to `3072/3276/3500/4096`, and gives exact range entry. | Produces `E_pair_entry` and `E_parallel_entry`. |
...
\]
For a mode whose current perpendicular ledger is dominated by any number
`widehat beta`, put `widehat eta=omega+widehat beta`.  The proof of the
accepted interface is monotone in this domination: its static quantity is
`S_0<=288omega+2widehat beta<=290widehat eta`.  Hence, if
`h_M<=2rho_ALS/3`, `Delta_J^M<=1/64`,
the direction/diagonal reserve
\[
 d_M:=\max_j\|G_MA_M(:,j)\|_2
 \le 2\kappa_{\rm res}(288\omega+2\widehat\beta)
      +128\omega+(2q_*+8\rho_{\rm ALS})h_M             \tag{10a}
\]
is below `1/8`, and the incoming scales are positive, then the exact
normalized output satisfies, simultaneously in both induced row/column
norms, ambient columns, and the common log register,
\[
 \operatorname{err}_M^+\le3072\widehat\eta+\ell_c h_M,
 \qquad
 \ell_c=16q_*+64\rho_{\rm ALS}={17\over256}<{1\over8}. \tag{11}
\]
This dominating-envelope form is only substitution into the accepted
displayed inequalities; it does not strengthen that dependency.  Moreover,
with `d_j=(H_MA_M)_{jj}`, the same accepted centered calculation gives
`|d_j|<=q_*d_M+d_M^2`, so
\[
 \nu_{M,j}^2
 =1+2d_j+(A_M^TH_MA_M)_{jj}
 \ge1-2q_*d_M-2d_M^2.                                  \tag{11a}
\]
Thus the proof below must establish `d_M<1/8` and positivity from the
pre-call record; neither is folded into an admissibility assertion.

After an exact predecessor solve, the accepted same-current construction is
also part of the interface.  If `A_N^{pred}` is the actual predecessor
coefficient and `A_N^{0,pred}` is obtained by setting only its carried
`E`-fields to zero while retaining the current `N_N,F_N`, define
\[
 (\nu_{N,j}^{pred})^2
 =\operatorname{diag}((I+A_N^{pred})^TH_N(I+A_N^{pred})+F_N)_j,
 \quad
 (\nu_{N,j}^{0,pred})^2
 =\operatorname{diag}((I+A_N^{0,pred})^TH_N
          (I+A_N^{0,pred})+F_N)_j,                    \tag{11b}
\]
and let each `D_nu` be the diagonal reciprocal normalizer.  Then
`B_N=(I+A_N^{pred})D_{nu,N}^{pred}` and
`B_N^0=(I+A_N^{0,pred})D_{nu,N}^{0,pred}` have unit-column identities
against the same current `F_N`.  This, rather than an identity comparator,
is the predecessor certificate consumed by the next call.
