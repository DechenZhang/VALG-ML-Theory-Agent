...
- Sketch review status: `ACCEPTED` (viability score 8, retry target `None`)
...
### Block G6: same-record carried resolvent and chronological entry sweep (`step_007c`, `step_007b`)

For every held mode retain the exact record
`bar P_N=G_NB_N+N_N`, `G_N^TN_N=0`,
`B_N=B_N^0+E_N=I+C_N+E_N`, and `F_N=N_N^TN_N`.  The comparator is not a
free normalization hypothesis: set only the carried coefficient error to
zero in the exact current-record equations, solve for `A_N^{0,pred}`, define
`(nu_{N,j}^{0,pred})^2=diag((I+A_N^{0,pred})^TH_N
(I+A_N^{0,pred})+F_N)_j`, and put
`B_N^0=(I+A_N^{0,pred})D_{nu,N}^{0,pred}`.  Hence actual and comparator
records obey the same-`F_N` unit-column identities and their subtraction gives
`diag(2(B_N^0)^TH_NE_N+E_N^TH_NE_N)=0`.  The initial U call has `E_N=0`;
after exact range entry the current record has `F_N=0`.

For a call in mode M, write `h_M=sum_(N in H_M)epsilon_N` and define the
actual-record perpendicular envelope
`beta_perp(M,h_M)=(1+q_*)[(1+48omega+h_M)^2t_(M'')^2
+(1+48omega+h_M)^2t_(M')^2]+r(t_(M')t_(M''))^2`.
The generated maximum `beta_FS=max_M beta_perp(M,h_M)` is closed before
consumption.  Direct expansion gives
`beta_perp(M,h)<=b_0+L_beta h`, with
`b_0<10^(-8)omega` and
`L_beta=2(1+q_*)(1+48omega+2rho_ALS/3)tau_r^2<=10^(-10)omega`.
Since the chronological outputs imply
`h_V<=3072(omega+beta_FS)` and `h_W<=6548(omega+beta_FS)`, solve
`beta_FS<=b_0+6548L_beta(omega+beta_FS)` to obtain
`beta_FS<2*10^(-8)omega` and `eta_FS:=omega+beta_FS<1.001q_*^2`.

The literal solve and centered normalization use the exact `A_M-A_M^0`
resolvent identity.  Define the two carried budgets
`u_C=(2q_*+8rho_ALS)h_M` and
`u_J=(4q_*+16rho_ALS)h_M`.  In each induced orientation the cross term is at
most `u_C`, the pair-resolvent term at most `u_J/2`, and their paired sum at
most `u_C+u_J/2`; the same calculation supplies ambient columns and the one
common product-log register.  With
`kappa_res<17/16` and `S_0<=290eta_FS`, the static part is below
`2977eta_FS<3072eta_FS`; the centered quadratic remainder yields
`epsilon_M^+<=3072eta_FS+ell_c h_M`, where
`ell_c=16q_*+64rho_ALS=17/256<1/8`.

The consumer discharges the calls in strict order:

- U has `h_U=0` and emits `epsilon_U<=3072eta_FS`, its same-record comparator, and `N_U^+=0`.
- V consumes that record with `h_V=epsilon_U`, emits `epsilon_V<=3276eta_FS`, its comparator, and `N_V^+=0`.
- W consumes both records with `h_W=epsilon_U+epsilon_V` and emits `epsilon_W<=3500eta_FS<4096eta_FS` and `N_W^+=0`.

The scalar bootstrap and `h_W<1/2048<2rho_ALS/3` verify
`Delta_J<1/32768<1/64`, `d_M<1/16384`, positivity, and ordinary inverse
branches before each call.  Thus
`||e^1||_Q<=4096eta_FS<rho_ALS/3`.  No target-span fact is assumed before its
own exact update, and zero padding follows from the block-diagonal active
pseudoinverse.
...
| `step_007c` | G6 | `step_001`, `step_007` | Same-current-`F_N` comparator, actual beta ledger/bootstrap, paired resolvent, centered row/column/ambient/log normalization | Produces `E_perp_pair,E_carry_resolvent`; step-local |
| `step_007b` | G6 | `step_001`, `step_007`, `step_007c` | Discharge U/V/W raw records, comparator handoffs, inverse floors, zero-padding, range chronology, completed `4096eta_FS` entry | Produces `E_pair_entry,E_parallel_entry`; step-local |
...
| `E_perp_pair,E_carry_resolvent` | Conditional one-block normalized transfer | Same-current comparator, `B=B^0+E`, defined `u_C,u_J`, beta bootstrap, centered normalization, pair floor and resolvent | Static `omega,beta_FS`; carried term linear in `h_M`; no repeated call beyond one sweep | Algebraic coupling: `epsilon_M^+<=3072eta_FS+ell_ch_M`, `ell_c=17/256<1/8` | Actual held pair Gram and same direction/product target; every residual class included | `step-local`; conditional record discharged before call; stationary actual defect zero |
| `E_pair_entry,E_parallel_entry` | One chronological sweep into exact spans | U/V/W inverse floors, comparator handoffs, scalar beta closure, exact unfolding range | Unknown-sign perpendicular terms bounded by `beta_FS`, then removed mode by mode | Finite recurrence: `3072,3276,3500`, final `4096` | Exact ALS outputs `G_MZ_M`; same rank-`k` tensor and quotient | `step-local`; no span premise before own update; shared-perpendicular stress passed |
...
| `E_carry_resolvent` | `step_007c` | Same-current comparator, `B=B^0+E`, defined `u_C,u_J`, total `Delta_C,Delta_J`, beta bootstrap | `2977eta_FS<3072eta_FS`, `ell_ch_M`, centered normalization, same product target | U/V/W calls in `step_007b` | valid conditional interface; proof pending |
| `E_parallel_entry` | `step_007b` | Per-call margins, comparator records, pair defects, exact range and positive scales | `3072/3276/3500`, final `4096eta_FS<rho_ALS/3`, exact `N^+=0` | `step_008`,`step_009` | valid interface; proof pending |
...
| `E_perp_pair` | `step_007c` | raw U/V/W ledger in `step_007b` | `E_quotient` -> actual `F_N,beta_perp(M,h_M),beta_FS` bootstrap -> consumer | derived | valid, proof pending |
| `E_carry_resolvent` | `step_007c` | chronological U/V/W calls in `step_007b` | `E_sm,E_best,E_quotient` -> same-record comparator and paired conditional lemma; consumer discharges each record | explicitly conditional | valid, proof pending |
...
5. `step_007c`: prove the same-current comparator identity, exact pair ledger, defined `u_C/u_J` paired resolvent bounds, beta bootstrap, centered normalization, positivity, and `3072eta_FS+ell_ch_M` output.
6. `step_007b`: discharge the U/V/W records with `h_U=0`, `h_V=epsilon_U`, `h_W=epsilon_U+epsilon_V`; verify comparator handoffs, all inverse margins, and exact range/zero-padding chronology; obtain `3072/3276/3500/4096` without an early span premise.
