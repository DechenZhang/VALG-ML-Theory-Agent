...
- Intended proof tool or cited result: the accepted weighted ledger, direct
  realized-factor correlation estimates, finite graph inequalities, and the
  raw right-sided same-state equation.
...
- Derived invariants supplied by accepted dependencies:
  - Accepted Proposition~\ref{prop:step-004-certificate} in
    ...
    `Step-Review Status = ACCEPTED`, supplies `E_cert`, `E_sm`,
    `E_cov`, `E_chart`, and `E_chart_l1`. In particular, every covered
    target has a certified stored state; a covered stored state has dual
    off-target mass at most `1/1024`, and every stored state has the
    same-state nonzero look-ahead required by the certificate equation.
  - Accepted Proposition~\ref{prop:step-004b-tail} and
    Lemma~\ref{lem:step-004b-barred} in
    ...
    alternative, the singleton interval
    `s_- lambda_j <= |theta_a| <= s_+ lambda_j`, the
    multi-support bound
    `|theta_a| <= lambda_max(1/sqrt(m)+160 eta_0)`, the singleton tail
    `a_cert=7q_*+8tau_r`, and the retained-floor profile with reserve
    `144 eta_0`. The raw equation has the same stored state in both held
    modes and has a column residual bounded by `d_M(a) tau_r`.
...
for `step_004b`. Their accepted reviews are consumed as the review gate;
no archival or prior-branch artifact is used.
...
| `unit_006` | lemma | Certificate signs equal \(\operatorname{sgn}(\theta)\); absorbing that sign in the initialized third factor and applying a product-one direction gauge preserves the initialized rank-one term and yields the barred post-score equation with \(\bar\theta=|\theta|>0\). Define \(s_{\rm comp}=|\theta|/\lambda_{\pi(a)}\). | Produces `E_gauge`, positive scalar, component score ratio, and the exact sign convention for downstream equations. |
...
- Restated statement in current notation: conditional on `E_sm` and
  `E_cov`, every target has a covered proposal state stored after the
  finite certification window; the stored state itself is used; its
  same-state look-ahead is nonzero; and for a covered stored state the
  target-dual off-target mass is at most `1/1024`. The chart has
  \(\chi_{\rm chart}=1/128\), and all realized Gram rows have off-diagonal
  mass at most \(q\le q_*\).
...
- Source: accepted Proposition~\ref{prop:step-004b-tail},
  Proposition~\ref{prop:step-004b-singleton},
  Proposition~\ref{prop:step-004b-multi}, and
  Lemma~\ref{lem:step-004b-barred} in `step_004b`.
- Restated statement in current notation: for every stored certified slot,
  ...
  \(\|R_M^{\rm raw}(:,a)\|_2\le d_M(a)\tau_r\); before any label or graph,
...
modes. Proposition~\ref{prop:step-004b-singleton} then gives (1).
...
The lower bound in (7) follows from unit_001 applied to a target attaining
\(\lambda_{\max}\). For the upper bound, LOW slots are below
...
On the retained pool from Proposition~\ref{prop:step-005-filter}, assign
each slot its singleton index \(j\). For two retained slots \(a,b\), in
each mode:
\[
\begin{cases}
...
\tag{23}
\]
Consequently the setting graph edge criterion
...
Use the oriented copies from Lemma~\ref{lem:step-005-chart}. If two slots
...
On every retained slot, all three certificate signs in
\(E_{\rm cert\_eq}\) equal \(\operatorname{sgn}(\theta_a)\). With
\(\beta_{M,a}\) from Lemma~\ref{lem:step-005-chart},
...
s_{{\rm comp},a}:={|\theta_a|\over\lambda_{\pi(a)}}\in[s_-,s_+].
\tag{26}
\]

**Proof / justification.**

For the raw equation supplied by \(E_{\rm cert\_eq}\), take the inner
product of its \(a\)-th column with \(p_{M,a}^{\rm raw}\). Since
\(\langle p_{M,a}^{\rm raw},v_{M,a}\rangle=\theta_a\), the certificate
inequality gives
\[
...
The second assertion is Lemma~\ref{lem:step-005-chart}. Define the
...
\(s_{\rm comp}\) is the singleton interval from the ledger and the
permutation in Proposition~\ref{prop:step-005-clusters}. \(\square\)
...
exact frozen `step_005` sketch row without a target-label or later-ALS
assumption.
...
- \(\chi_{\rm dual}=a_{\rm cert}+4q_*\) and
  ...
  \(s_{\rm comp,a}=|\theta_a|/\lambda_{\pi(a)}\) is the minimal
...
- The paired `global_proof.md` and `global_proof_review.md` were read
  only after confirming `Global-Proof Review Status = ACCEPTED`. Their
  step-relevant planning suggestion was to preserve the weighted ledger,
  strict score/graph gaps, and sign absorption. They were not used as proof
