...
Under accepted Proposition~\ref{prop:step-001-smoothed-event} and
Lemma~\ref{lem:step-004-dual-entry}, fix a target `j`, put
...
Under accepted Proposition~\ref{prop:step-003-coverage} and
Proposition~\ref{prop:step-004-chart}, choose
...
\[
 D_s:=\max_M\|h_M^{t_b+s+1}-h_M^{t_b+s}\|_2,
\]

the contraction estimate and the chart diameter give
...
The projective minimum in `zeta` can only decrease the oriented distance.
The harmless constant `3` bounds the chart diameter (the sharper bound is
`2 chi_chart`). At `s=L_cert`, (47) gives
...
Proposition~\ref{prop:step-004-chart} proves closedness, self-mapping,
