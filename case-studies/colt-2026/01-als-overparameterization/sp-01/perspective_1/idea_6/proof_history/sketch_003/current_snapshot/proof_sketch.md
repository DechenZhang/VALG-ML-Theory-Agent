...
This is proof-sketch attempt 3 for `perspective_1/idea_6`, run in
...
and repairs only the carried-cancellation/normalization interface requested
by the triggering `step_007b` review, frozen at SHA-256
`9acdab69d7c88328d2596737974dd9cedc084455daf1cf2657111c115e9289d0`.
The attempt-1 sketch and its runtime repair remain lineage only; they are not
current evidence for this attempt.
...
for the present repair.  Attempt 3 leaves that wrapper and every
setting-level object unchanged; it repairs only the theorem-facing producer
interface for the first active sweep.
...
- Triggering review:
  `perspective_1/idea_6/proof_history/sketch_002/proof_steps/step_007b/attempt_002/review.md`,
  SHA-256
  `9acdab69d7c88328d2596737974dd9cedc084455daf1cf2657111c115e9289d0`
- Triggering attempted proof:
  `perspective_1/idea_6/proof_history/sketch_002/proof_steps/step_007b/attempt_002/proof.md`,
  SHA-256
...
- Dependencies and scope: `step_001` and `step_007` only.  For a mode `M`,
  ...
  `I_M=(B_{M'},N_{M'},B_{M''},N_{M''},Delta_C,Delta_J)` and by
  `epsilon_N` for the held direction errors.  Its only hypotheses are
  `||B_N-I||_(row/col,1)<=48omega`,
  `max_j||N_N(:,j)||_2<=t_N`,
  `||Delta_C||_rc<=96omega`, `||Delta_J||_rc<=192omega+2b_perp`, and
  `||Delta_J||_rc<=1/64`; no target-span membership, future range result, or
  `E_parallel_entry` is in this interface.  `step_007b` proves these raw
  inequalities before each chronological call (at U directly from
  `E_quotient`, then at V/W after the preceding exact range update).
...
- Constant-closed static calculation: expand the target/carry split before
  taking any norm.  With `B_N=I+C_N`, the direction-only cross expansion is
  ...
  `||Delta_J^0||_rc<=192omega+2beta_perp`.
  Set
  `S_0:=||Delta_C^0||_rc+||Delta_J^0||_rc
  <=96omega+(192omega+2beta_perp)=288omega+2beta_perp<=290eta`.
...
- Constant-closed carried calculation: let
  `h_M:=sum_(N in H_M)epsilon_N` and expose the held-state perturbation as
  `B_N=I+C_N+E_N` with
  `||E_N||_(row/col,1)<=epsilon_N` and zero perpendicular field only when it
  has already been proved by the preceding exact range update.  Expanding
  each mixed Hadamard product gives, in row and column order,
  `||Delta_C||_(rc),carried <=2q_*h_M+8rho_ALS h_M` and
  `||Delta_J||_(rc),carried <=4q_*h_M+16rho_ALS h_M`.
  Substitution into the same centered identity (before absolute values) yields
  
  `||A_M-Diag(d)||_(rc),carried <=(4q_*+16rho_ALS)h_M`,
  `d_M,carried <=(2q_*+8rho_ALS)h_M`.
  ...
  exported common bound is
  
  `max{||N_M(A_M)||_rc, max_j||G_M N_M(A_M)(:,j)||_2,
  ||s_M^+||_infty}_carried <=ell_0 h_M`,
  
  where `ell_0:=8q_*+32rho_ALS=17/512<1/8`.  This is a conditional local
...
| `E_carry_resolvent`: carried-cancellation, rank-free normalized-resolvent, and common-product transfer | `step_007c` | `E_sm`, `E_best`, `E_quotient`, and the explicitly conditional raw held-input record (`epsilon_N`, cross/pair deviations); no target-span premise | static cross/pair defects, diagonal amplitude defects, and carried off-diagonal/quadratic defects are separated; first-order normalization is canceled before both row/column norms; no `sqrt(r)` term | exact active solve and canonical direction/product representation use the same realized target and held columns; residual terms are measured in the consumed quotient interface | `S_0<=290eta`, `kappa_res<17/16`, `8kappa_res S_0+4||alpha||<=2977eta<3072eta`, and carried gain `ell_0=8q_*+32rho_ALS<1/8` | `1-q_*^2`, `1/64` resolvent margin, `Gamma<=1.01`, `rho_ALS=1/1024` | `step_007b` U/V/W subupdates | None |
| `E_pair_entry`: first-sweep collective perpendicular and pair-Gram control | `step_007b` | `E_sm`, `E_best`, `E_quotient`, `E_perp_pair`, `E_carry_resolvent`, dual row/column bounds, canonical represented-product coordinate | mixed target-span/perpendicular terms are at most `2(1+q_*)(1+48omega)^2 tau_r^2`; coherent shared-perpendicular term is exactly charged by `r tau_r^4`; no uncontrolled class | exact held Khatri-Rao pair Gram and cross Gram of the setting update; no surrogate | `beta_perp<10^(-8)omega`, normalized cross deviation `<=96omega`, pair deviation `<=192omega+2beta_perp<1/512`, inverse floor `31/32` | target pair floor `1-q_*^2` and `tau_r=q_*^2/(10^4r)` | first `U,V,W` subupdates and `E_parallel_entry` | None |
| `E_parallel_entry`: one-sweep reachable target-span entry | `step_007b` | `E_pair_entry`, `E_carry_resolvent`, exact unfolding range, three intermediate ordinary inverses, product-one quotient rebalance | coefficient, amplitude, and perpendicular pair-Gram defects controlled separately; perpendicular defects vanish mode by mode | updated factors are the exact ALS outputs `G_M Z_M`; the post-sweep quotient is the same represented rank-k tensor with frozen zeros | sequential `3072,3456,4096` envelopes; `4096(omega+beta_perp)<rho_ALS/3`; exact `N_M^+=0` | `rho_ALS=1/1024` and intermediate inverse floor `31/32` | `step_008`, `step_009` | None |
