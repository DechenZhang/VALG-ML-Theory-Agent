...
- Triggering review SHA-256: none (attempt 1 was dependency-invalidated when
  `step_005` was replaced by accepted attempt 3)
...
- Derived invariants supplied by accepted dependencies:
  - Accepted `step_004` proof/review (proof SHA-256
    `aff54178e9b82621c707d850d5b5d138ff44710610b997947c7be94a13f535b`,
    review SHA-256
  ...
  - Accepted `step_005` proof/review (proof SHA-256
    ...
    supplies the retained singleton pool `\mathcal A_{\rm sel}`, the explicit
    proof-only bijection `\pi:\mathcal A_{\rm sel}\to[r]`, the oriented
    barred representatives and residuals, the exact barred same-state
    equation (with `f_{M,a}=G_MK_M^{\rm raw}(:,a)` defined from the same
    stored state), the dual off-target column mass
...
- Source: accepted `step_004b` raw/barred equation and dual-decomposition
  lemmas, with the same-state contraction
  `f_{M,a}=G_MK_M^{\rm raw}(:,a)`, consumed through the accepted
  `E_cert_eq` and `E_gauge` outputs of current `step_005`.
- Restated statement in current notation: for a stored certified state,
  accepted `step_005` defines the retained set `\mathcal A_{\rm sel}` and
  the bijection `\pi:\mathcal A_{\rm sel}\to[r]`. Write
  `\bar p_{M,a}` and `\bar r_{M,a}` for the barred direction and barred
  residual columns that current `step_005` denotes by `\bar P_M(:,a)` and
  `\bar R_M(:,a)`, respectively. Put
  `\iota:=\pi^{-1}` and, for each target index `j`, define the reindexed
  selected objects
  \[
  \bar P_M(:,j):=\bar p_{M,\iota(j)},\qquad
  d_{M,j}:=d_M(\iota(j)),\qquad
  D_M:=\operatorname{diag}(d_{M,j}),\qquad
  \bar R_M(:,j):=\bar r_{M,\iota(j)}.
  \]
  After the accepted score-sign and product-preserving proof-only
  orientation, the current `step_005` definition of `\bar K_M` and the
  same-state identity `f_{M,a}=G_MK_M^{\rm raw}(:,a)` give exactly
...
- Object mapping: `G_M` is the realized target matrix from `setting.md`;
  `\bar p_{M,a}` and `\bar r_{M,a}` are the current `step_005` barred
  same-state objects; `\iota` applies the same column permutation in every
  mode and to `D_M,\bar R_M`; and all columns in the pair coefficient are
  therefore from the same selected stored state. No target permutation is
  applied to the rows of `G_M` or to `\operatorname{diag}(\lambda)`.
...
- Restated statement in current notation: the observable graph has exactly
  `r` components; the accepted map `\pi` is a bijection, so
  `\iota=\pi^{-1}` makes selected column `j` belong to target `j`; after
  orientation, `B_M(j,j)>0`,
...
let `\mathcal A_{\rm sel}` and `\pi` be the selected set and explicit
bijection from the current accepted `step_005`, put `\iota=\pi^{-1}`, and
form `\bar P_M,D_M,\bar R_M` by the reindexing displayed in (1), so column
`j` is paired with `\lambda_j`. For each mode let

\[
H_M=G_M^TG_M=I+F_M,\qquad
\|F_M\|_1,\|F_M\|_\infty\le q,\qquad
L_M=G_MH_M^{-1},\qquad
...
The map `\iota` applies one common column permutation to the finite selected
pool, its same-state denominators, and its residuals; it does not permute
target rows. Since `L_M^TG_M=I`, `N_M` is orthogonal to the target span and
the accepted `step_004b` dual decomposition, transported by the current
`step_005` signs and this common permutation, gives
...
`||e||_2<=||e||_1<=chi`, `|e^TH_Me|<=(1+q)chi^2`, and
...
\[
\widehat\eta
\le\frac{32}{31}\left[1.01 a(q_*+2\widehat\eta+2q_*^2) +2r\tau_r\right].
\]
The l1 Hadamard bound gives the product of the largest entry in (21) and
the row mass in (20). Since `a<1` and `(32/31)1.01<4`, this display
implies the claimed (17) after the coarse but valid replacement of the
linear factor by `4a` and the residual by `4r\tau_r`.
...
`H_M=I`, `\bar P_M=G_M`, `D_M=\operatorname{diag}(\lambda)`, and the exact
certificate equation has `\bar R_M=0`. Thus `B_M=I` and `Z_M=0`; the exact
baseline is preserved, while a finite nonstationary certificate transient
is still charged by `2r tau_r`.

## Target-Step Assembly

The current accepted `step_005` map `\pi` gives the explicit inverse
reindexing `\iota`; applying it simultaneously to directions, denominators,
and residuals turns the barred equation from `E_gauge` into exactly (1).
The identity `f_{M,a}=G_MK_M^{\rm raw}(:,a)` and the current definition of
`\bar K_M` verify that its two pair coefficients are formed from the same
selected stored state. Lemma~\ref{lem:step-006-normalized-column}
...
- `\mathcal A_{\rm sel}` and `\pi` are the accepted current-`step_005`
  selected set and proof-only target bijection. The proof-local inverse
  `\iota=\pi^{-1}` simultaneously reindexes the three barred direction
  matrices, their denominator diagonals, and their residual matrices; it
  never reindexes target rows or weights.
