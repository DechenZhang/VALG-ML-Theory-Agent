...
- Step ID: `step_005`
- Unit attempt: 2
- Repair trigger review SHA-256: `4d4c2981e33c673fa4dab2b6b82a3a258bb6e88bf2549ab9e3185dcc5083b0cf`
...
| `unit_006` | lemma | For the defined same-state contraction \(f_{M,a}=T(\mathord{\cdot}_M,h_{M',a},h_{M'',a})=G_MK_M^{\rm raw}(:,a)\), \(\langle p_{M,a}^{\rm raw},f_{M,a}\rangle=\theta_a\), so the certificate signs equal \(\operatorname{sgn}(\theta)\); absorbing that sign in the initialized third factor and applying a product-one direction gauge preserves the initialized rank-one term and yields the barred post-score equation with \(\bar\theta=|\theta|>0\). Define \(s_{\rm comp}=|\theta|/\lambda_{\pi(a)}\). | Produces `E_gauge`, positive scalar, component score ratio, and the exact sign convention for downstream equations. |
...
\(E_{\rm cert\_eq}\) equal \(\operatorname{sgn}(\theta_a)\). More
