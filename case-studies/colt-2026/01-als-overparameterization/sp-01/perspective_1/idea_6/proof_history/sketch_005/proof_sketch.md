...
This is proof-sketch attempt 5 for `perspective_1/idea_6`, run in
...
by the live attempt-3 sketch review, frozen at SHA-256
`4992ce89c1b3734ed9df158501aa571af6491a66e01bc0fa00e3bba6421831f9`.
...
and every other pre-attempt-5 proof-stage artifact, are lineage only; all step
...
for the present repair.  Attempts 3 and 4 leave that wrapper and every
...
- Triggering review:
  `perspective_1/idea_6/proof_steps/step_007c/review.md`, SHA-256
  `2edfeef18bbe3674fb3c513fac81a6afe97ce81970feff3031a239e62df22798`
...
## Attempt-5 Carried-Interface Repair

The exhausted `step_007c` attempts exposed a missing exported inequality, not
an idea-level obstruction.  The revised roadmap therefore makes the carried
producer checkable without assuming an exclusive static/carried convention.
For every held record write

`B_N=I+C_N+E_N`,
`||C_N||_{row,1},||C_N||_{col,1}<=48omega`,
`||E_N||_{row,1},||E_N||_{col,1}<=epsilon_N`, and
`h_M=sum_N epsilon_N`,

allowing both `C_N` and `E_N` to be nonzero.  A new `step_007c` unit expands
the exact Hadamard products into `C-C`, `C-E`, `E-C`, and `E-E` classes before
either row or column absolute values are taken.  It records the source and
sign of every class and derives, rather than assumes,

`u_C=(2q_*+8rho_ALS)h_M`,
`u_J=(4q_*+16rho_ALS)h_M`.

The same unit applies the centered identity
`N=(A-Diag(d))+Diag(nu^{-1}-1+d)+A(D_nu-I)` once in the original
orientation and once after transposition.  Separate local subclaims bound
the diagonal remainder, the quadratic normalization reserve, the ambient
column norm, and the common product-log increment.  Under
`h_M<=2rho_ALS/3`, `d_M<1/8`, `Delta_J<1/64`, and the ordinary inverse branch,
the displayed row and column sums must close to

`2u_C+u_J=(8q_*+32rho_ALS)h_M=17h_M/512`,

with no rank or hidden condition-number factor.  Diagonal-similarity and
weight cancellation are written as exact matrix identities, not prose
absorptions.  A separate chronology unit checks the domains before each call,
sets `h_U=0`, `h_V=epsilon_U`, and `h_W=epsilon_U+epsilon_V`, and exports
`epsilon_U<=3072eta`, `epsilon_V<=3456eta`, and
`epsilon_W<=3888eta<4096eta` as conclusions.  It then passes only the exact
range output to `step_007b`; target-span membership is never an input.
This is a target-preserving bridge repair: primitive assumptions, algorithm,
metric, rate, success criterion, and the exact orthogonal baseline remain
unchanged.
...
- Dependencies and scope: `step_001` and `step_007` only.  For a mode `M`,
  ...
  **exported decomposition**
  `B_N=I+C_N+E_N`, with
  `||C_N||_(row/col,1)<=48omega`,
  `||E_N||_(row/col,1)<=epsilon_N`, and
  `max_j||N_N(:,j)||_2<=t_N`.  Here `C_N` is the static selected-entry
  coefficient field, `E_N` is the previously produced carried field, and
  `N_N` is the exact Euclidean perpendicular field.  Let
  `mathscr H_M` be the set of carried held records used in the call and set
  `h_M:=sum_(N in mathscr H_M)epsilon_N`.  The initial branch has
  `mathscr H_M=emptyset`, `h_M=0`, and no `E_N`; a carried branch adds only
  already-produced `E_N` records.  An exact range update may set the
  corresponding `N_N` to zero, but target-span membership is never an input
  to this lemma.
  ...
  +(4q_*+16rho_ALS)h_M`, with the further usable bound
  `beta_act,M<=beta_perp` and the ordinary-branch check
  `||Delta_J||_rc<=1/64`.  The producer also exports `d_M<1/8`, positive
  input scales, and `h_M<=2rho_ALS/3`; these are discharged chronologically
  by `step_007b` below, rather than hidden as admissibility assumptions.  No
  target-span membership, future range result, or `E_parallel_entry` is in
  this interface.
  Put `q=q_*`, `omega=q^2+r tau_r`, and define the ledger producer
  
  `F_N:=N_N^T N_N`, `H_N^par:=B_N^T H_N B_N`,
  `b_perp(t',t''):= (1+q)(1+48omega)^2((t')^2+(t'')^2)
  +r(t't'')^2`.
  
  Orthogonality gives the exact pair defect
  `beta_act,M=||H_(M')^par circ F_(M'') + F_(M') circ H_(M'')^par
  + F_(M') circ F_(M'')||_rc <= b_perp(t_{M'},t_{M''})`.
  `E_quotient` supplies `t_N<=tau_r`, so the exported numerical ledger is
  `beta_perp:=b_perp(tau_r,tau_r)=2(1+q_*)(1+48omega)^2tau_r^2+r tau_r^4`.
  The actual `beta_act,M` is kept separate from this upper bound, and
  `eta:=omega+beta_perp` is a bookkeeping envelope, not a new assumption.
...
- Constant-closed static calculation: expand the target/carry split before
  ...
  `||Delta_J^0||_rc<=192omega+2beta_act,M
  <=192omega+2beta_perp`.
  Set
  `S_0:=||Delta_C^0||_rc+||Delta_J^0||_rc
  <=96omega+(192omega+2beta_act,M)
  <=288omega+2beta_perp<=290eta`.
  ...
  `omega<=1.0001q_*^2`, `beta_perp<10^(-8)omega`, and
  ...
  `8(17/16)(290eta)+4(128eta)=2977eta<3072eta`.
