...
This is proof-sketch attempt 6 for `perspective_1/idea_6`, run in
...
by the attempt-5 sketch review, frozen at SHA-256
`e67175fe9629ff5f23b3a8143a217e8f3dea1028900a35cc55e7dcff9f6a8cf9`.
...
and every other pre-attempt-6 proof-stage artifact, are lineage only; all step
...
for the present repair.  Attempts 3, 4, and 5 leave that wrapper and every
...
- Triggering review:
  ...
  `e67175fe9629ff5f23b3a8143a217e8f3dea1028900a35cc55e7dcff9f6a8cf9`
- Triggering attempt-5 sketch: SHA-256
  `a4a7aa39dcb10492d903c3ba3f1b6eb41c6a5caf7341c4941450c316003962a4`
...
## Attempt-6 Carried-Interface Repair

The attempt-5 review identified two source-boundary defects.  A norm bound on
...
For every held mode `N`, retain the mixed record

`bar P_N=G_N B_N+N_N`, `G_N^T N_N=0`,
`B_N=I+C_N+E_N`,

where both `C_N` and `E_N` may be nonzero.  The static field satisfies
`||C_N||_{row,1},||C_N||_{col,1}<=48omega`.  If `N` is carried, it also has a
named predecessor call `pred(N)` with predecessor matrix
`A_N^pred`, predecessor normalization `D_{nu,N}^pred`, and output

`E_N=(I+A_N^pred)D_{nu,N}^pred-I`.

This is an accepted-derived output certificate, not a new theorem assumption:
`step_007b` emits it immediately after the predecessor's exact range update
and passes the certificate to the next call.  The certificate includes
`epsilon_N:=max(||E_N||_{row,1},||E_N||_{col,1})`,
`max_j||N_N(:,j)||_2=t_N`, and the two orientation identities

`2(H_N A_N^pred)_{jj}+
 ((A_N^pred)^T H_N A_N^pred)_{jj}=0`,

`diag(B_N^T H_N B_N-I)=0`,

with the second identity also written after transposition (the diagonal is
unchanged, while row and column sums are checked separately).  Consequently,
if `L_N:=H_NA_N^pred-Diag(H_NA_N^pred)`, then for `O` equal to `row` or
`col`,

`||L_N||_{O,1} <= (1+q_*)epsilon_N`,
`||Diag(H_NA_N^pred)||_{O,1} <= epsilon_N^2/2`, and
`||B_N^T H_N B_N-I||_{O,1} <= 3epsilon_N`.

The predecessor identity is the raw-control-to-export path that was missing in
attempt 5.  It is the only source used to suppress a first-order carried
diagonal; `||E_N||` alone is never used for that purpose.
...
and split it as
`beta_act,M=beta_perp^0(M)+beta_perp^carry(M)`, where

`beta_perp^0(M)<=2(1+q_*)(1+48omega)^2 tau_r^2+r tau_r^4`,
`beta_perp^carry(M)
 <= 6(1+q_*)(48omega+epsilon_{M'}+epsilon_{M''})
    (t_{M'}^2+t_{M''}^2)+2r(t_{M'}t_{M''})^2`.

The second line is charged to the carried pair class in `Delta_J`; at the
actual chronology `t_{M'}=t_{M''}=tau_r` and
`epsilon_{M'}+epsilon_{M''}<=h_M`.  The exported numerical envelope is

`beta_perp(M,h_M):=beta_perp^0(M)+beta_perp^carry(M)`,
`beta_*:=max_{M in {U,V,W}} beta_perp(M,h_M)`,

and `eta_M:=omega+beta_perp(M,h_M)`.  This replaces the invalid fixed
`(1+48omega)^2` bound at V/W while preserving
`beta_*<2e-8 omega` under the explicit chronological margins below.
...
`A_M-A_M^0
 =Lambda(C_M-C_M^0)J_M^{-1}Lambda^{-1}
  -Lambda(C_M^0-J_M^0)(J_M^0)^{-1}
   (J_M-J_M^0)J_M^{-1}Lambda^{-1}`,
...
- Dependencies and scope: `step_001` and `step_007` only.  For a mode `M`,
  ...
  **mixed exported decomposition**
  ...
  `I+C_N+E_N=(I+A_N^pred)D_{nu,N}^pred`.  It also exports, in both the
  original and transposed induced orientations,
  `2diag(H_NA_N^pred)+diag((A_N^pred)^TH_NA_N^pred)=0` and
  `diag(B_N^TH_NB_N-I)=diag((B_N^TH_NB_N-I)^T)=0`.
  Subtracting the analogous static identity gives the centered carried
  relation; in either orientation `O` it yields
  `||H_NE_N-Diag(H_NE_N)||_{O,1}<=(1+q_*)epsilon_N`,
  `||Diag(H_NE_N)||_{O,1}<=epsilon_N^2/2+48omega epsilon_N`, and
  `||B_N^TH_NB_N-(I+C_N)^TH_N(I+C_N)||_{O,1}
  ...
  `beta_perp(M,h_M):=(1+q_*)[(1+48omega+epsilon_(M'))^2t_(M'')^2
  +(1+48omega+epsilon_(M''))^2t_(M')^2]+r(t_(M')t_(M''))^2`.
  Thus `beta_act,M<=beta_perp(M,h_M)`, with `h_U=0`,
  `h_V=epsilon_U`, and `h_W=epsilon_U+epsilon_V`; define
  `eta_M:=omega+beta_perp(M,h_M)`.  Carried-perpendicular terms are charged
  explicitly in `Delta_J^carry`, so no V/W term is hidden in a fixed
  `(1+48omega)^2` bound.
...
- Constant-closed static calculation: expand the target/carry split before
  ...
  `||Delta_J^0||_rc<=192omega+2beta_perp(M,0)`.
  Set
  `S_0:=||Delta_C^0||_rc+||Delta_J^0||_rc
  <=288omega+2beta_perp(M,0)<=290eta_M`.
  ...
  `omega<=1.0001q_*^2`, `beta_perp(M,0)<10^(-8)omega`, and
  ...
  `8(17/16)(290eta_M)+4(128eta_M)=2977eta_M<3072eta_M`.
