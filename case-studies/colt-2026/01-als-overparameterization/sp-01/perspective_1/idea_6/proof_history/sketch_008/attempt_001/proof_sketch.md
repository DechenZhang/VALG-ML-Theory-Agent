...
This is proof-sketch attempt 8 for `perspective_1/idea_6`, run in
`revise_sketch` mode.  It preserves the accepted attempt-2 sketch frozen at
SHA-256
`c6126ab9b81d7a7cbcae3b9b857923f3fe8e275ed1ba4376fd830270c5ec95e1`
and repairs only the carried-cancellation/normalization interface identified
by the attempt-7 sketch review, frozen at SHA-256
`9e4aa409877f792fe143293af1e88d3ebf4ae9ede6bca06aadf37748927a2804`.
The attempt-1 sketch and its runtime repair remain lineage only; they are not
current evidence for this attempt.  The attempt-2 step proofs and reviews,
and every other pre-attempt-8 proof-stage artifact, are lineage only; all step
outputs must be regenerated and freshly reviewed after this sketch.
...
- Triggering review:
  `perspective_1/idea_6/proof_sketch_review.md`, SHA-256
  `9e4aa409877f792fe143293af1e88d3ebf4ae9ede6bca06aadf37748927a2804`
- Triggering attempt-7 sketch: SHA-256
  `9663efc957e3895be7a92831077b36481eea97542513abf25f5648ffe2597d1b`
- Triggering attempt-3 sketch: SHA-256
  `21150504d753137ae38a019ac14a8b17bd3238d9239cc1086eea0c737f033bde`
- Accepted attempt-2 sketch: SHA-256
  `c6126ab9b81d7a7cbcae3b9b857923f3fe8e275ed1ba4376fd830270c5ec95e1`
- Accepted attempt-2 sketch review: SHA-256
  `524bc3e32fc6d334d4c6c78bf8e347888df2cd4bbbd21bf7c3dc1414c0c9b3a4`
- Attempt-1 lineage sketch (not consumed): SHA-256
...
- Attempt-4 local repair summary: retain the centered cancellation and all
  ...
  `||E_N||_(row/col,1)<=epsilon_N`,
  `max_j||N_N(:,j)||_2<=t_N`, and
  `h_M=sum_(N in H_M)epsilon_N`.  Export the total bounds
  `Delta_C<=96omega+(2q_*+8rho_ALS)h_M` and
  `Delta_J<=192omega+2beta_act,M+(4q_*+16rho_ALS)h_M`, together with
  `Delta_J<=1/64`, `d_M<1/8`, positive input scales, the ordinary inverse
  branch, and `h_M<=2rho_ALS/3`.  The first-sweep consumer now instantiates
  `h_U=0`, `h_V=epsilon_U`, and `h_W=epsilon_U+epsilon_V`, with
  `epsilon_U<=3072eta`, `epsilon_V<=3456eta`, and
  `epsilon_W<=3888eta<4096eta`; explicit inequalities below discharge every
  V/W margin.  Incoming scales are handled by exact product cancellation and
...
## Attempt-8 Carried-Interface Repair
...
Both `C_N` and `E_N` may be nonzero; `epsilon_N` always means
`max(||E_N||_{row,1},||E_N||_{col,1})`, and
...
`step_007b` immediately after the predecessor's exact solve and is passed to
the next call; it is not a primitive assumption.
...
`||Diag(H_NE_N)||_{O,1}<=epsilon_N^2/2+48omega epsilon_N`, and
`||B_N^T H_N B_N-(B_N^0)^T H_NB_N^0||_{O,1}
 <=(2q_*+8rho_ALS)epsilon_N`.
...
`h_M=sum_{N in H_M}epsilon_N`, where `H_M` contains exactly the already
...
`||J_M-J_M^0||_{O,1}
 <= [4q_*+16rho_ALS]h_M+beta_perp(M,h_M)`.
...
`sum_{ell}|Diag(H_NE_N)\circ Q^0_{N'}|_{iell}
 <=rho_ALS epsilon_N`,
`sum_{ell}|L_N\circ C_{N'}|_{iell}<=2rho_ALS epsilon_N`,
`sum_{ell}|L_N\circ L_{N'}|_{iell}<=rho_ALS epsilon_N`,
...
inequalities use only `epsilon_N<=rho_ALS/3`, the two post-normalization
...
`beta_perp(M,h_M):=(1+q_*)[(1+48omega+h_M)^2t_{M''}^2
 +(1+48omega+h_M)^2t_{M'}^2]+r(t_{M'}t_{M''})^2`.

For bookkeeping only, write
`beta_perp^0(M):=beta_perp(M,0)` and
`beta_perp^carry(M,h_M):=beta_perp(M,h_M)-beta_perp^0(M)`; all producer,
consumer, flow, and gate rows use the unified `beta_perp(M,h_M)`, never one
of these components alone.  The actual matrix satisfies
`beta_act,M<=beta_perp(M,h_M)`.  The chronological envelope is
`beta_FS:=max_M beta_perp(M,h_M)`, `eta_FS:=omega+beta_FS`, with
`h_U=0`, `h_V=epsilon_U`, `h_W=epsilon_U+epsilon_V`.  The explicit domain
checks below give `beta_FS<2e-8omega` and hence `eta_FS<1.001q_*^2`.
...
`||Lambda(C-C^0)J^{-1}Lambda^{-1}||_{O,1}
 <= u_C+u_J/2`,
`||Lambda(C^0-J^0)(J^0)^{-1}(J-J^0)J^{-1}Lambda^{-1}||_{O,1}
 <= u_C+u_J/2`,

for `O=row` and `O=col`, with no `kappa_res` multiplier on the carried
sum.  The same paired calculation gives
...
`N_M(A_M)-N_M(A_M^0)
 =[A_M-A_M^0-Diag(Delta d_M)]
  ...
  The predecessor relation and `d_M^0<=720eta_FS` give, with
`x_M:=u_C+u_J/2=(4q_*+16rho_ALS)h_M`,
...
`4d_M^0x_M` and `2d_M^0x_M` terms; they are not dropped.  Since
  `h_M<=2rho_ALS/3`, `x_M<=68q_*h_M`, `eta_FS<=1.001q_*^2`, and
`q_*=1/4096`, the explicit substitution is

`2x_M^2+q_*x_M+6d_M^0x_M
 <=70q_*^2h_M+4320(1.001q_*^2)^2h_M
 <=(4rho_ALS)q_*h_M`,

which is within the `8rho_ALS h_M` cross reserve.  Therefore, in both
orientations,

`||N_M(A_M)-N_M(A_M^0)||_{row,1},
 ||N_M(A_M)-N_M(A_M^0)||_{col,1}
  <=2(2u_C+u_J)=(16q_*+64rho_ALS)h_M`.
...
The static comparator contributes
  `8kappa_res(288omega+2beta_FS)+4||alpha||_infty
  <=2977eta_FS<3072eta_FS`, while the carried increment is bounded by

  `ell_c h_M`, `ell_c:=16q_*+64rho_ALS=17/256<1/8`.

Thus the exported one-call interface is
  `epsilon_M^+<=3072eta_FS+ell_ch_M` in both induced orientations, for the
ambient direction columns, and for the one common product-log register.

The chronology unit checks domains separately.  With
  `eta_FS = omega+beta_FS`, `h_U=0`, it proves
  `Delta_J^U<=192eta_FS<1/32768`,
`d_U<1/16384`, `nu_U^2>1/2`, and
`epsilon_U<=3072eta_FS`.  The exact U unfolding update then emits the
predecessor certificate `pred(U)` and sets `t_U^+=0`.  At V it sets
`h_V=epsilon_U`, uses the full `beta_perp(V,h_V)`, and checks explicitly
`Delta_J^V<1/32768`, `d_V<1/16384`, `nu_V^2>1/2`, followed by
  `epsilon_V<=3072eta_FS+ell_c epsilon_U<=3276eta_FS`.  Only after the V range
update emits `pred(V)` does W run; with `h_W=epsilon_U+epsilon_V`, it checks
`Delta_J^W<1/32768`, `d_W<1/16384`, `nu_W^2>1/2`, and
  `epsilon_W<=3072eta_FS+ell_c h_W<=3500eta_FS<4096eta_FS`.  Numerically,
  `h_V<1/4096`, `h_W<1/2048<2rho_ALS/3`, and every incoming scale is
`lambda_j^(1/3)exp(s_j/3)>0`; `nu_M>0` supplies the next positive scale.
No target-span membership is consumed before the corresponding exact range
update, and no initial `|alpha|` bound is reused for V/W.

The actual orthogonal equal-weight stationary boundary has
`C=C^0=J=J^0=I`, `F_N=0`, `A=A^0=0`, `nu=nu^0=1`, and `s^+=0`.  The
  positive `beta_FS` envelope is only a finite-transient upper bound; the
...
3. **Selected-state bridge and local refinement.**
   ...
   - Fixed quantities: `q_*`, the target chart thresholds, gauge-invariant
     represented-product coordinates, the chronological envelope `beta_FS`
     (with local `beta_perp(M,h_M)` records), and
...
- `E_perp_pair` (the explicit `F_M=N_M^TN_M`, actual-record
  `beta_perp(M,h_M)`, and global `beta_FS` ledger) is derived by `step_007c`
  directly from `E_quotient`; `step_007b` instantiates it with the entry
  budget `t_M<=tau_r` and propagates the actual contamination separately.
...
- Dependencies and scope: `step_001` and `step_007` only.  For a mode `M`,
  ...
  **mixed exported decomposition**
  ...
  `||E_N||_(row/col,1)<=epsilon_N`, and
  `max_j||N_N(:,j)||_2<=t_N`.  Both `C_N` and `E_N` may be nonzero.  If N is
  carried, the predecessor output certificate names its predecessor matrix
  `A_N^pred` and normalization `D_{nu,N}^pred` and exports the exact identity
  ...
  The named predecessor comparator
  `B_N^0=(I+A_N^{0,pred})D_{nu,N}^{0,pred}` (equivalently `I+C_N`)
  obeys the same identity with the same `F_N`; subtracting these two valid identities gives
  ...
  held records used in the call and set `h_M:=sum_(N in mathscr H_M)epsilon_N`.
  ...
  `||Delta_C||_(row/col,1)<=96omega+(2q_*+8rho_ALS)h_M` and
  `||Delta_J||_(row/col,1)<=192omega+2beta_perp(M,h_M)
  +(4q_*+16rho_ALS)h_M`, with the ordinary-branch check
  `||Delta_J||_(row/col,1)<=1/64`.  The producer also exports `d_M<1/8`,
  positive input scales, and `h_M<=2rho_ALS/3`; these are discharged
  ...
  `beta_perp(M,h_M):=(1+q_*)[(1+48omega+h_M)^2t_(M'')^2
  +(1+48omega+h_M)^2t_(M')^2]+r(t_(M')t_(M''))^2`.
  Thus `beta_act,M<=beta_perp(M,h_M)`, with `h_U=0`,
  `h_V=epsilon_U`, and `h_W=epsilon_U+epsilon_V`; define the single global
  envelope `beta_FS:=max_M beta_perp(M,h_M)` and `eta_FS:=omega+beta_FS`.
  Carried-perpendicular terms are included in this same `Delta_J` ledger; no
  V/W term is hidden in a fixed static formula.
...
- Constant-closed static calculation: expand the target/carry split before
  ...
  `||Delta_J^0||_rc<=192omega+2beta_FS`.
  Set
  `S_0:=||Delta_C^0||_rc+||Delta_J^0||_rc
  <=288omega+2beta_FS<=290eta_FS`.
  ...
  `omega<=1.0001q_*^2`, `beta_FS<2*10^(-8)omega`, and
  `kappa_res<17/16`, gives a reserve no larger than `eta_FS` and hence
  ...
  `8(17/16)(290eta_FS)+4(128eta_FS)=2977eta_FS<3072eta_FS`.
...
- Constant-closed carried calculation: with the exported decomposition above,
  ...
  `||Delta_C^carry||_rc<=2q_*h_M+8rho_ALS h_M` and
  `||Delta_J^carry||_rc<=4q_*h_M+16rho_ALS h_M`.  Therefore the **total**
  producer interface is
  `||Delta_C||_rc<=96omega+(2q_*+8rho_ALS)h_M` and
  `||Delta_J||_rc<=192omega+2beta_perp(M,h_M)
  +(4q_*+16rho_ALS)h_M`.
  Substitution into the same centered identity (before absolute values) yields
  `||A_M-Diag(d)||_(rc),carry<=(4q_*+16rho_ALS)h_M` and
  `d_M,carry<=(2q_*+8rho_ALS)h_M`; together with the static bound this gives
  `d_M<=2kappa_res S_0+||alpha||_infty
  +(2q_*+8rho_ALS)h_M`.

  The quadratic normalization reserve is bounded explicitly by
  `(2q_*+8rho_ALS)h_M/2` whenever the already-produced input satisfies
  `h_M<=2rho_ALS/3`; the reserve is not hidden in an admissibility label.
  ...
  ||s_M^+||_infty}_carry <=ell_c h_M`,
  where `ell_c:=16q_*+64rho_ALS=17/256<1/8`.  The full output envelope is
  consequently `epsilon_M^+<=3072eta_FS+ell_c h_M` in every induced direction
  norm and in the common product register.  This is a conditional local
  implication whose inputs are discharged by `step_007b`; it does not assume
  that the held columns already lie in `range(G_N)`.

  For completeness, the per-call numerical discharge is part of this
  producer interface.  Since `omega<=1.0001q_*^2` and
  `beta_FS<2*10^(-8)omega`, `eta_FS=omega+beta_FS<1.001q_*^2`.  The
  chronology supplies `h_U=0`, `h_V=epsilon_U<=3072eta_FS<1/4096`, and
  `h_W=epsilon_U+epsilon_V<=6548eta_FS<1/2048`; because
  `2rho_ALS/3=1/1536`, every call has `h_M<=2rho_ALS/3`.  Also
  `4q_*+16rho_ALS=68q_*` and `2q_*+8rho_ALS=34q_*`, so at the largest
  carried budget
  `d_M< (2(17/16)(290)+128)1.001q_*^2+34q_*/2048
  <1/16384<1/8`, while
  `Delta_J<192(1.001q_*^2)+2(2*10^(-8)omega)+68q_*/2048
  <1/32768<1/64`.  Thus the Neumann ordinary-inverse branch is active at
  U, V, and W, not merely at the initial call.
  ...
  from `d_M<1/8`, which gives `nu_j^2>0`; the identity
  `(lambda_j^(1/3)e^(s_j/3))^2
  (lambda_j^(1/3)e^(-2s_j/3)nu_j)=lambda_j nu_j`
  cancels the historical `s` before the next held solve.  The envelope
  `|s_j|<=4096eta_FS<1/2048` (initially `|s_j|<=512omega`) also gives a finite
  positive ratio, but no carried `|alpha|<=128omega` assertion is reused.
- Exported interfaces `E_perp_pair` and `E_carry_resolvent`: the former
  contains `F_N`, `beta_act,M`, and the numerical
  `beta_perp(M,h_M)` ledger produced directly from the actual held records;
  the latter states that every raw held-input record satisfying the displayed
  conditional bounds obeys
  `epsilon_M^+<=3072eta_FS+ell_c sum_(N in H_M)epsilon_N`, with the same bound
  for direction row/column masses, ambient direction columns, and the one
  common product log register.  The source path is
  `E_sm,E_best,E_quotient -> (F_N,beta_perp(M,h_M),Delta_C,Delta_J) -> exact
  resolvent and centered cancellation -> E_carry_resolvent`; no
...
- Boundary and baseline stress: at an exact orthogonal equal-weight
  ...
  `beta_FS` remains a positive bookkeeping upper bound
...
- Accumulation and noncircular closure: this is a one-block conditional
  interface.  `step_007b` first derives `E_perp_pair` from the raw selected
  columns, verifies the U pair/cross bounds, and applies the lemma with
  `H_M=emptyset`, obtaining `epsilon_U<=3072eta_FS<rho_ALS/3`.  The exact
  unfolding range then proves `N_U^+=0`; only after that generated fact does
  `step_007b` construct the V held-input record, and similarly for W.
  Therefore
  `epsilon_U<=3072eta_FS`, `epsilon_V<=3072eta_FS+ell_c epsilon_U<=3276eta_FS`, and
  `epsilon_W<=3072eta_FS+ell_c(epsilon_U+epsilon_V)<=3500eta_FS<4096eta_FS`.
  The producer is local and conditional; the consumer path is strictly
  `E_quotient -> E_perp_pair,E_carry_resolvent -> step_007b ->
...
- Collective perpendicular field: consume the `E_perp_pair` ledger from
  ...
  `||H_M^par||_rc<=(1+q_*)(1+48omega+h_M)^2`,
  ...
  It dominates the actual contamination and is evaluated chronologically with
  `h_U=0`, `h_V=epsilon_U`, and `h_W=epsilon_U+epsilon_V`; set
  `beta_FS:=max_M beta_perp(M,h_M)`.  At the selected entry
  `t_M<=tau_r`, while after each exact range update the updated mode has
  `t_M=0`; the explicit domain arithmetic gives
  `beta_FS<2*10^(-8)omega`.  Thus shared perpendicular components, including
...
- Khatri-Rao pair-Gram and cross-Gram adequacy: after writing
  ...
  `||Delta_pair||_rc<=192omega+2beta_FS<1/512` at the first update, and the
...
- Normalized transfer consumption: `step_007c` is the sole producer of the
  ...
  static envelope `3072eta_FS`; after its exact range identity proves
  `N_U^+=0`, the V call receives a newly generated held-input record, and W
  receives the record generated after V.  The already-carried contribution is
  `ell_c sum epsilon_N` with `ell_c=16q_*+64rho_ALS=17/256<1/8`; the common product
  register is transferred by the exact scale-cancellation identity in
  `step_007c`.  After each exact predecessor solve and its range update,
  `step_007b` emits the named actual/comparator predecessor certificate and
  passes it to the next chronological call.  This keeps the source and consumer separate and prevents a
  second normalization charge from being silently added here.
- First cyclic update: use the exact unfolding
  ...
  update.  The `U` update consumes the full `beta_FS` bound and proves
  ...
  chronological order, followed by column normalization and one
  product-one equal-norm rebalance after the complete sweep, gives the
  explicit sequential envelopes `3072eta_FS`,
  `3276eta_FS`, and `3500eta_FS<4096eta_FS` for the updated mode
  fields.  In particular,
  `E_parallel_entry` exports
  `N_U^+=N_V^+=N_W^+=0` and
  `norm(e^1)_Q<=4096eta_FS<rho_ALS/3`.
...
- Accumulation behavior / scope compatibility: one finite cyclic sweep.  The
  ...
  Gram defects are bounded absolutely by the global `beta_FS` envelope and disappear mode by
...
- Noncircular closure status: `step_007` produces the raw entry controls;
  `step_007c` derives `E_perp_pair` and its parameterized normalized
  held-input transfer without consuming a later span or basin fact;
  `step_007b` discharges the conditional pair/cross inequalities before each
  call, proves the exact range after that call, and only then constructs the
  next held record.  `step_008` consumes the completed `E_parallel_entry`.
...
- Baseline conclusion preserved: at the exact orthogonal equal-weight
  ...
  first-sweep error is exactly zero.  The numerical upper bound `beta_FS`
...
- Producer-consumer provenance:
  `step_001, step_007 -> step_007c(E_perp_pair,E_carry_resolvent) ->
...
| `E_perp_pair`: actual-record perpendicular pair ledger | `step_007c` | `E_quotient`, produced comparator certificate, actual `B_N,N_N,F_N`, `t_N`, `epsilon_N`, realized Gram floor | static/perpendicular and carried/perpendicular terms are split without a rank-free max-column shortcut | exact same realized held factors and pair Gram; `F_N=N_N^TN_N` is the actual residual Gram | actual contamination `beta_act,M` is bounded by the actual-record `beta_perp(M,h_M)`; the chronological global envelope is `beta_FS:=max_M beta_perp(M,h_M)<2*10^(-8)omega` | `tau_r`, `q_*`, `48omega`, `epsilon_N`, `h_M`, produced `F_N` | chronological U/V/W ledger in `step_007b` | None |
| `E_carry_resolvent`: carried-cancellation, rank-free normalized-resolvent, and common-product transfer | `step_007c` | `E_sm`, `E_best`, `E_quotient`, produced `B_N,B_N^0,E_N,F_N` certificates, `h_M`, and actual-record `beta_perp(M,h_M)`; no target-span premise | all six cross/pair classes are split; the exact two-term resolvent difference is summed in row and column orientations; centered post-normalization controls the quadratic remainder | exact active solve and canonical direction/product representation use the same realized target and held columns | `eta_FS:=omega+beta_FS`; output `epsilon_M^+<=3072eta_FS+ell_c h_M`, `ell_c=17/256<1/8`, in both induced norms, ambient columns, and common log register | `S_0<=290eta_FS`, `kappa_res<17/16`, full similarity/inverse factors charged, `h_M<=2rho_ALS/3`, ordinary inverse and positivity domains | chronological U/V/W calls in `step_007b` | None |
| `E_pair_entry`: first-sweep collective perpendicular and pair-Gram control | `step_007b` | `E_sm`, `E_best`, `E_quotient`, `E_perp_pair`, `E_carry_resolvent`, dual row/column bounds, and canonical represented-product coordinate | actual-record mixed target-span/perpendicular terms use `beta_perp(M,h_M)` and the coherent `r(t_{M'}t_{M''})^2` charge; carried classes are bounded through the producer's `h_M` envelope and no class is uncontrolled | exact held Khatri-Rao pair Gram and cross Gram of the setting update; no surrogate; the ordinary inverse is legal at every call | `beta_FS<2*10^(-8)omega`; total cross/pair deviations use `96/192` plus `h_M` terms; `Delta_J<1/32768<1/64`; inverse floor `31/32`; `epsilon_U<=3072eta_FS`, `epsilon_V<=3276eta_FS`, `epsilon_W<=3500eta_FS<4096eta_FS`; `h_U=0,h_V=epsilon_U,h_W=epsilon_U+epsilon_V` | target pair floor `1-q_*^2`, `tau_r=q_*^2/(10^4r)`, and `h_W<2rho_ALS/3` | first `U,V,W` subupdates and `E_parallel_entry` | None |
| `E_parallel_entry`: one-sweep reachable target-span entry | `step_007b` | `E_pair_entry`, `E_carry_resolvent`, exact unfolding range, three intermediate ordinary inverses, positive scale bridge, and product-one quotient rebalance | coefficient, amplitude, carried, and actual-record perpendicular defects are controlled by the single `eta_FS` envelope and vanish mode by mode | updated factors are the exact ALS outputs `G_M Z_M`; incoming product scales cancel exactly | `epsilon_U<=3072eta_FS`, `epsilon_V<=3276eta_FS`, `epsilon_W<=3500eta_FS<4096eta_FS`; `4096eta_FS<rho_ALS/3`; exact `N_M^+=0` | `rho_ALS=1/1024`, `h_M<=2rho_ALS/3`, `d_M<1/8`, positive scales, inverse floor `31/32` | `step_008`, `step_009` | None |
...
| `E_perp_pair` | `step_007c` | `step_007b` raw U/V/W ledger | exposes the actual `F_M=N_M^TN_M`, actual-record `beta_perp(M,h_M)`, and global `beta_FS` before any range claim | `E_quotient` -> orthogonal projector identity -> `F_M` and actual-record `beta_perp(M,h_M)` -> `E_perp_pair`; no future ALS state is used | derived | None |
| `E_carry_resolvent` | `step_007c` | `step_007b` chronological U/V/W subupdates | closes the normalized first-sweep constants without a rank or hidden-condition-number loss; exports `eta_FS:=omega+beta_FS`, `epsilon_M^+<=3072eta_FS+ell_c h_M`, and `ell_c=17/256<1/8` | `E_sm,E_quotient` -> conditional raw cross/pair interface -> exact resolvent -> centered column normalization in both orientations -> rank-free row/column and product-log bounds -> `E_carry_resolvent`; each exact predecessor solve emits the named comparator certificate consumed by the next call | explicitly conditional | None |
| `E_pair_entry` | `step_007b` | the three first-sweep subupdates and `E_parallel_entry` | controls shared perpendicular components and every first-sweep Khatri-Rao inverse using actual-record `beta_perp(M,h_M)` and global `beta_FS` | `E_sm,E_quotient,E_perp_pair` -> actual-record cross/pair-Gram deviations -> `E_carry_resolvent` invocation -> intermediate inverse floors | derived | None |
| `E_parallel_entry` | `step_007b` | `step_008`, `step_009` | reaches the exact target-span contraction domain after one algorithmic sweep | `E_pair_entry,E_carry_resolvent` -> exact `U,V,W` unfolding range trace -> `N_M^+=0` -> `norm(e^1)_Q<rho_ALS/3` -> consumers | derived | None |
...
| `step_007c` | Before the first-sweep assembly, derive `E_perp_pair` from the accepted `E_quotient` decomposition, then state the source-adequate parameterized carried-cancellation interface: exact amplitude-conjugated solve, centered column-normalization identity, both induced row/column diagonal-similarity bounds, Neumann resolvent factor `kappa_res<17/16`, static bound `8kappa_res(288omega+2beta_FS)+4||alpha||<=2977eta_FS<3072eta_FS`, and carried gain `ell_c=16q_*+64rho_ALS=17/256<1/8`.  State actual orthogonal baseline equality (`beta_act=0`, `A=0`, `nu=1`, `s=0`) while retaining positive `beta_FS` bookkeeping.  The raw held-input conditions are conditional records that `step_007b` proves before each call; target-span membership is not an input. | `step_001`, `step_007` | `E_sm`, `E_best`, `E_quotient` (derived); explicitly conditional raw held-input record (`epsilon_N`, cross/pair deviations, perpendicular budgets) | normalization first-order cancellation, induced-norm resolvent transfer without rank or hidden condition number, common-register scale cancellation, legal `F_M`/actual-record `beta_perp(M,h_M)` producer, and baseline object identity | exact inverse identity, centered normalization decomposition, diagonal similarity, row/column Hadamard bounds, actual-record `beta_perp(M,h_M)` ledger, and direct current-notation calculation | `E_perp_pair`, `E_carry_resolvent` | Constant-explicit one-block transfer; `S_0<=290eta_FS`, `2977eta_FS<3072eta_FS`, `ell_c<1/8`, no `sqrt(r)` charge | PENDING |
| `step_007b` | Consume `E_perp_pair` and `E_carry_resolvent` together with the accepted selected entry decomposition `bar P_M=G_M B_M+N_M`; verify normalized cross/pair-Gram deviations and a `31/32` inverse floor at all three first-sweep subupdates, prove exact zero-padding equivalence and range containment in chronological U/V/W order, and show the completed first cyclic sweep has every `N_M^+=0` and `norm(e^1)_Q<=4096eta_FS<rho_ALS/3`.  No target-span/range fact is assumed before its own exact unfolding update. | `step_001`, `step_007`, `step_007c` | `E_sm`, `E_best`, `E_quotient`, `E_perp_pair`, `E_carry_resolvent` (derived), `assump:base-scale` (primitive procedure scale) | coherent shared perpendicular components, amplitude-conjugated Khatri-Rao pair Grams, chronological held-state transfer, and exact range entry | orthogonal projector identity, induced row/column Hadamard bounds, actual-record `E_perp_pair`, `E_carry_resolvent`, exact unfolding range, Moore--Penrose block identity, product-one rebalance | `E_pair_entry`, `E_parallel_entry`, active rank-r/zero-padding equivalence | Entry-sweep structural objective; explicit `tau_r^2`, `r tau_r^4`, `96/192/3072/3276/3500/4096` constants, inverse floor, and one-sweep cost | PENDING |
...
The accepted `step_007` decomposition is consumed first by the conditional
`step_007c` interface.  That row itself produces `E_perp_pair` from
`P_M^T P_M=B_M^T H_M B_M+N_M^T N_M` and exposes the exact mixed `tau_r^2`
and coherent `r tau_r^4` charges; it does not assume a future range result.
`step_007b` then discharges the raw cross/pair bounds before each invocation,
and the first cyclic range trace removes `N_U`, then `N_V`, then `N_W`.
Only the resulting `E_parallel_entry` is consumed by the target-span
