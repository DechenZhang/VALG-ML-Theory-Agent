...
This is proof-sketch attempt 7 for `perspective_1/idea_6`, run in
...
by the attempt-6 sketch review, frozen at SHA-256
...
and every other pre-attempt-7 proof-stage artifact, are lineage only; all step
...
for the present repair.  Attempts 3, 4, 5, and 6 leave that wrapper and every
...
- Triggering review:
  ...
  `75c2c40b9fd68ccab6e77224ca7f3931479310b17a2e18fff74bfdf1744b2f22`
- Triggering attempt-6 sketch: SHA-256
  `9d1ab5197a301564f2b58b41d155ddc12b4f467dba811d46aabfd66e0c126d97`
...
## Attempt-7 Carried-Interface Repair

The attempt-6 review identified two source-boundary defects.  A norm bound on
...
`B_N=I+C_N+E_N`, and `E_N:=B_N-B_N^0` with `B_N^0:=I+C_N`.
...
names the predecessor matrix `A_N^pred` and normalization
`D_{nu,N}^pred`, and defines the *actual post-normalization* output
`B_N=(I+A_N^pred)D_{nu,N}^pred`.  This certificate is emitted by
`step_007b` immediately after the predecessor's exact solve and is passed to
the next call; it is not a primitive assumption.

The exact unit-column identity is, in both original and transposed induced
orientations,
`diag(B_N^T H_N B_N+F_N)=1`, `F_N:=N_N^TN_N`, or equivalently
`diag(B_N^T H_N B_N-I)=-diag(F_N)`.  Only after the exact range update may
the producer set `F_N=0`.  The separately normalized comparator `B_N^0`
satisfies the same identity with the same `F_N`.  Subtracting these valid
...
The mixed expansion is split into six source classes before absolute values:
static-static, static-carried, carried-static, carried-carried, static-
perpendicular, and carried-perpendicular.  For each `O` in `{row,col}`, the
termwise ledger to be proved by the local unit is
...
where the superscript `span` excludes perpendicular terms, and the complete
pair difference is

`||J_M-J_M^0||_{O,1}
 <= [4q_*+16rho_ALS]h_M+beta_perp^carry(M)`.

To make the constants auditable, the row sums and their transposed column
counterparts are recorded separately.  For `i` fixed and then `j` fixed,

`sum_{j != i}|(L_{M'}\circ H_{M''})_{ij}|
 <= (q_*+2rho_ALS)epsilon_{M'}`,
`sum_i_{i != j}|(L_{M'}\circ H_{M''})_{ij}|
 <= (q_*+2rho_ALS)epsilon_{M'}`,

and the corresponding `M''` terms have the same two bounds.  The diagonal
and mixed quadratic terms obey, in both displayed orientations,

`sum_j |Diag(H A^pred)\circ H'| <= rho_ALS epsilon_N`,
`sum_j |(H A^pred)\circ C'| <= 2rho_ALS epsilon_N`,
`sum_j |(H A^pred)\circ(H' A'^pred)| <= rho_ALS epsilon_N`,

with the identical inequalities under `sum_i` for a fixed column.  Adding
...
inequalities use only `epsilon_N<=rho_ALS/3`, `d_N^pred<1/8`,
...
`beta_*:=max_M beta_perp(M,h_M)`, `eta_*:=omega+beta_*`, with
`h_U=0`, `h_V=epsilon_U`, `h_W=epsilon_U+epsilon_V`.  The explicit domain
checks below give `beta_*<2e-8omega` and hence `eta_*<1.001q_*^2`.
...
The predecessor relation and `d_M^0<=720eta_M` give, with
...
`h_M<=2rho_ALS/3`, `x_M<=68q_*h_M`, `eta_M<=1.001q_*^2`, and
...
`||N_M(A_M)-N_M(A_M^0)||_{row,1},
 ...
 <=2u_C+u_J=(8q_*+32rho_ALS)h_M`.

The same centered calculation gives
`max_j||G_M[N_M(A_M)-N_M(A_M^0)](:,j)||_2<=2u_C+u_J` and
...
`8kappa_res(288omega+2beta_perp^0)+4||alpha||_infty
 <=2977eta_M<3072eta_M`, while the carried increment is exactly

`ell_0 h_M`, `ell_0:=8q_*+32rho_ALS=17/512<1/8`.

Thus the exported one-call interface is
`epsilon_M^+<=3072eta_M+ell_0h_M` in both induced orientations, for the
ambient direction columns, and for the one common product-log register.

The chronology unit checks domains separately.  With
`eta_U=omega+beta_perp(U,0)`, `h_U=0`, it proves
`Delta_J^U<=192eta_U+2beta_perp^carry(U,0)<1/32768`,
`d_U<1/16384`, `nu_U^2>1/2`, and
`epsilon_U<=3072eta_U`.  The exact U unfolding update then emits the
...
`epsilon_V<=3072eta_V+ell_0epsilon_U<=3456eta_V`.  Only after the V range
update emits `pred(V)` does W run; with `h_W=epsilon_U+epsilon_V`, it checks
`Delta_J^W<1/32768`, `d_W<1/16384`, `nu_W^2>1/2`, and
`epsilon_W<=3072eta_W+ell_0h_W<=3888eta_W<4096eta_W`.  Numerically,
...
positive `beta_perp^0` envelope is only a finite-transient upper bound; the
...
- `E_perp_pair` (the explicit `F_M=N_M^TN_M` and
  `beta_perp(t_{M'},t_{M''})` ledger) is derived by `step_007c` directly from
  `E_quotient`; `step_007b` instantiates it with the entry budget
  `t_M<=tau_r` and propagates the actual contamination separately.
...
- Dependencies and scope: `step_001` and `step_007` only.  For a mode `M`,
  ...
  **mixed exported decomposition**
  ...
  The separately normalized comparator `B_N^0=I+C_N` obeys the same identity
  with the same `F_N`; subtracting these two valid identities gives
  ...
  `||Delta_J||_(row/col,1)<=192omega+2beta_act,M
  ...
  envelope `beta_*:=max_M beta_perp(M,h_M)` and `eta_*:=omega+beta_*`.
...
- Constant-closed static calculation: expand the target/carry split before
  ...
  `||Delta_J^0||_rc<=192omega+2beta_*`.
  Set
  `S_0:=||Delta_C^0||_rc+||Delta_J^0||_rc
  <=288omega+2beta_*<=290eta_*`.
  ...
  `omega<=1.0001q_*^2`, `beta_*<2*10^(-8)omega`, and
  `kappa_res<17/16`, gives a reserve no larger than `eta` and hence
  ...
  `8(17/16)(290eta_*)+4(128eta_*)=2977eta_*<3072eta_*`.
...
- Constant-closed carried calculation: with the exported decomposition above,
  ...
  `||Delta_J||_rc<=192omega+2beta_act,M
  +(4q_*+16rho_ALS)h_M`.
  ...
  ||s_M^+||_infty}_carry <=ell_0 h_M`,
  where `ell_0:=8q_*+32rho_ALS=17/512<1/8`.  The full output envelope is
  consequently `epsilon_M^+<=3072eta+ell_0 h_M` in every induced direction
  ...
  `beta_perp<10^(-8)omega`, `eta=omega+beta_perp<1.001q_*^2`.  The
  chronology supplies `h_U=0`, `h_V=epsilon_U<=3072eta<1/4096`, and
  `h_W=epsilon_U+epsilon_V<=6528eta<1/2048`; because
  ...
  <1/16384<1/8`, while
  `Delta_J<192(1.001q_*^2)+2(10^(-8)omega)+68q_*/2048
  ...
  `|s_j|<=4096eta<1/2048` (initially `|s_j|<=512omega`) also gives a finite
...
- Exported interfaces `E_perp_pair` and `E_carry_resolvent`: the former
  ...
  `beta_perp=b_perp(tau_r,tau_r)` ledger produced directly from
  `E_quotient`; the latter states that every raw held-input record satisfying
  the displayed conditional bounds obeys
  `epsilon_M^+<=3072eta+ell_0 sum_(N in H_M)epsilon_N`, with the same bound
  for direction row/column masses, ambient direction columns, and the one
  common product log register.  The source path is
  `E_sm,E_best,E_quotient -> (F_N,beta_perp,Delta_C,Delta_J) -> exact
...
- Boundary and baseline stress: at an exact orthogonal equal-weight
  ...
  `beta_perp=b_perp(tau_r,tau_r)` remains a positive bookkeeping upper bound
...
- Accumulation and noncircular closure: this is a one-block conditional
  ...
  `H_M=emptyset`, obtaining `epsilon_U<=3072eta<rho_ALS/3`.  The exact
  ...
  Therefore
  `epsilon_U<=3072eta`, `epsilon_V<=3072eta+epsilon_U/8<=3456eta`, and
  `epsilon_W<=3072eta+(epsilon_U+epsilon_V)/8<=3888eta<4096eta`.
...
- Collective perpendicular field: consume the `E_perp_pair` ledger from
  ...
  `b_perp(t_{M'},t_{M''})`.  At the selected entry
  `t_M<=tau_r`, this is
  `beta_perp:=2(1+q_*)(1+48omega)^2tau_r^2+r tau_r^4`.
...
- Normalized transfer consumption: `step_007c` is the sole producer of the
  ...
  static envelope `3072eta`; after its exact range identity proves
  ...
  `ell_0 sum epsilon_N` with `ell_0=8q_*+32rho_ALS<1/8`; the common product
  register is transferred by the exact scale-cancellation identity in
  `step_007c`.  This keeps the source and consumer separate and prevents a
...
- First cyclic update: use the exact unfolding
  ...
  update.  The `U` update consumes the full `beta_perp` bound and proves
  ...
  explicit sequential envelopes `3072(omega+beta_perp)`,
  `3456(omega+beta_perp)`, and `4096(omega+beta_perp)` for the updated mode
...
- Baseline conclusion preserved: at the exact orthogonal equal-weight
  ...
  first-sweep error is exactly zero.  The numerical upper bound `beta_perp`
...
| `E_perp_pair`: explicit perpendicular pair ledger | `step_007c` | `E_quotient` entry decomposition, `G_M^T N_M=0`, `t_M<=tau_r`, realized Gram floor | mixed target-span/perpendicular terms and coherent shared-perpendicular term are separated; no rank-free max-column shortcut | exact same realized held factors and pair Gram; `F_M=N_M^TN_M` is the actual residual Gram | `b_perp(t',t'')=(1+q_*)(1+48omega)^2((t')^2+(t'')^2)+r(t't'')^2`; `beta_perp=b_perp(tau_r,tau_r)<10^(-8)omega` | `tau_r=q_*^2/(10^4r)`, `q_*`, `48omega` | `step_007b` raw U/V/W ledger | None |
| `E_carry_resolvent`: carried-cancellation, rank-free normalized-resolvent, and common-product transfer | `step_007c` | `E_sm`, `E_best`, `E_quotient`, plus the explicit held records `B_N=I+C_N+E_N`, `||C_N||_(row/col,1)<=48omega`, `||E_N||_(row/col,1)<=epsilon_N`, `t_N`, and `h_M=sum epsilon_N`; no target-span premise | static and carried cross/pair defects are split; total `Delta_C<=96omega+(2q_*+8rho_ALS)h_M`; total `Delta_J<=192omega+2beta_act,M+(4q_*+16rho_ALS)h_M`; centered normalization cancels first-order terms in both orientations; no `sqrt(r)` term | exact active solve and canonical direction/product representation use the same realized target and held columns; every carried residual is transferred in the consumed quotient interface and bounded by `epsilon_M^+<=3072eta+ell_0 h_M` | `S_0<=290eta`, `kappa_res<17/16`, `8kappa_res S_0+4||alpha||<=2977eta<3072eta`, `ell_0=17/512<1/8`; chronological `h_U=0`, `h_V=epsilon_U`, `h_W=epsilon_U+epsilon_V`, `h_W<1/2048<2rho_ALS/3` | `1-q_*^2`, `Delta_J<1/32768<1/64`, `d_M<1/16384<1/8`, positive `D_lambda R_in` and `nu_j`, `Gamma<=1.01`, `rho_ALS=1/1024` | `step_007b` U/V/W subupdates | None |
| `E_pair_entry`: first-sweep collective perpendicular and pair-Gram control | `step_007b` | `E_sm`, `E_best`, `E_quotient`, `E_perp_pair`, `E_carry_resolvent`, dual row/column bounds, and canonical represented-product coordinate | mixed target-span/perpendicular terms are at most `2(1+q_*)(1+48omega)^2 tau_r^2`; coherent shared-perpendicular term is exactly charged by `r tau_r^4`; carried classes are bounded through the producer's `h_M` envelope and no class is uncontrolled | exact held Khatri-Rao pair Gram and cross Gram of the setting update; no surrogate; the ordinary inverse is legal at every call | `beta_perp<10^(-8)omega`; total cross/pair deviations use `96/192` plus `h_M` terms; `Delta_J<1/32768<1/64`; inverse floor `31/32`; `h_U=0,h_V=epsilon_U,h_W=epsilon_U+epsilon_V` | target pair floor `1-q_*^2`, `tau_r=q_*^2/(10^4r)`, and `h_W<2rho_ALS/3` | first `U,V,W` subupdates and `E_parallel_entry` | None |
| `E_parallel_entry`: one-sweep reachable target-span entry | `step_007b` | `E_pair_entry`, `E_carry_resolvent`, exact unfolding range, three intermediate ordinary inverses, positive scale bridge, and product-one quotient rebalance | coefficient, amplitude, carried, and perpendicular pair-Gram defects controlled separately; perpendicular defects vanish mode by mode; no carried scale is bounded by the initial `alpha` estimate | updated factors are the exact ALS outputs `G_M Z_M`; the post-sweep quotient is the same represented rank-k tensor with frozen zeros; incoming product scales cancel exactly | chronological `epsilon_U<=3072eta`, `epsilon_V<=3456eta`, `epsilon_W<=3888eta<4096eta`; `4096(omega+beta_perp)<rho_ALS/3`; exact `N_M^+=0` | `rho_ALS=1/1024`, `h_M<=2rho_ALS/3`, `d_M<1/8`, positive `D_lambda R_in` and `nu_j`, intermediate inverse floor `31/32` | `step_008`, `step_009` | None |
...
| `E_perp_pair` | `step_007c` | `step_007b` raw U/V/W ledger | exposes the actual `F_M=N_M^TN_M`, `beta_act`, and numerical `beta_perp` before any range claim | `E_quotient` -> orthogonal projector identity -> `F_M` and `b_perp(t',t'')` -> `E_perp_pair`; no future ALS state is used | derived | None |
| `E_carry_resolvent` | `step_007c` | `step_007b` chronological U/V/W subupdates | closes the normalized first-sweep constants without a rank or hidden-condition-number loss | `E_sm,E_quotient` -> conditional raw cross/pair interface -> exact resolvent -> centered column normalization in both orientations -> rank-free row/column and product-log bounds -> `E_carry_resolvent` | explicitly conditional | None |
| `E_pair_entry` | `step_007b` | the three first-sweep subupdates and `E_parallel_entry` | controls shared perpendicular components and every first-sweep Khatri-Rao inverse | `E_sm,E_quotient,E_perp_pair` -> cross/pair-Gram deviations -> `E_carry_resolvent` invocation -> intermediate inverse floors | derived | None |
...
| `step_007c` | Before the first-sweep assembly, derive `E_perp_pair` from the accepted `E_quotient` decomposition, then state the source-adequate parameterized carried-cancellation interface: exact amplitude-conjugated solve, centered column-normalization identity, both induced row/column diagonal-similarity bounds, Neumann resolvent factor `kappa_res<17/16`, static bound `8kappa_res(288omega+2beta_perp)+4||alpha||<=2977eta<3072eta`, and carried gain `ell_0=8q_*+32rho_ALS<1/8`.  State actual orthogonal baseline equality (`beta_act=0`, `A=0`, `nu=1`, `s=0`) while retaining positive `beta_perp` bookkeeping.  The raw held-input conditions are conditional records that `step_007b` proves before each call; target-span membership is not an input. | `step_001`, `step_007` | `E_sm`, `E_best`, `E_quotient` (derived); explicitly conditional raw held-input record (`epsilon_N`, cross/pair deviations, perpendicular budgets) | normalization first-order cancellation, induced-norm resolvent transfer without rank or hidden condition number, common-register scale cancellation, legal `F_M`/`beta_perp` producer, and baseline object identity | exact inverse identity, centered normalization decomposition, diagonal similarity, row/column Hadamard bounds, explicit `b_perp` ledger, and direct current-notation calculation | `E_perp_pair`, `E_carry_resolvent` | Constant-explicit one-block transfer; `S_0<=290eta`, `2977eta<3072eta`, `ell_0<1/8`, no `sqrt(r)` charge | PENDING |
| `step_007b` | Consume `E_perp_pair` and `E_carry_resolvent` together with the accepted selected entry decomposition `bar P_M=G_M B_M+N_M`; verify normalized cross/pair-Gram deviations and a `31/32` inverse floor at all three first-sweep subupdates, prove exact zero-padding equivalence and range containment in chronological U/V/W order, and show the completed first cyclic sweep has every `N_M^+=0` and `norm(e^1)_Q<=4096(omega+beta_perp)<rho_ALS/3`.  No target-span/range fact is assumed before its own exact unfolding update. | `step_001`, `step_007`, `step_007c` | `E_sm`, `E_best`, `E_quotient`, `E_perp_pair`, `E_carry_resolvent` (derived), `assump:base-scale` (primitive procedure scale) | coherent shared perpendicular components, amplitude-conjugated Khatri-Rao pair Grams, chronological held-state transfer, and exact range entry | orthogonal projector identity, induced row/column Hadamard bounds, `E_perp_pair`, `E_carry_resolvent`, exact unfolding range, Moore--Penrose block identity, product-one rebalance | `E_pair_entry`, `E_parallel_entry`, active rank-r/zero-padding equivalence | Entry-sweep structural objective; explicit `tau_r^2`, `r tau_r^4`, `96/192/3072/3456/4096` constants, inverse floor, and one-sweep cost | PENDING |
