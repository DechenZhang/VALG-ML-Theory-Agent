...
This is proof-sketch attempt 10 for `perspective_1/idea_6`, run in
`revise_sketch` mode.  It preserves the exact formalized setting and goal and
repairs only the first-active-sweep producer interface identified by the live
`step_007b` unit-attempt-2 review, SHA-256
`fd4d84931b7276238a4897a82a30c80c35fecd5418a07591b50973509385281f`.
The accepted attempt-9 sketch and review and the accepted attempt-9 global
diagnostic and review are roadmap context, not proof evidence.  No artifact
under `proof_history/` is consumed.  All step rows are fresh and `PENDING`
under attempt 10.  The formalized setting is
`perspective_1/idea_6/setting.md`; all primitive assumptions are cited below
by their stable ids.  The tensor is
...
Attempt 2 added the sole controller-approved procedure-accounting repair.  At
...
for the present repair.  Attempts 3, 4, 5, 6, and 7 leave that wrapper and every
setting-level object unchanged; this attempt repairs only the theorem-facing
producer interface for the first active sweep.
...
- Triggering review: `perspective_1/idea_6/proof_steps/step_007b/review.md`,
  SHA-256
  `fd4d84931b7276238a4897a82a30c80c35fecd5418a07591b50973509385281f`.
- Binding setting SHA-256:
  `a6991b0e308a8a33e782be75f9f081325404974335e5c00df85e6d8a80921cab`.
- Accepted attempt-9 sketch/review SHA-256:
...
- Accepted attempt-9 global diagnostic/review SHA-256:
...
- Evidence boundary: the live setting, accepted attempt-9 sketch and review,
  accepted live global diagnostic and review, and the triggering live step
  review above.  No archived proof history is evidence.
...
- Attempt-10 repair summary: `step_007c` exports a deterministic
  trajectory-independent fixed point `bar_beta`, `bar_eta`, and staged U/V/W
  budgets before any conditional call.  `step_007b` validates U, emits a full
  same-current actual/comparator predecessor record, validates V from that
  record, emits the V record, and finally validates W and emits the W record.
  Actual record sizes `epsilon_N^act` and exact held sums `h_M^act` are never
  rebound to deterministic upper envelopes.  Both induced diagonal reserves,
  ordinary inverse margins, incoming-scale positivity, and actual/comparator
  normalizer positivity are proved before each conditional invocation.  Only
  after this finite induction is complete are `beta_FS^act` and
  `eta_FS^act` defined and the already-domain-valid inequalities reapplied to
  recover the sharp `3072/3276/3500/4096` conclusion.
- Attempt-2 local repair summary: before any active sweep, fix
  ...
  introduced.
- Attempt-3 local repair summary: preserve the accepted attempt-2 setting,
  goal, rank/probability/metric contracts, exact range/zero-padding
  chronology, and all `step_008`/`step_009` quotient interfaces, but insert a
  source-adequate `step_007c` row before the `step_007b` first-sweep row.
  `step_007c` first derives the perpendicular pair ledger from the accepted
  entry decomposition (so `F_M` and `beta_perp` have a legal producer), then
  states a parameterized held-input lemma.  Its held-input hypotheses are
  raw cross/pair-Gram bounds and direction/perpendicular budgets only; target
  span membership and a future range conclusion are never assumptions.
  The actual contamination
  `beta_act,M = ||H_(M')^par circ F_(M'') + F_(M') circ H_(M'')^par
  + F_(M') circ F_(M'')||_rc` remains distinct from the numerical upper bound
  `beta_perp`.  The lemma derives both induced orientations of the
  diagonal-similarity/resolvent transfer and uses the exact
  column-normalization cancellation before taking norms.  It exports the
  rank-free bounds `8 kappa_res S_0 + 4||alpha||_infty <= 3072 eta` and
  This historical attempt-3 gain is superseded by the current conservative
  `ell_c=16q_*+64rho_ALS=17/256<1/8`.
  Here `S_0=288omega+2beta_perp<=290eta` and
  `kappa_res=Gamma^2(1-q_*^2)^(-1)(1-(1-q_*^2)^(-1)/64)^(-1)<17/16`.
  `step_007b` discharges the conditional raw pair/cross bounds at U, then
  invokes the same lemma at V and W only after the preceding exact range
  update has produced the next held-state interface.  Consequently the
  legal chronology is `e_U<=3072eta`,
  `e_V<=3072eta+e_U/8`, and
  `e_W<=3072eta+(e_U+e_V)/8`, giving `e_W<4096eta` without circular range
  assumptions.  The positive bookkeeping bound is retained at the exact
  orthogonal baseline; only the actual contamination is zero there.
- Attempt-4 local repair summary: retain the centered cancellation and all
  numerical mechanisms above, but make the carried producer contract
  executable.  For every held mode record write
  `B_N=I+C_N+E_N`, with `||C_N||_(row/col,1)<=48omega`,
  `||E_N||_(row/col,1)<=epsilon_N^act`,
  `max_j||N_N(:,j)||_2<=t_N`, and
  `h_M^act=sum_(N in H_M)epsilon_N^act`.  Export the total bounds
  `Delta_C<=96omega+(2q_*+8rho_ALS)h_M^act` and
  `Delta_J<=192omega+2beta_act,M+(4q_*+16rho_ALS)h_M^act`, together with
  `Delta_J<=1/64`, `d_M<1/8`, positive input scales, the ordinary inverse
  branch, and `h_M^act<=2rho_ALS/3`.  Attempt 10 replaces this historical
  chronology by the exact actual/barred interface in its binding addendum;
  no unadorned epsilon or h symbol from this summary is active below.
  Incoming scales are handled by exact product cancellation and
  positivity of exponentials, never by reusing the initial
  `|alpha|<=128omega` bound.  All step review fields remain `PENDING` for
  this fresh sketch attempt.
...
## Attempt-10 Repair Addendum

This addendum is binding for the first-sweep interfaces and supersedes every
older historical use of unadorned `epsilon_M`, `h_M`, `beta_FS`, and
`eta_FS`.  Older summaries remain lineage only.

Define a trajectory-independent perpendicular envelope.  With
`omega=q_*^2+r tau_r`, set

`b_0:=2(1+q_*)(1+48omega)^2tau_r^2+r tau_r^4`,

`L_beta:=2(1+q_*)(1+48omega+2rho_ALS/3)tau_r^2`.

These deterministic quantities satisfy `b_0<10^(-8)omega`,
`L_beta<=10^(-10)omega`, and `6548L_beta<1/2`.  Before any trajectory output
define

`bar_beta:=(b_0+6548L_beta omega)/(1-6548L_beta)`,
`bar_eta:=omega+bar_beta`.

Thus `bar_beta=b_0+6548L_beta bar_eta`,
`bar_beta<2*10^(-8)omega`, and `bar_eta<1.001q_*^2`.  Define deterministic
budgets

`bar_epsilon_U:=3072bar_eta`,
`bar_epsilon_V:=3072bar_eta+ell_c bar_epsilon_U=3276bar_eta`,
`bar_epsilon_W:=3072bar_eta+
 ell_c(bar_epsilon_U+bar_epsilon_V)<3500bar_eta`,

`bar_h_U:=0`, `bar_h_V:=bar_epsilon_U`, and
`bar_h_W:=bar_epsilon_U+bar_epsilon_V=6348bar_eta<6548bar_eta`.

The affine coefficient is for the staged records: U has `h=0`; before V the
exact U range output makes one of the two held `t_N` values zero; and before W
both preceding range outputs do so.  Hence every call with `h>0` has
`t_(M')^2+t_(M'')^2<=tau_r^2`, which gives the displayed `L_beta` without a
missing factor two.  The functional ledger
`beta_perp(M,h)<=b_0+L_beta h` on `0<=h<=2rho_ALS/3` therefore implies
`beta_perp(M,h)<=bar_beta` whenever
`h<=bar_h_M`, without using a call output; moreover
`bar_h_W<1/2048<2rho_ALS/3`.

For a generated record define
`epsilon_N^act:=max{||E_N||_(row,1),||E_N||_(col,1)}` and let
`err_N^act` be the maximum of this coefficient error, the ambient
direction-column error, and the common-log error.  The exact chronology is

`h_U^act:=0`, `h_V^act:=epsilon_U^act`,
`h_W^act:=epsilon_U^act+epsilon_V^act`.

The staged induction proves `err_N^act<=bar_epsilon_N` and
`h_M^act<=bar_h_M`; an actual quantity is never rebound to a barred budget.
Only after all three calls have been legalized define

`beta_FS^act:=max_M beta_perp(M,h_M^act)`,
`eta_FS^act:=omega+beta_FS^act`.

Reapplying the already-domain-valid one-call inequality with
`widehat beta=beta_FS^act` yields
`epsilon_U^act<=3072eta_FS^act`,
`epsilon_V^act<=3276eta_FS^act`, and
`epsilon_W^act<3500eta_FS^act`.  It also gives
`beta_FS^act<=b_0+6348L_beta(omega+beta_FS^act)`, hence
`beta_FS^act<2*10^(-8)omega`.  This second pass sharpens an already legal
trajectory and cannot justify an earlier call.

After each exact solve `N in {U,V,W}`, `step_007b` emits `Pred_N` before any
later consumer.  It retains the current exact perpendicular output and
`F_N=N_N^TN_N` and only afterward records the range consequence `F_N=0`.
It records the actual coefficient solve `A_N^pred`; constructs
`A_N^{0,pred}` by setting only the carried `E`-fields in that same predecessor
equation to zero while retaining all static fields and the same current
`F_N`; and defines

`(nu_{N,j}^pred)^2:=diag((I+A_N^pred)^T H_N(I+A_N^pred)+F_N)_j`,

`(nu_{N,j}^{0,pred})^2:=diag((I+A_N^{0,pred})^T H_N
 (I+A_N^{0,pred})+F_N)_j`.

With `D_nu:=diag(nu_j^(-1))`, the producer sets
`B_N=(I+A_N^pred)D_{nu,N}^pred`,
`B_N^0=(I+A_N^{0,pred})D_{nu,N}^{0,pred}`,
`C_N:=B_N^0-I`, and `E_N:=B_N-B_N^0`.  It proves both normalizers positive
and exports the two unit-column identities against the same current `F_N`.
Their difference gives, for `O in {row,col}`,

`||H_NE_N-Diag(H_NE_N)||_(O,1)<=(1+q_*)epsilon_N^act`,

`||Diag(H_NE_N)||_(O,1)
 <=(epsilon_N^act)^2/2+48omega epsilon_N^act
 <=bar_epsilon_N^2/2+48omega bar_epsilon_N`.

The last quantity is the row/column diagonal reserve consumed by the next
call.  Setting `B_N^0=I` merely because range entry gives `F_N=0` is not an
allowed producer.

Before each conditional call `M`, the staged certificate substitutes
`bar_h_M` and `bar_beta` in both induced orientations and records

`bar_Delta_C^M:=96omega+(2q_*+8rho_ALS)bar_h_M`,

`bar_Delta_J^M:=192omega+2bar_beta+
 (4q_*+16rho_ALS)bar_h_M<1/32768<1/64`,

`bar_S_0:=288omega+2bar_beta<=290bar_eta`,

`bar_d_M^0:=2kappa_res bar_S_0+128omega`,
`bar_d_M:=bar_d_M^0+(2q_*+8rho_ALS)bar_h_M<1/16384`.

Here `bar_d_M^0` and `bar_d_M` bound comparator and actual ambient direction
deviations, not diagonal entries.  With
`u_C=(2q_*+8rho_ALS)h_M^act` and
`u_J=(4q_*+16rho_ALS)h_M^act`, the centered calculation also gives
`||Diag(H_M(A_M-A_M^0))||_(O,1)<=u_C+u_J/2`.  Before the conditional
invocation the same certificate derives

`(nu_{M,j}^pred)^2,(nu_{M,j}^{0,pred})^2
 >=1-2q_*bar_d_M-2bar_d_M^2>1/2`

and records positive incoming scales
`lambda_j^(1/3)exp(s_j/3)>0`.  The order is pre-call certificate,
conditional transfer, exact solve/range, and `Pred_M` export, first for U,
then V, then W.

## Attempt-8 Inherited Carried-Interface Repair

The attempt-7 review identified two source-boundary defects.  A norm bound on
the carried coefficient field does not by itself imply centered diagonal
cancellation, and the static perpendicular ledger cannot be reused after a
carried field changes the target-span coefficient matrix.  The attempt-6
roadmap therefore exports a normalized predecessor certificate and keeps the
full chronological perpendicular ledger.  This is still a target-preserving
bridge repair; no primitive assumption, update rule, rate, metric, probability
protocol, success criterion, or baseline statement changes.

For every held mode `N`, retain one mixed record and one carried-field
definition throughout:

`bar P_N=G_N B_N+N_N`, `G_N^T N_N=0`,
`B_N=B_N^0+E_N=I+C_N+E_N`, with `C_N:=B_N^0-I` and
`E_N:=B_N-B_N^0`.  At the initial U call set `B_N^0=B_N` and `E_N=0`.

Both `C_N` and `E_N` may be nonzero; `epsilon_N^act` means
`max(||E_N||_{row,1},||E_N||_{col,1})`, while `bar_epsilon_N` is only its
deterministic staged upper budget, and
`t_N:=max_j||N_N(:,j)||_2`.  If N is carried, its predecessor certificate
names both the actual predecessor matrix/normalization and the comparator
predecessor matrix/normalization, and defines the two *post-normalization*
outputs
`B_N=(I+A_N^pred)D_{nu,N}^pred` and
`B_N^0=(I+A_N^{0,pred})D_{nu,N}^{0,pred}`.  This certificate is emitted by
`step_007b` immediately after the predecessor's exact solve, retains the same
current `F_N` in both normalizers, and is passed to the next call; it is not a
primitive assumption.

The exact unit-column identities are, in both original and transposed induced
orientations,
`diag(B_N^T H_N B_N+F_N)=1` and
`diag((B_N^0)^T H_N B_N^0+F_N)=1`, `F_N:=N_N^TN_N`, or equivalently each
diagonal Gram defect equals `-diag(F_N)`.  Only after the exact range update
may the producer set the current `F_N=0`.  Subtracting these valid
post-normalization identities gives the centered relation
`diag(2(B_N^0)^T H_N E_N+E_N^T H_N E_N)=0`.
Consequently, for `O=row` and `O=col`,
`||H_NE_N-Diag(H_NE_N)||_{O,1}<=(1+q_*)epsilon_N^act`,
`||Diag(H_NE_N)||_{O,1}<=(epsilon_N^act)^2/2+48omega epsilon_N^act`, and
`||B_N^T H_N B_N-(B_N^0)^T H_NB_N^0||_{O,1}
 <=(2q_*+8rho_ALS)epsilon_N^act`.
These are the raw-control-to-exported-interface inequalities; no
pre-normalization identity or incompatible second definition of E_N is used.

For a current mode `M`, let `H_{0,M}=H_{M'}\circ H_{M''}` and define the
cross/pair matrices from the actual held records,
`C_M=(G_{M'}^T\bar P_{M'})\circ(G_{M''}^T\bar P_{M''})` and
`J_M=\bar P_{M'}^T\bar P_{M'}\circ\bar P_{M''}^T\bar P_{M''}`.
Define `C_M^0,J_M^0` by replacing each carried `E_N` by zero while retaining
the same `C_N`, `N_N`, and static fields.  Thus `A_M^0` below is a genuine
predecessor comparator, not an exclusive static/carried convention.  Put
`h_M^act=sum_{N in H_M}epsilon_N^act`, where `H_M` contains exactly the already
produced carried held modes in this call.

Define the predecessor-centered field
`L_N:=H_NE_N-Diag(H_NE_N)` and the normalized pair increment
`Delta_N:=B_N^TH_NB_N-(B_N^0)^TH_NB_N^0`.  The mixed expansion is split into
six source classes before absolute values: static-static, static-carried,
carried-static, carried-carried, static-perpendicular, and
carried-perpendicular.  For each `O` in `{row,col}`, the termwise ledger to
be proved by the local unit is

`||C_M-C_M^0||_{O,1}
 <= [2q_*+8rho_ALS]h_M^act`,
`||J_M-J_M^0||_{O,1}^{span}
 <= [4q_*+16rho_ALS]h_M^act`,

where the superscript `span` excludes only the two perpendicular classes, and
the complete pair difference is

`||J_M-J_M^0||_{O,1}
 <= [4q_*+16rho_ALS]h_M^act+beta_perp(M,h_M^act)`.

To make the constants auditable, the row sums and their transposed column
counterparts are recorded separately.  For a fixed row `i` and a fixed column
`j`, respectively,

`sum_{ell != i}|(L_{M'}\circ Q^0_{M''})_{iell}|
 <= (q_*+2rho_ALS)epsilon_{M'}`,
`sum_{i != j}|(L_{M'}\circ Q^0_{M''})_{ij}|
 <= (q_*+2rho_ALS)epsilon_{M'}`,

and the corresponding `M''` terms have the same two bounds.  The diagonal,
static-carried, and carried-carried terms obey, in both displayed orientations,

`sum_{ell}|Diag(H_NE_N)\circ Q^0_{N'}|_{iell}
 <=rho_ALS epsilon_N^act`,
`sum_{ell}|L_N\circ C_{N'}|_{iell}<=2rho_ALS epsilon_N^act`,
`sum_{ell}|L_N\circ L_{N'}|_{iell}<=rho_ALS epsilon_N^act`,

with the identical inequalities under `sum_i` for a fixed column (and with
`N'` replaced by `N''`).  Adding
the two cross occurrences gives `2q_*+8rho_ALS`; adding the two pair
occurrences and their carried-carried term gives `4q_*+16rho_ALS`.  These
inequalities use only `epsilon_N^act<=bar_epsilon_N<rho_ALS/3`, the two post-normalization
column identities, `d_N^pred<1/8`,
`||C_N||_{row/col,1}<=48omega`, and the predecessor centering identity.  No
rank factor or symmetry of a carried field is used.

The perpendicular part uses the same actual record rather than the old static
bound.  With
`F_N=N_N^TN_N` and `H_N^par=B_N^TH_NB_N`, define

`beta_act,M:=||H_{M'}^par\circ F_{M''}
 +F_{M'}\circ H_{M''}^par+F_{M'}\circ F_{M''}||_{rc}`,

The single exported envelope is defined from those actual held factors by

`beta_perp(M,h):=(1+q_*)[(1+48omega+h)^2t_{M''}^2
 +(1+48omega+h)^2t_{M'}^2]+r(t_{M'}t_{M''})^2`.

For bookkeeping only, write
`beta_perp^0(M):=beta_perp(M,0)` and
`beta_perp^carry(M,h):=beta_perp(M,h)-beta_perp^0(M)`; all producer,
consumer, flow, and gate rows use the unified `beta_perp(M,h)`, never one
of these components alone.  The actual matrix satisfies
`beta_act,M<=beta_perp(M,h_M^act)`.  The a priori and post-induction envelopes
are those in the attempt-10 addendum; no future maximum is used here.
The functional producer exports `b_0,L_beta,bar_beta,bar_eta` before the
sweep; it does not define a maximum from unavailable future records.
`step_007b` defines `beta_FS^act,eta_FS^act` only after the barred induction.

The exact solve and normalization are then compared to the static comparator.
With `Lambda=diag(lambda)` and positive `D_lambda=diag(lambda^(1/3))`,

`A_M=Lambda(C_M-J_M)J_M^{-1}Lambda^{-1}`,
`A_M^0=Lambda(C_M^0-J_M^0)(J_M^0)^{-1}Lambda^{-1}`,

and the exact resolvent/difference identities are

`A_M-A_M^0
 =Lambda[(C_M-C_M^0)J_M^{-1}
  -C_M^0(J_M^0)^{-1}(J_M-J_M^0)J_M^{-1}]Lambda^{-1}`,

equivalently, with `Delta_C=C_M-C_M^0` and `Delta_J=J_M-J_M^0`,

`A_M-A_M^0
 =Lambda[(Delta_C-Delta_J)J_M^{-1}
  -(C_M^0-J_M^0)(J_M^0)^{-1}Delta_JJ_M^{-1}]Lambda^{-1}`,

`J_M^{-1}-(J_M^0)^{-1}
 =-(J_M^0)^{-1}(J_M-J_M^0)J_M^{-1}`.

For every diagonal similarity `D`, the proof records both
`||D X D^{-1}||_{row,1}` and `||D X D^{-1}||_{col,1}`.  In the carried
terms, the factor `lambda_i/lambda_j` from `Lambda` is paired with the
reciprocal factor from the two held product scales in
`R_in^{-2}D_lambda^{-2}`; entrywise the product is one.  Thus the exact
paired inequalities are

`||Lambda(C-C^0)J^{-1}Lambda^{-1}||_{O,1}<=u_C`,
`||Lambda(C^0-J^0)(J^0)^{-1}(J-J^0)J^{-1}Lambda^{-1}||_{O,1}
 <=u_J/2`,

for `O=row` and `O=col`; therefore their sum is at most `u_C+u_J/2`,
with no uncharged `kappa_res` multiplier on the carried sum.  The same paired calculation gives
`max_j||G_M(A_M-A_M^0)(:,j)||_2<=u_C+u_J/2`; this is the ambient-column
interface and uses `||G_Mx||_2^2<= (1+q_*)||x||_2^2` plus the already charged
diagonal terms, not an `sqrt(r)` conversion.

Let `d_M^0=diag(H_MA_M^0)` and
`Delta d_M=diag(H_M(A_M-A_M^0))`.  The centered normalization difference is
written before absolute values as

`N_M(A_M)-N_M(A_M^0)
 =[A_M-A_M^0-Diag(Delta d_M)]
  +Diag(nu^{-1}-(nu^0)^{-1}+Delta d_M)
  +(A_M-A_M^0)(D_nu-I)
  +A_M^0(D_nu-D_{nu^0})`.

  For any domination `widehat beta>=beta_perp(M,h_M^act)`, put
`widehat eta:=omega+widehat beta`.  The predecessor relation and
`d_M^0<=720widehat eta` give, with
`x_M:=u_C+u_J/2=(4q_*+16rho_ALS)h_M^act`,

`|Delta d_M|<=x_M`,
`|nu^{-1}-(nu^0)^{-1}+Delta d_M|
 <=2x_M^2+q_*x_M+4d_M^0x_M`,
`||A_M^0(D_nu-D_{nu^0})||_{O,1}<=2d_M^0x_M`,

in both row and column orientations.  The static/carry cross terms are the
`4d_M^0x_M` and `2d_M^0x_M` terms; they are not dropped.  Since
  `h_M^act<=2rho_ALS/3`, `x_M<=68q_*h_M^act`,
`widehat eta<=bar_eta`, and
`q_*=1/4096`, the explicit substitution is

`2x_M^2+q_*x_M+6d_M^0x_M
 <=70q_*^2h_M^act+4320(1.001q_*^2)^2h_M^act
 <=(4rho_ALS)q_*h_M^act`,

which is within the `8rho_ALS h_M^act` cross reserve.  Therefore, in both
orientations,

`||N_M(A_M)-N_M(A_M^0)||_{row,1},
 ||N_M(A_M)-N_M(A_M^0)||_{col,1}
  <=2(2u_C+u_J)=(16q_*+64rho_ALS)h_M^act`.

The same centered calculation gives
  `max_j||G_M[N_M(A_M)-N_M(A_M^0)](:,j)||_2
  <=2(2u_C+u_J)` and

`|log(nu_j)-log(nu_j^0)|
 <=2|Delta d_{M,j}|+2x_M^2
  <=2(2u_C+u_J)`.

The static comparator contributes
  `8kappa_res(288omega+2widehat beta)+4||alpha||_infty
  <=2977widehat eta<3072widehat eta`, while the carried increment is bounded by

  `ell_c h_M^act`, `ell_c:=16q_*+64rho_ALS=17/256<1/8`.

Thus the exported one-call interface is
  `err_M^act<=3072widehat eta+ell_c h_M^act` in both induced orientations,
for ambient direction columns and the one common product-log register.

The chronology unit first uses only `bar_beta,bar_eta`.  U has
`h_U^act=bar_h_U=0`; both induced `bar_Delta` bounds, comparator/actual
direction reserves, diagonal reserve, incoming-scale positivity, and both
normalizer floors are established before the conditional call.  Its output
obeys `err_U^act<=bar_epsilon_U`; the exact solve/range conclusion is followed
immediately by `Pred_U`, giving
`h_V^act=epsilon_U^act<=bar_h_V`.  The same pre-call checks legalize V; only
after its exact solve and `Pred_V` export does
`h_W^act=epsilon_U^act+epsilon_V^act<=bar_h_W` become available.  The W
certificate then legalizes the final call and emits `Pred_W`.  After this
finite induction the actual maximum is defined and the same valid
inequalities sharpen to `epsilon_U^act<=3072eta_FS^act`,
`epsilon_V^act<=3276eta_FS^act`, and
`epsilon_W^act<3500eta_FS^act<4096eta_FS^act`.  No target-span membership is
consumed before its own exact range update.

The actual orthogonal equal-weight stationary boundary has
`C=C^0=J=J^0=I`, `F_N=0`, `A=A^0=0`, `nu=nu^0=1`, and `s^+=0`.  The
  positive `bar_beta` envelope is only a finite-transient upper bound; the
actual stationary ledger is zero.  The remaining hard algebra is therefore
`step-local` under the normalized predecessor certificate, while the
source-to-claim and consumer paths are explicit.
...
same-state coefficient bridge -> best-scalar transfer -> carried-cancellation/
normalized-resolvent interface -> collective perpendicular pair-Gram audit and
first cyclic sweep -> invariant target-span
...
3. **Selected-state bridge and local refinement.**
   ...
   - Fixed quantities: `q_*`, the target chart thresholds, gauge-invariant
     represented-product coordinates, the deterministic `bar_beta,bar_eta`
     domain envelope and the post-induction `beta_FS^act,eta_FS^act`
     (with local `beta_perp(M,h_M^act)` records), and
...
- `E_quotient` (gauge-invariant represented-product quotient coordinates) is derived in
  `step_007` from the product-one equal-norm gauge.  That pending step must
  supply the entry decomposition `bar P_M=G_M B_M+N_M` with
...
- `E_FS_apriori` (the deterministic `b_0,L_beta,bar_beta,bar_eta`, barred
  U/V/W budgets, and both-orientation pre-call reserve formulas) is produced
  by `step_007c` from `E_sm,E_quotient` before any sweep output and consumed
  by the staged induction in `step_007b`.
- `E_carry_resolvent` (the carried-cancellation, rank-free normalized
  resolvent, and common-product transfer) is a parameterized conditional
  bridge produced by `step_007c` from `E_sm`, `E_best`, and `E_quotient`.
  Its raw held-input interface records direction/perpendicular budgets and
  cross/pair-Gram deviations; it does not assume target-span membership.
  `step_007b` discharges that interface before each chronological invocation,
  and only its exact range lemma produces the next target-span held state.
  Thus `E_carry_resolvent` never consumes `E_parallel_entry` and has no
  circular range premise.
- `E_perp_pair` (the explicit `F_M=N_M^TN_M` and functional actual-record
  `beta_perp(M,h)` ledger) is derived by `step_007c` directly from
  `E_quotient`; `step_007b` instantiates it at exact actual held sums and
  defines the trajectory maximum only after its staged domains are proved.
- `E_pair_entry` (collective perpendicular and normalized pair-Gram control)
  ...
  derived in `step_007b` from `E_sm`, `E_best`, `E_quotient`, and the
  `E_carry_resolvent` interface.  The
...
### `step_007c`: carried-cancellation and normalized-resolvent interface

- Claim class: theorem-critical conditional transfer from held Khatri--Rao
  cross/pair Grams to a normalized direction and the single represented-product
  register; theorem role: quantitative producer consumed by `step_007b` before
  its V/W subupdates.  This is a repair interface, not an additional
  initialization or basin assumption.
- Dependencies and scope: `step_001` and `step_007` only.  For a mode `M`,
  let `H_0=J_{0,M}=H_(M') circ H_(M'')`,
  `C=C_M^cr`, `J=J_M`, `Delta_C=C-H_0`, and `Delta_J=J-H_0`.
  The lemma is parameterized by a raw held-input record
  `I_M=(B_{M'},N_{M'},B_{M''},N_{M''},Delta_C,Delta_J)` and by the
  carried direction errors.  For every held mode record `N` write the
  **mixed exported decomposition**
  `bar P_N=G_NB_N+N_N`, `G_N^TN_N=0`, and `B_N=I+C_N+E_N`, with
  `||C_N||_(row/col,1)<=48omega`,
  `||E_N||_(row/col,1)<=epsilon_N^act`, and
  `max_j||N_N(:,j)||_2<=t_N`.  Both `C_N` and `E_N` may be nonzero.  If N is
  carried, `Pred_N`, emitted only after N's exact solve, names its actual
  predecessor matrix `A_N^pred` and normalization `D_{nu,N}^pred` and exports
  the exact identity
  `I+C_N+E_N=(I+A_N^pred)D_{nu,N}^pred`.  The actual post-normalization
  certificate is, in both original and transposed induced orientations,
  `diag(B_N^TH_NB_N+F_N)=1`, equivalently
  `diag(B_N^TH_NB_N-I)=-diag(F_N)`, where `F_N=N_N^TN_N`.
  The same record names the predecessor comparator
  `B_N^0=(I+A_N^{0,pred})D_{nu,N}^{0,pred}` (equivalently `I+C_N`)
  obeys the same identity with the same `F_N`; here `A_N^{0,pred}` is produced
  from that same predecessor equation with only the carried coefficient-error
  fields set to zero, retaining the current `N_N,F_N`, and
  `(nu_{N,j}^{pred})^2=diag((I+A_N^{pred})^TH_N
  (I+A_N^{pred})+F_N)_j` and
  `(nu_{N,j}^{0,pred})^2=diag((I+A_N^{0,pred})^TH_N
  (I+A_N^{0,pred})+F_N)_j` defines its normalizer.  Thus this is a
  same-record derived output, including the initial `E_N=0` and post-range
  `F_N=0` specializations, rather than an admissibility premise; subtracting
  these two valid identities gives
  `diag(2(B_N^0)^TH_NE_N+E_N^TH_NE_N)=0`.
  In either orientation `O` this yields
  `||H_NE_N-Diag(H_NE_N)||_{O,1}<=(1+q_*)epsilon_N^act`,
  `||Diag(H_NE_N)||_{O,1}<=(epsilon_N^act)^2/2
  +48omega epsilon_N^act`, and
  `||B_N^TH_NB_N-(B_N^0)^TH_NB_N^0||_{O,1}
   <=(2q_*+8rho_ALS)epsilon_N^act`.
  This normalized predecessor certificate is a derived output from the
  preceding exact solve, not a primitive assumption; a norm bound on E_N is
  never used as a substitute for it.  Let `mathscr H_M` be the set of carried
  held records used in the call and set
  `h_M^act:=sum_(N in mathscr H_M)epsilon_N^act`.
  An exact range update may set the corresponding `N_N` to zero, but
  target-span membership is never an input to this lemma.

  The complete raw interface exported before the solve is
  `||Delta_C||_(row/col,1)<=96omega+(2q_*+8rho_ALS)h_M^act` and
  `||Delta_J||_(row/col,1)<=192omega+2beta_perp(M,h_M^act)
  +(4q_*+16rho_ALS)h_M^act`, with the ordinary-branch check
  `||Delta_J||_(row/col,1)<=1/64`.  The producer also exports `d_M<1/8`,
  positive input scales, and `h_M^act<=2rho_ALS/3`; these are discharged
  chronologically by `step_007b`, rather than hidden as admissibility
  assumptions.  No target-span membership, future range result, or
  `E_parallel_entry` is in this interface.
  Put `q=q_*`, `omega=q^2+r tau_r`, `F_N:=N_N^TN_N`, and
  `H_N^par:=B_N^TH_NB_N`.  The actual perpendicular ledger is
  `beta_act,M=||H_(M')^par circ F_(M'')+F_(M') circ H_(M'')^par
  +F_(M') circ F_(M'')||_(row/col,1)`.  From the actual held records, not a
  static surrogate, the common numerical envelope is
  `beta_perp(M,h):=(1+q_*)[(1+48omega+h)^2t_(M'')^2
  +(1+48omega+h)^2t_(M')^2]+r(t_(M')t_(M''))^2`.
  Thus `beta_act,M<=beta_perp(M,h_M^act)`.  Direct expansion on the staged
  records (U has `h=0`; V has one zero `t_N`; W has two) and
  `0<=h<=2rho_ALS/3` gives the deterministic affine ledger
  `beta_perp(M,h)<=b_0+L_beta h` with the `b_0,L_beta` fixed in the attempt-10
  addendum.  `step_007c` exports that function and the already-solved fixed
  point `bar_beta,bar_eta`; it does not form a maximum over future actual
  records.  Carried-perpendicular terms remain in the same `Delta_J` ledger.
- Exact coefficient and scale identity: write the held direction-pair
  cross Gram and pair Gram as `C_M` and `J_M` in the target dual basis, and
  write `D_lambda=diag(lambda^(1/3))`,
  `R_in=diag(exp(s/3))`, and `Lambda=D_lambda^3`.  The literal active
  least-squares solve, before any proof-only rebalance, is the exact identity
  
  `Z_M=Lambda C_M J_M^(-1) R_in^(-2)D_lambda^(-2)`.
  
  This is obtained by substituting the two held factor matrices into the
  unfolding normal equations and cancelling their positive column scales;
  it is not a projected or target-span update.  Strip the incoming scale and
  set
  `A_M:=Lambda(C_M-J_M)J_M^(-1)Lambda^(-1)`.
  The target-basis solved direction is then `G_M(I+A_M)`.  Define
  `nu_j^2:=1+2(H_M A_M)_(jj)+(A_M^T H_M A_M)_(jj)`.
  Canonical unit-column normalization and product-preserving balancing give
  exactly
  `N_M(A_M):=(I+A_M)diag(nu^(-1))-I` and
  `s_M^+:=log(nu)`, while the incoming common register cancels componentwise:
  
  `(lambda_j^(1/3)e^(s_j/3))^2
  (lambda_j^(1/3)e^(-2s_j/3)nu_j)=lambda_j nu_j`.
  
  Thus the historical `s_U^+` and `s_V^+` values are overwritten globally
  before the next held-input solve and are not retained in the completed
  state.
- Carried-cancellation identity (both induced orientations): write
  `d_j=(H_M A_M)_(jj)` and
  `D_nu=diag(nu^(-1))`.  Before taking absolute values, expand the exact
  centered identity
  ...
  The unit-column identity
  `nu_j^2=1+2d_j+(A_M^T H_M A_M)_(jj)` and
  `|d_j|<=q_*d_M+d_M^2` imply, for
  `d_M:=max_j||G_M A_M(:,j)||_2<1/8`,
  `|nu_j^(-1)-1+d_j|<=2d_M^2+q_*d_M` and
  `|nu_j^(-1)-1|<=2d_M`.  Hence the first-order diagonal term is cancelled
  before either induced norm is taken.  In row orientation the three terms
  are bounded respectively by the centered row mass, the quadratic reserve,
  and the centered mass times `2d_M`; applying the transposed identity gives
  the column orientation with the same constants.  In particular the proof
  never appends `2(1+||A_M||)d_M` to both orientations, which was the source of
  the rejected `16 Gamma^2 S` estimate.
- Rank-free resolvent transfer: let `E_J:=J-H_0`.  The accepted target
  pair floor and the explicit raw pair ledger give
  `||H_0-I||_rc<=q_*^2` and `||E_J||_rc<=1/64` before each invocation.
  In either induced orientation, the Neumann identities are written out as
  
  `H_0^(-1)=sum_(m>=0)(I-H_0)^m`,
  `J^(-1)=H_0^(-1)sum_(m>=0)(-E_J H_0^(-1))^m`,
  
  so
  `||H_0^(-1)||_rc <= (1-q_*^2)^(-1)` and
  `||J^(-1)||_rc <= (1-q_*^2)^(-1)
  [1-(1-q_*^2)^(-1)/64]^(-1)`.
  For every positive diagonal `D` with
  `max(D)/min(D)<=Gamma^(1/3)`, direct entrywise multiplication gives
  `||D E D^(-1)||_(row,1),||D E D^(-1)||_(col,1)
  <=Gamma^(1/3)||E||_(row/col,1)`, while
  `max_j||G_M D E D^(-1)(:,j)||_2
  <=sqrt(1+q_*)Gamma^(1/3)||E||_(col,1)`.
  These estimates use only induced norms and the realized Gram bound; no
  `sqrt(r)`, latent-factor condition number, or range premise is introduced.
  Set the deliberately looser common factor
  `kappa_res:=Gamma^2(1-q_*^2)^(-1)
  [1-(1-q_*^2)^(-1)/64]^(-1)<17/16`.
- Constant-closed static calculation: expand the target/carry split before
  taking any norm.  The initial branch has `E_N=0`, so `B_N=I+C_N`; its
  direction-only cross expansion is
  
  `Delta_C^0 = C_(M') circ H_(M'') + H_(M') circ C_(M'')
  + C_(M') circ C_(M'')`,
  
  and the pair expansion has the same four first/second-order terms on both
  held modes, plus the explicitly ledgered perpendicular terms.  The two
  induced orientations of the Hadamard inequality therefore give, line by
  line,
  `||Delta_C^0||_rc<=96omega` and
  `||Delta_J^0||_rc<=192omega+2widehat_beta` for any domination
  `widehat_beta>=beta_perp(M,h_M^act)`.
  Set
  `S_0:=||Delta_C^0||_rc+||Delta_J^0||_rc
  <=288omega+2widehat_beta<=290widehat_eta`, where
  `widehat_eta=omega+widehat_beta`.
  The exact resolvent identity
  `J^(-1)-H_0^(-1)=-H_0^(-1)(J-H_0)J^(-1)` is then multiplied in both
  diagonal orientations and inserted into the centered normalization
  identity.  The resulting current-notation inequalities are
  `||A_M-Diag(d)||_rc <=4 kappa_res S_0+2||alpha||_infty` and
  `d_M<=2 kappa_res S_0+||alpha||_infty`.
  The displayed normalization reserve is charged explicitly: substituting
  the latter bound into `2d_M^2+q_*d_M`, using
  `omega<=1.0001q_*^2`, `widehat_eta<=bar_eta`, and
  `kappa_res<17/16`, gives a reserve no larger than `widehat_eta` and hence
  
  `max{||N_M(A_M)||_rc,
  max_j||G_M N_M(A_M)(:,j)||_2,||s_M^+||_infty}
  <=8 kappa_res S_0+4||alpha||_infty`.
  
  Since `||alpha||_infty<=128omega`, the public arithmetic is
  `8(17/16)(290widehat_eta)+4(128widehat_eta)
  =2977widehat_eta<3072widehat_eta`.
  Every scalar normalization term is inside this calculation; no second
  post-hoc normalization charge is permitted.
- Constant-closed carried calculation: with the exported decomposition above,
  expand each mixed Hadamard product before taking either induced norm.  The
  static part is the preceding `Delta_C^0,Delta_J^0`; the carried part obeys
  `||Delta_C^carry||_rc<=2q_*h_M^act+8rho_ALS h_M^act` and
  `||Delta_J^carry||_rc<=4q_*h_M^act+16rho_ALS h_M^act`.  Therefore the **total**
  producer interface is
  `||Delta_C||_rc<=96omega+(2q_*+8rho_ALS)h_M^act` and
  `||Delta_J||_rc<=192omega+2beta_perp(M,h_M^act)
  +(4q_*+16rho_ALS)h_M^act`.
  Substitution into the same centered identity (before absolute values) yields
  `||A_M-Diag(d)||_(rc),carry<=(4q_*+16rho_ALS)h_M^act` and
  `d_M,carry<=(2q_*+8rho_ALS)h_M^act`; together with the static bound this gives
  `d_M<=2kappa_res S_0+||alpha||_infty
  +(2q_*+8rho_ALS)h_M^act`.

  The quadratic normalization reserve is bounded explicitly by
  `(2q_*+8rho_ALS)h_M^act/2` whenever the already-produced input satisfies
  `h_M^act<=2rho_ALS/3`; the reserve is not hidden in an admissibility label.
  The row and column calculations are identical after transposition, so the
  carried increment exported to the consumer is
  `max{||N_M(A_M)||_rc, max_j||G_M N_M(A_M)(:,j)||_2,
  ||s_M^+||_infty}_carry <=ell_c h_M^act`,
  where `ell_c:=16q_*+64rho_ALS=17/256<1/8`.  The full output envelope is
  consequently `err_M^act<=3072widehat_eta+ell_c h_M^act` in every induced
  direction norm and in the common product register.  This is a conditional local
  implication whose inputs are discharged by `step_007b`; it does not assume
  that the held columns already lie in `range(G_N)`.

  The numerical discharge is now a pre-call output `E_FS_apriori`, not a
  consequence of the actual recurrence.  For each `M`, substitute
  `bar_h_M,bar_beta` to obtain `bar_Delta_J^M<1/32768`,
  `bar_d_M^0,bar_d_M<1/16384`, and, in both induced orientations,
  `bar_r_N^diag:=bar_epsilon_N^2/2+48omega bar_epsilon_N` for every carried
  record.  The centered diagonal relation gives the actual and comparator
  normalizer squares at least
  `1-2q_*bar_d_M-2bar_d_M^2>1/2`.  Thus U is legal before `Pred_U`; that
  record proves the premise for V, and `Pred_V` proves the premise for W.

  Finally, positivity of carried input scales is discharged by the exact
  product cancellation, not by an initial amplitude bound.  Every incoming
  diagonal is `D_lambda R_in` with `D_lambda` strictly positive and
  `R_in=diag(exp(s_j/3))` strictly positive.  The output positivity follows
  from the pre-call actual/comparator bound above, which gives both
  normalizers positive; the identity
  `(lambda_j^(1/3)e^(s_j/3))^2
  (lambda_j^(1/3)e^(-2s_j/3)nu_j)=lambda_j nu_j`
  cancels the historical `s` before the next held solve.  The envelope
  `|s_j|<=bar_epsilon_W<1/2048` (initially `|s_j|<=512omega`) also gives a finite
  positive ratio, but no carried `|alpha|<=128omega` assertion is reused.
- Exported interfaces `E_FS_apriori`, `E_perp_pair`, and
  `E_carry_resolvent`: the first contains the deterministic fixed point,
  staged budgets, and pre-call row/column reserve formulas; the second
  contains `F_N`, `beta_act,M`, and the numerical
  `beta_perp(M,h)` function produced directly from the actual held-record
  formula;
  the latter states that every raw held-input record satisfying the displayed
  conditional bounds obeys
  `err_M^act<=3072widehat_eta+
  ell_c sum_(N in H_M)epsilon_N^act`, with the same bound
  for direction row/column masses, ambient direction columns, and the one
  common product log register.  The source path is
  `E_sm,E_best,E_quotient -> (F_N,beta_perp(M,h),b_0,L_beta) ->
  E_FS_apriori` and `raw record -> exact resolvent and centered cancellation
  -> E_carry_resolvent`; no
  `E_parallel_entry`, later basin estimate, or post-update target-span fact is
  consumed by this producer.
- Boundary and baseline stress: at an exact orthogonal equal-weight
  stationary state, `H_M=I`, `B_M=I`, `N_M=F_M=0`, `alpha=0`, and
  `Delta_C=Delta_J=0`; hence `beta_act,M=0`, `A_M=0`, `nu=1`, `Z_M=D_lambda`,
  `s_M^+=0`, and the output is exactly stationary.  The **actual** ledger
  value is therefore zero, while the theorem's numerical envelope
  `bar_beta` remains a positive bookkeeping upper bound
  for a finite certificate transient; it is not substituted for the baseline
  equality.  A finite `q_real=0` certificate transient may have
  `beta_act,M>0`, and its charge is retained.
- Accumulation and noncircular closure: this is a one-block conditional
  interface plus a deterministic three-call envelope.  `E_FS_apriori` exists
  before U.  `step_007b` proves U's domains, calls the interface with
  `widehat_beta=bar_beta`, performs the exact solve/range, and emits `Pred_U`;
  it repeats this sequence for V and W.  Only after `Pred_W` does it define
  `beta_FS^act,eta_FS^act` and reapply the already-legal inequalities for the
  sharp constants.  The producer-consumer path is strictly
  `E_quotient -> E_FS_apriori,E_perp_pair,E_carry_resolvent -> step_007b ->
  E_parallel_entry -> step_008`, so no circular basin or target-span premise
  is introduced.
- Obligation locality classification: `step-local`.  The source identities,
  source-convention match, rank-free transfer, exact cancellation, numerical
  margins, and baseline object equality are all fixed at sketch level.
...
- Mechanism source: accepted `step_007` gives, at the actual selected
  ...
  These are derived output targets of the earlier pending step, not new
  conditions on arbitrary tube points.  The realized target Grams and weights
  come from `E_sm`.
- Collective perpendicular field: consume the `E_perp_pair` ledger from
  `step_007c`, and verify it directly at each chronological input.  Put
  ...
  `||H_M^par||_rc<=(1+q_*)(1+48omega+h_M^act)^2`,
  `max_(i,j)|F_M(i,j)|<=t_M^2`, and the final Hadamard term has at most r
  entries per row or column.  The actual-record ledger is therefore
  
  `beta_perp(M,h_M^act):=(1+q_*)[(1+48omega+h_M^act)^2t_{M''}^2
   +(1+48omega+h_M^act)^2t_{M'}^2]+r(t_{M'}t_{M''})^2`.
  
  It dominates the actual contamination.  The pre-call pass uses
  `h_M^act<=bar_h_M` and `beta_perp(M,h_M^act)<=bar_beta`.  At the selected
  entry `t_M<=tau_r`, while after each exact range update the updated mode has
  `t_M=0`.  Only after `Pred_W` is emitted set
  `beta_FS^act:=max_M beta_perp(M,h_M^act)` and derive
  `beta_FS^act<2*10^(-8)omega`.  Thus shared perpendicular components, including
  the coherent `r(t_{M'}t_{M''})^2` term, are retained without a rank-free
  ambient max-column shortcut.
...
- Khatri-Rao pair-Gram and cross-Gram adequacy: after writing
  ...
  `||Delta_pair||_rc<=192omega+2bar_beta<1/512` in the a priori pass, and the
...
- Normalized transfer consumption: `step_007c` is the sole producer of the
  rank-free diagonal-similarity/resolvent and normalization estimates.  In
  this step retain the exact amplitude-conjugated formula
  `Z_M=diag(lambda) C_pair J_pair^(-1)R_in^(-2)D_lambda^(-2)` and the
  ordinary inverse floors, but invoke `E_carry_resolvent` for the centered
  output bound rather than re-deriving a second, looser norm estimate.  The
  U call first supplies the raw `t_{V},t_{W}<=tau_r` ledger and therefore has
  static envelope `3072bar_eta`; after its exact range identity proves
  `N_U^+=0`, it emits
  `Pred_U=(A_U^pred,nu_U^pred,A_U^{0,pred},nu_U^{0,pred},
  B_U,B_U^0,C_U,E_U,F_U)` before V.  V emits the analogous `Pred_V` before W,
  and W emits `Pred_W` for completed-state assembly.  The already-carried
  contribution is `ell_c sum epsilon_N^act` with
  `ell_c=16q_*+64rho_ALS=17/256<1/8`; the common product
  register is transferred by the exact scale-cancellation identity in
  `step_007c`.  Each record retains the current `F_N` in both actual and
  comparator normalizers, proves their squares exceed `1/2`, and exports its
  row/column diagonal reserve before the next call.  This prevents an
  identity-comparator substitution or second normalization charge.
- First cyclic update: use the exact unfolding
  ...
  changing the algorithm.  No `N_M^+=0` fact is assumed before its own
  update.  The U/V/W calls first consume the deterministic `bar_beta` bound
  under their staged domains and prove
  `N_U^+=0`; only then does the `V` update use the single remaining `N_W`
  perpendicular ledger and prove `N_V^+=0`; the `W` update then proves
  `N_W^+=0`.  Applying the `E_carry_resolvent` interface in this
  chronological order, with a pre-call display of both induced
  `bar_Delta_C^M,bar_Delta_J^M`, every carried `bar_r_N^diag`, the
  `bar_d_M^0,bar_d_M` bounds, positive incoming scales, and both normalizer
  floors.  After the staged pass, define `beta_FS^act,eta_FS^act` and reapply
  the already-valid estimate to obtain `3072eta_FS^act`,
  `3276eta_FS^act`, and `3500eta_FS^act<4096eta_FS^act`.  In particular,
  `E_parallel_entry` exports
  `N_U^+=N_V^+=N_W^+=0` and
  `norm(e^1)_Q<=4096eta_FS^act<rho_ALS/3`.
...
- Accumulation behavior / scope compatibility: one finite cyclic sweep.  The
  ...
  Gram defects are bounded first by `bar_beta` and then sharply by
  `beta_FS^act`; they disappear mode by
...
- Noncircular closure status: `step_007` produces the raw entry controls;
  `step_007c` derives `E_FS_apriori`, `E_perp_pair`, and the parameterized
  held-input transfer without consuming a later span or basin fact.
  `step_007b` certifies U from barred quantities, solves U and emits `Pred_U`;
  certifies V from `Pred_U`, solves V and emits `Pred_V`; then certifies W and
  emits `Pred_W`.  Only afterward does it define and sharpen by the actual
  beta.  `step_008` consumes the completed `E_parallel_entry`.
  Target-span membership is not used to prove the first U update and
  therefore is not assumed circularly.
...
- Baseline conclusion preserved: at the exact orthogonal equal-weight
  stationary state the actual contamination `beta_act,M` is zero and the
  first-sweep error is exactly zero.  The numerical upper bound `bar_beta`
  remains positive bookkeeping for fixed `tau_r`; finite certified orthogonal
...
- Producer-consumer provenance:
  `step_001, step_007 -> step_007c(E_FS_apriori,E_perp_pair,
  E_carry_resolvent) ->
  step_007b -> step_008, step_009`.
...
| `E_FS_apriori`: trajectory-independent first-sweep domain | `step_007c` | `omega,tau_r,q_*,rho_ALS`, entry `t_N<=tau_r`, and the functional `beta_perp(M,h)` formula | both mixed `tau_r^2` terms and coherent `r tau_r^4` term are in deterministic `b_0`; affine carry has coefficient `L_beta`; no actual call output is used | same held-factor pair ledger consumed by the literal first sweep | `bar_beta=(b_0+6548L_beta omega)/(1-6548L_beta)`, `bar_eta=omega+bar_beta`; barred U/V/W budgets imply every beta, `Delta_C`, `Delta_J`, direction, and normalizer domain | `6548L_beta<1/2`, `bar_h_W<2rho_ALS/3`, `bar_Delta_J^M<1/32768`, `bar_d_M<1/16384`, normalizer squares `>1/2` | staged calls in `step_007b` | None |
| `E_perp_pair`: actual-record perpendicular pair ledger | `step_007c` | `E_quotient`, actual `B_N,N_N,F_N`, `t_N`, exact `epsilon_N^act,h_M^act`, realized Gram floor | static/perpendicular and carried/perpendicular terms are split without a rank-free max-column shortcut | exact same realized held factors and pair Gram; `F_N=N_N^TN_N` is the actual residual Gram | `beta_act,M<=beta_perp(M,h_M^act)`; the function is produced before calls, while `beta_FS^act` is formed only after the staged trajectory exists | `tau_r`, `q_*`, `48omega`, exact actual records, and `E_FS_apriori` | chronological U/V/W ledger in `step_007b` | None |
| `E_carry_resolvent`: carried-cancellation, rank-free normalized-resolvent, and common-product transfer | `step_007c` | `E_sm`, `E_best`, `E_quotient`, a produced `Pred_N` with actual/comparator `A^pred,nu^pred` and common current `F_N`, defined `u_C,u_J`, exact `h_M^act`, and a domination `widehat_beta`; no target-span premise | all six cross/pair classes, both row/column diagonal reserves, and the paired resolvent terms are explicit; centered normalization controls the remainder | exact active solve and canonical direction/product representation use the same realized target and held columns | `err_M^act<=3072widehat_eta+ell_c h_M^act`, `widehat_eta=omega+widehat_beta`, in both induced norms, ambient columns, and common log register | `S_0<=290widehat_eta`, `kappa_res<17/16`, `u_C,u_J`, `h_M^act<=2rho_ALS/3`, ordinary inverse, incoming-scale positivity, and both normalizer floors | staged U/V/W calls in `step_007b` | None |
| `Pred_U,Pred_V,Pred_W`: same-current predecessor records | `step_007b`, immediately after the corresponding exact solve/range output | legal pre-call inverse, actual solve `A_N^pred`, proof-only comparator solve `A_N^{0,pred}` obtained by zeroing only carried E-fields, current `N_N,F_N`, and positive actual/comparator normalizers | actual/comparator coefficient difference, normalization difference, and diagonal remainder are split in row and column orientations; `F_N` is retained identically in both records | both records normalize the same literal solved mode against the same realized target and current perpendicular Gram; no identity or surrogate comparator | define `B_N,B_N^0,C_N,E_N,epsilon_N^act`; subtract exact unit-column identities to get off-diagonal and `bar_r_N^diag` bounds before the next call | pre-call `bar_d_N<1/16384` gives both normalizer squares `>1/2`; exact range gives current `F_N=0` only after it is retained in the definitions | next V or W call; completed-state assembly for `Pred_W` | None |
| `E_pair_entry`: first-sweep collective, comparator, reserve, and pair-Gram control | `step_007b` | `E_FS_apriori`, `E_perp_pair`, `E_carry_resolvent`, selected entry, and exact post-solve `Pred_U,Pred_V,Pred_W` producers | actual mixed perpendicular terms use exact `h_M^act`; actual and barred epsilon symbols are distinct; row/column diagonal reserves and current `F_N` are retained | exact held Khatri-Rao Grams and literal solves; no identity comparator or surrogate | U/V/W staged induction under `bar_beta`; afterward `beta_FS^act<2*10^(-8)omega` and sharp `3072/3276/3500/4096` bounds | exact `h_U^act=0,h_V^act=epsilon_U^act,h_W^act=epsilon_U^act+epsilon_V^act`, barred domination, inverse floor `31/32`, positive scales/normalizers | `E_parallel_entry` | None |
| `E_parallel_entry`: one-sweep reachable target-span entry | `step_007b` | `E_pair_entry`, exact unfolding range, three ordinary inverses, `Pred_U,Pred_V,Pred_W`, positive scale bridge, and product-one quotient rebalance | coefficient, amplitude, carried, and actual perpendicular defects vanish mode by mode; the a priori envelope only legalizes calls and the actual envelope gives the final scale | updated factors are exact ALS outputs `G_M Z_M`; incoming product scales cancel exactly | exact `N_M^+=0`; `epsilon_U^act<=3072eta_FS^act`, `epsilon_V^act<=3276eta_FS^act`, `epsilon_W^act<3500eta_FS^act`; `4096eta_FS^act<rho_ALS/3` | `rho_ALS=1/1024`, both-orientation pre-call reserves, positive actual/comparator normalizers, inverse floor `31/32` | `step_008`, `step_009` | None |
...
| `E_FS_apriori` | `step_007c` | staged U/V/W domain induction in `step_007b` | supplies `b_0,L_beta,bar_beta,bar_eta`, barred epsilon/held budgets, both-orientation `bar_Delta` and diagonal reserves, and positive actual/comparator normalizer margins before U | `E_sm,E_quotient` -> functional beta ledger -> deterministic scalar fixed point and reserve substitution -> `E_FS_apriori` | derived | None |
| `E_perp_pair` | `step_007c` | `step_007b` actual U/V/W ledger | exposes `F_M=N_M^TN_M` and the function `beta_perp(M,h)`; does not form a maximum over unavailable future records | `E_quotient` -> orthogonal projector identity -> `F_M,beta_perp(M,h)` -> `E_perp_pair`; actual evaluation occurs on produced records | derived | None |
| `Pred_U,Pred_V,Pred_W` | `step_007b`, immediately after each exact solve | next chronological call; completed-state assembly for `Pred_W` | supplies actual/comparator `A_N^pred,nu_N^pred`, common current `F_N`, `B_N,B_N^0,C_N,E_N`, exact `epsilon_N^act`, and row/column diagonal reserve | pre-call certificate -> exact solve/range -> retain current `F_N` -> actual/comparator normalizers and unit-column identities -> `Pred_N` -> next call | derived | None |
| `E_carry_resolvent` | `step_007c` | `step_007b` chronological U/V/W calls | parameterized monotone estimate `err_M^act<=3072widehat_eta+ell_c h_M^act` after both-orientation reserve and positivity checks | `E_sm,E_quotient` plus produced `Pred_N` -> paired resolvent -> centered normalization in row/column/ambient/log interfaces -> conditional output | explicitly conditional | None |
| `E_pair_entry` | `step_007b` | the three first-sweep calls and `E_parallel_entry` | legalizes calls with `E_FS_apriori`, emits `Pred_N` after solves, then defines `beta_FS^act,eta_FS^act` and sharpens the already-valid trajectory | `E_FS_apriori` -> U certificate/solve/`Pred_U` -> V certificate/solve/`Pred_V` -> W certificate/solve/`Pred_W` -> actual-beta sharpening | derived | None |
| `E_parallel_entry` | `step_007b` | `step_008`, `step_009` | reaches the exact target-span contraction domain after one algorithmic sweep | `E_pair_entry,E_carry_resolvent,Pred_U,Pred_V,Pred_W` -> exact U/V/W range trace -> `N_M^+=0` -> `norm(e^1)_Q<rho_ALS/3` -> consumers | derived | None |
...
| `step_007c` | Derive the functional actual-record perpendicular ledger; define deterministic `b_0,L_beta` and the a priori fixed point `bar_beta,bar_eta`; export barred U/V/W budgets, both-orientation cross/pair and diagonal-reserve formulas, actual/comparator normalizer lower bounds, and the monotone conditional estimate `err_M^act<=3072widehat_eta+ell_c h_M^act`.  Specify the exact same-current post-solve `Pred_N` record schema retaining current `F_N`, and preserve the exact orthogonal baseline without target-span inputs. | `step_001`, `step_007` | `E_sm`, `E_best`, `E_quotient` (derived); local conditional held record only | trajectory-independent beta closure, same-current comparator schema, rank-free row/column transfer, diagonal reserve, positivity, and common-register cancellation | deterministic scalar fixed point, exact comparator/normalizer identities, paired resolvent, centered normalization, and baseline substitution | `E_FS_apriori`, `E_perp_pair`, `E_carry_resolvent`, `Pred_N` schema | `bar_h_W<2rho_ALS/3`, `bar_Delta_J^M<1/32768`, `bar_d_M<1/16384`, normalizer squares `>1/2`, `ell_c=17/256` | PENDING |
| `step_007b` | Consume `E_FS_apriori`, `E_perp_pair`, and `E_carry_resolvent` in a strict U-to-V-to-W induction.  Before every call display both induced `bar_Delta` bounds, carried row/column diagonal reserves, inverse floor, positive incoming scales, and actual/comparator normalizer margins.  After each exact solve/range output construct and export the full same-current `Pred_U`, `Pred_V`, or `Pred_W` record with actual/comparator `A^pred,nu^pred` and current `F_N`.  Keep exact `h_U^act=0`, `h_V^act=epsilon_U^act`, `h_W^act=epsilon_U^act+epsilon_V^act` distinct from barred budgets.  Only after the staged domains close define `beta_FS^act,eta_FS^act`, sharpen to `3072/3276/3500/4096`, and prove exact zero-padding/range entry with `norm(e^1)_Q<rho_ALS/3`. | `step_001`, `step_007`, `step_007c` | `E_sm`, `E_best`, `E_quotient`, `E_FS_apriori`, `E_perp_pair`, `E_carry_resolvent` (derived), `assump:base-scale` (primitive procedure scale) | staged noncircular domains, actual/envelope chronology, same-current comparator production, diagonal reserve and positivity, shared perpendicular control, and exact range entry | first-exit-free finite induction, actual-record ledger, paired reserve, exact unfolding range, Moore--Penrose block identity, and product-one rebalance | `Pred_U,Pred_V,Pred_W`, `E_pair_entry`, `E_parallel_entry`, active rank-r/zero-padding equivalence | `tau_r^2`, `r tau_r^4`, barred domain constants and sharp actual `3072/3276/3500/4096`, inverse floor, finite one-sweep cost | PENDING |
...
the selected bridge precedes the conditional carried-cancellation interface
`step_007c` (listed before `step_007b` despite the historical suffix), which
precedes the collective first-sweep audit, which precedes
...
The accepted `step_007` decomposition is consumed first by `step_007c`, which
produces the functional perpendicular ledger, deterministic
`E_FS_apriori`, and the parameterized `E_carry_resolvent` without a future
range input.  Internally `step_007b` is ordered
`E_FS_apriori -> U pre-call certificate -> U solve/range -> Pred_U ->
V pre-call certificate -> V solve/range -> Pred_V -> W pre-call certificate
-> W solve/range -> Pred_W -> beta_FS^act/eta_FS^act sharpening`.
Each pre-call certificate includes both induced cross/pair bounds, every
available row/column diagonal reserve, ordinary inverse margin, positive
incoming scale, and actual/comparator normalizer floor.  Exact
`h_U^act,h_V^act,h_W^act` are generated only by preceding records and remain
distinct from barred budgets.  The range trace removes `N_U`, then `N_V`,
then `N_W`.  Only the resulting `E_parallel_entry` is consumed by the target-span
contraction in `step_008`.  Thus target-span membership is a noncircular
generated output, and the false unrestricted-tube interface from attempt 8
...
None.  The triggering step review's repeated defects are repaired at the
sketch/interface level: a deterministic beta/eta fixed point exists before
U; a finite U-to-V-to-W induction proves every conditional domain; each exact
solve emits a genuine same-current actual/comparator predecessor record; both
induced diagonal reserves and positivity margins precede each consumer; and
actual epsilon/h quantities are never rebound to upper envelopes.  The exact
goal, procedure, rates, probability modes, metric, rank, runtime wrapper, and
orthogonal equal-weight baseline are unchanged.  Remaining derivations are
localized to the listed `PENDING` steps; no route to idea generation is
requested.
