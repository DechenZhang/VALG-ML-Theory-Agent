...
This is `perspective_1/idea_4`, sketch attempt 9 (`revise_sketch` after the
archived sketch-008 review).  The realized tensor is
...
\(|\delta_{M,j}|\).  Bounding these three terms separately and using
...
\[
 (1-\Gamma q_*R_t)\bigl(|\delta_{M,j}|+\|E_M(:,j)\|_1+\|F_M(:,j)\|_1\bigr)
...
\(1-\Gamma q_*R_t\ge31/32\), (S1a) implies (S1) with the displayed factor
four.  In particular it controls the \(E_M\) columns as well as the \(F_M\)
columns; it is not an inference from a pair product alone.  This is a
three-mode inversion of the same last-update equation, so no member-local
quantity is promoted to a global row premise.
...
matrix.  Let \(D_M(e)\) be the positive target-coordinate diagonal of the
unit-direction correlation matrix and define
\[
C_M(e):=(M_M^*)^\top M_M(e),\qquad
E_M(e):=C_M(e)-G_M D_M(e),
\]
where \(G_M=(M_M^*)^\top M_M^*\) has diagonal one.  Include the relative
raw scalar-coordinate error \(\delta_M(e):=D_M(e)-I\) separately; the
raw/unit conversion is bounded by \((2\kappa_0)^{\pm1}\) and is absorbed in
the allowed polynomial scale dependence.
...
and column l1 norms, and scalar sup norm after projection to the same
canonical chart.  In A1--A2 we write \(\eta(e,e')\) for
\(\eta_\Delta(e,e')\).
...
\(Y=Y^\circ D_Y\), \(Z=Z^\circ D_Z\), where \(Y^\circ,Z^\circ\) are the
unit-direction matrices from the canonical chart, and set
\(K=Z\odot Y=K^\circ D_K\), \(K^\circ=Z^\circ\odot Y^\circ\),
\(D_K=D_ZD_Y\).  The following diagonal conjugation, rather than a
...
- Mechanism source: the **joint selected-matrix row-small-gain lemma** is the
  ...
  \(H_M=G_M-I\).  The following indexed expansion fixes the residual source.
  Put \(c^-_{M,\ell j}=\langle m_\ell^*,m_j^-\rangle\) for the direction
  correlation immediately before the last simultaneous update, put
  \(d^-_{M,j}=c^-_{M,jj}\), and write
  \[
  \alpha_{M,j}:=d^-_{M',j}d^-_{M'',j}
    +\sum_{a\ne j}{\lambda_a\over\lambda_j}
       G_{M,ja}c^-_{M',aj}c^-_{M'',aj}.
  \]
  The denominator invariant in `step_004` gives
  \(\alpha_{M,j}\ge1-\Gamma q_*R_t\).  For \(\ell\ne j\), let
  \(h_{M,\ell a}=H_M(\ell,a)\) and define the dual-basis residual
  \[
  \begin{aligned}
  r_{M,\ell j}:={}&
   (\alpha_{M,j}^{-1}-1)
       h_{M',\ell j}h_{M'',\ell j}d^-_{M',j}d^-_{M'',j}\\
  &+\alpha_{M,j}^{-1}\left({\lambda_\ell\over\lambda_j}-1\right)
       h_{M',\ell j}h_{M'',\ell j}d^-_{M',j}d^-_{M'',j}\\
  &+\alpha_{M,j}^{-1}
  ...
  \end{aligned} \tag{J0r}
  \]
  This is the coefficient left after contracting the exact last-update normal
  equation with the realized dual basis and moving all off-diagonal selected
  terms to the left.  Thus, entrywise (including the diagonal scale factors),
  \[
  Q_M(\ell,j):={r_{M,\ell j}\over\alpha_{M,j}},\qquad
  F_M=
   (H_{M'}D_{M'})\circ E_{M''}
   +E_{M'}\circ(H_{M''}D_{M''})
   +E_{M'}\circ E_{M''}+Q_M. \tag{J0a}
  \]
  Here the first three terms contain every occurrence of an off-diagonal
  \(E\); \(Q_M\) contains only the displayed denominator, weight, diagonal
  scale, and two-static-Gram terms.  In particular, no \(E_M\)-dependent
  term is hidden in \(Q_M\), and the identity is valid before taking any
  row/column maximum.

  Taking absolute values before summing and using
  \(\|H_M\|_{\rm row,1},\|H_M\|_{\rm col,1}\le q_*\),
  \(\|G_M^{-1}\|_2\le8/7\), and the last-update bounds on the diagonal factors
  gives, independently for rows and columns,
  \[
  \max\left\{\max_j\sum_\ell|r_{M,\ell j}|,
             \max_\ell\sum_j|r_{M,\ell j}|\right\}
   \le 2q_*^2+2q_*^2+4q_*^2.
  \]
  The three charges are, respectively, denominator, weight, and two-static-
  Gram defects; they use no sign cancellation and no selected-row closure.
  Consequently
  \[
  \|Q_M\|_{\rm row,1},\|Q_M\|_{\rm col,1}\le8q_*^2. \tag{J0b}
  \]
  This is a direct normal-equation producer, has zero forcing at \(q=0\),
  and remains valid for an adversarially aligned single row.
  ...
  \]
  The column part is the explicit three-mode inversion consequence of (S1a):
  \[
