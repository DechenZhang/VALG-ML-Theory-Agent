...
normalization/weight budget.  Thus (S1) controls the \(E_M\) columns as well
as the \(F_M\) columns; it is not merely a pair-product statement.
...
- Mechanism source: the **joint selected-matrix row-small-gain lemma** is the
  ...
  \(H_M=G_M-I\).  For each mode, let \(\alpha_{M,j}\) be the signed target
  denominator and let \(r_{M,\ell j}\) be the dual-basis contraction of the
  last-update normal equation after removing the three \(E/H\) terms.  Define
  \[
  Q_M(\ell,j):=\frac{r_{M,\ell j}}{\alpha_{M,j}},\qquad
  F_M=
  E_{M'}\circ H_{M''}+H_{M'}\circ E_{M''}
  +E_{M'}\circ E_{M''}+Q_M. \tag{J0}
  \]
  The residual \(r_{M,\ell j}\) contains only the denominator defect
  \(\alpha_{M,j}^{-1}-1\), the weight defect
  \(\lambda_\ell/\lambda_j-1\), and terms with two static Gram factors.
  Contracting with the realized dual bases charges these pieces by
  \(2q_*^2,2q_*^2,4q_*^2\), respectively, hence
  \[
  \|Q_M\|_{\rm row,1},\|Q_M\|_{\rm col,1}\le8q_*^2.
  \]
  This is a direct normal-equation producer, has zero forcing at \(q=0\),
  and contains no \(E_M\)-dependent term hidden inside \(Q_M\).
  ...
  \]
  The first inequality is the F-to-E column bridge supplied by the definition
  of \(m_i\) and (S1), not an inference from a pair product alone.
