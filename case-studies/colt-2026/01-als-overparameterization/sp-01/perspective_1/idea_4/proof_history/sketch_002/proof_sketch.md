...
This is `perspective_1/idea_4`, sketch attempt 2 (`revise_sketch` after the
archived attempt-1 review).  The realized tensor is
...
`technical_survey.md`, repaired after the attempt-1 review.  The repair fixes
...
\]
gives \(f([0,19/20])\subset[q_*^2,0.922]\),
...
For the target-indexed balanced representatives, let `m_t` be the maximum
transient off-diagonal coefficient and `eta_t` the maximum of its row and
column absolute l1 masses.  The direct mode expansion in `step_007` is fixed
with the numerical inequalities
\[
m_{t+1}\le4m_t^2+4q_*m_t+8q_*^2,
\qquad
\eta_{t+1}\le8q_*^2+8(q_*+m_t)\eta_t.
\]
At the end of the recurrence burn-in, `m_t <= 1/64`, so the coefficient of
`eta_t` is at most `1/4`.  With `eta_{t_b} <= 2r`, the choice `C_0 >= 64`
therefore gives
\[
 m_{L_0}\le16q_*^2,qquad
 \eta_{L_0}\le17q_*^2.
\]
...
\(256(18q_*^2)<2\times10^{-5}\), this is far below the explicit local radius
`1/256` used below.  The decomposition includes separately the diagonal scalar
...
For an active mode-one update write `K=Z odot Y`.  At the exact balanced point,
\[
 G_K=K^T K=(G_Z\circ G_Y),
 \qquad \lambda_{\min}(G_K)\ge1-q_*^2.
\]
Inside the quotient ball of radius `varrho=1/256`, the row/column bound from
`step_007` gives \(\|K-K_*\|_2\le8\eta_{L_0}\), hence
\[
 \|G_K-G_{K_*}\|_2\le16\eta_{L_0}<1/64,
 \qquad \lambda_{\min}(G_K)>15/16.
\]
Consequently the Moore--Penrose branch equals the ordinary inverse throughout
the ball and
\[
 \|G_K^{-1}\|_2\le16/15,qquad
 \|G_K^{-1}-G_{K'}^{-1}\|_2
 \le(16/15)^2\|G_K-G_{K'}\|_2.
\]
...
\(12q_*/(1-q_*^2)<1/300\).  The inverse perturbation bound above and the
second-derivative estimate on the `1/256` ball contribute at most `1/32`.
...
the `1/256` basin condition; and the inverse bounds imply the `nu=1/8`
...
- Mechanism source: write the target-indexed mode-correlation matrices as
  ...
  with diagonal normalization retained, gives the concrete inequalities
  \[
  m_{t+1}\le4m_t^2+4q_*m_t+8q_*^2,\qquad
  \eta_{t+1}\le8q_*^2+8(q_*+m_t)\eta_t,
  \]
  with `m_0 <= 1` and `eta_0 <= 2r`.  The first inequality is supplied by
  `R_t`; after the tenth-sweep burn-in its transient part is at most `1/64`,
  so the coefficient of `eta_t` in the second inequality is at most `1/4`.
  The second inequality then contracts the initial `2r` mass for `C_0 log r`
  sweeps and gives `eta_L <= 17 q_*^2`.  This is the explicit row/column bridge;
...
- Closure/dominance/absorption relation: with `C_0 >= 64`, the transient term
  satisfies `2r 4^{-(L_0-10)} <= q_*^2/128`; solving the displayed small-gain
  recurrence gives `eta_L <= 17q_*^2`.  Therefore the quotient distance is
  at most `256(18q_*^2)<2e-5<1/256`, the radius exported by `step_009`.
...
- Source-to-claim adequacy: the wrapper fixes the source object mapping:
  ...
  `K=Z odot Y`, the exact source Gram is `G_K=G_Z o G_Y` with
  `lambda_min >= 1-q_*^2`; in the proposed ball the perturbation is below
  `1/64`, so the Moore--Penrose update is the ordinary inverse and
  `||G_K^{-1}|| <= 16/15` with the displayed inverse-difference bound.  Work
  in the componentwise balanced quotient, where a common rescaling of a true
  component is removed before measuring distance.  The `1-q_*^2` floor and
  `Gamma <= 1.01` give the explicit scale-invariant radius `varrho_ALS=1/256`
  and contraction `nu=1/8`.  Raw-coordinate normal-equation and arithmetic
  bounds may carry only the allowed polynomial dependence on `kappa_0`.  The
  direct Lipschitz calculation makes the Taylor remainder at most `(1-nu)/2`
  rather than appealing to an unspecified existential neighborhood.
...
- Closure/dominance/absorption relation: inside `varrho_ALS=1/256`, the
  Khatri--Rao Gram has eigenvalue at least `15/16`, the Moore--Penrose branch
  is the ordinary inverse, the inverse perturbation is bounded by
  `(16/15)^2`, and the source block-Gauss--Seidel derivative bound
  `12q_* /(1-q_*^2) < 1/300` plus the second-derivative remainder is at most
  `1/8`.  Thus the normal equations stay nonsingular and the ball is invariant.
...
| Joint representative coefficient row/column mass and best-scalar residual | `step_007` direct Hadamard/small-gain bridge | `R_t`, `m_t`, `eta_t`, symmetric Gram row/column mass, selected clusters | Persistent forcing is `8q_*^2`; transient starts `eta_0<=2r` and contracts with coefficient `<=1/4`; no uncontrolled incoming term remains | `||T-T_rep||_F <=256(eta_L+q_*^2)||T||_F` and quotient distance same scale | Explicit recurrences yield `m_L<=16q_*^2`, `eta_L<=17q_*^2`; `C_0>=64` | `q_*=1/4096` | `step_008`, `step_009` | None |
...
| Explicit local basin and contraction factor | `step_009` current-notation wrapper around Uschmajew Lemma 3.2/Theorems 3.3, 3.5 | Quotient margin, `G_K` inverse bound, derivative Lipschitz bound, representative residual | Taylor remainder and Moore--Penrose branch are controlled inside the ball | Best-scalar active initialization lies in the same quotient ball | `varrho_ALS=1/256`, `lambda_min(G_K)>15/16`, inverse difference `<= (16/15)^2`, derivative plus remainder `<=1/8` | Scale-invariant `1/256`, `nu=1/8`; raw costs may depend poly(`kappa_0`) | `step_010` | None |
