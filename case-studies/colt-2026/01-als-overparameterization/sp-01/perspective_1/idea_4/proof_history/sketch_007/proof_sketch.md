...
This is `perspective_1/idea_4`, sketch attempt 7 (`revise_sketch` after the
...
- Mechanism source: the **joint selected-matrix row-small-gain lemma** is the
  ...
  \(H_M=G_M-I\).  For each mode, the last-update normal equations give a
  residual matrix \(Q_M\) satisfying the deterministic bounds
  \[
  \|Q_M\|_{\rm row,1},\|Q_M\|_{\rm col,1}\le8q_*^2. \tag{J0}
  \]
  This follows by contracting the update with the realized dual bases:
  every term containing one static off-diagonal Gram factor is charged to a
  row or column sum of \(H_M\), and every term containing two such factors is
  charged to \(H_{M'}\circ H_{M''}\); the remaining denominator and weight
  defects are bounded by \(8q_*^2\).  No sign independence or magnitude
  conditioning is used.
  Let \(\rho:=\max_M\|E_M\|_{\rm row,1}\) and
  \(\kappa:=\max_M\|E_M\|_{\rm col,1}\).  Expanding the pair-product equation
  \[
  ...
  \tag{J1}
  \]
  and applying the induced row/column Hadamard inequality gives the
  deterministic small-gain system
  \[
  \kappa\le\chi_{\max},\qquad
  \rho\le4q_*\chi_{\max}+q_*\rho+16q_*^2. \tag{J2}
  \]
  Since \(\chi_{\max}\le56q_*^2\) and \(q_*=1/4096\),
  \[
  \rho\le\frac{4q_*\chi_{\max}+16q_*^2}{1-q_*}
  <17q_*^2.
  \]
  ...
  Define the deterministic generated event \(E_{\rm row}:=\{\rho<17q_*^2\}\).
  Then
  \[
  \eta_{\rm in}^{\rm sel}:=\max\{\rho,\kappa\}\le17q_*^2,\qquad
