...
This is `perspective_1/idea_4`, sketch attempt 1.  The realized tensor is
...
`technical_survey.md`.  It uses the parallel ALS convention from
...
- Source-to-claim adequacy: condition on the target coordinates in
  ...
  bounded by \(q_* t_r\), covariance eigenvalues lie in
  \([1-q_*,1+q_*]\), and the three target windows contribute
  \(\Theta(r^{-5/3}(\log r)^{-3/2})\).  A union bound over the three pair
  families and \(r-1\) competitors leaves the threshold \(b_*\log r\) with
  the required universal comparison constants.
...
- Closure/dominance/absorption relation: choose `C_rank` so
  \(k p_{\rm win}\ge C\log r\); the per-target miss probabilities are then
  summable, yielding a universal positive coverage probability.  This is a
  finite coupon event, not an all-time invariant.
...
- Null or boundary regime tested: `R_0=19/20`, `q=q_*`, `Gamma=1.01` is the
  worst numerical boundary; the denominator remains strictly positive.
- Target conclusion false or theorem-critical obstruction present if source
...
- Mechanism source: direct stationary-equation analysis of the simultaneous
  ...
  `Gamma`.
- Source-to-claim adequacy: a covered slot has score
  \(\sigma_i\ge(1-Cq_*)\lambda_j\) and displacement below `16q_*`.  Conversely,
  any final proposal with displacement at most `64q_*` and score at least
  `0.85 sigma_max` is shown by the fixed-point equations to have one dominant
  target coordinate; a multi-support stationary branch has score at most
  \((1/\sqrt2+Cq_*)\lambda_{\max}\).  The numerical margins leave the
  `0.85` filter gap.  The angle bound from `step_005` gives graph edges within
  a target and excludes edges across targets using the realized Gram row sum.
...
- Closure/dominance/absorption relation: the gap dominates all defects because
  `q_*` is fixed at `1/4096`; connected components merge same-target proposals
  and cannot merge distinct targets. Coverage from `step_003` guarantees every
  target has at least one retained member.
...
- Producer-consumer provenance: `step_005` + observable filter -> `step_006`
  -> `step_007`.
- Null or boundary regime tested: score ties and graph threshold equality are
  resolved by the stated largest-score representative rule; the proof uses
  strict margins before tie-breaking.
- Target conclusion false or theorem-critical obstruction present if source
...
- Mechanism source: write the target-indexed mode-correlation matrices as
  ...
  matrices.  Let `m_t` be the maximum entry of the transient `F_M` matrices
  and let `eta_t` be their maximum row/column l1 mass.  The direct expansion
  gives the two coupled, target-indexed inequalities
  \[
  m_{t+1}\le C_m(m_t^2+q m_t),\qquad
  \eta_{t+1}\le C_\eta\{q^2+(q+m_t)\eta_t\},
  \]
  with `m_0 <= 1` and `eta_0 <= C r`.  The first inequality is supplied by
  `R_t`; after the universal `R_t` burn-in choose the numerical split so that
  `C_eta(q+m_t) <= 1/2`.  The second inequality then contracts the initial
  `C r` mass for `C_0 log r` sweeps, and its fixed point is
  `eta <= 2 C_eta q^2`.  This is the explicit row/column bridge;
...
- Residual-to-target adequacy: after sign alignment and quotient rescaling,
  ...
  balanced CP factors by `O(eta+q_*^2)` in the operator/quotient norm.  The
  tensor residual decomposition has target term, row/column leakage term, and
  Gram term; each is bounded by the same row/column inequality, so the produced
  object is the same target tensor, not a surrogate.
- Key positive/control term or structural source: symmetric Gram row/column
  mass and the contractive transient `r R_t^{tr}`.
...
- Closure/dominance/absorption relation: choose `C_0` so the transient is
  below `q_*^2`; solve the scalar small-gain inequality for `eta` with
  `q_*=1/4096`.  The resulting quotient distance is below the radius exported
  by `step_009`.
...
- Theorem role: discharge the local ALS source assumption at the selected
  exact decomposition.
- Mechanism source: `step_001` gives full column rank and Khatri--Rao Gram
  floors.  Construct dual vectors for each realized factor matrix.  If a
  tangent perturbation of the CP factors has zero tensor derivative, contract
  with the dual vectors in two modes to isolate each component; only the three
  componentwise scaling directions remain.
- Source-to-claim adequacy: this direct dual-basis argument exactly matches
  Uschmajew Assumption 1 (Hessian kernel equals the `2r`-dimensional scaling
  tangent).  The source object is the same exact CP decomposition and the
  same Frobenius least-squares loss; no equilibrated or whitened surrogate is
  consumed.  The Khatri--Rao spectral floor supplies the positive margin.
...
- Closure/dominance/absorption relation: quotient out the exact scaling kernel;
  the remaining eigenvalues retain a fixed positive margin for `q_*<1`.
...
- Mechanism source: Uschmajew Lemma 3.2 (local differentiability and
  block-Gauss--Seidel derivative) and Theorems 3.3/3.5 (local linear ALS under
  the quotient-Hessian kernel condition), wrapped by a direct current-
  notation bound on the block normal equations.
- Source-to-claim adequacy: the wrapper fixes the source object mapping:
  ...
  held fixed and therefore does not alter the active normal equations.  Work
  in the componentwise balanced quotient, where a common rescaling of a true
  component is removed before measuring distance.  The `1-q_*^2` Khatri--Rao
  floor and the weight ratio `Gamma <= 1.01` then give an explicit
  scale-invariant radius
  \(\varrho_{\rm ALS}=c_{\rm ALS}(1-q_*^2)^2/(1+\Gamma)^d\) and
  \(\nu<1\); raw-coordinate normal-equation and arithmetic bounds may carry
  only the allowed polynomial dependence on `kappa_0`.  The direct Lipschitz
  calculation makes the Taylor remainder at most `(1-nu)/2` rather than
  appealing to an unspecified existential neighborhood.
...
- Closure/dominance/absorption relation: the radius is chosen so the normal
  equations stay nonsingular, the Taylor remainder is absorbed by the
  quotient margin, and the block-Gauss--Seidel derivative has norm at most
  `nu<1` throughout the ball.
...
- Entry-state / first-update stress result: the first active mode update is a
  well-defined inverse on the `1-q_*^2` spectral floor and maps the radius to
  at most `nu` times the error plus the absorbed quadratic remainder.
...
| Per-target window event and `R_0 <= 19/20`, `S_0 <= r R_0` | `step_002` | Correlated Gaussian regression, target window, competitor pair tails | Conditional means/covariances and finite competitor union are controlled | Produced raw event maps exactly to target-relative ratios in `setting.md` | `b_*/a_*=19/20` and `b_*>1` | `step_003`, `step_004` | None |
| All-target coverage at rank `U(r)` | `step_003` | Independent slots and per-target event probability | Slot misses are independent across slots; no dynamic claim is hidden | Coverage event is consumed only by the proposal recurrence | `k p >= C log r` gives a universal positive one-run probability | `step_004` | None |
| Finite-horizon `R_t,S_t` invariant and numerical floor | `step_004` direct expansion | `q_real`, `Gamma`, entry margins | Gram leakage is additive and denominator-controlled; no cyclic reuse | Same target-relative state is consumed by `step_005` | Interval invariance, dissipative map, and bounded fixed point | `R_0 <= 19/20`, `q_*`, `Gamma <=1.01` | `step_005`, `step_007` | None |
...
| One data-driven cluster per target | `step_006` | Observable score/displacement, angle certificate, stationary equations | Mixture branches, Gram leakage, and ties are controlled by the constant score gap | Representatives remain in the exact target direction metric | `1-1/sqrt(2)` dominates `O(q_*)`; graph threshold separates targets | `0.85`, `64q_*`, and row-sum `q_*` | `step_007` | None |
| Joint representative coefficient row/column mass and best-scalar residual | `step_007` direct Hadamard/small-gain bridge | `R_t` transient, symmetric Gram row/column mass, selected clusters | Persistent Gram forcing is `O(q^2)`; transient is `r R_t` and decays; no uncontrolled incoming term remains | Explicit same-target quotient and Frobenius residual bridge | Two-sided l1 Hadamard inequality and small-gain fixed point | `q_*=1/4096`, chosen `C_0` | `step_008`, `step_009` | None |
...
| Explicit local basin and contraction factor | `step_009` current-notation wrapper around Uschmajew Lemma 3.2/Theorems 3.3, 3.5 | Quotient margin, normal-equation inverse, derivative Lipschitz bound, representative residual | Taylor remainder and pseudoinverse branch are controlled inside the ball | Best-scalar active initialization lies in the same quotient ball | Choose radius so remainder is absorbed and derivative norm is `<1` | `varrho_ALS`, `nu<1` derived from `q_*` and `kappa_0` | `step_010` | None |
...
| Joint coefficient row/column mass and same-target residual | `step_007` Hadamard/small-gain derivation | `step_008`, `step_009` | Explicit basin-entry interface | `step_004`/`step_006` -> `step_007` -> local geometry | derived | None |
...
| Explicit basin and contraction factor | `step_009` current-notation local wrapper | `step_010` | Basin preservation and local refinement | `step_007`/`step_008` -> `step_009` -> contraction | derived | None |
