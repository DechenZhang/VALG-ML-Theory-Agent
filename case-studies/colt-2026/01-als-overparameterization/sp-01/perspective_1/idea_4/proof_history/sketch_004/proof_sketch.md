...
This is `perspective_1/idea_4`, sketch attempt 4 (`revise_sketch` after the
...
`technical_survey.md`, repaired after the archived review.  The repair fixes
 the Gaussian tail constants, the numerical score gap, the selected-member
 certificate transfer, the row/column-controlled reachable tube, the
 post-update coefficient arithmetic, the diagonal inverse/pseudoinverse branch,
...
member.  The following deterministic transfer lemma attaches the certificate
to every member that the data-only rule can select.  If `i_cov(j)` is the
covered member for target `j`, the recurrence and contraction expansion give
the sharper score bound `sigma_i_cov(j) >= (1-8 q_*^2) lambda_j`.  If `i` is
any retained member in the same target chart, the direct last-update
calculation gives
\[
 d_i+m_i+eta_i \le 4(1-sigma_i/lambda_j)+24q_*^2. \tag{S1}
\]
...
\[
 d_i\le32q_*^2,\qquad m_i\le32q_*^2,\qquad eta_i\le32q_*^2. \tag{S2}
\]
Thus the selected representative, rather than merely an existential covered
proposal, inherits the row/column certificate consumed by `step_007`.  A
cross-target member cannot enter the cluster by the strict graph separation in
`step_006`; ties are resolved only after (S1) is applied to every target-chart
member.
...
`m_10 <= 2.3e-6 < 1/64`; the normalized-column bound gives `eta_10 <= 2r`.
...
factor bases.  The selected initialization satisfies `eta <=32q_*^2` by
(S2).  For every `e` in this tube, the Hadamard row/column estimate gives
...
`||Phi_cyc(e)||_col <= (1/16)||e||_col+32q_*^2` and
...
`64q_*^2`, far below `1/256`.  Thus the contraction is a full reachable-tube
...
- Source-to-claim adequacy: the perturbative stationary-support lemma is
  ...
  realized Gram row sum.  The same lemma includes the observable selected-
  member transfer (S1)--(S2): every retained target-chart member, not only the
  covered trajectory, has `d_i,m_i,eta_i <= 32q_*^2`; hence the largest-score
  representative inherits the certificate.
- Residual-to-target adequacy: the produced graph representative is compared
  ...
  metric; (S2) supplies its row/column mass and same-target residual inputs,
  with error below the later basin radius.
...
- Closure/dominance/absorption relation: the displayed numerical gap is
  ...
  the selected-member transfer without using a hidden target label.
...
- Mechanism source: write the target-indexed mode-correlation matrices as
  ...
  `m_10 <= 2.3e-6 < 1/64`, and the normalized-column bound gives
...
- Noncircular closure status: `step_004` produces `R_t` for covered slots;
  the observable transfer (S1)--(S2) in `step_006` produces the same
  post-update certificate for every selected member; the row/column inequality
  then produces `eta`, and the local basin in `step_009` consumes `eta`.
...
- Source-to-claim adequacy: the wrapper fixes the source object mapping:
  ...
  `kappa_0`.  Work in the componentwise balanced quotient with the row/column
  mass `eta`; the normalized `1-q_*^2` floor and `Gamma <= 1.01` give the
  explicit tube `||e||_col<=1/512, eta<=1/256` and contraction `nu=1/8`.
  The derivative/remainder inequality holds pairwise for every two points in
  this tube, not only at the initialization.
...
| Correct graph clusters and representatives | `step_006` stationary score-gap plus observable selected-member transfer | `step_007` | Active rank-`r` initialization object with the covered-trajectory certificate | `step_005` -> `step_006` -> (S1)-(S2) -> joint matrix bridge | derived | None |
| Joint coefficient row/column mass and same-target residual | `step_007` post-first-update indexed Hadamard/small-gain derivation, seeded by (S2) for each selected member | `step_008`, `step_009` | Explicit basin-entry interface | `step_004`/`step_006` -> selected-member `m_s`/`eta_s` -> `step_007` -> local geometry | derived | None |
...
| `step_006` | Prove high-score/small-displacement proposals are target-near, establish the component/mixture score gap, certify one graph cluster per target, and transfer the covered trajectory certificate to every retained target-chart member via (S1)-(S2). | `step_005` | Derived `E_ang`, `E_sm`; observable filter definition | Perturbative stationary-point classification and observable-to-certificate transfer | Orthogonal support classification, direct `q`-perturbation, and last-update residual identity | `E_cluster`, selected representatives, `d_i,m_i,eta_i <=32q_*^2` | Objective 3: data-driven membership | PENDING |
| `step_007` | Bound the target-indexed selected-representative coefficient matrix in both row and column l1 mass, and transfer best-scalar representatives to an explicit same-target quotient residual. | `step_004`, `step_006` | Derived `E_RS`, `E_cluster`, selected-member (S2), `E_sm` | Incoming cross-target leakage and `r`-uniform accumulation | Post-update Hadamard row/column inequality and small-gain recurrence | `eta <= C q_*^2`, quotient distance/residual certificate for the actual selected factors | Objective 3: joint basin bridge | PENDING |
