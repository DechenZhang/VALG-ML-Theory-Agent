...
- Current idea: `idea_4`, proof-sketch attempt 6 after the archived
...
- Branch notes: This framework is selected because it repairs only the failed proposal update interface. The orthogonal equal-weight specialization remains exact: `(A^+, B^+, C^+) = (BC, AC, AB)`, `R^+ <= R^2`, and `S^+ <= R S`. Attempt 6 makes the global row/column source an explicit conditional-sign concentration lemma, defines all selected coefficient matrices and their induced norms, fixes a unique equal-norm quotient gauge, and derives the selected residual and ALS contraction in that same tube norm. The row/column bridge is the required new interface for the active joint refinement and is not treated as an unproved generated condition.
