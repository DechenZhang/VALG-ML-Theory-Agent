...
This is `perspective_1/idea_4`, sketch attempt 6 (`revise_sketch` after the
...
4. angular/displacement plus stationary-point classification -> score filter and
   graph clusters;
5. selected representatives -> member-local coefficients, then a conditional-sign
   row/column concentration lemma in `step_007` -> a joint coefficient-matrix
...
between different target labels is assumed or needed.  Intersecting with the
conditional-sign row event from `step_007` costs at most \(12r^{-6}\), so for
all \(r\ge3\)
\[
\Pr(E_{\rm cov}\cap\mathcal E_{\rm row}\mid E_{\rm sm})\ge p_0:=1/4.
\]
This conservative constant is the one used by the restart bridge; the rank
constant remains independent of either confidence parameter.
...
Let \(D_M=\operatorname{diag}(C_M)\) after sign alignment and put
\[
 \delta_{M,j}:=D_M(j,j)-1,\qquad E_M:=C_M-G_MD_M.
\]
...
\[
 ...
 F_M:=P_M-P_M^*D_{M'}D_{M''}.
\]
Define the member-local off-diagonal coefficient error and its score proxy by
\[
 m_i:=\max_{M}\left\{|\delta_{M,j}|,
       \sum_{\ell\ne j}|F_M(\ell,j)|\right\},\qquad
 \chi_i:=d_i+m_i.
\]
...
in the same target chart, the direct last-update calculation gives
\[
...
constants; it is not substituted for the selected-matrix seed (S5).  For the
...
B_Z(j,j)=g_j/c_j.
\]
Then \(B_XB_YB_Z=I\) and all three gauged column norms equal \(g_j\).
This equal-norm rule is the unique positive gauge satisfying product one and
equal norms; its logarithmic Jacobian is nonsingular in `T_ALS`, and
\[
\max_M\|B_M(e)-B_M(e')\|_\infty
\le4\|e-e'\|_{\rm col} \tag{G1}
\]
throughout the tube.  Thus the pairwise chart does not depend on an
existential gauge.
Writing \(M_M(e)\) for the gauged mode matrix and \(M_M^*\) for the exact
realized factor matrix, let \(D_M(e)\) be the positive target-coordinate
diagonal of the correlation matrix and define
\[
C_M(e):=(M_M^*)^\top M_M(e),\qquad
E_M(e):=C_M(e)-G_M D_M(e),
\]
where \(G_M=(M_M^*)^\top M_M^*\).  Include the relative
scalar-coordinate error \(\delta_M(e):=D_M(e)-I\) separately.
...
\|e\|_{\rm col}:=\max\{\max_{M,j}\|M_M(e)_{:j}-M_M^*{}_{:j}\|_2,
                         \max_M\|\delta_M(e)\|_\infty\},
\]
\[
\eta(e):=\max\{\max_{M\in\{X,Y,Z\}}\{\|E_M(e)\|_1,\|E_M(e)\|_\infty\},
                    \max_M\|\delta_M(e)\|_\infty\},
\qquad
...
\max_M\|\delta_M(e)-\delta_M(e')\|_\infty\right\}.
\]
...
\|e_{\rm sel}\|_T\le128q_*^2<1/512. \tag{G2}
\]
...
`T_ALS := { ||e||_col <= 1/512, eta(e) <= 1/256 }`, where `eta(e)` is the
maximum row and column l1 mass of the coefficient error matrices in the true
factor bases.  The selected initialization satisfies (S5), in fact
...
\|b_X(e)-b_X(e')\|_T
\le\left(\frac1{300}
+4\|e-e'\|_{\rm col}+4\eta(e,e')\right)\|e-e'\|_T,
\]
\[
...
\tag{A2}
\]
The same estimates hold cyclically.  Summing the three block updates and
absorbing the scalar and gauge components yields the displayed pairwise
coefficient; at the tube boundary it is
\(1/300+8(2/256+2/512)=0.0971<1/8\).  This is a direct normal-equation
remainder proof; the cited local theorem is used only after (A1)--(A2).
...
1/log(1/(1-p_0))`, and `m >= 8 log(2^12/epsilon)` active sweeps.  The
  ...
  conditional-sign bounds imply `E_cov`, `E_row`, `E_ang`,
  and `E_cluster`; the row/column inequalities imply
...
- Source-to-claim adequacy: the perturbative stationary-support lemma is
  ...
  realized Gram row sum.  The same lemma includes the observable selected-member
  transfer (S1)--(S2): every retained target-chart member has the per-column
  bound `chi_i=d_i+m_i<=56q_*^2`; it does not assert a global row mass.
  The separate selection/Hadamard lemma (S3)--(S5) then supplies
  `eta_{\rm in}^{sel}<=17q_*^2` and
  `eta_{\rm sel}<=80q_*^2` for the largest-score representatives.
- Residual-to-target adequacy: the produced graph representative is compared
  ...
  metric; (S2) supplies its per-column input and (S3)--(S5) supply the global
  row/column mass and same-target residual input, with error below the tube
  radius.
...
- Closure/dominance/absorption relation: the displayed numerical gap is
  ...
  the per-column transfer without using a hidden target label.  The induced
  row/column bound is then obtained independently from (S3), before any
  target-index sum.
...
- Mechanism source: the **joint selected-matrix lemma** is the sole producer of
  the global row/column output.  After the proof-only permutation and sign map
  from `step_006`, use the matrices \(G_M,C_M,D_M,E_M,P_M,P_M^*,F_M\) and
  the member coefficient \(m_i\) defined above.  Put
  \(H_M=G_M-I\), so both induced norms of \(H_M\) are at most \(q_*\).
  Conditional on the sigma-field generated by target-coordinate magnitudes,
  scores, displacements, and graph membership, the off-target Gaussian signs
  remain independent Rademacher variables: every selection observable is
  invariant under these signs.  Define the row-source event
  \(\mathcal E_{\rm row}\) by the three inequalities
  \[
  \max_M\{\|F_M\|_{\rm row,1},\|F_M\|_{\rm col,1}\}
  \le4q_*\chi_{\max}+16q_*^2. \tag{J1}
  \]
  The column part follows from the definition of \(m_i\); the row part is
  an independent Bernstein calculation on the conditional Rademacher sums.
  The deterministic square-sum bound used in that calculation is
  \[
  \max_{M,a}\sum_j |F_M(a,j)|^2
  \le 4q_*^2\chi_{\max}^2+16q_*^4,
  \]
  obtained by expanding
  \[
  (I+H_{M'}+E_{M'})\circ(I+H_{M''}+E_{M''})
 -(I+H_{M'})\circ(I+H_{M''})
 =E_{M'}\circ H_{M''}+H_{M'}\circ E_{M''}
  +E_{M'}\circ E_{M''}.
  \]
  Bernstein plus a union bound over the three modes and \(r\) rows gives
  \(\Pr(\mathcal E_{\rm row}^c\mid E_{\rm sm},E_{\rm cov},E_{\rm cluster})
  \le12r^{-6}\).  This source controls aligned selected columns and is not
  inferred from their individual \(m_i\) bounds.
  For covered trajectories, the separate audit recurrence remains
  \[
  m_{s+1}^{\rm cov}\le4R_s+8q_*^2,\qquad
  \eta_{s+1}^{\rm cov}\le8q_*^2+
  ...
  On \(\mathcal E_{\rm row}\),
  \[
  \eta_{\rm in}^{\rm sel}:=
  \max_M\{\|F_M\|_{\rm row,1},\|F_M\|_{\rm col,1}\}
  \le4q_*\chi_{\max}+16q_*^2\le17q_*^2,
  \qquad
  \eta_{\rm sel}:=\chi_{\max}+\eta_{\rm in}^{\rm sel}\le80q_*^2. \tag{J2}
  \]
- Source-to-claim adequacy: the joint selected-matrix lemma is a direct
  current-notation conditional-sign derivation.  Its row source is the
  Rademacher/Bernstein event (J1), while its column source is the explicitly
  defined `m_i`; the two controls are proved separately before any
  target-index sum.  The covered `S_t` recurrence is used only for the
  finite proposal audit and never as a global row premise.
- Residual-to-target adequacy: after the permutation, sign alignment, and
  ...
  row and column bounds from (J1)--(J2) therefore control every column,
  every induced row/column norm of \(E_M\), and every scalar delta in
  \({bt}norm_T`.  The four residual contributions are bounded separately:
...
- Key positive/control term or structural source: the symmetric Gram
  ...
  the conditional-sign row-source event (J1); the global incoming row mass is
  controlled before summing selected labels.
...
- Closure/dominance/absorption relation: with `C_0 >= 64`, the covered-slot
  transient satisfies `2r 4^{-(L_0-10)} <= q_*^2/128`.  On the
  independently generated event (J1), (J2) gives `eta_sel <=80q_*^2`;
...
- Noncircular closure status: `step_004` produces `R_t` for covered slots;
  ...
  step alone produces the conditional-sign event (J1) and the global
  `eta_sel` in (J2); the local basin in `step_009` consumes that
...
- Repair route if source is unsupported: tighten the conditional Bernstein
  budget in (J1); no new primitive assumption or protocol change is needed.
...
- Source-to-claim adequacy: the wrapper fixes the source object mapping:
  ...
  every point in the reachable tube `T_ALS`, write `K=K^circ D_K`; the
  normalized Gram has `lambda_min(G_Kcirc)>=7/8` and inverse difference at most
  `(8/7)^2` times the Gram difference.  The raw Moore--Penrose update is
  conjugate by `D_K^(-1)`, whose norm and inverse norm are polynomial in
  `kappa_0`.  Work in the explicitly defined componentwise balanced norm
  `||e||_T=max{||e||_col,eta(e)}`; the normalized `1-q_*^2` floor and
  `Gamma <= 1.01` give the explicit tube `||e||_col<=1/512,
  eta<=1/256` and contraction `nu=1/8`.  The gauge map satisfies
  (G1), the normal-equation differences satisfy (A1)--(A2), and the
  derivative/remainder inequality therefore holds pairwise for every two
  points in this tube.
- Residual-to-target adequacy: `step_007` exports (G2) component by component:
  the selected direction columns, every row and column norm of `E_M`, and all
  scalar deltas are bounded in the canonical equal-norm gauge.  Hence the
  quotient distance is at most `256(81q_*^2)=1.23597e-3<1/512` after
  best-scalar balancing, so the actual active initialization lies in `T_ALS`
  (direct tube entry, without a radius-quarter claim).  The tensor residual
  and parameter distance are related by the same local Jacobian margin, with
  no error floor.
...
- Closure/dominance/absorption relation: for every `e` in `T_ALS`, the
  ...
  pairwise tube inequality is at most `1/8`; the separate column and eta
...
| Joint representative coefficient row/column mass and best-scalar residual | `step_007` direct selection/Hadamard bridge | Per-member `chi_i`, global `eta_in^sel`, `eta_sel`, plus covered `m_s^cov,eta_s^cov` recurrence | Score ordering controls columns only; (S3) controls incoming row/column mass before target summation; covered transient has forcing `8q_*^2` | Frobenius residual relative bound uses `256(eta_sel+q_*^2)`; quotient distance `1.23597e-3<1/512` | `chi_max<=56q_*^2`, `eta_in^sel<=17q_*^2`, `eta_sel<=80q_*^2`; `C_0>=64` for covered transient | `q_*=1/4096` | `step_008`, `step_009` | None |
...
| Correct graph clusters and representatives | `step_006` stationary score-gap plus observable selected-member transfer | `step_007` | Active rank-`r` initialization object with per-column certificate | `step_005` -> `step_006` -> (S1)-(S2) -> (S3)-(S5) joint matrix bridge | derived | None |
| Joint coefficient row/column mass and same-target residual | `step_007` post-selection Hadamard bridge plus covered Q1 audit | `step_008`, `step_009` | Explicit basin-entry interface | `step_004`/`step_006` -> `chi_max` and `eta_in^sel` -> `step_007` -> local geometry | derived | None |
...
| `step_006` | Prove high-score/small-displacement proposals are target-near, establish the component/mixture score gap, certify one graph cluster per target, transfer per-column selected-member bounds via (S1)-(S2), and invoke the independent selection/Hadamard interface (S3)-(S5). | `step_005` | Derived `E_ang`, `E_sm`; observable filter definition | Perturbative stationary-point classification and separation of member-local versus global errors | Orthogonal support classification, direct `q`-perturbation, last-update identity, and induced Hadamard norms | `E_cluster`, `chi_i<=56q_*^2`, `eta_in^sel<=17q_*^2` | Objective 3: data-driven membership | PENDING |
| `step_007` | Bound the target-indexed selected-representative coefficient matrix in both row and column l1 mass using (S3)-(S5), and transfer best-scalar representatives to an explicit same-target quotient residual. | `step_004`, `step_006` | Derived `E_RS`, `E_cluster`, selected-member (S2)-(S5), `E_sm` | Incoming cross-target leakage and `r`-uniform accumulation | Independent selection/Hadamard row/column inequality plus covered-trajectory Q1 audit | `eta_sel<=80q_*^2`, quotient distance <=1.23597e-3<1/512, residual certificate for actual selected factors | Objective 3: joint basin bridge | PENDING |
