...
- Current idea: `idea_4`, proof-sketch attempt 10 after the archived
...
- Branch notes: This framework is selected because it repairs only the failed proposal update interface. The orthogonal equal-weight specialization remains exact: `(A^+, B^+, C^+) = (BC, AC, AB)`, `R^+ <= R^2`, and `S^+ <= R S`. Attempt 10 keeps the deterministic row-small-gain route, but separates the exact algebraic pair residual from the E-left last-update equation: the former is (J0a), while the latter defines the nontrivial dual residual and its induced-l1 budget in (J0b). The three-mode S1 inversion controls direction and coefficient columns together; raw amplitudes use a distinct diagonal from unit correlations, and the local tube is narrowed to `||e||_col<=1/4096, eta<=1/2048` so the full A1-A2 boundary arithmetic certifies `nu=1/8`. The row/column bridge is the required new interface for the active joint refinement and is not treated as an unproved generated condition.
