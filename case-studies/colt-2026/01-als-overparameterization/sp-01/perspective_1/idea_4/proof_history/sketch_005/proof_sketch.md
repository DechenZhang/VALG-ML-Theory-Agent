...
This is `perspective_1/idea_4`, sketch attempt 5 (`revise_sketch` after the
...
member.  The following transfer is deliberately split into a member-local
column statement and a matrix-level incoming-row statement.  If `i_cov(j)` is
the covered member for target `j`, the recurrence and contraction expansion
give the sharper score bound `sigma_i_cov(j) >= (1-8 q_*^2) lambda_j`.  If
`i` is any retained member in the same target chart, the direct last-update
calculation gives
\[
...
For the joint selected matrix, let \(\widehat M\) be the selected, sign-aligned
direction matrix in mode \(M\), let \(G_M\) be the true realized Gram, and let
\(D_M\) be the positive diagonal of selected target-coordinate scalars.  Write
\(\widehat G_M=G_MD_M+E_M\).  For each updated mode, let
\(\mathcal F_M\) denote the residual in the pair-product coefficient matrix
formed from the other two modes.  The independent selection/Hadamard lemma
applies to these three matrices, not to a single member.  Using
\(\|G_M-I\|_{\rm row},\|G_M-I\|_{\rm col}\le q_*\), the column bounds (S2),
and the identity
\[
 (I+H_1+E_1)\circ(I+H_2+E_2)
 -(I+H_1)\circ(I+H_2)
 =E_1\circ H_2+H_1\circ E_2+E_1\circ E_2,
 \tag{S3}
\]
the induced row and column norms obey
\[
 \eta_{\rm in}^{\rm sel}:=
 \max_{M}\{\|\mathcal F_M\|_1,\|\mathcal F_M\|_\infty\}
 \le4q_*\chi_{\max}+16q_*^2
 \le17q_*^2,\qquad
 \chi_{\max}:=\max_j\chi_j\le56q_*^2. \tag{S4}
\]
The estimate is made before summing target labels: the Hadamard row/column
norm inequality controls the incoming mass and prevents an \(r\)-factor.  It
therefore remains valid when all selected column errors align in one row.
Define the total selected coefficient mass
\[
 \eta_{\rm sel}:=\chi_{\max}+\eta_{\rm in}^{\rm sel}
 \le73q_*^2\le80q_*^2. \tag{S5}
\]
This is the joint theorem-facing seed for `step_007`; it is independent of the
covered-slot transient recurrence and is attached to the data-selected
representatives.
...
The quotient chart and tube norm used below are fixed explicitly.  For each
iterate choose the unique nearby positive diagonal gauge
\(D_X,D_Y,D_Z\), with \(D_XD_YD_Z=I\), that makes every target coordinate
positive; the observable third-mode sign is fixed before this gauge choice.
Writing \(M_M(e)\) for the gauged mode matrix and \(M_M^*\) for the exact
realized factor matrix, let \(D_M(e)\) be its positive target-coordinate
diagonal and define
\[
...
\]
For a pair \(e,e'\), the same canonical gauge is used and
\(\|e-e'\|_T\) is defined by replacing \(E_M(e)\) and the column differences
by their pairwise differences.  Thus the norm controls the per-column
quotient error, both incoming and outgoing coefficient mass, and the scalar
coordinates in one fixed chart; it is not an existential norm.
...
- Mechanism source: write the target-indexed mode-correlation matrices as
  ...
  term.  For a covered trajectory, index the transient after each simultaneous
  update as `m_s^cov`, `eta_s^cov`; the direct expansion gives
  \[
  m_{s+1}^{\rm cov}\le4R_s+8q_*^2,\qquad
  \eta_{s+1}^{\rm cov}\le8q_*^2+
  8(q_*+m_s^{\rm cov})\eta_s^{\rm cov}.
  \]
  At `s=9`, `m_10^cov <= 2.3e-6 < 1/64`, and for `s >= 10`
  this gives `eta_{L_0}^cov <=17q_*^2`.  This covered-slot recurrence is
  not used as the selected-matrix seed.  Instead, apply the independent
  selection/Hadamard expansion (S3) to the three pair-product coefficient
  matrices.  It gives
  \[
  \eta_{\rm in}^{\rm sel}\le4q_*\chi_{\max}+16q_*^2\le17q_*^2,
  \qquad
  \eta_{\rm sel}\le80q_*^2,
  \]
  before any target-index sum, so incoming mass cannot acquire an `r` factor.
...
- Residual-to-target adequacy: after sign alignment and quotient rescaling,
  ...
  balanced CP factors by at most `256(eta_sel+q_*^2)` in the operator/quotient
...
- Key positive/control term or structural source: the symmetric Gram
  ...
  the selection/Hadamard bound (S4); the global incoming row mass is controlled
  before summing selected labels.
...
- Closure/dominance/absorption relation: with `C_0 >= 64`, the covered-slot
  transient satisfies `2r 4^{-(L_0-10)} <= q_*^2/128`.  Independently,
  (S4)--(S5) give `eta_sel <=80q_*^2`; hence the selected quotient distance is
...
- Noncircular closure status: `step_004` produces `R_t` for covered slots;
  (S1)--(S2) in `step_006` produce per-column `chi_i`, while the
  independent Hadamard selection lemma (S3)--(S5) produces global `eta_sel`.
  The local basin in `step_009` consumes that output; no basin property is
  used to prove it.
...
- Source-to-claim adequacy: the wrapper fixes the source object mapping:
  ...
  eta<=1/256` and contraction `nu=1/8`.  The derivative/remainder
  inequality holds pairwise for every two points in this tube, not only at
  initialization.
- Residual-to-target adequacy: `step_007` gives quotient distance at most
  `256(81q_*^2)=1.23597e-3<1/512` after best-scalar balancing, so the
  actual active initialization lies in `T_ALS` (direct tube entry, without
  a radius-quarter claim).  The tensor residual and parameter distance are
...
- Entry-state / first-update stress result: the first active mode update is a
  well-defined inverse on the tube's `7/8` normalized Gram floor and maps both
  the column error and row/column mass by the displayed `1/16` recurrence plus
  `32q_*^2`; the diagonal scale conjugation preserves the same quotient target.
