...
- Current idea: `idea_4`, proof-sketch attempt 5 after the archived
...
- Branch notes: This framework is selected because it repairs only the failed proposal update interface. The orthogonal equal-weight specialization remains exact: `(A^+, B^+, C^+) = (BC, AC, AB)`, `R^+ <= R^2`, and `S^+ <= R S`. Attempt 5 additionally separates the selected-member per-column certificate from the global incoming row/column mass, proves the latter with an independent Hadamard selection lemma, defines the reachable tube norm, and attaches the corrected direct-entry residual to the actual data-selected factors. The row/column bridge is the required new interface for the active joint refinement and is not treated as an unproved generated condition.
