...
Scalar Gaussian Chernoff bound. If $Z\sim {\cal N}(0,\sigma^2)$,
   then
...
Chi-square upper tail. For $z\sim {\cal N}(0,I_n)$,
   \[
...
Independent Gaussian bilinear tail. For independent
   $z,z'\sim {\cal N}(0,I_n)$,
   \[
   ...
   Assumption~\ref{assump:gaussian-smoothing}.
Khatri--Rao Gram identity and spectral comparison. For unit-column
   matrices $A,B$,
