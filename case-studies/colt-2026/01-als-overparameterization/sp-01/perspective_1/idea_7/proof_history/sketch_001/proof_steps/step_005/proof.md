...
below are proof-only functions of a stored observable slot.
...
\bar r_{M,a}:=\alpha_{M,a}r_{M,a},
\]

where $(\alpha_{U,a},\alpha_{V,a},\alpha_{W,a})=(s_a,1,1)$, gives the
...
replaced by $\sigma_a$, the only additional term is
$(d_{M,a}/\sigma_a-1)B_{M,a}/d_{M,a}$, whose $\ell_1$ norm is at most
...
\tag{20}
\]
Consequently, with the observable $1/8$-threshold sets
\[
...
slot and every (1/8)-threshold/tail set, while retaining the unnormalized
...
- The selector (w), threshold sets (T_{M,a}), high pool \(\mathcal H\),
  and witness index (j) are proof-local.  The high pool and threshold are
  observable score/inner-product rules; witness indices are used only in the
  coverage-floor proof and are not supplied to the procedure.
