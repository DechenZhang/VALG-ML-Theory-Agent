...
| unit_003 | lemma | Under E_land_pre, normalized outputs have target-span coefficient fields \(C_M^{\rm land}\) with both induced masses and every Euclidean column error at most \(2\beta/(1-\beta)\). | Directional quotient entry. |
...
1. Accepted step_009 pre-solve reserve: in current notation it supplies
   \[
   \widetilde X_M
...
**Lemma (normalized direction fields; label lem:step-010-directions).** Under
unit_002, define \(D_{a,M}:={\rm diag}(a_{M,1},\ldots,a_{M,r})\) and
\[
...
prop:step-010-rebalance).** After all three independent frozen outputs have
been computed, the setting's joint commit and rebalance are defined.  With
\[
...
**Proof / justification.** Every mode formula in unit_002 uses only the same
frozen \((X^0,Y^0,Z^0)\), so the three outputs are computed and then
committed as one transaction.  Positivity follows from unit_002, and
\[
...
prop:step-010-quotient-entry).** Under units 001--004 and
...
quantities and the scale log.  Unit_003 bounds every direction quantity by
\(3\beta\le384\omega\), so (11) follows.  Finally,
\[
...
Moore--Penrose output without assuming a nonsingular design.  On E_land_pre,
Proposition~\ref{prop:step-010-frozen-solves} uses the positive diagonal
congruence and pair-Gram floor from step_009 to replace each pseudoinverse by
its own ordinary inverse, derives the exact coefficient identity, and proves
all active normalizers positive.  The three formulas are evaluated from the
same frozen seed before any commit.

Lemma~\ref{lem:step-010-directions} converts the coefficient reserve into
both induced direction masses and Euclidean column errors, with zero
perpendicular fields and a fixed target orientation.  Proposition
\ref{prop:step-010-rebalance} performs the single prescribed joint
commit/rebalance and proves exact preservation of every represented term.
Finally, Proposition~\ref{prop:step-010-quotient-entry} gives
...
- \(C_M^{\rm land},P_M^{\rm land},s^{\rm land}\) are the minimal
  ...
  proof-local.  Constants \(2,3,1024\) are proved in units 003 and 005.
- Exact target-span membership is proved in unit_001, and positive normalizers
  in unit_002.  The joint commit is a finite operation after all formulas are
  complete.  Singular or zero-normalizer tapes retain observable
