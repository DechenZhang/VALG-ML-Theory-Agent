...
- E_cert_eq (step_005): after the accepted proof-only sign convention,
  P_M D_M = G_M K_M + R_M, where
  K_M = D_lambda (c_M' circ c_M''), D_M(a,a)=d_M,a>0, and
  ||R_M(:,a)||_2 <= d_M,a tau_r.  Every selected P_M column is in
  range(G_M).
- E_support, E_cluster, and E_gauge (step_006): a proof-only permutation
  associates selected column a with target a.  The product-one sign gauge
  preserves every represented rank-one term.  After this gauge, the target
  correlations c_M,aa are positive and
  max_{j != a} |c_M,ja| < 2 eta,
  sum_{j != a} |c_M,ja| <= ell_* = eta/(1-(14/5) eta),
  where eta = eta_score(r) < 1/2000.  The retained score/coverage interface
  and Gamma <= 1.01 imply
  sigma_a >= 0.85 sigma_max > 0.765 lambda_min
  > (3/4) lambda_max, and ||c_M(:,a)||_2 <= sqrt(1+q).
...
| --- | --- | --- | --- |
| unit_001 | lemma | The target-span dual equation has a positive diagonal; projected residuals normalized by d_M,a are entrywise at most 2 tau_r; off-diagonal normalized coefficients obey a product inequality. | E_row_residual and the input to E_col. |
...
| unit_003 | proposition | The column bound gives a small-gain row recurrence; its positive denominator selects the small root and both induced masses of the unnormalized dual coefficients are at most 44 omega. | E_row. |
...
2. The accepted step_005 signed equation and step_006 support/cluster/gauge
   results are used in exactly the current realized-factor notation.  No
   landing state or later ALS invariant is imported.
...
Lemma [lem:step-007-dual-equation] (dual equation, positive diagonal, and
projected residual).

Under E_sm, E_cert_eq, E_support, and E_gauge, define

    A_M = G_M^dagger P_M = H_M^(-1) G_M^T P_M,     c_M = G_M^T P_M = H_M A_M.

For every selected column a, the diagonal coefficient a_M,a = A_M(a,a)
satisfies 0.74 < a_M,a < 1.001.  Put
...
The product of the three dominant correlation signs is positive.  Otherwise
the dominant score term is negative and the selected off-target bounds give

    sigma_a <= lambda_max x_* ||c_M',-a||_2 ||c_M'',-a||_2
             <= lambda_max x_*(1+q_*) < (3/4) lambda_max,

where the retained score floor used here is
sigma_a > (3/4) lambda_max as recorded in the dependency interface.  Hence the three
individual dominant sign flips form a product-one gauge, so c_M,aa > 3/4
in all modes.  Combining this with (C) and |c_M,aa| <= 1 proves
0.74 < a_M,a < 1.001 and h_M,aa > 2/3.

Apply G_M^dagger to the selected equation.  Columnwise this is
...
residual used by the E_row_residual ledger obeys, separately,
...
Under the preceding units, let

    R = max_M ||Z_M - I||_row,1,    C = max_M ||Z_M - I||_col,1.

Then R <= 5 omega and

    max_M ( ||A_M-I||_row,1 or ||A_M-I||_col,1 ) <= 44 omega.       (J)
...
stored state per target while preserving the represented tensor.  Lemma
[lem:step-007-dual-equation] applies the true-factor dual to its exact
same-state equation, proves the positive diagonal, and bounds every projected
residual entry before summation.  Proposition [prop:step-007-column] uses
...
- In the exact orthogonal/equal-weight limit q = tau_r = 0, all off-target
  coordinates and residuals vanish, A_M = Z_M = I, and the original
  same-state equation is unchanged.
...
- H_M, c_M, A_M, Z_M, h_M, and eps_M are appendix-local translations of
  the setting's realized Gram, correlation, dual, normalized-dual,
...
- E_sm, E_cert_eq, E_support, E_cluster, and E_gauge are generated outputs
  ...
  permutation and sign gauge are proof-only and preserve represented terms.
