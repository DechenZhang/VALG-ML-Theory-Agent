...
1. Scalar Gaussian Chernoff bound. If $Z\sim {\cal N}(0,\sigma^2)$,
   ...
   The covariance bound comes from Assumption~\ref{assump:base-scale} and
   Assumption~\ref{assump:gaussian-smoothing}.
...
vacuously. Thus the exact displayed claim is proved using only its five
listed primitive assumptions and no additional assumption. diagnostic is not used in this assembly.
\end{proof}
...
The static event and realized factors are supplied by
Proposition~\ref{prop:step-001-geometry}; on $E_{\rm sm}$ every realized
factor column is well defined and (1) holds simultaneously in all three
modes. The proposal analysis below uses only this exported interface.
...
Proposition~\ref{prop:step-002-joint-window}
   proves (1), uniformly in slot \(i\) and
...
which the Proposition~\ref{prop:step-002-joint-window} interface holds, and condition on its
...
Under Assumption~\ref{assump:random-initialization} and the Proposition~\ref{prop:step-002-joint-window}, fix \(j\in[r]\) and define
...
and the Proposition~\ref{prop:step-002-joint-window}, define
...
1. Realized Gram interface. In current notation,
   Proposition~\ref{prop:step-001-realized-gram} says that on the event $E_{\rm sm}$ from Proposition~\ref{prop:step-001-geometry} each
...
product inequality yields (11), and applying (G\_M^\dagger) to (5) gives
...
2. Certificate equation and weighted ledger.
   ...
   witness score bounds. Their assumptions are exactly the conclusions of Propositions~\ref{prop:step-004-certificate} and \ref{prop:step-001-geometry}; no
...
The observable support and gauge results imply, in current notation,
...
The two-orientation coefficient result gives (C), the exact target-span identity
...
\[
 P_M^0=P_M, \tag{5}
\]
where $P_M$ is the oriented selected matrix in the named result
...
By Proposition~\ref{prop:step-001-geometry}, $\lambda_{\min}(H_M)\ge1-q>0$, so $G_M$ has full column rank. The
the row-closure conclusion of Proposition~\ref{prop:step-007-row} conclusion gives $P_M=G_MA_M$ with (C). The orthogonal
...
\end{proposition}
\begin{proof}[Step composition]
The named result graph/gauge result first identifies, purely for proof,
...
The pre-solve reserve from Lemma~\ref{lem:step-009-congruence} and
Proposition~\ref{prop:step-009-normalizer} supplies, in current notation,
   \[
   \widetilde X_M
   =G_MD_\lambda Q_M^0(J_M^0)^{-1}(D_{-M}^0)^{-1}
   =G_M\Xi_ML_M^0, \tag{C1}
   \]
   before any landing commit. Its static and seed inputs are not landing outputs.
Current-setting linear algebra: right multiplication cannot enlarge a
...
\]
By (the landing conclusion of Proposition~\ref{prop:step-010-quotient-entry}), \(H_M^0\) has full column rank, so its Moore--Penrose inverse is
...
\(v_{M,j}:=\xi_{M,j}-e_j\), (the static geometry conclusion of Proposition~\ref{prop:step-001-geometry}) and unit columns of \(G_M\) imply
\[
...
after (the landing conclusion of Proposition~\ref{prop:step-010-quotient-entry}).
...
\]
and (the static geometry conclusion of Proposition~\ref{prop:step-001-geometry}) give two terms each at most \(\beta/(1-\beta)\) in either
...
\begin{proof}
From (the static geometry conclusion of Proposition~\ref{prop:step-001-geometry}), every \(G_M^\top G_M\) is positive
...
Under the static geometry conclusion of Proposition~\ref{prop:step-001-geometry}, let
...
Under the static geometry conclusion of Proposition~\ref{prop:step-001-geometry},
...
Under the static geometry conclusion of Proposition~\ref{prop:step-001-geometry},
let the landing conclusion of Proposition~\ref{prop:step-010-quotient-entry}
supply the entry state \(e^{\rm land}\). Using
...
\tag{24}
\]
In particular, starting from the landing conclusion of Proposition~\ref{prop:step-010-quotient-entry} state, the generated
chronological orbit remains in the chart and defines the generated basin event; writing
...
1. the coupon dependency.  The preceding
   Proposition~\ref{prop:step-003-coupon}, under
   Assumptions~\ref{assump:subquadratic-rank} and
   \ref{assump:random-initialization}, together with
   Proposition~\ref{prop:step-002-joint-window}, proves its coverage conclusion
   conditional on the fixed smoothed tensor, without targetwise
   ...
   conclusions.
2. the cyclic dependency.  The preceding
   ...
   \(E_{\rm sm}\) and \(E_{\rm land}\) interfaces, proves the landing-state
   contraction,
...
Under Assumption~\ref{assump:base-scale}, the static-geometry
...
Under Assumptions~\ref{assump:base-scale} and
\ref{assump:accuracy-confidence}, the Proposition~\ref{prop:step-011-chronological-contraction}, and
...
the landing, the literal cyclic update also uses a Moore--Penrose
...
invoke the Proposition~\ref{prop:step-003-coupon}, the Proposition~\ref{prop:step-011-chronological-contraction}, and the local
...
\[
 ...
 \le(1-p\_0)^J=27^{-J}\le\delta\_{\rm init}.
\tag{16}
\]
...
the exact-span chronology and (the landing conclusion of Proposition~\ref{prop:step-010-quotient-entry}).  Lemma~\ref{lem:step-012-residual-transfer}
...
Under Assumptions~\ref{assump:base-scale},
\ref{assump:subquadratic-rank}, and
\ref{assump:accuracy-confidence}, condition on the static event supplied by
Proposition~\ref{prop:step-001-geometry}.  Choose
\[
...
Let
\[
 \bar k:=1+C_{\rm rank}r^{5/3}(\log r)^{5/2},\quad
...
W_{\rm total}
\le c_{\rm cost}\bigl(1+C_{\rm rep}\log(1/\delta_{\rm init})\bigr)
\Big[&n^3+\bar k\{2+(C_{\rm burn}+C_{\rm cert})\log r\}(n^3+n)\\
&+\bar k n^3+\bar k^2n
+R_\epsilon(n^3\bar k+n^2\bar k^2+\bar k^3)+\bar k n\Big].
\end{aligned}
\tag{B.5}
...
the declared envelopes for $\kappa _0$ and $\rho^{-1}$ are the only
structural substitutions.  The probability in (B.4) is conditional on the
static event, while the first probability remains the separate
$1-\delta_{\rm sm}$ event probability.
\end{proposition}

\begin{proof}
The ceiling in the definition of $k$ gives the first inequality in (B.3).
The definitions $L_{\rm prop}=L_{\rm burn}+L_{\rm cert}$ and
$L_{\rm burn},L_{\rm cert}\le1+C\log r$ give the second.  Since
...
(B.4), and the choice of $C_{\rm rep}$ gives the second.

Substituting (B.3) into the per-run expression of
Proposition~\ref{prop:step-012-dense-cost} and multiplying by the bound on
$J$ gives (B.5).  Expanding
$\bar k=O(r^{5/3}(\log r)^{5/2})$ yields the three displayed rank powers in
(B.6); every term is retained, and the only absorbed scalar is
$R_\epsilon=O(1+\log\kappa _0+\log(1/\epsilon))$.  The finite envelopes
$\kappa _0\le r^{d_\kappa}$ and $\rho^{-1}\le r^{d_\rho}$ then make the
hidden dependence polynomial in the stated structural variables.  No
confidence parameter is absorbed into $k$, and the two probability modes are
not multiplied until the tower calculation in the main theorem.
\end{proof}
...
residual have the exact values required by the theorem. The argument is a
...
The theorem assumptions provide the primitive conditions used by the
...
exactly the generated interfaces stated in theorem, and the direct
