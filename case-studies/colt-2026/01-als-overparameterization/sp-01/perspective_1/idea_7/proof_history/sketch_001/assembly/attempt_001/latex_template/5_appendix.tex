...
the four deterministic interfaces are exactly the conclusions of the named
local results, so their conjunction holds on the same event.
\end{proof}
\end{proposition}
...
1. the preceding result the preceding result. Its specified conclusion is: on the
   ...
   The dependency assumptions were discharged in its specified proof and
...
Under Assumption~\ref{assump:random-initialization} and the specified $E_{\rm sm}$ conclusion (1), for every mode $M$ and every $\ell\ne j$,
...
Under Assumption~\ref{assump:random-initialization}, the specified $E_{\rm sm}$ conclusion, and Lemmas~\ref{lem:step-002-regression} and
...
Under Assumption~\ref{assump:random-initialization}, the specified $E_{\rm sm}$ conclusion, Lemma~\ref{lem:step-002-target-mass}, and
...
Fix any realized tensor in the preceding result event $E_{\rm sm}$.
...
This is the exact specified sketch-row claim. It is a one-slot probability
...
1. the preceding result the preceding result. Proposition~\ref{prop:step-002-joint-window}
   in the preceding dependency proves (1), uniformly in slot \(i\) and
...
which the preceding result\_002 interface holds, and condition on its
...
the preceding result, (1) gives \(p_j\ge c_{\rm win}r^{-5/3}(\log r)^{-3/2}\).


\begin{lemma}[Fixed-target slot-miss bound]\label{lem:step-003-slot-miss}
Under Assumption~\ref{assump:random-initialization} and the specified Proposition~\ref{prop:step-002-joint-window}, fix \(j\in[r]\) and define
...
\texttt{   \textbackslash\{\}left(c\_\{\textbackslash\{\}rm win\}r\textasciicircum\{\}\{-5/3\}(\textbackslash\{\}log r)\textasciicircum\{\}\{-3/2\}\textbackslash\{\}right)}
...
and the specified Proposition~\ref{prop:step-002-joint-window}, define

\[
E_{\rm cov}:=\bigcap_{j=1}^r\bigcup_{i=1}^k\mathscr E_{ij}
\texttt{   =\textbackslash\{\}bigcap\_\{j=1\}\textasciicircum\{\}r B\_j\textasciicircum\{\}\{\textbackslash\{\},c\}.}
\tag{6}
\]

Then, for every fixed smoothed instance in the preceding result domain,
...
the preceding result\_002 conclusion then gives
...
Fix a realized smoothed instance for which the preceding result\_002 interface
...
specified sketch-row output \(E_{\rm cov},p_0\), ready for the preceding result's
trajectory induction and the preceding result's restart calculation.
...
tower property and the specified instance bound give
...
but theorem-facing one-run initialization guarantee remains the
conditional bound (13), with \(\delta_{\rm init}\) reserved for independent
restarts in the preceding result.

\subsection{Jacobi chart recurrence}\label{app:step-004}

\paragraph{Auxiliary facts.}

1. the preceding result\_003 coverage interface: on \(E_{\rm cov}\), a witness
   exists for every target with \(R_0\le19/20\) and \(S_0\le rR_0\).
2. the preceding result\_001 realized Gram interface: on \(E_{\rm sm}\), each mode
...
Under the specified \(E_{\rm sm}\) Gram
interface and the covered-slot initial interface from the preceding result, suppose at
...
Under
Assumption~\ref{assump:subquadratic-rank}, the preceding result\_003 coverage
...
On \(E_{\rm sm}\cap E_{\rm cov}\), choose the preceding result witness \(i(j)\) for
each target \(j\). The argument in fact applies to every pair \((i,j)\) for
which the specified window event occurs; \(E_{\rm cov}\) is used only to
ensure that at least one such pair exists for every target. The specified initial interface gives nonzero target
...
Concretely, \(E_{\rm RS}\) is the finite intersection, over all specified window pairs and all \(t\le L_{\rm prop}\), of (3)--(4) and (9);
...
1. Realized Gram interface (the preceding result 001).  In current notation,
   Proposition~\ref{prop:step-001-realized-gram} says that on E\_sm each
   ...
   factors.
2. Stored-state interface (the preceding result 004).
...
Under the derived interfaces `E\_cert`
and E\_sm, for every certified slot (a) and mode (M), let
...
\tag{13}
\]
The specified E\_sm row/column bounds imply
...
product inequality yields (11), and applying (G\_M\textasciicircum{}\dagger) to (5) gives
...
and mode, define the finite event E\_cert\_tail; it is formed before any
target label, representative ordering, or clustering operation.

\begin{proof}
On a coverage witness, let
$\rho_{M,\ell}:=|\bar c_{M,a,\ell}/\bar c_{M,a,j}|$.  The the preceding result
...
slotwise finite deterministic statements on E\_cert and E\_sm; no label is
...
Fix E\_sm and the generated E\_cert event.  For each certified slot,
...
still the realized (G\_U,G\_V,G\_W,D\_\lambda) tensor.
...
(\sqrt{r(1+q)}\tau\_r), rather than being silently multiplied by a future
selection or ALS horizon.  The exact (d)-normalized identity records the
scalar cancellation; conversion to the score (sigma\_a) adds only the
explicit one-time term in (19).

Finally, Proposition~\ref{prop:step-005-threshold-ledger} uses only the
already generated coverage witnesses to lower-bound (sigma\_{\max}).  It
...
E\_cert\_eq and E\_cert\_tail.  It is produced before the score-filter output
...
2. Certificate equation and weighted ledger (the preceding result 005, attempt 2).
   ...
   witness score bounds. Their assumptions are exactly E\_cert and E\_sm; no
...
Fix the generated event E\_sm and the finite certified pool. For a barred slot
...
Under E\_sm, E\_cert\_eq, and E\_cert\_tail, for
...
\begin{proof}
The the preceding result retained-pool ledger gives, for each mode, a
...
\begin{proof}
Let \(a(j)\) be the coverage witness supplied by E\_cert\_tail for
...
the lower norm bound in E\_cert\_tail imply
\[
...
\texttt{             +\textbackslash\{\}|\textbackslash\{\}widehat p\_\{M,b\}-g\_\{M,j\}\textbackslash\{\}|\_2)\textasciicircum\{\}2\textbackslash\{\}\textbackslash\{\}}
...
exactly the interface exported as E\_gauge. \(\)

\end{proof}
\end{lemma}
\paragraph{Assembly.}

Fix E\_sm and the generated the preceding result interfaces. The specified weighted ledger
...
1. The the preceding result\_001 realized-geometry result is used only through the
   current H\_M, q, Gamma, eigenvalue, and pseudoinverse bounds listed above.
2. The the preceding result\_005 signed equation and the preceding result support/cluster/gauge
   results are used in exactly the current realized-factor notation.  The
   the preceding result gauge contributes only the proof-only permutation and
   term-preserving product-one option; positivity is derived in unit\_001.
...
\texttt{q = q\_real,  t = r tau\_r,  omega = q\_*\textasciicircum\{\}2 + t,  eta = eta\_score(r).}

The displayed definition of eta\_score, tau\_r = q\_*\textasciicircum{}2/(10000 r), and
q <= q\_* imply

\texttt{eta <= (3/2) q\_*,       ell\_* <= (31/20) q\_*,       x\_* := 2 eta <= 3 q\_*,}
\texttt{x\_* ell\_* <= (93/20) q\_*\textasciicircum\{\}2.                                    (A)}

For example,
sqrt(r(1+q\_*)) tau\_r <= q\_*/10\textasciicircum{}6 and
1.4(1+q\_*) tau\_r/(1-tau\_r) <= q\_*/10\textasciicircum{}6; hence
eta <= 1.4(1+q\_*) q\_* + 2 q\_*/10\textasciicircum{}6 < (3/2) q\_*.
Since eta < 1/2000, division by 1-(14/5)eta gives the stated ell\_* bound.
All constants in (A) are universal and remain valid when q\_real = 0.


\begin{lemma}[sign-covariant dual equation, positive diagonal, and projected residual]\label{lem:step-007-dual-equation}

Under E\_sm, E\_cert\_eq, E\_support, and E\_cluster, first keep the score-oriented
the preceding result objects barred.  After the proof-only target permutation, write

\texttt{bar c\_M,ja = (G\_M\textasciicircum\{\}T bar P\_M)(j,a),   bar sigma\_a = |theta\_a|}
\texttt{  = sum\_j lambda\_j bar c\_U,ja bar c\_V,ja bar c\_W,ja}
\texttt{  > (3/4) lambda\_max.}

For each a let xi\_M,a = sign(bar c\_M,aa).  Then
xi\_U,a xi\_V,a xi\_W,a = 1.  Define the transformed, product-one objects

\texttt{p\_M,a = xi\_M,a bar p\_M,a,}
\texttt{c\_M,ja = xi\_M,a bar c\_M,ja,}
\texttt{k\_M,a = xi\_M',a xi\_M'',a bar k\_M,a = xi\_M,a bar k\_M,a,}
\texttt{r\_M,a = xi\_M,a bar r\_M,a.}

Stacking the transformed columns defines P\_M, K\_M, and R\_M.

The transformed objects obey

\texttt{P\_M D\_M = G\_M K\_M + R\_M,   c\_M = G\_M\textasciicircum\{\}T P\_M,                 (B0)}

and the score identity is unchanged.  Set sigma\_a = bar sigma\_a and relabel
these transformed objects without bars.  Define

\texttt{A\_M = G\_M\textasciicircum\{\}dagger P\_M = H\_M\textasciicircum\{\}(-1) G\_M\textasciicircum\{\}T P\_M,   c\_M = H\_M A\_M.}

For every selected column a, a\_M,a = A\_M(a,a) is positive and
0.74 < a\_M,a < 1.001.  Put

\texttt{Delta\_M = diag(a\_M,a),   Z\_M = A\_M Delta\_M\textasciicircum\{\}(-1),}
\texttt{h\_M,ja = (H\_M Z\_M)(j,a) = c\_M,ja / a\_M,a.}

Then h\_M,aa > 2/3.  If

\texttt{eps\_M,ja = (G\_M\textasciicircum\{\}dagger R\_M)(j,a) / d\_M,a,}

then |eps\_M,ja| <= 2 tau\_r for every M,j,a.  For j != a,

\texttt{|Z\_M(j,a)| <= 4 |h\_M',ja h\_M'',ja| + 3 tau\_r.                 (B)}

\begin{proof}

The barred score identity and the specified retained-pool bounds are the
pre-gauge facts.  If xi\_U,a xi\_V,a xi\_W,a were negative, the dominant term
in that identity would be negative and the off-target bounds would give

\texttt{bar sigma\_a <= lambda\_max x\_* ||bar c\_M',-a||\_2}
\texttt{                        ||bar c\_M'',-a||\_2}
\texttt{             <= lambda\_max x\_*(1+q\_*) < (3/4) lambda\_max,}

contradicting the displayed retained score floor.  Thus the signs xi\_M,a
have product one.  Multiplying the barred same-state equation in mode M by
xi\_M,a gives (B0), because
xi\_M',a xi\_M'',a = xi\_M,a.  The residual column is transformed by the same
sign, so its norm bound remains ||R\_M(:,a)||\_2 <= d\_M,a tau\_r.  The product
...
product-one gauge; no positivity is imported from E\_gauge.  By construction
c\_M,aa = |bar c\_M,aa| > 3/4.

Since P\_M is in range(G\_M), A\_M is well-defined and P\_M = G\_M A\_M.
The unit column norm and the eigenvalue floor give
||A\_M(:,a)||\_2 <= (1-q)\textasciicircum{}(-1/2).  Therefore

\texttt{|c\_M,aa - A\_M(a,a)|}
\texttt{  <= ||H\_M(a,-a)||\_2 ||A\_M(-a,a)||\_2}
\texttt{  <= q / sqrt(1-q).                                         (C)}

Combining (C) with c\_M,aa > 3/4 and |c\_M,aa| <= 1 proves
0.74 < a\_M,a < 1.001 and h\_M,aa > 2/3.

Apply G\_M\textasciicircum{}dagger to (B0).  Columnwise this is

\texttt{d\_M,a A\_M(:,a) = K\_M(:,a) + (G\_M\textasciicircum\{\}dagger R\_M)(:,a).             (D)}

The pseudoinverse norm and the residual bound give

\texttt{||eps\_M,:,a||\_2 <= tau\_r / sqrt(1-q) < 2 tau\_r,              (E)}

which proves the dual-residual entrywise assertion.  The directly projected
residual used by E\_row\_residual obeys, separately,

\texttt{|(G\_M\textasciicircum\{\}T R\_M)(j,a)| / d\_M,a}
\texttt{   <= sqrt(1+q) tau\_r < 2 tau\_r.                             (E')}

Consequently, both the dual residual array eps and the directly projected
array in (E') have every induced row and column sum bounded by 2 r tau\_r.

Also

\texttt{d\_M,a <= ||G\_M||\_2 ||K\_M(:,a)||\_2}
\texttt{       <= lambda\_max (1+q),}

because ||c\_M' circ c\_M''||\_2 <= sqrt(1+q).  Hence
d\_M,a / lambda\_a <= Gamma(1+q) < 1.02.

For j != a, divide the j coordinate of (D) by the a coordinate.  The
denominator, divided by lambda\_a, has absolute value at least

\texttt{9/16 - 1.02 tau\_r / sqrt(1-q) > 0.56.                       (F)}

Using c\_M',ja = a\_M',a h\_M',ja, c\_M'',ja = a\_M'',a h\_M'',ja and the
upper bound 1.001 on each diagonal factor, the product part of the numerator
divided by lambda\_a is at most 1.02 |h\_M',ja h\_M'',ja|.  The residual part
divided by lambda\_a is at most 1.02 tau\_r.  Division by (F) makes both
...
\texttt{max\_M ||Z\_M - I||\_col,1 <= 42 omega.                          (G)}

\begin{proof}

For each selected column a, (A), (C), and a\_M,a > 0.74 give

\texttt{||h\_M,-a||\_2}
\texttt{  <= ||c\_M,-a||\_2 / a\_M,a}
\texttt{  <= (3/2) sqrt(x\_* ell\_*).                                  (H)}

Sum (B) over j != a and use Cauchy-Schwarz:

\texttt{sum\_\{j != a\} |Z\_M(j,a)|}
\texttt{  <= 4 ||h\_M',-a||\_2 ||h\_M'',-a||\_2 + 3(r-1) tau\_r}
\texttt{  <= 9 x\_* ell\_* + 3 t}
\texttt{  <= 42 (q\_*\textasciicircum\{\}2+t) = 42 omega.                                 (I)}
...
\texttt{R = max\_M ||Z\_M - I||\_row,1,    C = max\_M ||Z\_M - I||\_col,1.}

Then

\texttt{R <= 5 omega,}
\texttt{max\_M ||A\_M-I||\_row,1 <= 7 omega,}
\texttt{max\_M ||A\_M-I||\_col,1 <= 44 omega.                            (J)}

The normalized residual entries still obey |eps\_M,ja| <= 2 tau\_r, and
their row or column sums are charged once as 2 r tau\_r.

\begin{proof}

Fix M and row j.  For a != j, set v\_M,ja = (H\_M Z\_M)(j,a).  From (G),

\texttt{max\_\{a != j\} |v\_M,ja| <= q + (1+q) C =: u,                   (K)}

since the direct H\_M(j,a) term has row mass q, the off-diagonal Z entry is
at most C, and the remaining Gram row contributes at most q C.  Also,

\texttt{sum\_\{a != j\} |v\_M,ja|}
\texttt{  <= q + (1+q) ||Z\_M-I||\_row,1.                              (L)}

To verify (L), the l=j term in H\_M Z\_M contributes the row error.  Each
l != j term contributes its off-diagonal row mass and its one diagonal
entry Z\_M(l,l)=1, weighted by the H\_M row mass q.

Sum (B) over a != j, use (K) for one factor and (L) for the other, and
take the maximum over modes.  This gives

\texttt{R <= 4 u \{ q + (1+q) R \} + 3 t.                              (M)}

By (G), t = q\_*\textasciicircum{}2/10000, and q <= q\_*,

\texttt{u <= q + 42(1+q\_*)(1+10\textasciicircum\{\}(-4)) q\_*\textasciicircum\{\}2 < (51/50) q\_*,}
\texttt{4 u (1+q) < 1/1000.                                         (N)}

Thus (M) has a positive denominator and

\texttt{R <= (4 u q + 3 t) / (1 - 4 u (1+q)) < 5 omega.               (O)}
...
It remains to transfer from normalized columns to A\_M.  Let z be a column
of Z\_M, with z\_a=1 and ||z-e\_a||\_1 <= C, and write a0 = a\_M,a.  The unit
norm identity 1 = (a0 z)\textasciicircum{}T H\_M (a0 z) gives

\texttt{|z\textasciicircum\{\}T H\_M z - 1| <= 2 q C + (1+q) C\textasciicircum\{\}2 =: delta\_a.              (P)}

At q <= q\_* and C <= 42 omega, delta\_a < 1/100 and
|a0-1| <= 2 delta\_a <= omega.  (The last inequality is the direct endpoint
check using omega = (1+10\textasciicircum{}(-4)) q\_*\textasciicircum{}2 and q\_* = 1/4096.)
Since a0 < 1.001, (G), (O), and this diagonal bound imply

\texttt{||A\_M-I||\_col,1 <= 1.001 C + omega < 44 omega,}
\texttt{||A\_M-I||\_row,1 <= 1.001 R + omega < 7 omega.}

This proves (J).  Equation (E) gives the entrywise 2 tau\_r residual, and
summing over at most r entries gives 2 r tau\_r in either orientation, never
r\textasciicircum{}2 tau\_r.  QED.

\end{proof}
\end{proposition}
\paragraph{Assembly.}

The the preceding result\_006 support and gauge interfaces first provide one selected
...
barred the preceding result score identity, proves the dominant sign product, explicitly
...
only the specified per-column correlation tails to produce the 42 omega
...
inequality, and transfer from normalized dual columns to A\_M-I.  Therefore
E\_col, E\_row\_residual, and E\_row hold with both induced orientations at
O(omega), and with one residual charge r tau\_r.  All objects are from the
selected stored state; no landing or cyclic object is consumed.
Because the specified stored-state interface places every selected column in
range(G\_M), the orthogonal complement field is exactly zero at this stage
and therefore satisfies the later the preceding result budget tau\_r.

\subsection{Balanced observable seed}\label{app:step-008}

\paragraph{Auxiliary facts.}

1. Observable support and gauge (the preceding result 006): in current notation,
...
2. Selected coefficient closure (the preceding result 007): the specified two-orientation result gives (C), the exact target-span identity
...
Under E\_sm, E\_gauge, and E\_row, after the
...
\]
The score-preserving orientation from E\_gauge has the same rank-one product
...
\[
 P_M^0=P_M, \tag{5}
\]
where $P_M$ is the oriented selected matrix in the preceding result
...
By E\_sm, $\lambda_{\min}(H_M)\ge1-q>0$, so $G_M$ has full column rank. The
specified E\_row conclusion gives $P_M=G_MA_M$ with (C). The orthogonal
...
Under
E\_sm, E\_support, and E\_row, with the oriented matrices in
...
It remains to transfer this diagonal control to the scalar. By the specified support ledger (A), every off-target column satisfies the same $x_*,\ell_*$
...
The the preceding result graph/gauge result first identifies, purely for proof,
...
orthogonal complement is identically zero; the specified row/column closure
...
the target Gram margin, and the specified off-target ledger. It first derives
...
1. specified static geometry (the preceding result). In current notation, the
   specified result states \(q\le q_*\), \(\Gamma\le1.01\), and the displayed
   induced row/column Gram bounds for every realized \(G_M\). These are
   discharged by the preceding the preceding result artifact and are used only as
   derived controls.

2. specified balanced seed (the preceding result). In current notation, the specified result states the decomposition, orthogonality, \(48\omega\) row/column
   fields, \(\tau_r\) perpendicular-column bound, unit normalized columns,
   and \(D_M^0=D_\lambda^{1/3}E^0\). These are discharged by the preceding
   the preceding result artifact; no landing object is imported.
...
Under assump:subquadratic-rank and the specified E\_sm bound
...
\eta_A<104q_*^2, \tag{N2}
\]
and, using the specified \(\Gamma\le1.01\),
\[
...
Under the specified derived interfaces E\_sm
and E\_seed, for every mode \(M\),
\[
...
\texttt{  =I+E\_\{M'\}\textbackslash\{\}circ E\_\{M''\}. \textbackslash\{\}tag\{11\}}
\]
For either \(\nu={\rm row}\) or \(\nu={\rm col}\),
\[
\|E_{M'}\circ E_{M''}\|_\nu
...
Under Proposition~\ref{prop:step-009-pair-reserves}
and the specified scale identity in E\_seed, let
\[
...
The scale identity from E\_seed gives
\[
...
\texttt{   =D\_\textbackslash\{\}lambda\textasciicircum\{\}\{1/3\}(E\textasciicircum\{\}0)\textasciicircum\{\}\{-2\},\textbackslash\{\}qquad}
\ell_{M,j}^0:=(L_M^0)_{jj}
\texttt{   =\textbackslash\{\}lambda\_j\textasciicircum\{\}\{1/3\}e\textasciicircum\{\}\{-2s\_j\textasciicircum\{\}0/3\}>0,}
\]
and
\[
...
the generated interface E\_land\_pre, with no post-solve or predecessor input.

\subsection{Synchronized frozen landing}\label{app:step-010}

\paragraph{Auxiliary facts.}

1. the preceding result\_009 pre-solve reserve (in particular
   ...
   =G_M\Xi_ML_M^0, \tag{C1}
   \]
   together with (A1)--(A2), before any landing commit.  Its specified E\_sm/E\_seed inputs are not landing outputs.
...
Under E\_land\_pre, for each mode
\[
\widetilde X_M=G_M\widetilde B_M,\qquad
...
\begin{proof}
The frozen scaling and specified congruence give
\[
...
On the derived event \(E_{\rm land\_pre}\), assume
the preceding result conclusions of Lemma~\ref{lem:step-009-congruence} and
...
On the derived event \(E_{\rm land\_pre}\), under
...
On the derived conjunction
\(E_{\rm land\_pre}\cap E_{\rm seed}\), assume the specified seed-scale
conclusion \(\|s^0\|_\infty\le128\omega\), the preceding result reserve
...
every represented term.  Finally, under the specified \(E_{\rm seed}\) scale
...
1. the preceding result landing interface. In current notation, the
   preceding dependency states the Moore--Penrose range inclusion, the
   positive active norms on E\_land\_pre, the joint commit, the single
...
2. specified static geometry interface. The the preceding result result,
   preceding as recorded above and as used by the preceding result, is restated
...
3. the standard cyclic ALS quotient interpretation~\citep{Uschmajew2012}. The result is used only as a
   ...
   the target-span map, the Refresh\_s identity, all numerical constants, and
...
\texttt{      \textbackslash\{\}max\_\{i\textbackslash\{\}ne j\}|B\_\{ij\}|}
\texttt{   +\textbackslash\{\}max\_i|A\_\{ii\}B\_\{ii\}|,}
   \qquad \nu\in\{{\rm row},{\rm col}\}.
   \tag{C1}
   \]
...
\texttt{  =D\_\textbackslash\{\}lambda\textasciicircum\{\}\{1/3\}E,}
...
\begin{proof}
From (A1), every \(G_M^\top G_M\) is positive
...
\texttt{   +F\_\{M'\}\textbackslash\{\}circ(L\_\{M''\}-F\_\{M''\}).}
\tag{13}
\]
...
Under E\_sm and its specified static-geometry interface (A1), and under the hypotheses and conclusions of
...
product-preserving equal-norm representative. The specified E\_land interface
...
G\_M. To identify the refreshed product log, the two held balanced norms are
...
Under E\_sm and its specified static-geometry interface (A1), let the specified E\_land interface (A2) supply
...
\tag{24}
\]
In particular, starting from the specified E\_land state, the generated
chronological orbit remains in the chart and defines E\_basin; writing
...
For the specified E\_land entry, (A2) gives
...
The specified E\_land interface (A2), together with E\_sm and its static
...
the dynamic pair-Gram reserve from the specified realized interference bound
...
each consumer, and exports the uniform map estimate (24) and the E\_land
no-forcing recurrence (25). Therefore the generated event E\_basin and
...
2. specified cyclic dependency.  The preceding
   ...
   specified \(E_{\rm sm}\) and \(E_{\rm land}\) interfaces, proves (A2),
...
Under Assumption~\ref{assump:base-scale}, the specified static-geometry
...
\texttt{  +\textbackslash\{\}|\{\textbackslash\{\}cal A\}-I\textbackslash\{\}|\_2\textbackslash\{\}|\textbackslash\{\}Delta(\textbackslash\{\}lambda)\textbackslash\{\}|\_F\textbackslash\{\}\textbackslash\{\}}
 &<(1.003)(1.001)x\|\lambda\|_2+3.004x\|\lambda\|_2
 <4.1x\|\lambda\|_2.
\end{aligned}
\tag{5}
\]
The specified static geometry gives
...
\texttt{      =H\_U\textbackslash\{\}circ H\_V\textbackslash\{\}circ H\_W,}
\]
...
\texttt{   \textbackslash\{\}sum\_\{\textbackslash\{\}ell\textbackslash\{\}ne j\}|(H\_V)\_\{j\textbackslash\{\}ell\}|}
 \le q_*^2.
\]
...
\texttt{    u\_j\textbackslash\{\}otimes v\_j\textbackslash\{\}otimes w\_j\textbackslash\{\}right\textbackslash\{\}|\_F\textasciicircum\{\}2}
 =\|T\|_F^2
...
Under Assumptions~\ref{assump:base-scale} and
\ref{assump:accuracy-confidence}, the specified Proposition~\ref{prop:step-011-chronological-contraction}, and
...
quotient entry \(x_0=0\) is supplied, the specified contraction forces
...
\texttt{   \textbackslash\{\}|\textbackslash\{\}xi\_i\textasciicircum\{\}\{(M)\}\textbackslash\{\}|\_2>0\textbackslash\{\} \textbackslash\{\}text\{for every \}i,M\textbackslash\{\}right\textbackslash\{\}\},}
 \qquad
 \Omega_{\rm raw}^{(J),\circ}:=
\texttt{   (\textbackslash\{\}Omega\_\{\textbackslash\{\}rm raw\}\textasciicircum\{\}\{(1),\textbackslash\{\}circ\})\textasciicircum\{\}J.}
\tag{12}
\]
...
specified landing, the literal cyclic update also uses a Moore--Penrose
...
Refresh\_s proof; no d\_Q, target-span, or contraction claim is made there.
...
\tag{13}
\]
There is a universal c\_cost such that one complete run, including all
...
O(n\textasciicircum{}3) once. A contraction of an n by n\textasciicircum{}2 unfolding with one Khatri--Rao
vector costs O(n\textasciicircum{}3); three contractions per Jacobi commit, including
certification look-ahead, give O(k L n\textasciicircum{}3). Generating and normalizing raw
vectors costs O(kn). Scores use at most O(k n\textasciicircum{}3) dense tensor contractions.
The three modewise inner products for every retained pair and the connected
graph cost O(k\textasciicircum{}2 n), covering filtering, clustering, and tie handling.

The frozen landing pass has three products of an n by n\textasciicircum{}2 unfolding with an
n\textasciicircum{}2 by k design, dense Gram formation, and three k-column Moore--Penrose
factorizations. Their
combined cost is O(B\_k); the one rebalance is lower order. A cyclic sweep has
the same dense upper bound O(B\_k), including construction of the reconstructed
tensor for the original residual test (O(n\textasciicircum{}3 k)). Inactive k-r columns may
be removed for a sharper B\_r bound, but retaining the prescribed zero-padded
arrays is covered by B\_k. Singular pseudoinverses use the same
...
invoke the specified Proposition~\ref{prop:step-003-coupon}, the specified Proposition~\ref{prop:step-011-chronological-contraction}, and the local
...
\[
 ...
 \le(1-p\_0)\textasciicircum{}J=27\textasciicircum{}{-J}\le\delta\_{\rm init}.
\tag{16}
\]
...
at least \(p_0=26/27\). On a fixed \(E_{\rm sm}\) tensor, the specified deterministic chain through
...
the specified generated interfaces on its fresh tape, and define
\[
...
the separate \(E_{\rm baseline}\) obligation of the preceding result.

\subsection{Exact orthogonal baseline}\label{app:step-013}

\paragraph{Auxiliary facts.}

1. specified balanced-seed interface (step 008). The preceding
...
2. specified frozen landing interface (step 010). In current notation,
   for each \(M\), the setting computes
   \[
   \widetilde X_M=T_{(M)}H_M^0
\texttt{   ((H\_M\textasciicircum\{\}0)\textasciicircum\{\}\textbackslash\{\}top H\_M\textasciicircum\{\}0)\textasciicircum\{\}\textbackslash\{\}dagger}
   \]
...
3. specified chronological interface (step 011). Proposition
   ...
   equation and that Refresh\_s represents the literal chronological update.
   We instantiate and check the normal equations for all three modes directly.
4. specified stopping interface (step 012). The original test is
...
Under the local
specialization \(\mathsf B\) and the preceding result seed convention, the
...
For completeness, the specified scale-equivariance interface can be checked
...
the preceding result residual-test semantics. Thus all three frozen landing
...
The assumptions in the theorem provide the primitive conditions used by the
first step.  Proposition~\ref{prop:step-001-realized-gram} gives the static event
with probability at least $1-\delta_{\rm sm}$.  Conditional on that event,
Propositions~\ref{prop:step-002-joint-window} and
\ref{prop:step-003-coupon} provide the universal covered-run event.  The
recurrence and certificate conclusions in Proposition~\ref{prop:step-004-certificate}
and Proposition~\ref{prop:step-005-threshold-ledger} feed the label-free selection
and gauge results of Proposition~\ref{prop:step-006-graph}; the two
orientation closure and seed bounds follow from Proposition~\ref{prop:step-007-row}
and Proposition~\ref{prop:step-008-seed-interface}.  The frozen reserves and
joint landing are then supplied by Proposition~\ref{prop:step-009-normalizer}
and Proposition~\ref{prop:step-010-quotient-entry}.  Finally,
Proposition~\ref{prop:step-011-chronological-contraction} and
Proposition~\ref{prop:step-012-restart} yield the original Frobenius stopping
criterion, polynomial runtime, and conditional restart confidence.  These are
exactly the generated interfaces stated in the theorem, and the direct
composition introduces no additional assumption or rate loss.  In the exact
orthogonal equal-weight specialization, Proposition~\ref{prop:step-013-zero-residual}
gives the stated zero-residual fixed point.
