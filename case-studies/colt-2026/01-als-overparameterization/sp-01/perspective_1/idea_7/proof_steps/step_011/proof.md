...
\[
 ...
 \quad\text{for every exact-span balanced positive-product }e
 \text{ with }d_Q(e,0)\le\rho_{\rm ALS},
...
- At any maintained in-chart state, exact target-span membership is proved
  below from the ordinary full-rank unfolding formula; it is not assumed as a
  primitive event. No quantitative claim is made for a singular/off-chart
  cyclic tape.

### Local conditional hypotheses

- For Proposition~\ref{prop:step-011-refresh-equivariance}, the solved-column
  condition \(\min_j\|\widetilde X_M(:,j)\|_2>0\) is a local condition for
  defining canonical Refresh_s; Proposition~\ref{prop:step-011-block-contraction}
  proves it before that proposition is invoked on the maintained chart.
- The radius condition \(d_Q(e,0)\le\rho_{\rm ALS}\) is a local condition in
  the block lemmas and is maintained by the induction in
  Proposition~\ref{prop:step-011-chronological-contraction}; it is the domain
  of the exported map estimate, not a new primitive assumption.
...
| unit_002 | proposition | On a full-rank target-span block whose solved active columns all have positive norm, positive product-one rescaling is exactly equivariant with literal ALS, and the canonical Refresh_s representative is quotient-equivalent to the literal post-block state. | Proves scale equivariance and literal-ALS chronology. |
...
| unit_004 | proposition | Under E_sm and its accepted static-geometry interface (A1), plus the unit-001/003 conclusions, one exact in-chart full-rank mode solve proves positive solved columns, exact target-span output, and a direction/scale defect at most \((8q_*+32\rho_{\rm ALS})x\). | Gives the one-block contraction and positivity interface. |
| unit_005 | proposition | Under E_sm/(A1), the accepted E_land interface (A2), and Propositions~\ref{prop:step-011-refresh-equivariance} and~\ref{prop:step-011-block-contraction}, every exact-span balanced positive-product state in the \(\rho_{\rm ALS}\) chart has a chronological U/V/W map with uniform factor \(\ell=17/512\); the E_land entry then yields E_basin and the no-forcing recurrence with literal-orbit equivalence. | Exports the map-wide contraction and generated basin event. |
...
3. **Uschmajew, Local Convergence of the Alternating Least Squares
   ...
   block-Gauss--Seidel chronology. In the present setting nonzero held factors
   come from the positive-product chart, nonsingular block equations are
   checked locally in Lemma~\ref{lem:step-011-gram-reserve}, and positive
   solved factors are proved in Proposition~\ref{prop:step-011-block-contraction};
   the target-span map, the Refresh_s identity, all numerical constants, and
   the contraction are derived directly below. No radius, theorem constant, or
   existence neighborhood is imported from the cited paper.
...
prop:step-011-block-contraction).** Under E_sm and its accepted
static-geometry interface (A1), and under the hypotheses and conclusions of
...
product-preserving equal-norm representative. The accepted E_land interface
(A2) supplies one such in-chart input, but the estimate below is uniform over
...
prop:step-011-chronological-contraction).** Under E_sm and its accepted
static-geometry interface (A1), let the accepted E_land interface (A2) supply
the entry state \(e^{\rm land}\). Using
...
\tag{24}
\]
In particular, starting from the accepted E_land state, the generated
chronological orbit remains in the chart and defines E_basin; writing
...
**Proof / justification.** Fix an arbitrary input state in the domain of the
...
For the accepted E_land entry, (A2) gives
...
The accepted E_land interface (A2), together with E_sm and its static
geometry interface (A1), supplies the legal positive exact-span entry.
...
each consumer, and exports the uniform map estimate (24) and the E_land
no-forcing recurrence (25). Therefore the generated event E_basin and
\(\Psi^q\) satisfy the exact sketch-row claim, with no predecessor
...
- **Admissibility conditions and auxiliary tolerances:**
  \(x\le\rho_{\rm ALS}\), \(q\le q_*\), \(\Gamma\le1.01\), positive
  represented products, and the fixed orientation chart (with E_land as the
  generated entry). The explicit inequalities are
...
- \(\varrho\) is a proof-local alias for the setting-defined
  ...
  \(a_j,b_{M,j}\) are proof-local translations of the setting's target Gram,
...
- E_sm, E_land, exact-span membership, full-rank block designs, positive
  ...
  proved in this step and then consumed only after their producer. The
  positive solved-column condition is a local hypothesis for Refresh_s and
  is discharged by Proposition~\ref{prop:step-011-block-contraction}; no
  singular/off-chart cyclic range claim is exported.
