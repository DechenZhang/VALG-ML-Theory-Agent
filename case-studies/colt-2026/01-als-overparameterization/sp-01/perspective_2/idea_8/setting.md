...
- Basic setup:
  ...
  **Symmetric J-aware history update and all-commit budget.** The proof must
  ...
  \tag{J-diag}
  \]
  Hence on the approved two-label audit, where
  \(H_B(p,j)=H_C(p,j)=h\), \(H_A(p,j)=0\), and \(h>0\), this charge is
  required to satisfy
  \[
  \widetilde\Upsilon^A_{p;j,p}
  \ge\eta_0h^2\ge C_\eta\Gamma_\star h^4,
  \qquad
  \widetilde\Upsilon^A_{p;j,p}
  =O(h^4+P^{-D_{\rm stat}}h^2),
  \tag{J-two-label}
  \]
  where \(\nu_{\rm real}\ge h\). The fixed leading constant must make
  (J-update) dominate the exact \(2h^4+O(h^6)\) increment. When
  \(\ell\ne p\),
...
- Goal statement:
  ...
  diagonal identity (J-diag), the dominance check (J-two-label), and the
  off-diagonal reduction \(J_M(p,\ell)=H_M(p,\ell)\) when \(\ell\ne p\).
...
- Rigor:
  ...
  statewise Neumann envelope. Replace only its false H-only accepted-history
  charge by the symmetric J-aware positive/baseline charge (J-charge), and
  add the generated rowwise and one-time global accumulation obligations
  (J-row) and (J-global). No projection, transitivity axiom, modified
  objective, observable target information, or new primitive assumption is
  introduced.
  ...
  asymmetric class. The J-aware repair restores the captured-label identity
  while retaining direct, transitive, and longer correlation paths without
  orthogonalizing true target directions or leaving a terminal floor.
  ...
  On the approved two-label family its leading \(O(h^4)\) term can dominate
  the exact \(2h^4+O(h^6)\) increment, while for \(\ell\ne p\) one has
