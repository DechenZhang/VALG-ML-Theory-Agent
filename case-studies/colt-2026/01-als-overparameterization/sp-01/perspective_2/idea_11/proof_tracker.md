...
| -------------- | --------------- | --------- | ------- | ------------ | ---------------------- |:-----:| -------------- | ------------ | ------- |
| 1 | Two-mode contraction, zero-safe relative certificate, raw capture ledger, and four-block terminal route | sketch | sketch | 1 | Fail | N/A | IDEA_FAIL | /subagent-idea-generator | Empty-prefix exact-orthogonal counterexample: fixed resolvent padding yields rootwise `Omega(sqrt(r) P^-Dstat)` but adaptive Raw-J permits only `O(P^-Dstat/sqrt(r))`; changing padding/Raw-J is a theorem-contract repair. |
