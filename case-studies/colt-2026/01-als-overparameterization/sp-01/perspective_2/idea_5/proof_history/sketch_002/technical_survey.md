...
- Current idea: `perspective_2/idea_5`, transactional one-probe discovery with a radial sign bracket, an angular-only strict-saddle certificate, exact rollback, probe-local response isolation, and a protected terminal chart.
- Setting / goal summary: On the modewise-near-orthogonal Gaussian-smoothed class in `setting.md`, establish a conditional finite-tape reachable-landscape theorem at \(k=\Theta(r^{3/2}(\log r)^2)\), followed by same-target balanced PL refinement to arbitrary relative Frobenius accuracy. The theorem must derive, rather than assume, certificate reachability, target assignment, span transfer, chart entry, and the separate instance/tape probability statements.
...
- Branch notes: This is the only selected framework because it matches the changed certificate tangent and the transactional state interface. All cited papers are motivation or component tools; the theorem-critical isolation, probability, and chart bridges remain direct current-notation obligations. The allowed orthogonal/equal-weight boundary is explicitly tested: pure channels have positive angular curvature, while mixed two-channel states have a negative angular direction.
...
- Disposition: `backup`
- Branch notes: This framework supplies static and probabilistic ingredients only. It cannot be used to import the NeurIPS 2025 quadratic-rank ALS theorem into this GD branch.
...
- Disposition: `backup`
- Branch notes: This framework is downstream-only. It is selected as a fallback local module after the generated discovery entry is established, but it cannot discharge certificate reachability or target assignment.
