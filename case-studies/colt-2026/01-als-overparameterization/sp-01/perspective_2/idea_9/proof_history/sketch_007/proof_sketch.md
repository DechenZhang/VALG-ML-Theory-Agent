...
This is proof-sketch attempt 7 for `perspective_2/idea_9`, rerun in
`revise_sketch` mode after the repeated within-span activation/exchange
failure in the archived attempt-6 step review.  The binding
...
For the repaired activation interface, use the following proof-local
quantities (they are outputs, not assumptions):

```
s_min = (1/4) c_R tau_min P^(-D_cap/2),
eps_act = C_act [tau_max (nu^2 + r nu^3 + sqrt(r)e)
                 + tau_max ||A_I||_1 + B_perp + N_I(g)e/g^3],
zeta_act = 3 eps_act/s_min,
theta_act = min{1/8, (s_min/(32 r tau_max))^(1/3)},
mu_act = c_act tau_min theta_act^4/r.
```

`step_003c` proves the displayed source-coordinate and derivative bounds and
uses the actual values of `eps_act` and `s_min`.  On the branch
`zeta_act<=1/32` it applies the normalized support partition; on its
complement it uses the low-score/defect alternative and either rules out the
certificate or emits the same exchange witness.  It never replaces them by a
uniform guessed `1/r` or `1/sqrt(r)` scale.  If a certificate has a score
below the defect scale, the same step applies the coordinate equations to the
*source part* of the score and either rules out the certificate or returns an
explicit exchange tangent.  Thus a low-score state is not assigned a
positive-curvature claim merely because it is outside all source spans.
The exponent order is fixed after the raw capture and source-normal
comparisons: choose `D_act` and then `D_gap` so that every non-singleton
exchange witness has `g^3 mu_act >= 4 N_I(g) P^(-D_gap)` whenever its
certificate can pass.  The same comparison and `(SE-raw)` give
`3 B_perp <= mu_act/2` at the transfer point.  These are derived margin
comparisons, not new primitive conditions.
...
3. Project the current residual with `Pi_star`, observe that all target-frame
   ...
   actual residuals.  Then show that
...
- Sketch attempt: 7
- Retry mode: `revise_sketch` after the repeated `step_004` blocker
- Triggering artifact: `perspective_2/idea_9/proof_steps/step_004/review.md`
  (SHA-256 `398e81936eded3372cbff74c9e7a3aebc40ca9085c8dab55d9e93534826059b2`)
...
- Local repair: preserve every accepted J-ledger, chart, entry, baseline, and
  generated-flow repair from attempts 4--6.  Keep `step_003b` as the producer
  of full-history source-normal exclusion and add `step_003c`, a separate
  diagonal activation/exchange bridge.  `step_003c` first derives the
  source-perpendicular exclusion and approximate diagonal coordinate
  equations, then performs an adaptive support partition and a finite
  pair/three-mode tangent enumeration.  It exports either a sign-safe
  singleton score/isolation interface or a quantitatively normalized exchange
  tangent.  `step_004` only transfers that already-produced witness to the
  actual residual and compares it with the observable Hessian certificate.
...
The dependency-ordered roadmap has eighteen stable rows, retaining all
existing numeric Step IDs and inserting repair IDs `step_003b` and `step_003c`
before `step_004`.
...
4. `step_003b` uses prior commit-time radial/angular stationarity, generated
   ...
   compatibility.
5. `step_003c` converts the projected residual into an approximate diagonal
   source-coordinate problem.  It proves source-perpendicular exclusion,
   sign-safe coordinate stationarity, adaptive low-score/common-support/
   straddling support cases, and a finite normalized tangent family.  It
   exports a singleton score bridge or an exchange witness with margin
   `mu_act`.
6. `step_004` transfers the `step_003c` witness through `(SE-perp)` and
   `(SE-raw)`, rules out the exchange branch using the *same* `mu_act` scale,
   and exports the certificate dichotomy without redoing support selection.
7. `step_005` turns a certificate into a unique unresolved label and calibration.
8. `step_006` expands one accepted error using the local `z` profile and
   proves the J charge/update, including the repaired O.1 audit and cyclic
   forms.
9. `step_007` derives the quantitative zero-diagonal rootwise l2 bound for
   the already defined `z` profile from a prior prefix budget.
10. `step_008` performs the rootwise factorization and one-time global charge
   sum, retaining all numerical terms.
11. `step_009` closes the protected-state induction from the empty prefix.
12. `step_010` proves deterministic transaction dynamics and exact rollback.
13. `step_011` proves history-uniform full-Haar transaction reachability.
14. `step_012` amplifies the independent tape to all distinct commits.
15. `step_013` proves exact-target balanced Jacobian conditioning, the
    weight-scaled radius `c_chart g_minus/r`, a quadratic sandwich, and PL.
16. `step_014` aligns the generated injection/signs with the exact target orbit
    and proves same-target full-state entry directly from `R-euc`.
17. `step_015` proves first-exit invariance and floor-free terminal descent.
18. `step_016` performs the public rate specialization and baseline assembly.
...
by induction, with the empty prefix as the zero base case.  The bridge itself
  ...
  interface.  Within a landscape call, `step_003c` is the sole producer of
  the activation/exchange witness; `step_004` is only a transfer and
...
- Exposed variables: `r,nu,Gamma_star,P,D_stat,D_gap,D_cap,D_act,eta0,e,`
  ...
  `s_min,eps_act,zeta_act,theta_act,mu_act`, and all displayed budgets.
...
- Required bridge or simplification obligations: `step_003` defines the local
  ...
  The new `step_003c` keeps the activation defect `eps_act` separate from the
  capture score `s_min`, derives source-perpendicular and coordinate
  stationarity bounds, and chooses `theta_act` and `mu_act` from the actual
  certificate scale.  Its finite tangent enumeration exports a margin only
  after proving `g^3 mu_act >= 4 N_I(g)P^(-D_gap)`; no `1/r` versus
  `1/sqrt(r)` conversion is deferred to `step_004`.
...
- The activation/exchange witness is a derived local output of `step_003c`,
  not a conditional premise hidden inside `step_004`.  Its inputs are the
  same-target projected residual from `step_003b`, the observable certificate,
  and the already bounded target-frame/history defect `eps_act`.  The step
  derives source-perpendicular bounds from the projected contraction equations,
  then derives approximate diagonal coordinate equations and chooses adaptive
  thresholds `theta_act` and `mu_act`.  A finite enumeration of pair rotations
  and synchronized three-mode rotations handles low-score, one-mode-straddling,
  common-support, sign, and threshold-equality cases.  It exports a singleton
  score bridge or a normalized exchange tangent; it never exports a target
  label before the singleton branch is established.
...
- Theorem role: supplies the first legal full-space history control consumed by
  `step_003c`/`step_004` chain; it also prevents the invisible outside-span certificate from the
...
- Closure/dominance relation: `q_nor<=1/4` gives a finite triangular
  ...
  then receives the unchanged `A_I/Theta_I` target-frame controls, so Step
  004's exchange argument has a legal source for every defect class.
...
- Producer-consumer provenance: `step_003b` consumes `step_001` (source spans,
  ...
  identity to the current `step_003c`; `step_003c`/`step_004`/`step_005` produce the next
...
### Diagonal activation/exchange bridge (`step_003c`)

- Step ID: `step_003c`.
- Claim class: source-support activation, signed singleton isolation, or
  quantitatively normalized exchange curvature.
- Theorem role: this is the sole producer of the within-span
  activation/exchange witness. It separates the finite diagonal support
  algebra from the actual-residual transfer performed later by `step_004`.
- Mechanism source: let \(R^\parallel=\Pi_\star R_I\), and form
  \(v_A=R^\parallel\times_2v^\top\times_3w^\top\), cyclically. The angular
  certificate and exact identity
  \(\operatorname{grad}_{\cal S}f_I=-g^3\operatorname{grad}_{\cal S}s_I\)
  give \(\|P_{u^\perp}v_A\|\le N_I(g)e/g^3+\sqrt3B_\perp\).
  Since \(v_A\) lies in the realized source span, its normal component is
  exactly \(-s^\parallel u_\perp\). Capture, radial comparability, and
  `(SE-raw)` therefore imply
  \[
  \|u_\perp\|+\|v_\perp\|+\|w_\perp\|
  \le 3\{N_I(g)e/g^3+\sqrt3B_\perp\}/s^\parallel .
  \tag{Act-perp}
  \]
  This is the source-perpendicular exclusion; no flat outside-span state is
  assigned an unsupported positive-Hessian mechanism.
- Source-to-claim adequacy: orthonormalize each realized source frame only as
  a proof coordinate change, \(\widehat M_M=M_MG_M^{-1/2}\), and set
  \(x_{j,A}=a_j=\langle u,\widehat m_{j,A}\rangle\), with
  \(x_{j,B}=b_j\) and \(x_{j,C}=c_j\).
  In particular, the orthonormal-frame norm identities give
  \(\|a\|_2^2,\|b\|_2^2,\|c\|_2^2\ge1-9\zeta_{\rm act}^2\) on the
  small-defect branch; on its complement the low-score clause below is used
  without treating a missing norm as an assumption.
  Direct expansion of the projected target and accepted history, using
  `S`, `A_I`, `Theta_I`, and the accepted
  `(Hist-F)` record, gives a displayed derivative remainder
  \(\varepsilon_{\rm act}\le\epsilon_{\rm act}\).
  Before any root is selected, derive for every unresolved \(j\)
  \[
  |\tau_jb_jc_j-s^\parallel a_j|,\quad
  |\tau_ja_jc_j-s^\parallel b_j|,\quad
  |\tau_ja_jb_j-s^\parallel c_j|
  \le\epsilon_{\rm act}.
  \tag{Act-eq}
  \]
  No root-indexed `Theta` entry is used to derive these equations.
- Key positive/control term or structural source: the positive projected
  score floor \(s^\parallel\ge s_{\min}\), the three equations
  `(Act-eq)`, source-coordinate normalization from
  `(Act-perp)`, and the exact product-sphere Hessian identity.
- Opposing defect terms: source-frame remainder, accepted-history
  contraction, numerical stationarity, and \(B_\perp\). They are displayed
  separately inside \(\epsilon_{\rm act}\); none is replaced by an unstated
  \(1/r\) or \(1/\sqrt r\) comparison.
- Closure/dominance relation: define
  \[
  L=\{j\in U_I:\tau_j|a_jb_jc_j|\ge8\epsilon_{\rm act}\},\qquad
  W_L=\sum_{j\in L}\tau_j|a_jb_jc_j|.
  \]
  The score identity and `(Act-eq)` yield the sign-safe mass
  inequality \(W_L\ge s^\parallel-4\epsilon_{\rm act}\).
  With the adaptive threshold \(\theta_{\rm act}\), normalization and
  `(Act-eq)` give an exhaustive partition: (i) one common label is
  active in all three modes; (ii) at least two labels have common three-mode
  support; or (iii) support is low-score or mode-straddling. Threshold
  equality, \(|U_I|=1\), and every mode permutation are assigned explicitly.
- Exchange tangent relation: for each active pair \(p\ne q\), define
  \[
  d_M^{p,q}=
  {x_{q,M}\widehat m_{p,M}-x_{p,M}\widehat m_{q,M}\over
   (x_{p,M}^2+x_{q,M}^2)^{1/2}}.
  \tag{Act-rot}
  \]
  Enumerate the three two-mode tangents and the synchronized three-mode
  tangent, with signs chosen from the finite product-one sign set. Define
  \(Q_{\rm ex}(p,q)\) as the maximum exact score-Hessian quadratic form over
  this finite family. Substitution of `(Act-rot)` into the exact
  Hessian formula, followed by `(Act-eq)`, proves either the
  singleton branch or
  \[
  Q_{\rm ex}(p,q)\ge\mu_{\rm act}
  =c_{\rm act}\tau_{\min}\theta_{\rm act}^4/r.
  \tag{Act-ex}
  \]
  The calculation retains the \(-s^\parallel\|\xi\|^2\) normalization term,
  both \(p,q\) contributions, all sign choices, and every
  \(\epsilon_{\rm act}\) remainder. It never assumes that
  \(1-x_{p,M}^2-x_{p,N}^2\) is positive.
- Low-score and boundary handling: if
  \(s^\parallel\le8\epsilon_{\rm act}\), `(Act-eq)`, the capture
  floor, and source-coordinate normalization either contradict the existence
  of a certificate or produce a mode-straddling pair satisfying
  `(Act-ex)`. A direction perpendicular in all three source modes is
  handled by `(Act-perp)`, not by a largest-singular-vector
  assertion.
- Sign-safe singleton export: in branch (i), multiply the three equations in
  `(Act-eq)` by their corresponding coordinates and sum over all
  unresolved labels. Define the retained other-label term explicitly as
  \(R_{\rm oth}=\sum_{j\ne p}\tau_ja_jb_jc_j\). The same three equations,
  the normalization deficits from `(Act-perp)`, and the active-set threshold
  give the finite support bound
  \(|R_{\rm oth}|\le\tfrac14\tau_p|a_pb_pc_p|+3\epsilon_{\rm act}\); this is
  proved before any score simplification and is not a deletion of the other
  labels. After a product-one sign choice,
  \[
  s^\parallel\ge\tau_p|a_pb_pc_p|-4\epsilon_{\rm act}
  =:s_{\rm iso}>0.
  \tag{Act-singleton}
  \]
  This, rather than deletion of the other labels, is the singleton score
  producer.
- Residual-to-target adequacy: `step_003c` analyzes the same-target
  projection only. It exports either `(Act-singleton)` or a unit
  tangent with `(Act-ex)`; `step_004` is responsible for
  transferring that output to the actual residual through the already proved
  \(B_\perp\) bounds.
- Accumulation behavior / scope compatibility: one finite support calculation
  per prefix. The only history terms are the already finite
  `A_I`/`B_hist,F` and \(B_\perp\) budgets, entering with
  unknown sign through \(\epsilon_{\rm act}\); no new repeated forcing is
  introduced.
- Obligation locality classification: step-local. The repeated attempt-6
  issue is repaired by a new producer interface, not relabeled as an idea
  failure.
- Noncircular closure status: `steps_001--003 -> step_003b -> step_003c`;
  `step_003c` produces the root-free support alternative before
  `step_004` or any root-indexed ledger consumer.
- Entry-state / first-update stress result: at the empty prefix,
  \(A_I=B_\perp=0\); positive-score atoms satisfy the exact coordinate
  equations, zero-score seeds are rejected, and a fully source-perpendicular
  state is excluded by `(Act-perp)`. In the exact \(H=e=0\) branch,
  \(\epsilon_{\rm act}=0\) and the bridge is the exact diagonal calculation.
- Baseline conclusion preserved: exact atom certificates, exact rollback,
  exact representation, and floor-free terminal refinement are unchanged.
- Producer-consumer provenance: `step_003c` consumes
  `step_003b`'s \(R^\parallel\), residual reserve, and differential
  transfer. It exports `(Act-singleton)` or
  `(Act-ex),mu_act` only to `step_004`; only a proved
  singleton is allowed to flow to `step_005`.
- Null or boundary regime tested: zero/defect-scale score,
  one/two/all source-perpendicular modes, \(|U_I|=1\), threshold equality,
  opposite product signs, common support, one-mode straddling, and exact
  orthogonal/noiseless data.
- Target conclusion false if source vanishes: without `(Act-eq)` or
  the finite tangent family, mixed support can evade the Hessian comparison.
  Both are direct consequences of the unchanged setting.
- Repair route if source is unsupported: `/proof-sketch` for a
  missing interface; idea generation only after an actual contradiction to
  this same-setting diagonal lemma, not after another local derivation
  failure.

### Conditional landscape transfer summary (`step_004` and `step_005`)

The former monolithic activation prose is superseded in attempt 7 by the
dedicated `step_003c` interface inserted below.  The rows in this section
record only the downstream transfer and matching obligations; no support
trichotomy or exchange margin is consumed from this summary.
...
- Mechanism source: the exact product-sphere Hessian identity and the
  already-produced `Act-singleton`/`Act-ex` witness from `step_003c`, combined
...
- Closure/dominance relation: `(Act-ex)` is compared with the observable
  Hessian threshold using `g^3(mu_act-3B_perp) >= 2N_I(g)P^(-D_gap)`;
  `(Act-singleton)` is transferred to the actual score and becomes the sole
  downstream branch.
- Accumulation behavior / scope compatibility: conditional on one legal prefix;
  no cross-prefix claim is made until `step_009`.
- Obligation locality classification: step-local.
- Noncircular closure status: `step_003b -> step_003c -> step_004 ->
  step_005`; `step_009` later supplies the prior-prefix records used by the
...
- Producer-consumer provenance: only the transferred singleton output feeds
  `step_005` (which stores the post-fit audit), then `step_006`, `step_009`,
...
| Diagonal activation/exchange witness | `step_003c` | `R_I^parallel`, `(Res)`, `(SE-raw)`, `S`, `A_I`, `Theta_I`, the certificate score/gradient, and the exact source-frame Gram bounds | frame remainder, accepted-history contraction, numerical stationarity, and `B_perp` are all retained in `epsilon_act`; no sign or scale is hidden | the witness is expressed in the same realized source-frame product spheres; `step_004` transfers it back to the exact actual residual with the same `B_perp` metric | `(Act-perp)` and `(Act-eq)` yield the adaptive support partition; finite pair/three-mode tangent enumeration gives `Q_ex>=mu_act`, or `(Act-singleton)` gives a positive singleton score | `s_min`, `theta_act`, `mu_act`, and the explicit comparison `g^3(mu_act-3B_perp)>=2N_I(g)P^(-D_gap)`; all are derived after raw comparisons | `step_004` only; singleton output then flows to `step_005`, while exchange output is consumed solely by the certificate contradiction | None |
| Certificate matching and calibration | `steps_004--005` | `Act-singleton`, radial equations, `(SE-perp)`, `(SE-raw)`, branch `(SE-lb)`, capture, and separation | projected target-frame, source-normal, numerical, and guard defects are already split; mixed-support defects have been discharged by `step_003c` | `step_004` transfers the same-target projected singleton to the actual residual; the accepted slot is compared directly with the exact realized atom in `(R-euc)` | `(Act-singleton)` plus `(SE-raw)` gives the isolation margin; `(SE-lb)` is used only for eta-scale calibration on `E_lb` | positive `s_iso`, radial root, angular gap, separation `1/4`, and `B_perp` slack | `steps_006,009,011,014`, plus the next temporal `step_003b` consumer | None |
...
| Diagonal activation/exchange witness | source-support activation and strict-saddle alternative | exact projected contraction equations, source-perpendicular identity, adaptive product-mass partition, and finite pair/three-mode rotations | direct current-notation derivation from `step_003b` plus `S,A_I,Theta_I`; no external theorem wrapper | `Act-perp`, `Act-eq`, `Act-singleton`, and `Act-ex` have the exact projected source-frame interface consumed by `step_004` | orthonormal frame is only a coordinate device; the produced witness is transferred to the same actual target by `B_perp` in `step_004` | `W_L>=s_parallel-4 eps_act`; either `s_iso>0` or `Q_ex>=mu_act`; choose exponents so `g^3(mu_act-3B_perp)>=2N P^(-D_gap)` | `s_min`, `eps_act`, `theta_act`, `mu_act`, source-coordinate norms, and all sign/tangent choices are displayed before export | projected residual is a same-target analysis object; `step_004` dominates its transfer residual at the Hessian/capture scale | one finite support calculation; history forcing enters only through the already finite `A_I` and `B_perp` budgets with no hidden cancellation | `step_001/002/003 -> step_003b -> step_003c -> step_004`; `step_009` supplies prior records to the next temporal bridge | `step_003c` -> `step_004`; singleton only -> `step_005`; exchange is consumed by the certificate contradiction | zero score, all-mode perpendicular, low-score defect scale, threshold equality, common support, straddling, signs, and `|U_I|=1` are explicit | step-local | None |
| Matching, calibration, injection | support/membership, generated entry | radial root, angular stationarity, finite sign orbit, and the transferred activation witness | direct derivation from `step_003c -> step_004` and setting equations | `R-euc`,`R-dir`, and the stored normal audit are the exact consumed interfaces | signs are analysis-only; target labels are not algorithm inputs; projection uses the same realized source spans | singleton score bridge plus `(SE-raw)` gives the isolation margin; `(SE-lb)` is used only on `E_lb` | score, gap, capture, separation, `Theta`, `B_perp`, and `E_lb` are available before export | actual accepted slot is compared with exact realized atom; projected residual error is bounded in the same ambient metric | one output per accepted commit, finite at most r | activation -> transfer -> matching/audit -> next temporal bridge | `step_005` -> `step_003b,step_006,step_009,step_011,step_014` | empty state, first positive score, mixed support already discharged, separation threshold, and invisible witness | step-local | None |
...
| Conditional `Theta_I` | `step_003` | `step_003b,step_003c,steps_004,005,007` | current residual control | `step_002 -> step_003 ->` target-frame bridge/activation/landscape and later quantitative profile | explicitly conditional | None |
...
| Diagonal activation/exchange witness | `step_003c` | `R_I^parallel`, `(Res)`, `(SE-raw)`, `S`, `A_I`, `Theta_I`, and the observable certificate | source-frame, history, numerical, and source-normal defects are retained in `eps_act`; exchange margin is not inferred from a later row bound | the projected source-coordinate calculation is a same-target analysis bridge; `step_004` transfers the witness to `R_I` with `B_perp` | `Act-perp` + `Act-eq` + finite tangent family -> either `Act-singleton` or `Act-ex` with `mu_act`; no root is selected before this output | `step_003c` -> `step_004`; only the singleton branch -> `step_005`; exchange is consumed by the certificate contradiction | `derived` | None |
| Certificate matching and calibration | `steps_004--005` | `step_003c` singleton output, transferred `(SE-perp)/(SE-raw)`, radial equations, and separation | unique labels, factor errors, and stored post-fit normal audit | `step_003c -> step_004 -> current step_005 audit -> next temporal step_003b -> transition/entry` | derived after activation and prefix bridge | None |
...
Sketch Steps = 18 stable rows (including repair IDs `step_003b` and
`step_003c`); all rows
...
| `step_003c` | From `R_I^parallel` and the observable certificate, derive source-perpendicular exclusion and approximate diagonal coordinate equations; partition low-score, common-support, and straddling cases; enumerate normalized pair/three-mode tangents; export either sign-safe `(Act-singleton)` or `(Act-ex)` with `mu_act`. | `step_001`, `step_002`, `step_003`, `step_003b` | derived event, `(Res)`, `(SE-raw)`, conditional `A_I/Theta_I`, and the local observable certificate; no target label or source-support condition is assumed | source-frame coordinate normalization, adversarial signs, low-score flat states, threshold equality, and one consistent curvature scale | direct contraction equations, source-perpendicular identity, finite support mass partition, and exact product-sphere Hessian calculation over a finite tangent family | `s_min`, `eps_act`, `theta_act`, `mu_act`, `(Act-perp)`, `(Act-eq)`, `(Act-singleton)`, `(Act-ex)` | R2; retain raw frame/history/numerical defects and choose `D_act,D_gap` only after the displayed comparisons | PENDING |
| `step_004` | Transfer the already-produced activation witness through `(SE-perp)` and `(SE-raw)`, rule out `(Act-ex)` with the same `mu_act` scale, and export the singleton certificate dichotomy without redoing support selection. | `step_003b`, `step_003c` | derived event and `E_lb` only where explicitly used; conditional prior-prefix records; `assump:base-product-balance` | actual-vs-projected score/gradient/Hessian transfer and exact threshold comparison | direct product-sphere Hessian identity plus accepted same-target transfer; no unproved support or `1/r` to `1/sqrt(r)` conversion | isolation margin and certificate dichotomy | R2; retain `B_perp`, `eps_act`, `D_gap`, and branch-qualified calibration | PENDING |
...
| `step_009` | Close the abstract protected-state induction from `I=emptyset,A=0` through all at most r commits, carrying raw numerical terms, invoking `step_003b`, `step_003c`, and `step_004` before every landscape call, and using the `E_lb`/exact-limit branch for any simplified row interface; concrete transaction dynamics consume this invariant only afterward. | `step_001`, `step_002`, `step_003`, `step_003b`, `step_003c`, `step_004`, `step_005`, `step_006`, `step_007`, `step_008` | all nine primitive assumptions; prior generated certificate/matching/normal records; derived `E_inst`, `E_lb`; no source-normal or activation condition is assumed | noncircular prefix ordering, temporal normal recurrence, activation witness flow, and unresolved bookkeeping | finite induction, triangular normal solve, diagonal activation bridge, exact rollback-preservation interface, and one-time ledger | raw prefix invariant, `(SE-perp)/(SE-raw)`, `(Act-singleton)/(Act-ex)`, all budgets, and branch-qualified `(SE-lb)`/`J-row` | R2; uniform finite horizon with no unsupported absorption; carries raw profile outside `E_lb` and specialized rows only on `E_lb` or exact H=e=0 | PENDING |
...
- Diagonal activation boundary: apply `(Act-perp)` before using any source
  coordinate.  Test score below `8 eps_act`, one active label, two-label common
  support, one-mode straddling, opposite product signs, exact threshold
  equality, and `|U_I|=1`.  The finite tangent family includes all three
  two-mode rotations and the synchronized three-mode rotation; the maximum is
  compared to the single declared `mu_act` scale.  A fully source-perpendicular
  point is handled by the normal component of the contraction equation and is
  never treated as a positive-curvature witness.
...
-> step_004 -> current step_005 audit -> next step_003b`.
...
`(SE-raw)` before `step_004`; only the named `E_lb` branch additionally exports
`(SE-lb)`.  `step_003c` then derives the source-perpendicular relation,
coordinate equations, adaptive support partition, and finite tangent witness;
`step_004` transfers that witness and `step_005` performs matching.  These
...
interface, retain its positive terms at `h=e^2`, and attempt 6 completed the
same-setting full-history source-normal producer through the explicit
triangular normal bridge.  The unresolved-target lower bound and accepted-error
reverse-triangle reserve `(Res)` are displayed before any capture comparison.
The unconditional `(SE-raw)` comparisons exclude the invisible outside-span
witness at capture and curvature scales; the eta-scale `(SE-lb)` comparison is
explicitly conditioned on the derived `E_lb` event, with exact `H=e=0`
equality handled separately.  The projected residual preserves the exact
`A_I/Theta_I` target-frame interface, while the numerical normal remainder is
controlled at each named branch.  Attempt 7 additionally gives the repeated
within-span obligation its own `step_003c` producer: source-perpendicular
exclusion, sign-safe coordinate equations, adaptive support cases, and a
finite tangent family with one `mu_act` scale are all declared before
`step_004`.  `step_004` performs only same-target transfer and certificate
comparison.  `R-acc` remains available for its exact ledger consumers, with no
unsupported factor-norm conversion.  The local z definition, temporal
normal-audit flow, rowwise bound, abstract prefix closure, all `step_003b`
consumers, and `C_L`/`C_path_star` dependence are declared in producer order.
