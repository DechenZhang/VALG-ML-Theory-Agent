...
This is proof-sketch attempt 8 for `perspective_2/idea_9`, rerun in
`revise_sketch` mode after the attempt-7 sketch review exposed a
first-order frame-convention defect in the activation bridge.  The binding
...
- Sketch attempt: 8
- Retry mode: `revise_sketch` after attempt-7 sketch review
- Triggering artifact: `proof_history/sketch_007/proof_sketch_review.md`
  (SHA-256 `f456e94b125f560233e27f2599a1b08126e987df47135ad18e42e4287562bbf6`)
...
- Local repair: preserve every accepted J-ledger, source-normal, chart, entry,
  baseline, and generated-flow repair from attempts 4--7.  Keep `step_003b`
  unchanged and replace `step_003c`'s orthonormalized coordinates by exact
  current/dual-frame coordinates.  The diagonal polynomial is retained with
  its exact Gram-ellipsoid stationarity equation, so first-order Gram mixing is
  structural rather than an additive defect.  Derive capture and radial
  comparability before any activation budget, use a simultaneous root-free
  `Theta_act` maximum, and audit every low/common/straddling/sign/mode boundary
  with normalized dual tangents.  `step_004` remains only the same-target
  transfer and observable-certificate comparison.
...
5. `step_003c` first derives `(Act-cap)--(Act-proj)`, then writes the projected
   ...
   source-perpendicular exclusion, the complete low/common/straddling/sign
   partition, and normalized single-label/pair dual tangent bounds.  It exports
   a singleton score bridge or an exchange witness with margin `mu_act`.
...
- Required bridge or simplification obligations: `step_003` defines the local
  ...
  `Theta_act` maximum before support selection.  Its finite dual-tangent
  enumeration exports `mu_act=c_act tau_min/r^2` only after proving all four
  inequalities in `(Act-margin)`; no first-order Gram term or `1/r` versus
...
- The activation/exchange witness is a derived local output of `step_003c`,
  ...
  the realized Gram matrices, and the simultaneously produced full
  candidate-indexed `Theta_I`.  The step first derives radial comparability,
  then uses exact correlations `x=M^T u` and dual coefficients
  `alpha=G^(-1)x`; it consumes only `Theta_act=max_(j,M)||Theta_(j,M,.)||_2`
  before root selection.  A finite enumeration of normalized dual
  single-label, two-mode pair, and synchronized three-mode pair tangents
  handles low score, one-mode straddling, common support, signs, all mode
  permutations, threshold equality, and `|U_I|=1`.  It exports a singleton
  score bridge or a normalized exchange tangent; a root-specific `Theta` row
  is unavailable until `step_005` has received the selected label.
...
#### Complete support and tangent alternative
...
- Residual-to-target adequacy: the dual map is built from the same realized
  primal factors and exactly diagonalizes label selection; no whitened or
  transformed target replaces \(T\). `step_004` transfers
  `(Dual-singleton)` or `(Dual-exchange)` to the actual
  residual using the accepted \(B_\perp\) bound.
...
- Noncircular closure status:
  `steps_001--003 -> step_003b -> ordered radial scale -> dual equations
  -> finite tangent alternative -> step_004`. A root-specific
  \(\Theta\)-row is consumed only after `step_005` receives \(p\).
- Entry-state / first-update stress result: the exact one-mode-correlated atom,
  empty prefix, low score, one/two/all normal modes, common support,
...
- Producer-consumer provenance: `step_003c` consumes the certificate,
  realized Grams, \(R_I^\parallel\), the root-free \(\Theta_{\rm act}\), and
  `step_003b`'s transfer outputs. It produces exactly one of
  `(Dual-singleton)` or `(Dual-exchange)` for
  `step_004`; only the singleton branch may flow to
  `step_005`.
...
The former orthonormal-frame activation prose is superseded in attempt 8 by
the dual/current-frame `step_003c` interface above.  The rows in this section
...
- Mechanism source: the exact product-sphere Hessian identity and the
  already-produced `Dual-singleton`/`Dual-exchange` witness from `step_003c`, combined
...
- Closure/dominance relation: `(Dual-exchange)` is compared with the observable
  Hessian threshold using
  `g^3(4mu_act-3B_perp) >= 3g^3mu_act >= 12N_I(g)P^(-D_gap)`;
  `(Dual-singleton)` is transferred to the actual score and becomes the sole
  downstream branch.
...
| Dual/current-frame activation witness | `step_003c` | certificate capture/radial equations, `R_I^parallel`, `(Res)`, `(SE-raw)`, realized `G_M`, complete candidate-indexed `Theta_I`, and the root-free `Theta_act` maximum | exact Gram coupling is retained through `x=M^T u`, `alpha=G^(-1)x`; history, numerical, and source-normal defects alone enter `epsilon_act` | primal and dual coordinates are built from the exact realized atoms; `Dual-stat` is exact on the one-mode-correlated atom and `step_004` transfers the resulting same-target witness by `B_perp` | ordered `(Act-rad)` first; then `Dual-stat`, explicit low/common/straddling/sign cases, and normalized dual tangents export `Dual-singleton` or `Dual-exchange` | `s_low`, `beta_act`, `delta_act`, `mu_act`, and all inequalities in `(Act-margin)`; no additive `O(nu)` frame defect | `step_004`; only a transferred singleton flows to `step_005` | None |
| Certificate matching and calibration | `steps_004--005` | `Dual-singleton`, `(Act-rad)`, `(SE-perp)`, `(SE-raw)`, branch `(SE-lb)`, capture, and separation | projected target-frame, source-normal, numerical, and guard defects are split; all mixed-support cases were discharged by `step_003c` | `step_004` transfers the same-target dual/current-frame singleton to the actual residual; the accepted slot is compared directly with the exact realized atom in `(R-euc)` | `Dual-singleton` plus `(SE-raw)` gives the isolation margin; `(SE-lb)` is used only for eta-scale calibration on `E_lb` | positive `s_iso`, radial root, angular gap, separation `1/4`, and `B_perp` slack | `steps_006,009,011,014`, plus the next temporal `step_003b` consumer | None |
...
| Dual/current-frame activation witness | source-compatible support activation and strict-saddle alternative | ordered capture/radial scale, exact biorthogonality, Gram-ellipsoid stationarity, root-free `Theta_act`, and normalized dual tangents | direct current-notation derivation from `steps_001--003b`; no external theorem wrapper | `Dual-stat` exactly selects the target label and passes the one-mode-correlated exact-atom test; `Dual-low/common/straddle/singleton` have the interfaces consumed by `step_004` | `D=MG^(-1)` is built from the same realized primal factors; Gram effects remain exact in `x=G alpha`, and the output is transferred by `B_perp` | ordered `(Act-rad)`; `Dual-energy/norm/select`; explicit low/common/straddling/sign/threshold/cardinality-one inequalities; net `Q>=4mu_act` | `s_low`, `beta_act`, `delta_act`, `eps_act`, `mu_act`, dual norm bounds, and all tangent normalizations/sign maxima are displayed | projected and actual residuals share the exact target; `step_004` dominates only the already bounded `B_perp` transfer | one-prefix calculation; history enters through simultaneous `Theta_act` and finite `B_perp`, with unknown signs charged in `eps_act` | `steps_001--003 -> step_003b -> Act-rad -> Dual-stat -> finite tangent alternative -> step_004` | `step_003c` -> `step_004`; singleton-only -> `step_005`; exchange is consumed by the certificate contradiction | exact atom with one-mode correlation, low score, normal modes, common/straddling support, signs, equality, cardinality one, and exact branch pass | step-local | None |
| Matching, calibration, injection | support/membership, generated entry | radial root, angular stationarity, finite sign orbit, and transferred `Dual-singleton` | direct derivation from `step_003c -> step_004` and setting equations | `R-euc`,`R-dir`, and the stored normal audit are the exact consumed interfaces | signs are analysis-only; target labels are not algorithm inputs; dual coordinates use the same realized source spans | `Dual-singleton` plus `(SE-raw)` gives the isolation margin; `(SE-lb)` is used only on `E_lb` | score, gap, capture, separation, root-specific `Theta` available after selection, `B_perp`, and `E_lb` are available | actual accepted slot is compared with the exact realized atom in the ambient factor metric | one output per accepted commit, finite at most r | dual activation -> transfer -> matching/audit -> next temporal bridge | `step_005` -> `step_003b,step_006,step_009,step_011,step_014` | empty state, first positive score, mixed support discharged, separation threshold, and invisible witness pass | step-local | None |
...
| Dual/current-frame activation alternative | `step_003c` from the observable certificate, realized Grams, `R_I^parallel`, root-free `Theta_act`, and `step_003b` transfer outputs | `step_004`; only the singleton branch is then consumed by `step_005` | excludes every non-singleton certificate or supplies the unique-label antecedent without a first-order frame defect | `step_001/002/003 -> step_003b -> Act-rad -> Dual-stat -> step_003c alternative -> step_004 -> singleton-only step_005` | derived | None |
| Certificate matching and calibration | `steps_004--005` | `step_003c` dual/current-frame singleton output, transferred `(SE-perp)/(SE-raw)`, radial equations, and separation | unique labels, factor errors, and stored post-fit normal audit | `step_003c -> step_004 -> current step_005 audit -> next temporal step_003b -> transition/entry` | derived after activation and prefix bridge | None |
...
| `step_003c` | First derive capture, radial comparability, positive `g^3`, and projected score; then use exact current correlations `x=M^T u` and dual coefficients `alpha=G^(-1)x` to prove `Dual-stat`, the exact-atom boundary, root-free `Theta_act`, every low/common/straddling/sign/mode/threshold/`|U|=1` case, and normalized dual-tangent margins. | `step_001`, `step_002`, `step_003`, `step_003b` | derived event, `(Res)`, `(SE-raw)`, complete candidate-indexed `Theta_I`, current prior-prefix raw budget, and the observable certificate; no target label or source-support condition is assumed | radial ordering, exact nonorthogonal Gram convention, root-free preselection interface, adversarial signs, and one normalized margin | biorthogonal current-frame identities, Gram-ellipsoid energy, exact dual selectors, and explicit product-sphere Hessian calculations | `(Act-rad)`, `Theta_act`, `eps_act`, `s_low`, `beta_act`, `delta_act`, `mu_act`, `Dual-stat`, `Dual-singleton`, `Dual-exchange` | R2; retain raw history/numerical terms, exact Gram coupling, and verify `(Act-margin)` on `E_lb`, its complement, and the exact branch | PENDING |
| `step_004` | Transfer the already-produced dual/current-frame activation witness through `(SE-perp)` and `(SE-raw)`, rule out `Dual-exchange` with the same `mu_act` scale, and export the singleton certificate dichotomy without redoing support selection. | `step_003b`, `step_003c` | derived event and `E_lb` only where explicitly used; conditional prior-prefix records; `assump:base-product-balance` | actual-vs-projected score/gradient/Hessian transfer and exact threshold comparison | direct product-sphere Hessian identity plus accepted same-target transfer; no coordinate, support, or scale conversion | isolation margin and certificate dichotomy | R2; retain `B_perp`, `eps_act`, `D_gap`, and branch-qualified calibration | PENDING |
...
| `step_009` | Close the abstract protected-state induction from `I=emptyset,A=0` through all at most r commits, carrying raw numerical terms and invoking `step_003b`, ordered-scale `step_003c`, and transfer `step_004` before every matching call; use `E_lb`/exact-limit only for named simplifications. | `step_001`, `step_002`, `step_003`, `step_003b`, `step_003c`, `step_004`, `step_005`, `step_006`, `step_007`, `step_008` | all nine primitive assumptions; prior generated certificate/matching/normal records; derived `E_inst`, `E_lb`; no source-normal or activation condition is assumed | noncircular prefix ordering, temporal normal recurrence, root-free/preselection versus selected-row ordering, and unresolved bookkeeping | finite induction, triangular normal solve, dual/current-frame activation bridge, exact rollback-preservation interface, and one-time ledger | raw prefix invariant, `(SE-perp)/(SE-raw)`, `Dual-singleton/Dual-exchange`, all budgets, and branch-qualified `(SE-lb)`/`J-row` | R2; uniform finite horizon with no unsupported absorption; carries raw profile outside `E_lb` and specialized rows only on `E_lb` or exact H=e=0 | PENDING |
...
- Dual/current-frame activation boundary: first derive `(Act-rad)`.  On the
  attempt-7 counterexample with only mode A correlated and the scratch state
  equal to atom 1, verify `x_A=G_Ae_1`, `alpha_A=e_1`, and exact zero label-2
  residual in `(Dual-stat)`.  Then test `s_parallel<s_low`, one active label,
  two-label common support, one- and two-mode straddling, opposite product
  signs, exact threshold equality, and `|U_I|=1`.  The finite tangent family
  includes normalized single-label dual residuals, all three pair two-mode
  rotations, and all four product-one synchronized three-mode rotations.  A
...
the complete finite tangent alternative.  `step_004` transfers that witness
...
controlled at each named branch.  Attempt 8 repairs the attempt-7 activation
interface with exact biorthogonal coordinates: the one-mode-correlated atom
passes `(Dual-stat)` exactly, capture/radial bounds precede every denominator,
the preselection history input is the root-free `Theta_act` maximum, and all
low/common/straddling/sign/mode/equality/`|U_I|=1` cases have normalized dual
tangents at the same `mu_act` scale.  The seven-column generated-flow row now
separates inputs, consumers, dependency path, and provenance.  `step_004`
performs only same-target transfer and certificate comparison.  `R-acc`
remains available for its exact ledger consumers, with no
