...
orthogonal/noiseless \(H_A=H_B=H_C=0,\ e_{\rm num}=0\) branch is recorded
...
- \(e_{\rm num}:=P^{-D_{\rm stat}}\) is the setting-defined analysis
  tolerance used only to name the exact numerical baseline interface. The
  exact-limit specialization sets this tolerance to zero; it is not a
  primitive assumption in this step.
...
- On the exact \(H_A=H_B=H_C=0,\ e_{\rm num}=0\) branch all centered Gram terms and numerical
...
The named normalization dependency is the simultaneous conclusion of
Lemma~\ref{lem:p2-i9-s8-step001-normalization}, whose primitive basis is the
same six assumptions displayed above together with the
Lemma~\ref{lem:p2-i9-s8-step001-perturbation} event. In particular, on
\(E_{\rm str}\), it supplies

\[
 (2\kappa)^{-1}\le\|m^{\rm raw}_{j,M}\|_2\le2\kappa
 \quad\text{for every }j\in[r],\ M\in\{A,B,C\},
 \tag{Norm-dep}
\]

as well as the normalized-Gram conclusion used by this unit. This named
dependency is part of the theorem interface, not an implicit consequence
attributed to \(E_{\rm str}\) alone.
...
Define the \(\mathcal F\)-measurable even-column fiber event

\[
 \mathcal G_{\rm even}:=
 \bigcap_{q=1}^{m}
 \left\{(2\kappa)^{-1}\le\|a_{j_q}\|_2\le2\kappa\right\}.
\]

On \(\mathcal G_{\rm even}\), the displayed standard-deviation lower bound
holds. On \(\mathcal G_{\rm even}^{c}\), the intersection with
\(E_{\rm str}\) is empty by (Norm-dep). Thus, on every nonempty conditional
fiber, where \(\mu_q\) is an arbitrary \(\mathcal F\)-measurable mean, Result
G2 with \(a=4\kappa^2t\) gives
...
\[
 ...
 \le {\bf 1}_{\mathcal G_{\rm even}}(r^{-40})^m
 \le r^{-40}.
\]
...
zero (\(e_{\rm num}=0\)), then the pair and triple gaps are equalities, the target
...
comparisons. On the exact \(H_A=H_B=H_C=0,\ e_{\rm num}=0\) baseline, they
instead use equality and
zero source terms.
