...
This is proof-sketch attempt 9 for `perspective_2/idea_9`, rerun in
`revise_sketch` mode after the attempt-8 `step_003c` review exposed a
...
Attempt 9 adds a separate matched-mass bridge after the current/dual
activation algebra. Support is enumerated over all labels, not only `U_I`.
With `V_I=pi_I(I)`, the projected protected-error tensor is
`E_I^mat=Pi_star R_I-sum_(j in U_I) tau_j phi_j`. A finite mass split proves
that either unresolved target mass supplies a singleton/straddle alternative,
or matched mass supplies a normalized all-label exchange tangent. A unique
matched dominant label is incompatible with `Sep <= 1/4`. The protected
mixture counterexample from the triggering review therefore receives an
explicit residual-curvature witness; it is not silently counted as an
unresolved active set. `E_I^mat` and its bound are generated from prior
`R-euc` records and are not additional assumptions.
...
setting.  Attempt 9 adds one local exchange lemma: a matched-residual
all-label alternative sits between the dual coordinate algebra and the
...
- Sketch attempt: 9
- Retry mode: `revise_sketch` after the repeated attempt-8 `step_003c`
  activation/exchange review
- Triggering artifacts: `perspective_2/idea_9/proof_steps/step_003c/review.md`
  (SHA-256 `8a663ae96e38471160088d53a880d1b6556093a79f7dac91f6072d3ed1f1b823`)
  and its blocked proof artifact (SHA-256
  `966413b489f6a56b61fddd0c6aa104c23694a5aceeec038bf7fb2e8f083de3e6`)
...
- Local repair: preserve every accepted J-ledger, source-normal, chart, entry,
  baseline, and generated-flow repair from attempts 4--8. Keep `step_003b`
  unchanged and retain `step_003c`'s exact current/dual coordinates, ordered
  radial interface, and root-free `Theta_act`. Add `step_003d` as a
  target-preserving matched-mass/all-label exchange bridge: define the
  projected protected-error tensor from generated `(R-euc)` records, split
  unresolved target score from matched residual score, enumerate active sets
  on all labels, and use a residual-curvature exchange margin whenever
  unresolved-only sets are empty. A common matched singleton is rejected by
...
6. `step_003d` consumes those equations and the generated protected-error
   ...
   `mu_mat` margin.  A matched singleton is ruled out by separation.  This is
   the theorem-critical matched-mass/all-label bridge requested by the failed
   proof-step review.
...
- Required bridge or simplification obligations: `step_003` defines the local
  ...
  equations to all labels, derives `E_I^mat`, and proves the unresolved/matched
  mass split and `(Mat-ex)` with the explicit capture-to-gap comparison
  `(Mat-margin)`. No first-order Gram term or `1/r` versus `1/sqrt(r)`
  conversion is deferred to `step_004`.
...
- **Producer and scope.** `step_003d` is the theorem-critical producer between
  ...
  available to the algorithm.
- **All-label stationarity.** Extend `(Dual-stat)` to every (j\in[r]) by
  defining (h^{\rm all}_{j,M}) as the exact contraction of
  (E_I^{\rm mat}) and the accepted target-frame history with the dual
  selector (d_{j,M}). Then
  \[
  \tau_jx_{j,N}x_{j,L}-s^\parallel\alpha_{j,M}
   =h^{\rm all}_{j,M}+\langle d_{j,M},\zeta_M\rangle .
  \tag{All-stat}
  \]
  The simultaneous root-free envelope and `(Mat-budget)` give
  \[
  \max_{j,M}|h^{\rm all}_{j,M}|+\max_M\|\zeta_M\|_2
  \le\epsilon_{\rm all}:=C_{\rm all}\left[
  \tau_{\max}(\nu^2+r\nu^3+\sqrt r e+\Theta_{\rm act})
  +B_{\rm mat}+N_I(g)e/t\right].
  \tag{All-def}
  \]
  No unresolved-label support condition is used in this extension.
...
- **Mass split.** Define
  ...
  \(W_U\ge|s^\parallel|/8\) or \(|s_M|\ge7|s^\parallel|/8\). The first
  ...
  lemma below.
- **Matched exchange lemma.** If the all-label support has two labels or
  ...
  {\cal Q}(\xi)
  \ge c_{\rm mat}{|s_M|^2\over r\tau_{\max}}
   -C_{\rm mat}\left[(\nu+\delta_{\rm act}){|s_M|^2\over r\tau_{\max}}
   +\epsilon_{\rm all}^2\right].
  \tag{Mat-ex}
  \]
  The positive term is the exact two-label residual exchange, while the
  displayed opposing terms are Gram mixing, unresolved leakage,
  source-normal transfer, and numerical stationarity. The biorthogonal
  selectors make this a same-target identity. Fixed exponents and constants
  are chosen so the capture lower bound implies
  \[
  s^\parallel\ge\sqrt2\|R_I\|_FP^{-D_{\rm cap}/2}
  \Longrightarrow
  \text{either }W_U\ge|s^\parallel|/8\text{ or }
...
#### Archived unresolved-only alternative (superseded in attempt 9)

The following attempt-8 bullets are retained only as historical context. They
are not a producer of any attempt-9 output; in particular, the assertion that
...
The former orthonormal-frame activation prose is superseded in attempt 9 by
...
| Matched-mass/all-label exchange alternative | `step_003d` | `step_003c` all-label equations, `E_I^mat` from prior `(R-euc)` records, `R_I^parallel`, `(Res)`, `(SE-raw)`, `Theta_act`, and the observable certificate | protected-error mass, unresolved target mass, Gram mixing, source-normal transfer, and numerical stationarity are separated; no unresolved support condition is assumed | `E_I^mat` and the consumed residual use the same realized `T`; `(Mat-ex)` is an ambient product-sphere Hessian identity, not a whitened surrogate | all-label energy makes every `L_M^all` nonempty; mass split gives unresolved structural alternative or matched residual exchange; matched singleton violates `Sep` | `(Mat-margin)` gives `Q>=8N_I(g)P^(-D_gap)` whenever capture holds in the matched branch; raw terms stay explicit outside `E_lb` | `step_004`; only the unresolved singleton flows to `step_005` | None |
...
| Matched-mass/all-label exchange bridge | source-compatible support activation and strict-saddle alternative | `E_I^mat` from prior `(R-euc)`, all-label energy, exact mass split, and finite residual pair/three-mode rotations | direct current-notation derivation from `step_003c` and `step_003b`; no external theorem wrapper | `(Mat-ex)` uses the same realized target and ambient Hessian; matched singleton is rejected by `Sep`; unresolved singleton is the only commit-facing output | `E_I^mat` is a projected protected-error tensor, not a surrogate target; `(SE-raw)` transfers the final witness | `(Mat-margin)` compares matched exchange curvature to `8N_I(g)P^(-D_gap)` whenever capture holds; raw terms remain explicit on `E_lb^c` | `B_mat`, `epsilon_all`, all-label active sets, sign maxima, and capture/radial lower bound | same-target residual decomposition and exact product-sphere tangent metric | one finite mass/exchange calculation per prefix; no new recurrence or hidden matched-mass assumption | `step_003c -> step_003d -> step_004`; `step_009` supplies prior records | `step_004`; unresolved singleton -> `step_005` | separated protected mixture with `W_U=0`, matched singleton, threshold equality, `|U_I|=1`, and exact H=e=0 pass | step-local | None |
...
| Matched-mass/all-label exchange alternative | `step_003d` from `step_003c`, `E_I^mat`, prior `(R-euc)` records, and the observable certificate | `step_004`; only the unresolved singleton is then consumed by `step_005` | excludes every matched-mixture certificate via `(Mat-ex)` or supplies the unique unresolved-label antecedent; matched singleton is excluded by `Sep` | `step_003c -> step_003d -> mass split -> structural/matched exchange or unresolved singleton -> step_004 -> singleton-only step_005` | derived | None |
...
| `step_003d` | From `(All-stat)`, `E_I^mat`, and the certificate, prove the matched-mass/all-label exchange bridge: all-label active sets are nonempty; unresolved mass gives the structural singleton/straddle alternative; matched mass gives `(Mat-ex)`; a matched singleton violates `Sep`; capture implies the exchange margin `(Mat-margin)`. | `step_003c`, `step_003b` | derived `E_I^mat` from prior `(R-euc)` records, `(Res)`, `(SE-raw)`, `Theta_act`, and the observable certificate; no matched-mass condition is assumed | protected-error versus unresolved-target score split, mixed signs, matched mixtures, and one common margin | exact source-frame mass decomposition, finite all-label pair/three-mode tangent enumeration, sign maximization, and residual-Hessian calculation | unresolved singleton, structural exchange, matched exchange `(Mat-ex)`, `mu_mat`, and the capture-to-gap implication `(Mat-margin)` | R2; retain `B_mat`, `epsilon_all`, raw branch terms, and explicit exponent comparisons | PENDING |
| `step_004` | Transfer the already-produced unresolved or matched exchange witness from `step_003d` through `(SE-perp)` and `(SE-raw)`, rule out either exchange branch at the observable Hessian threshold, and export the singleton certificate dichotomy without redoing support selection. | `step_003b`, `step_003c`, `step_003d` | derived event and `E_lb` only where explicitly used; conditional prior-prefix records; `assump:base-product-balance` | actual-vs-projected score/gradient/Hessian transfer and exact threshold comparison | direct product-sphere Hessian identity plus accepted same-target transfer; no coordinate, support, or scale conversion | isolation margin and certificate dichotomy | R2; retain `B_perp`, `epsilon_all`, `D_gap`, and branch-qualified calibration | PENDING |
...
None at sketch level. Attempt 9 preserves the honest
`O(h^4+e h+e^2)` interface and the accepted full-history source-normal bridge.
The repeated unresolved-support blocker is repaired by `step_003d`: it uses
all-label energy, the exact generated `E_I^mat`, an unresolved/matched score
mass split, and the explicit `(Mat-ex)` residual-curvature margin. A matched
singleton is ruled out by `Sep`, while a separated matched mixture is handled
by the finite all-label exchange family. The protected-mixture witness from
the triggering review is therefore covered without a new assumption, target
weakening, or algorithm change. The remaining proof obligation is exactly the
displayed capture-to-gap inequality `(Mat-margin)`; it is a step-local
lemma-sized obligation for a fresh `step_003d` proof, not a hidden condition or
an idea-generation route. If that explicit same-target inequality is
contradicted by an allowed instance, the controller should record that
precise obstruction and route back to `/proof-sketch` under the user's policy.
