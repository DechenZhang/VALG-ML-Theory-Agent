...
This is proof-sketch attempt 6 for `perspective_2/idea_9`, rerun in
`revise_sketch` mode after the archived attempt-5 sketch review.  The binding
...
- Sketch attempt: 6
- Retry mode: `revise_sketch` after attempt-5 sketch review
- Triggering artifact: `proof_history/sketch_005/proof_sketch_review.md`
...
- Local repair: preserve every accepted J-ledger, chart, entry, baseline, and
  ...
  bound; attempt 6 makes its `eta0` comparison explicitly `E_lb`-conditioned,
  exposes the unresolved-target reverse-triangle reserve, and synchronizes
  every temporal consumer without assuming source exclusion or altering any
  primitive condition.
...
- Noncircular closure status: `step_001` supplies the source spans, pair/triple
  ...
  target-frame `Theta`; each earlier accepted application of `steps_004--005`
  supplies matching, `R-euc`, and the post-fit record.  `step_003b` derives
  `B_hist,F`, `(Res)`, `a_(t,M)`, and `B_perp` before the current `step_004`
...
- Producer-consumer provenance: `step_003b` consumes `step_001` (source spans,
  ...
  identity to the current `step_004`; `step_004`/`step_005` produce the next
...
- Mechanism source: direct product-sphere score, gradient, Hessian, and radial
  ...
  `Theta_I` bound target-frame history defects, while `(SE-perp)` and
  `(SE-raw)` bound the remaining full-space defect; `(SE-lb)` is used only on
  `E_lb` for the eta0 calibration scale.
...
- Producer-consumer provenance: outputs feed `step_005` (which stores the
  post-fit audit), `step_006`, `step_009`, `step_011`, and `step_014`; the
  stored audit is a prior-iteration input to the next `step_003b` invocation.
...
| Conditional `Theta_I` and local precommit `z` definition | `step_003` | S, conditional actual A, numerical all-ones source, both q gaps, and the zero-diagonal convention | static, history, and numerical forcing classes remain separate; no quantitative row bound is exported here | exact current residual coordinates and the locally defined z array use the same root/mode/label frame consumed by the J charge | nonnegative Neumann series converges with factor `1/(1-q_res)`; define `z_(p,M,ell)=Theta_(I;p,M,ell)+C_num e` off diagonal and zero on the diagonal | `q_res,q_row<=1/4`, empty-history base, and proof-only numerical padding | `step_003b`, `steps_004,005,006,007` | None |
| Full-history source-normal exclusion and projected residual transfer | `step_003b` | `step_001` source spans, `E_lb`, pair/triple gap; `step_002` row sums; `step_003` Theta; prior-iteration `step_005` matching/R-euc/post-fit records supplied by `step_009` | numerical post-fit forcing is controlled by `e Lambda_nor`; structural within-span errors remain in `A_I/Theta_I`; off-diagonal normal coefficients are controlled by `q_nor` | produced `R_I^parallel=Pi_star R_I` and consumed `R_I` use the same target and ambient metric; target-frame contractions are exactly equal; `(Hist-F)` and `(Res)` give `norm_(sigma,F)((Id-Pi_star)R_I)<=B_perp` | triangular normal equations plus `(1-q_nor)^(-1)` give `B_perp`; `(SE-raw)` is unconditional, while `E_lb` gives `(SE-lb)` for `tau_min eta0` | pair-Khatri--Rao unit diagonal, `q_nor<=1/4`, explicit unresolved-target/error reserve, and fixed `D_stat` slack | current `step_004`, current `step_005` audit, `step_009`, `step_011`, `step_016`; the next temporal `step_003b` consumes the audit stored by current `step_005` | None |
| Certificate matching and calibration | `steps_004--005` | score, gradient, Hessian, radial equations, Theta, `(SE-perp)`, `(SE-raw)`, branch `(SE-lb)`, capture, and separation | projected target-frame, source-normal, numerical, mixed-support, and guard defects are split and bounded by their own sources | `step_003b` transfers the actual residual to its same-target projection with residual `B_perp`; `(Res)` supplies a positive residual scale; the accepted scratch slot is then compared directly with the exact realized atom in factor Euclidean norm | `(SE-raw)` excludes outside-only certificates and `(SE-lb)` supplies eta0 calibration only on `E_lb`; singleton/mixed-support dichotomy and radial root export unique label, `R-euc`, `R-dir`, and the next normal-audit record | positive score/capture, angular gap, `eta0`, `B_perp`, separation `1/4`, and `E_lb` | `steps_006,009,011,014`, plus prior-iteration `step_003b` consumer for the stored audit | None |
...
| Conditional `Theta_I` | `step_003` | `step_003b,steps_004,005,007` | current residual control | `step_002 -> step_003 ->` target-frame bridge/landscape and later quantitative profile | explicitly conditional | None |
...
| Full-history source-normal bound `(SE-perp)` and capture transfer `(SE-raw)/(SE-lb)` | `step_003b` | current `step_004`, current `step_005` audit path, `step_009`, `step_011`, `step_016`, and the next temporal `step_003b` | full-space landscape has no invisible outside-span history; `(Hist-F)/(Res)` and target-frame identity are exported; `(SE-lb)` is branch-qualified | `step_001,step_002,step_003` plus prior accepted `step_005` records (supplied by `step_009`) -> current `step_003b` -> current `step_004` -> current `step_005` audit -> next temporal `step_003b`; `step_009` reuses the same bridge at each later prefix | `derived` | None |
| Certificate matching and calibration | `steps_004--005` | `step_003b` (next temporal invocation), `steps_006,009,011,014` | unique labels, factor errors, and stored post-fit normal audit | `Theta -> current landscape/matching -> current step_005 audit -> next temporal step_003b -> transition/entry` | derived after prefix bridge | None |
...
| `step_004` | Under the complete conditional prefix interface from `step_003b`, prove score decomposition and singleton/mixed-support angular landscape dichotomy. | `step_003`, `step_003b` | derived event and `E_lb`; conditional prior-prefix certificate records; `assump:base-product-balance` | nonorthogonal paths, projected-vs-full residual transfer, and Hessian gap | direct product-sphere stationarity/Hessian expansion plus `(SE-perp)`, `(SE-raw)`, and branch `(SE-lb)` | isolation margin and certificate dichotomy | R2; retain all target-frame and source-normal defects | PENDING |
...
path is explicit:
`step_001/002/003 + prior-iteration step_005 audit -> step_003b -> current
...
`(SE-lb)`.  Conditional landscape and matching (`steps_004--005`) then precede
the exact transition and rootwise/global ledger (`steps_006--009`).  The local z
...
interface, retain its positive terms at `h=e^2`, and attempt 6 completes the
...
controlled at each named branch.  `R-acc` remains available for its exact
ledger consumers, with no unsupported factor-norm conversion.  The local z
definition, temporal normal-audit flow, rowwise bound, abstract prefix closure,
all `step_003b` consumers, and `C_L`/`C_path_star` dependence are declared in
producer order.
