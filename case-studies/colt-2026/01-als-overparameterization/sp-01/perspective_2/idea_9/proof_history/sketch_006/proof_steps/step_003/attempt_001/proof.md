...
- Derived invariants supplied by accepted dependencies:
  - On the accepted once-drawn event `E_inst`,
    Lemma~\ref{lem:step-002-static-kernels} and
    Proposition~\ref{prop:step-002-source-budget} supply the exact
    nonnegative `H/J/K/S` objects, their zero-diagonal conventions, and the
    static source bounds.
  - Lemmas~\ref{lem:step-002-transfer-l1} and
    \ref{lem:step-002-transfer-root}, followed by
    Proposition~\ref{prop:step-002-small-gain}, supply the exact nonnegative,
    root-preserving transfer `Tcal` and
...
- Accepted dependency artifacts:
  - `perspective_2/idea_9/proof_steps/step_002/proof.md`, sketch attempt 4,
    unit attempt 1, `Step Proof Status = COMPLETE`, SHA-256
    `7b7b4247671ae955f2451e8a533547762e3f2836add40889e32e2473a123dc7a`.
  - `perspective_2/idea_9/proof_steps/step_002/review.md`, which reviews that
    ...
    `3390c0244a17ef95d81327c6fd4fab749385873fa0483577a9ab1124e55b6883`.
...
1. **Accepted Step 002 static-source interface.** On `E_inst`,
   Lemma~\ref{lem:step-002-static-kernels} gives, for every mode `M`,
   ...
   Proposition~\ref{prop:step-002-source-budget} gives the nonnegative
...
2. **Accepted Step 002 transfer interface.** Let
   ...
   root coordinate `j`. Lemmas~\ref{lem:step-002-transfer-l1} and
   \ref{lem:step-002-transfer-root}, together with
   Proposition~\ref{prop:step-002-small-gain}, give
...
3. **Finite-dimensional geometric-series facts.** For any induced norm and
   ...
   Proposition~\ref{prop:step-003-neumann} applies these inequalities to its
...
The local interfaces are as follows. Lemma~\ref{lem:step-003-forcing}
supplies one source array legal in both norms.
Proposition~\ref{prop:step-003-neumann} supplies the simultaneous resolvent
and both bounds. Proposition~\ref{prop:step-003-local-profile} consumes that
same `Theta_I` to define the local profile and to verify the empty and exact
boundary cases.
...
\label{lem:step-003-forcing}

Under the accepted static source conclusions of
Lemma~\ref{lem:step-002-static-kernels} and
Proposition~\ref{prop:step-002-source-budget}, if `I` is a legal finite
...
\label{prop:step-003-neumann}

Under the accepted transfer conclusions of
Lemmas~\ref{lem:step-002-transfer-l1} and
\ref{lem:step-002-transfer-root} and
Proposition~\ref{prop:step-002-small-gain}, and under
Lemma~\ref{lem:step-003-forcing}, if `I` is a legal finite conditional
...
block-l2 norms. It is the unique solution of
...
Every coefficient of \(\mathcal L\), `B_I`, and hence every summand in (16) is
nonnegative. Passing coordinatewise to the limit shows `Theta_I` is
nonnegative. Summing (16) from `s=0` and using (4) gives the first lines of
(13)--(14); Lemma~\ref{lem:step-003-forcing} gives their second lines.
...
\label{prop:step-003-local-profile}

Under Proposition~\ref{prop:step-003-neumann}, if `I` is a legal finite
...
`S=Tcal=0`, hence `Theta_I=0` and every profile in (21) is exactly zero. In
...
**Proof.** Proposition~\ref{prop:step-003-neumann} makes every off-diagonal
...
specialization gives `S=Tcal=0`; substituting `e_num=A_I=0` into (11) and
...
For a fixed legal conditional history, Lemma~\ref{lem:step-003-forcing}
...
Proposition~\ref{prop:step-003-neumann} applies the two distinct accepted
small-gain margins to one common sequence of partial sums. It proves
...
Proposition~\ref{prop:step-003-local-profile} then substitutes the produced
`Theta_I` into the setting's algebraic (J-z) convention for every candidate
...
\longrightarrow \{\text{Steps 004,005,007}\},
\qquad
...
- Exposed variables: `r`, `P`, `D_stat`, `Gamma_star`, `C_0`, `C_num`,
  `nu_real`, `e_num=P^(-D_stat)`, `S`, `A_I`, `Tcal`, `q_res`, `q_row`,
...
- Baseline-reduction check: with `H=0`, `e_num=0`, and `A_I=0`, the accepted
  dependency gives `S=Tcal=0`, and (11), (21) give exact
...
- `D`, `Z_+`, the all-ones array, `S`, `A_I`, `Tcal`, `Theta_I`, and `z` are
...
- `B_I` in (5), \(\mathcal L\) in (3), the partial sums `Theta_I^(N)`, and
  ...
  Lemma~\ref{lem:step-003-forcing}. \(\mathcal L\) is directly defined from the
...
- The finite nonnegativity of `A_I` is proved in
  Lemma~\ref{lem:step-003-forcing} from its exact finite-sum definition and
  accepted positivity of the realized weights. The convergence and
  boundedness of `Theta_I` are proved in
  Proposition~\ref{prop:step-003-neumann}. The finite, nonnegative,
  zero-diagonal status of local `z` is proved in
  Proposition~\ref{prop:step-003-local-profile}.
...
- No global diagnostic artifact was read or used for this proof.
