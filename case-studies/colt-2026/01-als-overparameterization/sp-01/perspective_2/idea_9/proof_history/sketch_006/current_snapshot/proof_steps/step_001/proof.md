...
- assump:rank-regime: the binding rank branch has `r<k<r^2`, hence `k>=r`;
  this auxiliary domain source is used only to append the `k-r` joint-zero
  slots to the exact target, not in the random-event estimates.
...
The algorithmic rank k occurs through the setting-defined scale
P=16 n r k kappa rho^(-1) and the already defined rank-k target slot
space. The auxiliary Assumption~\ref{assump:rank-regime} source is used
only for k>=r when
padding Q^star; no rank-regime inequality is used in the probabilistic
derivation.
...
| --- | --- | --- | --- |
| unit_001 | lemma | Perturbation-Operator Concentration, label `lem:p2-i9-s6-step001-perturbation`: under Assumption~\ref{assump:base-column-conditioning}, Assumption~\ref{assump:gaussian-smoothing}, and Assumption~\ref{assump:dimension-regime}, the relative perturbation matrices obey a simultaneous operator bound on an event of probability at least 1-r^(-30). | Supplies the event on which normalization is stable. |
| unit_002 | lemma | Normalized-Column and Gram Transfer, label `lem:p2-i9-s6-step001-normalization`: under Assumptions~\ref{assump:base-column-conditioning}, \ref{assump:base-product-balance}, \ref{assump:gaussian-smoothing}, \ref{assump:dimension-regime}, \ref{assump:smoothing-scale}, and \ref{assump:modewise-gram-margin}, and the event from Lemma \ref{lem:p2-i9-s6-step001-perturbation}, normalized columns have the stated norm, weight-ratio, and realized-Gram bounds. | Produces the structural realized-instance controls. |
| unit_003 | proposition | Pair/Triple Conditioning and Exact Target, label `prop:p2-i9-s6-step001-target`: under Assumptions~\ref{assump:base-column-conditioning}, \ref{assump:base-product-balance}, \ref{assump:gaussian-smoothing}, \ref{assump:dimension-regime}, \ref{assump:smoothing-scale}, \ref{assump:modewise-gram-margin}, the structural controls from Lemma \ref{lem:p2-i9-s6-step001-normalization}, and Assumption~\ref{assump:rank-regime} for `k>=r`, pair/triple Hadamard Gram gaps, tensor norm bounds, and the exact balanced target representation hold. | Produces pair/triple conditioning, T, and Q^star. |
| unit_004 | lemma | Disjoint-Pair Gaussian Lower Tail, label `lem:p2-i9-s6-step001-lower-tail`: under Assumptions~\ref{assump:base-column-conditioning}, \ref{assump:base-product-balance}, \ref{assump:gaussian-smoothing}, \ref{assump:dimension-regime}, \ref{assump:smoothing-scale}, and \ref{assump:modewise-gram-margin}, the normalization conclusion of Lemma \ref{lem:p2-i9-s6-step001-normalization}, and the perturbation event, the derived event E_lb={nu_real>=P^(-D_lb)} has complement probability at most r^(-20) and structural intersection failure at most r^(-40), for fixed D_lb=60. | Supplies the only finite-smoothing lower scale used by later branch-qualified comparisons. |
| unit_005 | proposition | Once-Drawn Instance Assembly, label `prop:p2-i9-s6-step001-instance`: under Assumptions~\ref{assump:base-column-conditioning}, \ref{assump:base-product-balance}, \ref{assump:gaussian-smoothing}, \ref{assump:dimension-regime}, \ref{assump:smoothing-scale}, \ref{assump:modewise-gram-margin}, and \ref{assump:rank-regime} for the target padding, together with the named local conclusions, the intersection E_inst=E_str intersect E_lb has probability at least 1-r^(-10) and carries every requested output; its complement retains the raw structural branch, and the exact zero-defect limit is handled separately. | Assembles the exact target-step interface and probability modes. |
...
C_dim in Assumption~\ref{assump:dimension-regime} makes three such events
(one per mode)
...
Paper-ready label: lem:p2-i9-s6-step001-perturbation.
Paper-ready title: Perturbation-Operator Concentration.

Statement:

Under Assumption~\ref{assump:base-column-conditioning},
Assumption~\ref{assump:gaussian-smoothing}, and
Assumption~\ref{assump:dimension-regime}, let
bar D_M=diag(||bar m_1||_2,...,||bar m_r||_2),
...
enough in Assumption~\ref{assump:dimension-regime}. Define E_str to be the
event in the last display.
...
Paper-ready label: lem:p2-i9-s6-step001-normalization.
Paper-ready title: Normalized-Column and Gram Transfer.
...
\ref{assump:modewise-gram-margin}, and on the event E_str of Lemma
\ref{lem:p2-i9-s6-step001-perturbation},
...
Paper-ready label: prop:p2-i9-s6-step001-target.
Paper-ready title: Pair/Triple Conditioning and Exact Target.
...
\ref{assump:base-product-balance}, \ref{assump:gaussian-smoothing},
\ref{assump:dimension-regime}, \ref{assump:smoothing-scale}, and
\ref{assump:modewise-gram-margin}, Assumption~\ref{assump:rank-regime} for
`k>=r`, and the conclusion of Lemma \ref{lem:p2-i9-s6-step001-normalization}, the
centered matrices
...
nonzero. By Assumption~\ref{assump:rank-regime}, `k>=r`; the tensor of the
three factors in q_j^star is tau_j phi_j, and summing the r nonzero slots
and the k-r joint-zero slots is exactly T. From the norm bounds in
Lemma \ref{lem:p2-i9-s6-step001-normalization},
...
Paper-ready label: lem:p2-i9-s6-step001-lower-tail.
Paper-ready title: Disjoint-Pair Gaussian Lower Tail.
...
\ref{assump:base-product-balance}, \ref{assump:gaussian-smoothing},
\ref{assump:dimension-regime}, \ref{assump:smoothing-scale}, and
\ref{assump:modewise-gram-margin}, the perturbation event E_str of Lemma
\ref{lem:p2-i9-s6-step001-perturbation}, and the normalized-column and Gram
conclusion of Lemma \ref{lem:p2-i9-s6-step001-normalization} (which itself is
derived from the preceding primitive assumptions), let
...
\qquad
\mathbb P(E_{\rm str}\cap E_{\rm lb}^c)\le r^{-40}.
\]
...
Paper-ready label: prop:p2-i9-s6-step001-instance.
Paper-ready title: Once-Drawn Instance Assembly.
...
\ref{assump:dimension-regime}, \ref{assump:smoothing-scale}, and
\ref{assump:modewise-gram-margin}, Assumption~\ref{assump:rank-regime} for
the target padding, and the conclusions of Lemmas
\ref{lem:p2-i9-s6-step001-normalization} and
...
E_lb from conditional Gaussian small-ball estimates, exporting both
P(E_str intersect E_lb^c)<=r^(-40) and the weaker P(E_lb^c)<=r^(-20),
and explicitly leaves the complement raw. Proposition \ref{prop:p2-i9-s6-step001-instance}
...
- Admissibility conditions and auxiliary tolerances: n>=C_dim r log(2r),
  0<rho<=1, the six primitive assumptions (listed by their stable IDs above),
  sufficiently large fixed
...
- bar D_M, bar U_M, Xi_M, Z_M, W_M, beta, s_{j,M}, and D_{s,M} are
  ...
  exported downstream.
- The notation m_{j,M}^{raw} in the normalization statement is a proof-local
  alias for the setting-defined realized column (a_j, b_j, or c_j according
  to M); it is not a second generated object. The constant C_sb is a fixed
  proof-local absolute density constant from Result G2.
