...
\tag{15}
\]
Put \(\delta_0:=\tau_{\min}/(10^6r)\).  By the fixed order of
constant selection in the sketch (first \(C_\nu\), then \(D_{\rm stat}\)),
the accepted upper Gram margin, (10), and (12) imply
\[
\omega\le\delta_0,\qquad \gamma\le\delta_0.
\tag{16a}
\]
Indeed, the frame term is at most
\(2C_{\rm fr}\Gamma_\star\tau_{\min}/(C_\nu r\log(2r))\), the history
term is \(O(\tau_{\min}/(r\log^2(2r))+r\tau_{\max}e_{\rm num})\),
the normal term is bounded by (12), and the numerical term is bounded by
(10).  Enlarging the fixed constants gives (16a) uniformly for the fixed
asymptotic threshold.  This is a derived estimate, not a new hypothesis.
There are fixed choices of \(C_{\rm fr},C_{\rm hist}\) and the fixed
asymptotic threshold for which, using (10), (12), and the accepted
\(\eta_0=O(r^{-2})\) bound,
\[
\omega\le\frac{\tau_{\min}}{10^6r},\qquad
\gamma\le\frac{\tau_{\min}}{10^6r}.
\tag{16}
\]
For each mode put
...
For completeness, the support case split is written out with the correct
\(r^{-1}\) scale.  Put \(\theta=(16\sqrt r)^{-1}\) and
\(\alpha_M=\|x_{\cdot,M}\|_2\).  If
\(|S|\le\tau_{\min}/(256r)\), (18) gives
\[
 |x_{j,M}x_{j,N}|\le {|S|+\gamma\over\tau_{\min}}
 \le {1\over128r}\qquad(M\ne N).
 \tag{21a}
\]
The diagonal strict-saddle calculation used here is the following explicit
identity.  For a label \(p\), modes \(M,N\), and remaining mode \(L\), let
\(d_{p,M},d_{p,N}\) be the normalized residualized directions in (22).  Then
the diagonal part of (1), after inserting (18), satisfies
\[
\langle\operatorname{Hess}_{\cal S}s_I\xi,\xi\rangle
\ge {\tau_{\min}|x_{p,L}|(1-x_{p,M}^2-x_{p,N}^2)\over
2\sqrt{(1-x_{p,M}^2)(1-x_{p,N}^2)}}-|S|-8\gamma-3B_\perp,
\tag{21b}
\]
for \(\xi=2^{-1/2}(d_{p,M},\operatorname{sgn}(x_{p,L})d_{p,N},0)\).
The formula follows by writing the \(p\)-coordinate contribution and using
\(\sum_{j\ne p}\tau_jx_{j,A}x_{j,B}x_{j,C}=S-\tau_pa_pb_pc_p\);
the two residualized denominators are displayed, so no hidden normalization
is used.  If no \(\alpha_M\ge1/2\), the same calculation applied to the
largest singular vector of the restricted diagonal map gives a unit tangent
with score-Hessian at least \(\tau_{\min}/(32r)-8\gamma-3B_\perp\),
contradicting (25).  Thus some \(\alpha_L\ge1/2\), and a label \(p\) has
\(|x_{p,L}|\ge1/(2\sqrt r)\).  Equation (21a) then makes the other two
coordinates at \(p\) at most \(1/(64\sqrt r)\), so (21b) gives
\[
\langle\operatorname{Hess}_{\cal S}s_I\xi,\xi\rangle
\ge {\tau_{\min}\over256r}.
\tag{21c}
\]
This includes \(|U_I|=1\): the tangent uses only \(p\), so the formal
requirement \(p\ne q\) is dropped on this boundary.  It also covers a
one-mode-straddling attempt, because (21a) forces the two complementary
coordinates below the threshold rather than selecting an unrelated label.
Suppose now \(|S|>\tau_{\min}/(256r)\).  For every label with one coordinate
above \(\theta\), subtracting the corresponding two equations in (18), after
multiplication by the coordinates, gives
\[
\bigl||a_j|^2-|b_j|^2\bigr|+
\bigl||a_j|^2-|c_j|^2\bigr|
\le {8\gamma\over |S|}\le {1\over32r}.
\tag{24}
\]
Thus either all three coordinates at \(j\) exceed \(\theta/2\), or the two
smaller modes satisfy the low-support alternative (21a), which is precisely
the one-mode-straddling exchange branch.  In the all-three case, the three
products have the sign of \(S\): an opposite-sign product of magnitude at
least \(\theta^3/8\) would make one equation in (18) differ by at least
\(\tau_{\min}\theta^2/4>8\gamma\), contradicting (16).

If exactly one label \(p\) has all three coordinates above \(\theta/2\), the
unit norms and (24) imply
\[
|a_p|,|b_p|,|c_p|\ge\tfrac12,\qquad
\operatorname{sgn}(a_pb_pc_p)=\operatorname{sgn}(S).
\tag{25}
\]
If two labels \(p\ne q\) have all six coordinates above \(\theta/2\), define
\[
d_M^{p,q}={x_{q,M}\widehat m_{p,M}-x_{p,M}\widehat m_{q,M}
\over (x_{p,M}^2+x_{q,M}^2)^{1/2}},\qquad
\xi=3^{-1/2}(\epsilon_A d_A^{p,q},\epsilon_B d_B^{p,q},
\epsilon_C d_C^{p,q}),
\tag{26}
\]
where \(\epsilon_A\epsilon_B\epsilon_C=1\) and the common sign of the
support products is made positive.  The denominator in each mode is between
\(\theta\) and \(\sqrt2\), and each \(d_M^{p,q}\) is unit and tangent.  A
direct expansion of (1) gives
\[
\langle\operatorname{Hess}_{\cal S}s_I\xi,\xi\rangle
\ge {\tau_{\min}\over32r}-12\gamma-3B_\perp
\ge {\tau_{\min}\over128r}.
\tag{27}
\]
The cyclic mixed numerators are
\(\tau_p x_{q,M}x_{q,N}x_{p,L}+
\tau_q x_{p,M}x_{p,N}x_{q,L}\); (24) gives the same sign, and summing the
three normalized terms gives the displayed \(1/r\) coefficient.  The
\(12\gamma\) term is the complete frame/history/gradient defect from (15),
while \(3B_\perp\) is the accepted Hessian transfer (7).  This audits the
normalization and perturbation budget.

The preceding alternatives are exhaustive over the three mode permutations,
including threshold/sign-straddling cases.  The root \(p\) is selected only
after all arrays have been computed, and no
\(\Theta_{I;p,\cdot,\cdot}\) entry is used in this selection. \(\square\)
