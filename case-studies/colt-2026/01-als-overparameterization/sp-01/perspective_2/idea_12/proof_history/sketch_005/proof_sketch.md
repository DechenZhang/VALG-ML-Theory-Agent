...
This is sketch attempt 5 for perspective_2/idea_12 in revise_sketch mode.
...
The scale must dominate the accumulated defect rather than only the
dimension baseline. Put
\[
 a_n=\sqrt{L_{\rm ang}\log(2r)/n},\quad B_\chi=P^{-D_{\rm stat}},\quad
 B_A=C_A\Gamma_\star(\nu_{\rm real}^2+r\nu_{\rm real}^3)+C_AB_\chi,
\]
\[
 \kappa_T={\|T\|_F\over\tau_{\min}},\qquad
 \mathfrak D=C_D\kappa_T\!\left[
 \Gamma_\star(\nu_{\rm real}^2+r\nu_{\rm real}^3+B_A)
 +\sqrt r\,P^{-D_{\rm stat}}+B_\chi^{1/2}\right].
 \tag{global-defect}
\]
These analysis-only quantities use the finite objective telescope. Define
\[
 a_{\rm def}=(64\mathfrak D)^{1/3},\quad
 a_\mu=(8\kappa_TP^{-D_\mu})^{1/3},\quad
 a_\star=\max\{a_n,a_{\rm def},a_\mu\},\quad b_\star=4a_\star,
 \qquad \Lambda_\star:=n a_\star^2,
 \tag{adaptive-scale}
\]
\[
 \theta_\star={\tau_{\min}a_\star^3\over16},\qquad
 \mathcal M_\star(q)=\{j:|a_j(q)|\ge\theta_\star\}.
 \tag{adaptive-selector}
\]
Since (a_\star^3\ge8\kappa_TP^{-D_\mu}),
\(\theta_\star/\|T\|_F\ge P^{-D_\mu}/2\); the adaptive selector therefore
retains the certificate threshold while adding the defect margin.
Let \(a_{\max}\) denote the derived largest radius on which the existing
separation and radial reserves hold. For each unresolved \(j\), use
\[
 \mathcal W^{\rm ang}_{s,j}=\{(u,v,w):a_\star\le|\langle u_M,m_{j,M}\rangle|
 \le2a_\star,\ |\langle u_M,m_{\ell,M}\rangle|\le b_\star
 \ \forall\ell\ne j,M\},
\]
with positive product sign, and
\[
 \mathcal W^{\rm rad}_{s,j}=\{g:|g^3-s_s(u,v,w)|\le\tfrac18s_s(u,v,w),\
 \tfrac12(\tau_{\min}a_\star^3)^{1/3}\le g\le
 2(8\tau_{\max}a_\star^3)^{1/3}\}.
\]
Here \(g_{\min}=\tfrac12(\tau_{\min}a_\star^3)^{1/3}\),
\(\Delta_{\rm ang}\asymp a_\star\), and
\(\Delta_{\rm rad}\ge c_{\rm rad}\tau_{\min}^{1/3}a_\star\). The protocol
checks only observable certificates. Spherical density gives
\[
 p_\star\ge c\,a_\star^3(2r)^{-c_0}
 \exp(-C n a_\star^2). \tag{slab-mass}
\]
The polynomial branch is explicit:
\[
 a_\star\le a_{\max},\qquad a_\star\ge(nr)^{-c_\star},\qquad
 n a_\star^2\le C_{\rm mass}\log(nr). \tag{mass-feasibility}
\]
Then \(p_\star\ge(nr)^{-c_{\rm tx}}\). If the last inequality fails,
the same density estimate gives a super-polynomially small slab mass; that
unrestricted-\(n\) branch is recorded as a blocker below. Let \(d_L,d_{\rm lip}\)
be fixed guarded-box exponents and choose
\[
 D_{\rm stat}\ge2D_\mu+D_{\rm gap}+12,\quad D_{\rm cap}\ge D_{\rm stat}+12,\quad
 c_{\rm tx}\ge c_0+3+3c_\star,\quad
 d_{\rm tx}\ge20+d_L+c_\star d_{\rm lip}+c_\star+c_0+6.
 \tag{P-hierarchy}
\]
On the feasible branch, the public LP budget and \(a_\star^3\ge64\mathfrak D\)
give
\[
 C_{\rm seed}\{\Gamma_\star(\nu_{\rm real}^2+r\nu_{\rm real}^3+b_s)
 +\sqrt r\,e_I(q)\}\le\tfrac18\tau_{\min}a_\star^3. \tag{seed-dominance}
\]
Together with \(\sqrt r\,\nu_{\rm real}\le1/20\), this gives
\[
 s_s\ge\tfrac14\tau_{\min}a_\star^3,\qquad
 \Delta F_s\ge\tfrac1{16}\tau_{\min}^2a_\star^6,\qquad
 {s_s\over D_s},{\Delta F_s\over D_s^2},{g^3\over D_s}\ge4P^{-D_\mu},\qquad
...
updates. On the guarded box,
\[
 L_{{\rm tx},\star}\le P^{d_L}a_\star^{-d_{\rm lip}},\qquad
 H_{\rm tx}\Delta_{\rm tr}\ge4R_{\max}+2\Delta_{\rm ang},\qquad
 H_{\rm tx}\ge C_{\rm tx}L_{{\rm tx},\star}\Delta_{\rm tr}^{-1}\log P,
 \qquad \alpha_hL_{{\rm tx},\star}\le\tfrac14. \tag{guard-path}
\]
The lower feasibility bound on \(a_\star\) makes these path requirements
polynomial; \(H_{\rm tx}\) is the least integer satisfying them.
...
With
\[
 N_{\rm tx}=\left\lceil C_{\rm tape}r\,p_\star^{-1}\log(2r)\right\rceil
 \le C'_{\rm tape}r(nr)^{c_{\rm tx}}\log(2r),
\]
\((1-p_\star)^{N_{\rm tx}}\le r^{-11}\) per unresolved label and the
finite union gives conditional failure at most \(r^{-10}\). Work is polynomial
and independent of \(\epsilon\) on the feasible branch; otherwise the explicit
mass blocker is invoked rather than claiming polynomial recovery. The half-
slack export uses \(a_\star,b_\star,p_\star,\Lambda_\star\) and the margins
above, so no fixed \(a_n\)-scale is used downstream.
...
\(\mathcal M_\star=\{j:|a_j|\ge\theta_\star\}\), with
\(\theta_\star=\tau_{\min}a_\star^3/16\).
...
\]
For a passing candidate with two surviving coefficients, the adaptive
threshold gives both the coefficient-level source and its unnormalized form
\[
 m_{\rm sel}:={\min\{|a_j|,|a_\ell|\}\over\tau_{\min}}
 \ge {a_\star^3\over16},\qquad
 m_{\rm mix}:={|a_ja_\ell|\over\|T\|_F}
 \ge {\tau_{\min}a_\star^6\over256\kappa_T}.
 \tag{adaptive-mixed}
\]
The selector calculation factors one surviving coefficient, so its negative
source is of order \(\tau_{\min}a_\star^3\), while the displayed \(m_{\rm mix}\)
is the corresponding normalized Hessian source. The LP budget substitutes
...
\[
 ...
 \tag{U3-hierarchy}
\]
and \(a_\star^3\ge64\mathfrak D\) imply the displayed U3-dominance inequality,
after retaining the finite raw \(\sum\chi_t\) term. Thus the negative mixed
term dominates every structural, history, and numerical defect before
positive-Hessian acceptance.
...
dimension regime yield \(p_\star\ge c a_\star^3(2r)^{-c_0}e^{-C\Lambda_\star}\),
and hence \(p_\star\ge(nr)^{-c_{\rm tx}}\) on the explicit feasible branch
after the seed-dominance inequality. It also exports
\[
 s_s\ge\tfrac14\tau_{\min}a_\star^3,\quad
 \Delta F_s\ge\tfrac1{16}\tau_{\min}^2a_\star^6,\quad
 \mu_s\ge2P^{-D_\mu},\quad
...
derivative bound \(L_{{\rm tx},\star}\le P^{d_L}a_\star^{-d_{\rm lip}}\) and
the guard-path inequalities make the polynomial path reach the certificate
whenever \(a_\star\ge(nr)^{-c_\star}\). The first score is positive and the
half-slack export
\[
 e_{\rm rad},e_{\rm ang}\le\tfrac12e_I,\quad
 s_s/D_s,\Delta F_s/D_s^2,g^3/D_s\ge4P^{-D_\mu},\quad
 ...
 C_0\Gamma_\star\|\mathcal T^+\|_{{\rm root},2\to{\rm root},2}\le\tfrac18
\]
is invariant for H_tx updates or rollback. U5-amplify uses
\(N_{\rm tx}=\lceil C_{\rm tape}r p_\star^{-1}\log(2r)\rceil\), so
\((1-p_\star)^{N_{\rm tx}}\le r^{-11}\) per unresolved label and the
conditional failure is at most (r^{-10}) on the feasible branch. If
\(\Lambda_\star>C_{\rm mass}\log(nr)\), U5 stops at the explicit blocker
rather than asserting polynomial recovery. The analysis union over labels is
never queried by the protocol. Empty prefix, negative score, guard boundary,
tape exhaustion, and R=0 all stop or rollback. Obligation locality:
step-local; path U4 -> U5 -> U6/U7.

### Adaptive mass feasibility stress

The positive seed and mixed-selector sources are active on the feasible slab
because (a_\star^3\ge64\mathfrak D), while the lower bound
\(a_\star\ge(nr)^{-c_\star}\) keeps the guarded derivative path polynomial.
The spherical slab has the explicit mass (p_\star\) in (slab-mass). If
\(\Lambda_\star>C_{\rm mass}\log(nr)\), concentration instead gives
\(p_\star\le\exp(-c n a_\star^2)), so no polynomial-length fixed tape can
meet the recovery contract. The binding setting permits this branch through
an (n)-independent near-margin C-mode defect; the eleven primitive IDs do not
exclude it. Obligation locality is `step-local` on the feasible branch and
`idea/theorem-contract defect` for the unrestricted-(n) blocker; the route is
to `/subagent-idea-generator`, not a hidden rate simplification.
...
| 005 | Adaptive Haar slab (a_\star,b_\star,p_\star,\Lambda_\star), explicit score/gain/mu margins, derivative path, and guarded flow | U4 -> stopped tape amplification on the feasible branch | first sign/update, guard, exhaustion, mass blocker | step-local -> 006/007 |
...
| seed window/tape event | step 005 | a_\star,b_\star,p_\star,\Lambda_\star, score/gain/mu margins, sqrt(r) slack, guard-path, H_tx | Actual full-space state | Haar mass/amplification on the feasible branch | 005 -> 006/007 | mass-feasibility failure |
...
| Activation/tape event with adaptive (a_\star,b_\star,p_\star,\Lambda_\star) and feasibility gate | step 005 | 006,007 | Recovery probability on feasible branch; blocker otherwise | 004 + random tape -> 005 | derived/conditional |
...
| step_005 | Prove the adaptive seed window with \(s_s\ge\tfrac14\tau_{\min}a_\star^3\), \(\Delta F_s\ge\tfrac1{16}\tau_{\min}^2a_\star^6\), \(p_\star\), \(\mu_s\ge2P^{-D_\mu}\), \(\sqrt r\nu_{\rm real}\le1/20\), guarded path, and tape amplification on the feasible branch. | step_002, step_004 | assump:random-tape; derived invariant | Adaptive defect scale, Haar mass, derivative path, and explicit feasibility/blocker test | Spherical density, clipped flow, stopped trials | window, margins, tape event, discovery runtime | R3 | PENDING |
...
The seven-row organization is the concise strongest conditional route; it
does not hide the remaining unrestricted-parameter obstruction. The adaptive
scale is valid only when
\(a_\star\le a_{\max}\), \(a_\star\ge(nr)^{-c_\star}\), and
\(\Lambda_\star=n a_\star^2\le C_{\rm mass}\log(nr)\). If
\(\Lambda_\star>C_{\rm mass}\log(nr)\), spherical concentration gives
\(p_\star\le\exp(-c n a_\star^2)\), so the fixed polynomial tape cannot
recover even one unresolved label with the required probability. The setting
allows an (n)-independent near-margin C-mode defect, hence the eleven IDs do
not imply this mass condition for unrestricted (n). This is a concrete
theorem-contract blocker and must route to `/subagent-idea-generator` rather
than be presented as unconditional closure. Under the displayed feasibility
conditions, all seven rows remain `PENDING`, the target, algorithm, rank,
probability modes, progress type, metric, and no-floor baseline are unchanged,
and the adaptive branch is the strongest material-partial sketch.
