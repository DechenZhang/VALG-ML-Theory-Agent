...
- Current idea: `perspective_2/idea_12`, sketch attempt 2 (revise_sketch).
...
The live proof sketch is now attempt 2 (revise_sketch). The selected
candidate-relative framework is unchanged; the repair adds the explicit
local-prefix transition/induction interface, coefficient-level isolation
units, quantified slab/radial activation flow, and the same-target orbit
metric/rate bridge. The archived attempt-1 survey remains history only.
