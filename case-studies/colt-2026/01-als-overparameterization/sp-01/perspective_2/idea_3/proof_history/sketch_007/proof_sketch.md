...
This is sketch attempt 6 for the exact homogeneous asymmetric third-order CP
...
- Source-to-claim adequacy: write \(m_0=1/(8\log(2r))\).  The fresh event
  ...
  \le C\sqrt r.
  \]
  In the unequal-weight/queued case, condition on
  \(R_{Q,e}=\sum_{q\in Q_e}(\tau_qv_q-Q_{q,e})\) and require the signed
  aggregate condition
  \[
  \eta_i=\langle R_{Q,e}+E_e,d_i\rangle
  \ge-\frac14z_{ij}.
  \]
  The weighted product-normal wrapper normalizes by
  \(\sigma_{e}^2=\sum_{\ell\in U_e}\tau_\ell^2+\|R_{Q,e}\|_F^2\);
  the E0 bounds \(\tau_{\max}/\tau_{\min}\le P^6\), and symmetry of the
  conditional queued projection, give the same mild-gap probability uniformly
  over the allowed unequal weights:
  \[
  p_{\mathrm{mild}}\ge\frac{c}{r\log(2r)}.
  \]
...
- Residual-to-target adequacy: the positive lead is indexed by an actual j
  and \(z_{ij}\) is the signed target contribution.  The signed queued/error
  condition gives \(s_{i,0}=z_{ij}+\eta_i\ge3z_{ij}/4\ge3q_0/4\).
  Queued/error terms are normalized by the lead, rather than being required
  to be smaller than an absolute \(q_0\) scale.
...
- Source-to-claim adequacy: split the dynamics at the first parking time.
  For the probationary pre-parking phase define the signed lead
  \(\ell_t=s_j(t)\), the aggregate ratio
  \[
  \chi_t=\frac{\left(\sum_{\ell\ne j}z_{\ell,t}^2\right)^{1/2}
  +|\eta_t|}{\ell_t},
  \]
  and the angular gain \(b_t=\lambda_tg_t\ell_t\).  W0 supplies
  \(\ell_0\ge3q_0/4\), \(\chi_0\le C\sqrt r\), and a positive top-coordinate
  gap.  Direct balanced differentiation gives, while \(g_t<2\theta\),
  \[
  \ell_{t+1}\ge
  \ell_t\bigl(1-Cb_tP^{-24}\bigr)-Cb_tP^{-32}\ell_t,
  \]
  \[
  \chi_{t+1}\le(1-cb_t)\chi_t+
  Cb_t(P^{-16}+P^{-32}\chi_t).
  \]
  The radial identity is
  \(g_{t+1}\ge g_t+\lambda_tg_t^2\ell_t/2\).  Consequently
  \[
  B_{\mathrm{park}}=\sum_{t<\mathrm{park}}b_t
  \ge c\log(2\theta/\delta)\ge c'\log P,
  \]
  and the coupled inequalities give
  \(\ell_t\ge q_0/2\), \(\chi_t\le P^{-8}\) at the first parking
  boundary, with no sign loss.  This is the pre-D0 amplification; it uses
  only the angular \(g\)-power and never invokes target-scale queued updates.
  The first crossing is then project-and-parked by the fixed protocol.
  Separately, after D0 promotes j into Q, the queued post-promotion recurrence
  uses the target coefficient \(\tau_j\):
  \[
...
- Residual-to-target adequacy: after pre-parking amplification, D0 uses
  \[
  s_i=z_{ij}+\eta_i,\qquad |\eta_i|\le P^{-24}z_{ij},
  \]
  at the boundary.  The deterministic pre-parking winner-filter lemma shows
  that every candidate which did not enter a mild target basin has score at
  most \(q_0/4\), while the amplified candidate has score at least \(q_0/2\);
  thus the global largest score is a basin candidate.
...
- Closure/dominance/absorption relation: the pre-parking
  \((\ell,\chi)\) recurrence is lead-preserving because its multiplicative
  lead loss is \(P^{-24}\) relative to the angular gain, while chi contracts
  at the full \(b_t\) rate.  It outputs
  \(\ell\ge q_0/2,\chi\le P^{-8}\) before D0.  The separate queued recurrence
  then yields mature radii at most \(4\kappa\), target-error decrease at least
