...
This is sketch attempt 4 for the exact homogeneous asymmetric third-order CP
...
- Triggering artifact:
  perspective_2/idea_3/proof_history/sketch_003/proof_sketch_review.md
...
- Selected framework: guarded homogeneous refresh-and-promotion with a
  recursively stopped relative-winner event, one consistently scaled loss
  reserve, queued-target induction, and defect-free post-pruning PL.
...
5. W0 proves a relative target-basin event for a fresh block.  The event
   includes a weighted target lead, a relative competitor gap for every other
   candidate, score-floor and angular conditions, and the winner condition
   required by the global largest-score promotion rule.
...
7. P0 proves score persistence and queued radial/angular growth using the
   relative margin, with a supported \(P^{103}\) time envelope.  It also
   proves mature-cluster persistence and cap margin.  D0 then transfers a
   physically separated winner to Q without deleting its target error.
...
10. B1 bounds the Jacobian on the whole chart and compares the entry loss with
    the minimum loss on the chart boundary.  This proves first-exit
    preservation and a defect-free PL inequality.
...
- Probability mode: conditional high probability over the one finite tape.
  Let \(\mathsf G_e\) be the complete relative-winner event for the fresh
  block at epoch e, and define successful prefixes recursively:
  \[
  \mathsf E_0=\Omega,\qquad
  \mathsf E_{e+1}=\mathsf E_e\cap
  \bigl(\{U_e=\varnothing\}\cup\mathsf G_e\bigr),
  \]
  where \(\mathsf G_e\) is evaluated only on \(\mathsf E_e\) and
  \(U_e\ne\varnothing\).  Set
  \[
  ...
  \(\Pr(\mathsf G_e^c\mid\mathsf E_e)\le r^{-24}\), hence
...
- Step_006 derives a complete relative target-basin and global-winner event
  from a fresh block conditional on I_e.  Its stopped recursive event is
  nonanticipating and includes terminal U exhaustion.
...
### W0: relative target-basin and global-winner event

- Step ID: step_006.
- Claim class: adaptive anti-concentration, score separation, and winner
  exclusion.
- Theorem role: sole random source for each unresolved target promotion.
- Mechanism source: fresh Haar directions represented by normalized Gaussians,
  ...
  lemma.
- Source-to-claim adequacy: the good candidate event requires
  \[
  |z_{ij}|\ge q_0,\qquad
  |z_{ij}|\ge8\sum_{\ell\ne j}|z_{i\ell}|,
  \qquad
  |\eta_i|\le P^{-8}|z_{ij}|,
  \]
  where \(\eta_i\) is the queued/error/self contribution.  It also requires
  relative angular drift at most \(P^{-24}\) and the all-candidate winner
  condition
  \[
  s_i\ge(1+P^{-32})s_h\quad\text{for every }h\ne i.
  \]
  The direct relative-coordinate/order-statistic calculation gives
  \[
  \Pr(\mathsf G_e^c\mid\mathsf E_e,I_e)
  \le \exp[-c k/(r\log(2r))]+P^{-40}\le r^{-24}.
  \]
  The \(P^{-40}\) term is the small-ball cost of the score floor, not an
  absolute competitor window.
- Residual-to-target adequacy: the lead is indexed by an actual j in U; the
  relative error \(\eta_i/z_{ij}\) is controlled by I_e and the cone.
- Key positive/control term or structural source: weighted target lead and
  global score gap.
- Opposing defect terms: weighted mixed channels, queued residual, E_e,
  angular drift, and self interaction.
- Closure/dominance/absorption relation: the displayed ratios give
  \(s_i\ge(1-P^{-8})z_{ij}\) and exclude every larger mixed or duplicate
  candidate.  The dynamic cone margin is passed to P0.
- Accumulation behavior / scope compatibility: successful prefixes query at
  most r blocks; the terminal event includes U exhaustion, so no first-failure
...
- Entry-state / first-update stress result: at seed radius J0 supplies the
  cubic sign; a residual-orthogonal block is a failed \(\mathsf G_e\), not a
  claimed deterministic progress branch.
- Baseline conclusion preserved: candidates are compared with the actual T.
- Producer-consumer provenance: W0 produces \(\mathsf G_e\) and the winner
  interface for D0 and L1.
- Null or boundary regime tested: equal-weight orthogonal targets, mixed
  candidate, duplicate candidate, and score-floor small ball.
- Target conclusion false or theorem-critical obstruction present if source
...
- Source-to-claim adequacy: define score lead \(\Gamma_t\) and competitor
  ratio \(\chi_t\).  While \(g_t\le\theta\),
  \[
  \Gamma_{t+1}\ge\Gamma_t-
  C\lambda_tg_t^2(\chi_t+P^{-24}),
  \]
  \[
  \chi_{t+1}\le(1-c\lambda_tg_t^2)\chi_t+
  C\lambda_tg_t^2(P^{-32}+\zeta_t),
  \]
  with \(\Gamma_0\ge P^{-16}\), \(\chi_0\le P^{-24}\),
  \(\zeta_t\le P^{-32}\).  The coupled recurrence gives
  \(\Gamma_t\ge P^{-20}\) and \(s_t\ge q_0/2\).  Then
  \(g_{t+1}\ge g_t+\lambda_tg_t^2q_0/4\), so the queued growth time is
  at most P^103 and the proof envelope P^108.
- Residual-to-target adequacy: D0 uses
  \[
  s_i=z_{ij}+\eta_i,\qquad |\eta_i|\le P^{-8}|z_{ij}|,
  \]
  at the boundary.  Every nonwinner has score at most
  \(s_i(1-P^{-32})\).  Moving j to Q replaces \(\tau_j^2\) by the actual
...
- Closure/dominance/absorption relation: the two-dimensional
  \((\Gamma,\chi)\) recurrence is a small-gain system; P0 yields mature
...
- Producer-consumer provenance: P0/D0 produce the next I certificate and
  feed L1.
- Null or boundary regime tested: opposite sign, score gap at \(P^{-20}\),
  first crossing \(2\theta\), and empty U.
- Target conclusion false or theorem-critical obstruction present if source
...
- Source-to-claim adequacy: on the full chart \(\|h\|\le P^{-36}\),
  \[
  \sigma_{\min}(J(h))\ge P^{-4}/2,\qquad
  \|J(h)\|\le P^4,\qquad
  \|R(h)+J_\star h\|\le P^8\|h\|^2.
  \]
  At the chart boundary,
  \[
  \|R(h)\|_F\ge\frac12P^{-4}P^{-36}
  =\frac12P^{-40},\qquad
  F_{\partial}\ge P^{-80}/8.
  \]
  B0 gives \(F_0\le\|T\|^2P^{-124}/2\le P^{-116}\), strictly below
  \(F_{\partial}\) for large P.  Armijo monotonicity therefore rules out a
  first exit.  Inside the chart the same bounds yield
  \[
...
- Key positive/control term or structural source: uniform current-Jacobian
  singular value and the boundary-energy gap.
- Opposing defect terms: only the quadratic state-dependent remainder.
- Closure/dominance/absorption relation: the remainder is at most half the
  linear term on the chart; the entry/boundary energy comparison closes the
...
| Relative winner event | step_006 | fresh Haar block and I_e | relative mixed/error terms controlled | target-indexed score | relative lead and all-candidate gap | q0 floor and P^-32 gap | P0, D0, L1 | None |
| Score/maturation transition | step_007 | W0 margin, C0, E0 | evolving residual controlled by Gamma/chi recurrence | actual target tensor error | P^103 growth and P^-16 mass drop | P^-20 score lead | D0, L1 | None |
...
| PL/first exit | step_011 | current J bounds and entry energy | quadratic remainder only | exact protected residual | boundary F >=P^-80/8 versus entry | chart radius | R0 | None |
...
| I0 and local I_e interface | step_005 | step_006-step_009 | induction base | E0/C0/J0 -> I0 | derived for base; explicitly conditional for generic e | None |
| Relative winner \(\mathsf G_e\) | step_006 | step_007-step_009 | stopped tape event | I_e + fresh block -> G_e | explicitly conditional | None |
| Gamma/chi and maturation | step_007 | step_008-step_009 | physical target reduction/cap | I_e + G_e -> P0 | explicitly conditional | None |
...
| step_006 | W0 relative target-basin/global-winner event and recursive failure bound | step_005 | assump:random-tape; step_001, step_003-step_005 | scale-compatible order statistics | relative Gaussian/Haar lemma | G_e and stopped tape interface | Guarded burn-in | PENDING |
| step_007 | P0 score persistence, P^103 growth, mature cap margin, and target reduction | step_005, step_006 | step_001, step_003, step_005-step_006 | evolving residual | Gamma/chi small-gain recurrence | protected transition | Guarded burn-in | PENDING |
| step_008 | D0 all-candidate assignment and I_{e+1} physical transfer | step_007 | step_005-step_007 | largest-score winner | score-gap and M functional | next I | Guarded burn-in | PENDING |
...
only as a local conditional interface.  Step_006's relative-winner event is
evaluated only on a successful prefix.  Steps_007 and step_008 prove the
universal transition I_e to I_{e+1}; step_009 iterates that transition and
requires terminal unresolved-set exhaustion, so the tape event is nonvacuous.
...
None at sketch level.  The attempt-3 vacuous stopping event, absolute
competitor window, mixed reserve definitions, unattainable polishing target,
and overview-only chart/rate claims are replaced by explicit producer
relations in steps_006 through step_012.  Any failure of the relative-winner
lemma, Gamma/chi persistence, or the conditioned chart map is a localized
step obligation and must be surfaced by the corresponding proof-step review,
not hidden as an assumption.
